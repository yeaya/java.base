...
$Object* ProcessEnvironment$StringEntrySet$1::next() {
	return $new($ProcessEnvironment$StringEntry, $$cast($Map$Entry, $nc(this->i)->next()));
}
...
$Class* ProcessEnvironment$StringEntrySet$1::load$($String* name, bool initialize) {
	...
	$loadClass(ProcessEnvironment$StringEntrySet$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ProcessEnvironment$StringEntrySet$1);
	});
	return class$;
}
