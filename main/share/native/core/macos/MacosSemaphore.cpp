...
MacosSemaphore::MacosSemaphore(uint value) {
...
void MacosSemaphore::signal(uint count) {
	for (uint i = 0; i < count; i++) {
		kern_return_t ret = semaphore_signal(semaphore);
	}
}

void MacosSemaphore::wait() {
	kern_return_t ret;
...
bool MacosSemaphore::trywait() {
	return timedwait(0);
}

bool MacosSemaphore::timedwait(int64_t millis) {
	// relative timeout
