...
bool OS::addressToFunctionName(address addr, char* buf, int buflen) {
	...
	if (dladdr((void*)addr, &dlinfo) != 0) {
		...
		if (dlinfo.dli_saddr != NULL && dlinfo.dli_sname != NULL) {
			if (!(macosxDecoder.demangle(dlinfo.dli_sname, buf, buflen))) {
		...
		if (dlinfo.dli_fname != NULL && dlinfo.dli_fbase != NULL) {
			if (macosxDecoder.decode((address)(addr - (address)dlinfo.dli_fbase),
		...
		if (dlinfo.dli_fbase != NULL &&
			macosxDecoder.decode(addr, localbuf, MACH_MAXSYMLEN, dlinfo.dli_fbase)) {
			if (!(macosxDecoder.demangle(localbuf, buf, buflen))) {
...
void OS::Macosx::registerThreadWithCollectorFunction() {
