...
void OSThread::run(JavaThread* thread) {
	...
	OS::Macosx::registerThreadWithCollectorFunction();
...
bool OSThread::create(JavaThread* thread, int64_t reqStackSize) {
	...
	pthread_t tid;
	int ret = pthread_create(&tid, &attr, (void* (*)(void*))threadEntry, thread);
...
void OSThread::setInterrupted(bool) {
}