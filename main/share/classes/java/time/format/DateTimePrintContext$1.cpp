...
$ValueRange* DateTimePrintContext$1::range($TemporalField* field) {
	if (this->val$effectiveDate != nullptr && $nc(field)->isDateBased()) {
		return this->val$effectiveDate->range(field);
	}
...
$Object* DateTimePrintContext$1::query($TemporalQuery* query) {
	if (query == $TemporalQueries::chronology()) {
		return $of(this->val$effectiveChrono);
	}
	if (query == $TemporalQueries::zoneId()) {
		return $of(this->val$effectiveZone);
	}
	if (query == $TemporalQueries::precision()) {
		return $nc(this->val$temporal)->query(query);
	}
...
$String* DateTimePrintContext$1::toString() {
	...
	var$0->append(this->val$temporal);
	var$0->append(this->val$effectiveChrono != nullptr ? $$str({" with chronology "_s, this->val$effectiveChrono}) : ""_s);
	var$0->append(this->val$effectiveZone != nullptr ? $$str({" with zone "_s, this->val$effectiveZone}) : ""_s);
	return $str(var$0);
}
...
$Class* DateTimePrintContext$1::load$($String* name, bool initialize) {
	...
	$loadClass(DateTimePrintContext$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DateTimePrintContext$1);
	});
	return class$;
}
