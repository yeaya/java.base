...
$String* DateTimeFormatterBuilder$WeekBasedFieldPrinterParser::toString() {
	...
	if (this->chr == u'Y') {
		if (this->count == 1) {
			...
			$init($SignStyle);
			sb->append("WeekBasedYear,"_s)->append(this->count)->append(","_s)->append(19)->append(","_s)->append((this->count < 4) ? $SignStyle::NORMAL : $SignStyle::EXCEEDS_PAD);
		}
	} else {
		switch (this->chr) {
		case u'c':
...
$Class* DateTimeFormatterBuilder$WeekBasedFieldPrinterParser::load$($String* name, bool initialize) {
	...
	$loadClass(DateTimeFormatterBuilder$WeekBasedFieldPrinterParser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DateTimeFormatterBuilder$WeekBasedFieldPrinterParser);
	});
	return class$;
}
