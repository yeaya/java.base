...
bool DateTimeFormatterBuilder$InstantPrinterParser::format($DateTimePrintContext* context, $StringBuilder* buf) {
	$useLocalObjectStack();
	$init($ChronoField);
	$var($Long, inSecs, $nc(context)->getValue($ChronoField::INSTANT_SECONDS));
	...
	int32_t inNano = $ChronoField::NANO_OF_SECOND->checkValidIntValue(inNanos != nullptr ? inNanos->longValue() : 0);
...
int32_t DateTimeFormatterBuilder$InstantPrinterParser::parse($DateTimeParseContext* context, $CharSequence* text, int32_t position) {
	...
	int32_t sec = (secVal != nullptr ? secVal->intValue() : 0);
	int32_t nano = (nanoVal != nullptr ? nanoVal->intValue() : 0);
...
$Class* DateTimeFormatterBuilder$InstantPrinterParser::load$($String* name, bool initialize) {
	...
	$loadClass(DateTimeFormatterBuilder$InstantPrinterParser, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DateTimeFormatterBuilder$InstantPrinterParser);
	});
	return class$;
}
