...
$ValueRange* HijrahDate::range($TemporalField* field) {
	...
	if ($instanceOf($ChronoField, field)) {
		if (isSupported(field)) {
			...
			$init($HijrahDate$1);
			switch ($nc($HijrahDate$1::$SwitchMap$java$time$temporal$ChronoField)->get((f)->ordinal())) {
			case 1:
			...
			return $$sure($HijrahChronology, getChronology())->range(f);
		}
...
int64_t HijrahDate::getLong($TemporalField* field) {
	if ($instanceOf($ChronoField, field)) {
		$init($HijrahDate$1);
		switch ($nc($HijrahDate$1::$SwitchMap$java$time$temporal$ChronoField)->get($cast($ChronoField, field)->ordinal())) {
		case 4:
			return getDayOfWeek();
		case 5:
...
$ChronoPeriod* HijrahDate::until($ChronoLocalDate* endDate) {
	$useLocalObjectStack();
	$var(HijrahDate, end, $cast(HijrahDate, $$sure($HijrahChronology, getChronology())->date(endDate)));
	int64_t totalMonths = ($nc(end)->prolepticYear - this->prolepticYear) * 12 + ($nc(end)->monthOfYear - this->monthOfYear);
	...
	return $$sure($HijrahChronology, getChronology())->period($Math::toIntExact(years), months, days);
}
...
bool HijrahDate::equals(Object$* obj) {
	...
	return var$0 && $$sure($HijrahChronology, getChronology())->equals($(otherDate->getChronology()));
}
...
int32_t HijrahDate::hashCode() {
	...
	return ($$nc($$sure($HijrahChronology, getChronology())->getId())->hashCode() ^ (yearValue & -2048)) ^ ((yearValue << 11) + (monthValue << 6) + (dayValue));
}
...
$Class* HijrahDate::load$($String* name, bool initialize) {
	...
	$loadClass(HijrahDate, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
