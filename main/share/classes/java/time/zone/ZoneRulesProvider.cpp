...
ZoneRulesProvider* ZoneRulesProvider::getProvider($String* zoneId) {
	$init(ZoneRulesProvider);
	$useLocalObjectStack();
	$var(ZoneRulesProvider, provider, $cast(ZoneRulesProvider, ZoneRulesProvider::ZONES->get(zoneId)));
	if (provider == nullptr) {
...
void ZoneRulesProvider::registerProvider(ZoneRulesProvider* provider) {
	$init(ZoneRulesProvider);
	$Objects::requireNonNull(provider, "provider"_s);
	registerProvider0(provider);
	ZoneRulesProvider::PROVIDERS->add(provider);
}
	...
	$synchronized(class$) {
		$useLocalObjectStack();
		{
			...
			for (; $nc(i$)->hasNext();) {
				...
				{
					$Objects::requireNonNull(zoneId, "zoneId"_s);
					$var(ZoneRulesProvider, old, $cast(ZoneRulesProvider, ZoneRulesProvider::ZONES->putIfAbsent(zoneId, provider)));
					if (old != nullptr) {
		...
		$var($Set, combinedSet, $new($HashSet, $(ZoneRulesProvider::ZONES->keySet())));
...
bool ZoneRulesProvider::refresh() {
	...
	bool changed = false;
	{
		$var($Iterator, i$, ZoneRulesProvider::PROVIDERS->iterator());
		for (; $nc(i$)->hasNext();) {
...
void ZoneRulesProvider::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(ZoneRulesProvider::ZONES, $new($ConcurrentHashMap, 512, 0.75f, 2));
	{
		...
		$AccessController::doPrivileged($$new($ZoneRulesProvider$1, loaded));
		...
		while ($nc(it)->hasNext()) {
			...
			bool found = false;
			{
				...
				for (; $nc(i$)->hasNext();) {
					...
					if ($nc(p)->getClass() == $nc(provider)->getClass()) {
						found = true;
					}
				}
			}
...
$Class* ZoneRulesProvider::load$($String* name, bool initialize) {
	...
	$loadClass(ZoneRulesProvider, name, initialize, &classInfo$$, ZoneRulesProvider::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ZoneRulesProvider);
	});
	return class$;
}
