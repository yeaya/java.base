...
ZoneOffsetTransitionRule* ZoneOffsetTransitionRule::of($Month* month, int32_t dayOfMonthIndicator, $DayOfWeek* dayOfWeek, $LocalTime* time, bool timeEndOfDay, $ZoneOffsetTransitionRule$TimeDefinition* timeDefinition, $ZoneOffset* standardOffset, $ZoneOffset* offsetBefore, $ZoneOffset* offsetAfter) {
	...
	$Objects::requireNonNull(timeDefinition, "timeDefinition"_s);
	$Objects::requireNonNull(standardOffset, "standardOffset"_s);
...
ZoneOffsetTransitionRule* ZoneOffsetTransitionRule::readExternal($DataInput* in) {
	...
	int32_t dom = ((int32_t)((uint32_t)(data & (63 << 22)) >> 22)) - 32;
	int32_t dowByte = (int32_t)((uint32_t)(data & (7 << 19)) >> 19);
	$DayOfWeek* dow = dowByte == 0 ? ($DayOfWeek*)nullptr : $DayOfWeek::of(dowByte);
	int32_t timeByte = (int32_t)((uint32_t)(data & (31 << 14)) >> 14);
	$ZoneOffsetTransitionRule$TimeDefinition* defn = $($ZoneOffsetTransitionRule$TimeDefinition::values())->get((int32_t)((uint32_t)(data & (3 << 12)) >> 12));
	int32_t stdByte = (int32_t)((uint32_t)(data & (255 << 4)) >> 4);
	int32_t beforeByte = (int32_t)((uint32_t)(data & (3 << 2)) >> 2);
	int32_t afterByte = (data & 3);
...
$ZoneOffsetTransition* ZoneOffsetTransitionRule::createTransition(int32_t year) {
	...
	if (this->dom < 0) {
		...
		$assign(date, $LocalDate::of(year, this->month, this->dom));
		if (this->dow != nullptr) {
...
$String* ZoneOffsetTransitionRule::toString() {
	...
	buf->append("TransitionRule["_s)->append($nc(this->offsetBefore)->compareTo(this->offsetAfter) > 0 ? "Gap "_s : "Overlap "_s)->append(this->offsetBefore)->append(" to "_s)->append(this->offsetAfter)->append(", "_s);
	if (this->dow != nullptr) {
		if (this->dom == -1) {
	...
	buf->append(" at "_s)->append(this->timeEndOfDay ? "24:00"_s : $($nc(this->time)->toString()))->append(" "_s)->append(this->timeDefinition)->append(", standard offset "_s)->append(this->standardOffset)->append(u']');
	return buf->toString();
}

void ZoneOffsetTransitionRule::clinit$($Class* clazz) {
...
$Class* ZoneOffsetTransitionRule::load$($String* name, bool initialize) {
	...
	$loadClass(ZoneOffsetTransitionRule, name, initialize, &classInfo$$, ZoneOffsetTransitionRule::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ZoneOffsetTransitionRule);
	});
	return class$;
}
