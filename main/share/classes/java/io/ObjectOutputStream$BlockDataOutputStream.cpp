...
void ObjectOutputStream$BlockDataOutputStream::write(int32_t b) {
	...
	this->buf->set(this->pos++, (int8_t)b);
}
...
void ObjectOutputStream$BlockDataOutputStream::writeByte(int32_t v) {
	...
	this->buf->set(this->pos++, (int8_t)v);
}
...
void ObjectOutputStream$BlockDataOutputStream::writeBytes($String* s) {
	...
	for (int32_t off = 0; off < endoff;) {
		...
		while (this->pos < stop) {
			this->buf->set(this->pos++, (int8_t)this->cbuf->get(cpos++));
		}
		off += n;
	}
}
...
void ObjectOutputStream$BlockDataOutputStream::writeChars($chars* v, int32_t off, int32_t len) {
	...
	while (off < endoff) {
		if (this->pos <= limit) {
			...
			this->dout->writeChar($nc(v)->get(off++));
		}
	}
}
...
void ObjectOutputStream$BlockDataOutputStream::writeShorts($shorts* v, int32_t off, int32_t len) {
	...
	while (off < endoff) {
		if (this->pos <= limit) {
			...
			this->dout->writeShort($nc(v)->get(off++));
		}
	}
}
...
void ObjectOutputStream$BlockDataOutputStream::writeInts($ints* v, int32_t off, int32_t len) {
	...
	while (off < endoff) {
		if (this->pos <= limit) {
			...
			this->dout->writeInt($nc(v)->get(off++));
		}
	}
}
...
void ObjectOutputStream$BlockDataOutputStream::writeFloats($floats* v, int32_t off, int32_t len) {
	...
	while (off < endoff) {
		if (this->pos <= limit) {
			...
			this->dout->writeFloat($nc(v)->get(off++));
		}
	}
}
...
void ObjectOutputStream$BlockDataOutputStream::writeLongs($longs* v, int32_t off, int32_t len) {
	...
	while (off < endoff) {
		if (this->pos <= limit) {
			...
			this->dout->writeLong($nc(v)->get(off++));
		}
	}
}
...
void ObjectOutputStream$BlockDataOutputStream::writeDoubles($doubles* v, int32_t off, int32_t len) {
	...
	while (off < endoff) {
		if (this->pos <= limit) {
			...
			this->dout->writeDouble($nc(v)->get(off++));
		}
	}
}
...
void ObjectOutputStream$BlockDataOutputStream::writeUTFBody($String* s) {
	...
	for (int32_t off = 0; off < len;) {
		...
		for (int32_t cpos = 0; cpos < csize; ++cpos) {
			...
			if (this->pos <= limit) {
				if (c <= 127 && c != 0) {
					this->buf->set(this->pos++, (int8_t)c);
				} else if (c > 2047) {
					this->buf->set(this->pos + 2, (int8_t)(0x80 | ((c >> 0) & 0x3f)));
					this->buf->set(this->pos + 1, (int8_t)(0x80 | ((c >> 6) & 0x3f)));
					this->buf->set(this->pos + 0, (int8_t)(0xe0 | ((c >> 12) & 0x0f)));
					this->pos += 3;
				} else {
					this->buf->set(this->pos + 1, (int8_t)(0x80 | ((c >> 0) & 0x3f)));
					this->buf->set(this->pos + 0, (int8_t)(0xc0 | ((c >> 6) & 0x1f)));
					this->pos += 2;
				}
			} else if (c <= 127 && c != 0) {
				write(c);
...
$Class* ObjectOutputStream$BlockDataOutputStream::load$($String* name, bool initialize) {
	...
	$loadClass(ObjectOutputStream$BlockDataOutputStream, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
