...
void ObjectInputStream$FieldValues::init$($ObjectInputStream* this$0, $ObjectStreamClass* desc, bool recordDependencies) {
	...
	if (numObjFields > 0) {
		...
		for (int32_t i = 0; i < this->objValues->length; ++i) {
			$var($ObjectStreamField, f, fields->get(numPrimFields + i));
			this->objValues->set(i, $(this$0->readObject0($Object::class$, $nc(f)->isUnshared())));
			$nc(this->objHandles)->set(i, this$0->passHandle);
			if (recordDependencies && f->getField() != nullptr) {
...
$Object* ObjectInputStream$FieldValues::get($String* name, Object$* val) {
	...
	if (off >= 0) {
		...
		return (this->this$0->handles->lookupException(objHandle) == nullptr) ? $nc(this->objValues)->get(off) : ($Object*)nullptr;
	} else {
		return $of(val);
	}
}
...
int32_t ObjectInputStream$FieldValues::getFieldOffset($String* name, $Class* type) {
	...
	} else if ($$nc(this->desc->getLocalDesc())->getField(name, type) != nullptr) {
		return -1;
	} else {
...
$Class* ObjectInputStream$FieldValues::load$($String* name, bool initialize) {
	...
	$loadClass(ObjectInputStream$FieldValues, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ObjectInputStream$FieldValues);
	});
	return class$;
}
