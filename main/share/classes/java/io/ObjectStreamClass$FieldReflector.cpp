...
void ObjectStreamClass$FieldReflector::init$($ObjectStreamFieldArray* fields) {
	...
	for (int32_t i = 0; i < nfields; ++i) {
		...
		int64_t key = (rf != nullptr) ? $nc(ObjectStreamClass$FieldReflector::unsafe)->objectFieldOffset(rf) : $Unsafe::INVALID_FIELD_OFFSET;
		this->readKeys->set(i, key);
		this->writeKeys->set(i, usedKeys->add($($Long::valueOf(key))) ? key : $Unsafe::INVALID_FIELD_OFFSET);
		this->offsets->set(i, f->getOffset());
		this->typeCodes->set(i, f->getTypeCode());
		if (!f->isPrimitive()) {
			typeList->add((rf != nullptr) ? rf->getType() : ($Class*)nullptr);
		}
	}
	$set(this, types, $cast($ClassArray, typeList->toArray($$new($ClassArray, typeList->size()))));
...
void ObjectStreamClass$FieldReflector::getPrimFieldValues(Object$* obj, $bytes* buf) {
	...
	for (int32_t i = 0; i < this->numPrimFields; ++i) {
		int64_t key = this->readKeys->get(i);
		int32_t off = this->offsets->get(i);
		switch (this->typeCodes->get(i)) {
		case u'Z':
...
void ObjectStreamClass$FieldReflector::setPrimFieldValues(Object$* obj, $bytes* buf) {
	...
	for (int32_t i = 0; i < this->numPrimFields; ++i) {
		int64_t key = this->writeKeys->get(i);
...
void ObjectStreamClass$FieldReflector::getObjFieldValues(Object$* obj, $ObjectArray* vals) {
	...
	for (int32_t i = this->numPrimFields; i < $nc(this->fields)->length; ++i) {
		...
		case u'L':
		case u'[':
			$assign(var$0, $nc(ObjectStreamClass$FieldReflector::unsafe)->getReference(obj, this->readKeys->get(i)));
			break;
		default:
			$throwNew($InternalError);
		}
		$nc(vals)->set(this->offsets->get(i), var$0);
	}
}
...
void ObjectStreamClass$FieldReflector::setObjFieldValues(Object$* obj, $ObjectArray* vals, bool dryRun) {
	...
	for (int32_t i = this->numPrimFields; i < $nc(this->fields)->length; ++i) {
		...
		case u'[':
			{
				$var($Object0, val, $nc(vals)->get(this->offsets->get(i)));
				if (val != nullptr && !$nc($nc(this->types)->get(i - this->numPrimFields))->isInstance(val)) {
					$var($Field, f, $nc(this->fields->get(i))->getField());
					$var($StringBuilder, var$0, $new($StringBuilder));
					var$0->append("cannot assign instance of "_s);
					var$0->append($(val->getClass()->getName()));
					var$0->append(" to field "_s);
					var$0->append($($nc($nc(f)->getDeclaringClass())->getName()));
					var$0->append("."_s);
					var$0->append($(f->getName()));
					var$0->append(" of type "_s);
					var$0->append($($nc(f->getType())->getName()));
					var$0->append(" in instance of "_s);
...
void ObjectStreamClass$FieldReflector::clinit$($Class* clazz) {
...
$Class* ObjectStreamClass$FieldReflector::load$($String* name, bool initialize) {
	...
	$loadClass(ObjectStreamClass$FieldReflector, name, initialize, &classInfo$$, ObjectStreamClass$FieldReflector::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ObjectStreamClass$FieldReflector);
	});
	return class$;
}
