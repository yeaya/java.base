...
void ObjectStreamClass$EntryFuture::clinit$($Class* clazz) {
...
$Class* ObjectStreamClass$EntryFuture::load$($String* name, bool initialize) {
	...
	$loadClass(ObjectStreamClass$EntryFuture, name, initialize, &classInfo$$, ObjectStreamClass$EntryFuture::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ObjectStreamClass$EntryFuture);
	});
	return class$;
}
