...
void MergeCollation::init$($String* pattern) {
	...
	for (int32_t i = 0; i < this->statusArray->length; ++i) {
...
$String* MergeCollation::getPattern(bool withWhiteSpace) {
	...
	for (i = 0; i < $nc(this->patterns)->size(); ++i) {
		$var($PatternEntry, entry, $cast($PatternEntry, this->patterns->get(i)));
...
void MergeCollation::fixEntry($PatternEntry* newEntry) {
	bool var$0 = this->lastEntry != nullptr && $nc($nc(newEntry)->chars)->equals(this->lastEntry->chars);
	if (var$0 && $nc(newEntry->extension)->equals(this->lastEntry->extension)) {
	...
	if ($nc(newEntry)->strength != $PatternEntry::RESET) {
		...
		if ($nc(newEntry->chars)->length() == 1) {
			char16_t c = newEntry->chars->charAt(0);
			int32_t statusIndex = $sr(c, MergeCollation::BYTEPOWER);
			int8_t bitClump = $nc(this->statusArray)->get(statusIndex);
			int8_t setBit = (int8_t)($sl(MergeCollation::BITARRAYMASK, c & MergeCollation::BYTEMASK));
			if (bitClump != 0 && (bitClump & setBit) != 0) {
				oldIndex = $nc(this->patterns)->lastIndexOf(newEntry);
			} else {
				this->statusArray->set(statusIndex, (int8_t)(bitClump | setBit));
			}
		} else {
			oldIndex = $nc(this->patterns)->lastIndexOf(newEntry);
		}
...
int32_t MergeCollation::findLastEntry($PatternEntry* entry, $StringBuffer* excessChars) {
	...
	if ($nc(entry)->strength != $PatternEntry::RESET) {
		...
		if ($nc(entry->chars)->length() == 1) {
			int32_t index = $sr(entry->chars->charAt(0), MergeCollation::BYTEPOWER);
			if (($nc(this->statusArray)->get(index) & ($sl(MergeCollation::BITARRAYMASK, entry->chars->charAt(0) & MergeCollation::BYTEMASK))) != 0) {
		...
		for (i = $nc(this->patterns)->size() - 1; i >= 0; --i) {
			$var($PatternEntry, e, $cast($PatternEntry, this->patterns->get(i)));
			if ($nc($nc(e)->chars)->regionMatches(0, entry->chars, 0, $nc($nc(e)->chars)->length())) {
				$var($CharSequence, var$0, entry->chars);
				int32_t var$1 = e->chars->length();
...
$Class* MergeCollation::load$($String* name, bool initialize) {
	...
	$loadClass(MergeCollation, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MergeCollation);
	});
	return class$;
}
