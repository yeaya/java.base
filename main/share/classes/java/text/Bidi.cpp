...
void Bidi::init$($AttributedCharacterIterator* paragraph) {
	...
	this->bidiBase->setPara(paragraph);
}
...
void Bidi::init$($chars* text, int32_t textStart, $bytes* embeddings, int32_t embStart, int32_t paragraphLength, int32_t flags) {
	...
	if (textStart < 0 || paragraphLength > $nc(text)->length - textStart) {
		$throwNew($IllegalArgumentException, $$str({"bad range: "_s, $$str(textStart), " length: "_s, $$str(paragraphLength), " for text of length: "_s, $$str($nc(text)->length)}));
	}
	if (embeddings != nullptr && (embStart < 0 || paragraphLength > embeddings->length - embStart)) {
...
$Class* Bidi::load$($String* name, bool initialize) {
	...
	$loadClass(Bidi, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Bidi);
	});
	return class$;
}
