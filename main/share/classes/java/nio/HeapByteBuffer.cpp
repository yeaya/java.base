...
$CharBuffer* HeapByteBuffer::asCharBuffer() {
	...
	return (this->bigEndian ? $cast($CharBuffer, ($new($ByteBufferAsCharBufferB, this, -1, 0, size, size, addr, this->segment))) : $cast($CharBuffer, ($new($ByteBufferAsCharBufferL, this, -1, 0, size, size, addr, this->segment))));
}
...
$ShortBuffer* HeapByteBuffer::asShortBuffer() {
	...
	int32_t size = (limit() - pos) >> 1;
	int64_t addr = this->address + pos;
	return (this->bigEndian ? $cast($ShortBuffer, ($new($ByteBufferAsShortBufferB, this, -1, 0, size, size, addr, this->segment))) : $cast($ShortBuffer, ($new($ByteBufferAsShortBufferL, this, -1, 0, size, size, addr, this->segment))));
}
...
$IntBuffer* HeapByteBuffer::asIntBuffer() {
	...
	int64_t addr = this->address + pos;
	return (this->bigEndian ? $cast($IntBuffer, ($new($ByteBufferAsIntBufferB, this, -1, 0, size, size, addr, this->segment))) : $cast($IntBuffer, ($new($ByteBufferAsIntBufferL, this, -1, 0, size, size, addr, this->segment))));
}
...
$LongBuffer* HeapByteBuffer::asLongBuffer() {
	...
	int64_t addr = this->address + pos;
	return (this->bigEndian ? $cast($LongBuffer, ($new($ByteBufferAsLongBufferB, this, -1, 0, size, size, addr, this->segment))) : $cast($LongBuffer, ($new($ByteBufferAsLongBufferL, this, -1, 0, size, size, addr, this->segment))));
}
...
$FloatBuffer* HeapByteBuffer::asFloatBuffer() {
	...
	int32_t size = (limit() - pos) >> 2;
	int64_t addr = this->address + pos;
	return (this->bigEndian ? $cast($FloatBuffer, ($new($ByteBufferAsFloatBufferB, this, -1, 0, size, size, addr, this->segment))) : $cast($FloatBuffer, ($new($ByteBufferAsFloatBufferL, this, -1, 0, size, size, addr, this->segment))));
}
...
$DoubleBuffer* HeapByteBuffer::asDoubleBuffer() {
	...
	int32_t size = (limit() - pos) >> 3;
	int64_t addr = this->address + pos;
	return (this->bigEndian ? $cast($DoubleBuffer, ($new($ByteBufferAsDoubleBufferB, this, -1, 0, size, size, addr, this->segment))) : $cast($DoubleBuffer, ($new($ByteBufferAsDoubleBufferL, this, -1, 0, size, size, addr, this->segment))));
}

void HeapByteBuffer::clinit$($Class* clazz) {
	...
	HeapByteBuffer::ARRAY_INDEX_SCALE = $Buffer::UNSAFE->arrayIndexScale($getClass($bytes));
}
...
$Class* HeapByteBuffer::load$($String* name, bool initialize) {
	...
	$loadClass(HeapByteBuffer, name, initialize, &classInfo$$, HeapByteBuffer::clinit$, []($Class* clazz) -> $Object* {
