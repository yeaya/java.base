...
$Class* CharBuffer$$Lambda$lambda$chars$0::load$($String* name, bool initialize) {
	...
	$loadClass(CharBuffer$$Lambda$lambda$chars$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CharBuffer$$Lambda$lambda$chars$0);
	});
	return class$;
}
...
void CharBuffer::putBuffer(int32_t pos, CharBuffer* src, int32_t srcPos, int32_t n) {
	...
	if (src->isAddressable()) {
		...
		for (int32_t i = pos, j = srcPos; i < posMax; ++i, ++j) {
			put(i, src->get(j));
		}
	}
}
	...
	if (isAddressable() && ((int64_t)length << 1) > $Bits::JNI_COPY_FROM_ARRAY_THRESHOLD) {
		...
		try {
			if (order() != $ByteOrder::nativeOrder()) {
				$init($Buffer);
				$var($ScopedMemoryAccess$Scope, var$1, scope());
				$nc($Buffer::SCOPED_MEMORY_ACCESS)->copySwapMemory(nullptr, var$1, src, srcOffset, $(base()), bufAddr, len, $Character::BYTES);
			} else {
				$init($Buffer);
				$var($ScopedMemoryAccess$Scope, var$2, scope());
...
$Appendable* CharBuffer::append($CharSequence* csq, int32_t start, int32_t end) {
	$useLocalObjectStack();
	$var($CharSequence, cs, csq == nullptr ? $cast($CharSequence, "null"_s) : csq);
...
$IntStream* CharBuffer::chars() {
	return $StreamSupport::intStream($$new(CharBuffer$$Lambda$lambda$chars$0, this), $Buffer::SPLITERATOR_CHARACTERISTICS, false);
}
...
void CharBuffer::clinit$($Class* clazz) {
...
$Class* CharBuffer::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.nio.CharBuffer$$Lambda$lambda$chars$0")) {
	...
	$loadClass(CharBuffer, name, initialize, &classInfo$$, CharBuffer::clinit$, []($Class* clazz) -> $Object* {
