...
$Object* SelectionKey::attach(Object$* ob) {
	return $nc(SelectionKey::ATTACHMENT)->getAndSet($$new($ObjectArray, {this, ob}));
}
...
void SelectionKey::clinit$($Class* clazz) {
...
$Class* SelectionKey::load$($String* name, bool initialize) {
	...
	$loadClass(SelectionKey, name, initialize, &classInfo$$, SelectionKey::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SelectionKey);
	});
	return class$;
}
