...
void AsynchronousFileChannel::clinit$($Class* clazz) {
...
$Class* AsynchronousFileChannel::load$($String* name, bool initialize) {
	...
	$loadClass(AsynchronousFileChannel, name, initialize, &classInfo$$, AsynchronousFileChannel::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(AsynchronousFileChannel);
	});
	return class$;
}
