...
bool MappedMemoryUtils::isLoaded(int64_t address, bool isSync, int64_t size) {
	...
	return isLoaded0(var$0, length, $Bits::pageCount(length));
}
...
void MappedMemoryUtils::force($FileDescriptor* fd, int64_t address, bool isSync, int64_t index, int64_t length) {
	if (isSync) {
		...
		try {
			int64_t var$0 = mappingAddress(address, offset, index);
			force0(fd, var$0, mappingLength(offset, length));
...
bool MappedMemoryUtils::isLoaded0(int64_t address, int64_t length, int64_t pageCount) {
	$init(MappedMemoryUtils);
	$prepareNativeStatic(isLoaded0, bool, int64_t address, int64_t length, int64_t pageCount);
	bool $ret = $invokeNativeStatic(address, length, pageCount);
	$finishNativeStatic();
	return $ret;
}
...
void MappedMemoryUtils::load0(int64_t address, int64_t length) {
	$init(MappedMemoryUtils);
	$prepareNativeStatic(load0, void, int64_t address, int64_t length);
...
void MappedMemoryUtils::unload0(int64_t address, int64_t length) {
	$init(MappedMemoryUtils);
	$prepareNativeStatic(unload0, void, int64_t address, int64_t length);
	$invokeNativeStatic(address, length);
	$finishNativeStatic();
}
...
void MappedMemoryUtils::force0($FileDescriptor* fd, int64_t address, int64_t length) {
	$init(MappedMemoryUtils);
	$prepareNativeStatic(force0, void, $FileDescriptor* fd, int64_t address, int64_t length);
...
$Class* MappedMemoryUtils::load$($String* name, bool initialize) {
	...
	$loadClass(MappedMemoryUtils, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MappedMemoryUtils);
	});
	return class$;
}
