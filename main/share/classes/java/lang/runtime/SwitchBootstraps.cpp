...
$Class* SwitchBootstraps$$Lambda$verifyLabel::load$($String* name, bool initialize) {
	...
	$loadClass(SwitchBootstraps$$Lambda$verifyLabel, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SwitchBootstraps$$Lambda$verifyLabel);
	});
	return class$;
}
...
$Class* SwitchBootstraps$$Lambda$lambda$enumSwitch$0$1::load$($String* name, bool initialize) {
	...
	$loadClass(SwitchBootstraps$$Lambda$lambda$enumSwitch$0$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SwitchBootstraps$$Lambda$lambda$enumSwitch$0$1);
	});
	return class$;
}
...
$CallSite* SwitchBootstraps::typeSwitch($MethodHandles$Lookup* lookup, $String* invocationName, $MethodType* invocationType, $ObjectArray* labels$renamed) {
	...
	bool var$1 = var$2 || (!$$sure($Class, invocationType->returnType())->equals($Integer::TYPE));
	bool var$0 = var$1 || $$sure($Class, invocationType->parameterType(0))->isPrimitive();
	if (var$0 || !$$sure($Class, invocationType->parameterType(1))->equals($Integer::TYPE)) {
	...
	$$nc($Stream::of(labels))->forEach($$new(SwitchBootstraps$$Lambda$verifyLabel));
	...
	for (int32_t i = startIndex; i < $nc(labels)->length; ++i) {
		$var($Object0, label, labels->get(i));
		{
			...
			if (var$0) {
				...
				bool var$1 = $instanceOf($Integer, label);
				if (var$1) {
				...
				if (var$1) {
					...
					bool var$3 = $instanceOf($Number, target);
					if (var$3) {
					...
					bool var$2 = var$3;
					if (var$2) {
						int32_t var$4 = $nc(constant)->intValue();
						var$2 = var$4 == $nc(input)->intValue();
					}
					if (var$2) {
						return i;
					} else {
						bool var$6 = $instanceOf($Character, target);
						if (var$6) {
						...
						bool var$5 = var$6;
						if (var$5) {
							int32_t var$7 = $nc(constant)->intValue();
							var$5 = var$7 == $nc(input_1)->charValue();
						}
						if (var$5) {
							return i;
						}
					}
				} else if ($nc(label)->equals(target)) {
					return i;
				}
			}
		}
...
$CallSite* SwitchBootstraps::enumSwitch($MethodHandles$Lookup* lookup, $String* invocationName, $MethodType* invocationType, $ObjectArray* labels$renamed) {
	...
	bool var$2 = var$3 || (!$$sure($Class, invocationType->returnType())->equals($Integer::TYPE));
	bool var$1 = var$2 || $$sure($Class, invocationType->parameterType(0))->isPrimitive();
	bool var$0 = var$1 || !$$sure($Class, invocationType->parameterType(0))->isEnum();
	if (var$0 || !$$sure($Class, invocationType->parameterType(1))->equals($Integer::TYPE)) {
		$throwNew($IllegalArgumentException, $$str({"Illegal invocation type "_s, invocationType}));
	}
	$Objects::requireNonNull(labels);
	$assign(labels, $cast($ObjectArray, labels->clone()));
	$Class* enumClass = $cast($Class, invocationType->parameterType(0));
	$assign(labels, $$nc($$nc($Stream::of(labels))->map($$new(SwitchBootstraps$$Lambda$lambda$enumSwitch$0$1, lookup, enumClass)))->toArray());
...
$Object* SwitchBootstraps::lambda$enumSwitch$0($MethodHandles$Lookup* lookup, $Class* enumClass, Object$* l) {
	$init(SwitchBootstraps);
	return convertEnumConstants(lookup, enumClass, l);
}

void SwitchBootstraps::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(SwitchBootstraps::LOOKUP, $MethodHandles::lookup());
	{
		try {
			...
			$assignStatic(SwitchBootstraps::DO_ENUM_SWITCH, SwitchBootstraps::LOOKUP->findStatic(SwitchBootstraps::class$, "doEnumSwitch"_s, $($MethodType::methodType($Integer::TYPE, $Enum::class$, $$new($ClassArray, {
				$Integer::TYPE,
				$getClass($ObjectArray)
			})))));
...
$Class* SwitchBootstraps::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.lang.runtime.SwitchBootstraps$$Lambda$verifyLabel")) {
			return SwitchBootstraps$$Lambda$verifyLabel::load$(name, initialize);
		}
		if (name->equals("java.lang.runtime.SwitchBootstraps$$Lambda$lambda$enumSwitch$0$1")) {
	...
	$loadClass(SwitchBootstraps, name, initialize, &classInfo$$, SwitchBootstraps::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SwitchBootstraps);
	});
	return class$;
}
