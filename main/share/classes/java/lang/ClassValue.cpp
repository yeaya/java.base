...
void ClassValue::init$() {
	this->hashCodeForCache = ClassValue::nextHashCode->getAndAdd(ClassValue::HASH_INCREMENT) & ClassValue::HASH_MASK;
...
$Object* ClassValue::getFromHashMap($Class* type) {
	...
	for (;;) {
		...
		try {
			$var($ClassValue$Version, var$1, e->version());
...
void ClassValue::clinit$($Class* clazz) {
	$assignStatic(ClassValue::EMPTY_CACHE, $new($ClassValue$EntryArray, {nullptr}));
...
$Class* ClassValue::load$($String* name, bool initialize) {
	...
	$loadClass(ClassValue, name, initialize, &classInfo$$, ClassValue::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ClassValue);
	});
	return class$;
}
