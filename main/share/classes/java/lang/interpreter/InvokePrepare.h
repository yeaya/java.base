 ...
 */
	...
	$Array<Class>* typeArray = method->getSharedParameterTypes();
    int32_t typesLength = typeArray->length;
    Class** types = (Class**)typeArray->begin();
	$var($ObjectArray, args, $new<$ObjectArray>(typesLength));
	...
	for (int32_t index = 0; index < typesLength; index++) {
		...
		if (argType->isPrimitive()) {
			if (argType == Integer::TYPE) {
				...
				args->set(index, Float::valueOf(arg));
			} else if (argType == Double::TYPE) {
				double arg = va_arg(vargs, double);
				if (xmmIndex < 8) {
					int64_t i64 = xmmArgs[xmmIndex];
					arg = *(double*)&i64;
					xmmIndex++;
				}
				args->set(index, Double::valueOf(arg));
			}
		} else {
			Object* arg = va_arg(vargs, Object*);
			args->set(index, $fcast<Object>(arg));
		}
#endif
	}
    va_end(vargs);
