...
$URI* NamedPackage::location() {
	...
	if (var$0 && this->module$->getLayer() != nullptr) {
		$var($Configuration, cf, $$nc(this->module$->getLayer())->configuration());
		$var($ModuleReference, mref, $$sure($ResolvedModule, $$nc($nc(cf)->findModule($(this->module$->getName())))->get())->reference());
...
$Class* NamedPackage::load$($String* name, bool initialize) {
	...
	$loadClass(NamedPackage, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(NamedPackage);
	});
	return class$;
}
