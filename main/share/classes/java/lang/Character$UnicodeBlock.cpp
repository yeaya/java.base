...
Character$UnicodeBlock* Character$UnicodeBlock::of(int32_t codePoint) {
	...
	current = top / 2;
	while (top - bottom > 1) {
		if (codePoint >= Character$UnicodeBlock::blockStarts->get(current)) {
			bottom = current;
		} else {
			top = current;
		}
	...
	return Character$UnicodeBlock::blocks->get(current);
}
...
void Character$UnicodeBlock::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* Character$UnicodeBlock::load$($String* name, bool initialize) {
	$NamedAttribute SURROGATES_AREAfieldAnnotations$$$namedAttribute[] = {
		{"since", 's', "1.5"},
		{}
	};
	$CompoundAttribute SURROGATES_AREAfieldAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", SURROGATES_AREAfieldAnnotations$$$namedAttribute},
		{}
	};
	$FieldInfo fieldInfos$$[] = {
		...
		{"SURROGATES_AREA", "Ljava/lang/Character$UnicodeBlock;", nullptr, $PUBLIC | $STATIC | $FINAL | $DEPRECATED, $staticField(Character$UnicodeBlock, SURROGATES_AREA), SURROGATES_AREAfieldAnnotations$$},
	...
	$loadClass(Character$UnicodeBlock, name, initialize, &classInfo$$, Character$UnicodeBlock::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Character$UnicodeBlock);
	});
	return class$;
}
