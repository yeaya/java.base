...
void ProxyGenerator$ProxyMethod::init$($Method* method, $String* methodFieldName) {
	$useLocalObjectStack();
	$var($String, var$0, $nc(method)->toShortSignature());
	$var($ClassArray, var$1, method->getParameterTypes());
	$Class* var$2 = method->getReturnType();
	$var($ClassArray, var$3, method->getExceptionTypes());
	ProxyGenerator$ProxyMethod::init$(method, var$0, var$1, var$2, var$3, method->getDeclaringClass(), methodFieldName);
}
...
void ProxyGenerator$ProxyMethod::generateMethod($ClassWriter* cw, $String* className) {
	...
	int32_t accessFlags = 1 | 0x10;
	if ($nc(this->method)->isVarArgs()) {
		accessFlags |= 128;
	}
	$var($String, var$0, this->method->getName());
	$var($MethodVisitor, mv, $nc(cw)->visitMethod(accessFlags, var$0, desc, nullptr, $($ProxyGenerator::typeNames($($Arrays::asList(this->exceptionTypes))))));
	...
	for (int32_t i = 0; i < parameterSlot->length; ++i) {
		...
		nextSlot += $ProxyGenerator::getWordsPerType(this->parameterTypes->get(i));
	}
	$nc(mv)->visitCode();
	...
	if (this->parameterTypes->length > 0) {
		emitIconstInsn(mv, this->parameterTypes->length);
		mv->visitTypeInsn($Opcodes::ANEWARRAY, "java/lang/Object"_s);
		for (int32_t i = 0; i < this->parameterTypes->length; ++i) {
			mv->visitInsn(89);
			emitIconstInsn(mv, i);
			codeWrapArgument(mv, this->parameterTypes->get(i), parameterSlot->get(i));
...
void ProxyGenerator$ProxyMethod::codeWrapArgument($MethodVisitor* mv, $Class* type, int32_t slot) {
	if ($nc(type)->isPrimitive()) {
		...
		$nc(mv)->visitMethodInsn(184, $nc(prim)->wrapperClassName, "valueOf"_s, $nc(prim)->wrapperValueOfDesc, false);
	} else {
...
void ProxyGenerator$ProxyMethod::codeFieldInitialization($MethodVisitor* mv, $String* className) {
	...
	for (int32_t i = 0; i < this->parameterTypes->length; ++i) {
		...
		if ($nc(this->parameterTypes->get(i))->isPrimitive()) {
			$var($ProxyGenerator$PrimitiveTypeInfo, prim, $ProxyGenerator$PrimitiveTypeInfo::get(this->parameterTypes->get(i)));
			mv->visitFieldInsn(178, $nc(prim)->wrapperClassName, "TYPE"_s, "Ljava/lang/Class;"_s);
		} else {
			codeClassForName(mv, this->parameterTypes->get(i));
		}
		mv->visitInsn($Opcodes::AASTORE);
	}
...
$Class* ProxyGenerator$ProxyMethod::load$($String* name, bool initialize) {
	...
	$loadClass(ProxyGenerator$ProxyMethod, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ProxyGenerator$ProxyMethod);
	});
	return class$;
}
