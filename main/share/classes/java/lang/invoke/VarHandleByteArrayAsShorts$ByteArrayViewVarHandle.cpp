...
$Class* VarHandleByteArrayAsShorts$ByteArrayViewVarHandle::load$($String* name, bool initialize) {
	...
	$loadClass(VarHandleByteArrayAsShorts$ByteArrayViewVarHandle, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(VarHandleByteArrayAsShorts$ByteArrayViewVarHandle);
	});
	return class$;
}
