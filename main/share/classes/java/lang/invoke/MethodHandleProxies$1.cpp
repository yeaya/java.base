...
$Object* MethodHandleProxies$1::invoke(Object$* proxy, $Method* method, $ObjectArray* args) {
	...
	for (int32_t i = 0; i < $nc(this->val$methods)->length; ++i) {
		if ($nc(method)->equals(this->val$methods->get(i))) {
			return $nc($nc(this->val$vaTargets)->get(i))->invokeExact(args);
		}
	}
	...
	if ($MethodHandleProxies::isDefaultMethod(method)) {
		return $MethodHandleProxies::callDefaultMethod(this->val$defaultMethodMap, proxy, this->val$intfc, method, args);
	}
...
$Class* MethodHandleProxies$1::load$($String* name, bool initialize) {
	...
	$loadClass(MethodHandleProxies$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MethodHandleProxies$1);
	});
	return class$;
}
