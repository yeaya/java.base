...
$MethodHandle* MethodTypeForm::cachedMethodHandle(int32_t which) {
	...
	return (entry != nullptr) ? $cast($MethodHandle, entry->get()) : ($MethodHandle*)nullptr;
}
...
$MethodHandle* MethodTypeForm::setCachedMethodHandle(int32_t which, $MethodHandle* mh) {
	$synchronized(this) {
		...
		this->methodHandles->set(which, $$new($SoftReference, mh));
		return mh;
	}
}
...
$LambdaForm* MethodTypeForm::cachedLambdaForm(int32_t which) {
	...
	return (entry != nullptr) ? $cast($LambdaForm, entry->get()) : ($LambdaForm*)nullptr;
}
...
$LambdaForm* MethodTypeForm::setCachedLambdaForm(int32_t which, $LambdaForm* form) {
	$synchronized(this) {
		...
		this->lambdaForms->set(which, $$new($SoftReference, form));
		return form;
	}
}
...
void MethodTypeForm::init$($MethodType* erasedType) {
	...
	for (int32_t i = 0; i < erasedPtypes->length; ++i) {
		...
		if (ptype != $Object::class$) {
			...
			if (w->isSubwordOrInt() && ptype != $Integer::TYPE) {
...
$ClassArray* MethodTypeForm::canonicalizeAll($ClassArray* ts, int32_t how) {
	...
	for (int32_t imax = $nc(ts)->length, i = 0; i < imax; ++i) {
...
void MethodTypeForm::clinit$($Class* clazz) {
...
$Class* MethodTypeForm::load$($String* name, bool initialize) {
	...
	$loadClass(MethodTypeForm, name, initialize, &classInfo$$, MethodTypeForm::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(MethodTypeForm);
	});
	return class$;
}
