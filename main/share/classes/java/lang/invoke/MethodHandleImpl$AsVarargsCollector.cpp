...
$MethodHandle* MethodHandleImpl$AsVarargsCollector::asTypeUncached($MethodType* newType) {
	...
	if (newArity == collectArg + 1 && $$sure($Class, type->parameterType(collectArg))->isAssignableFrom($$cast($Class, newType->parameterType(collectArg)))) {
...
$Object* MethodHandleImpl$AsVarargsCollector::invokeWithArguments($ObjectArray* arguments) {
	...
	bool var$0 = arguments == nullptr || (argc = arguments->length) <= MAX_SAFE;
	if (var$0 || argc < $nc(type)->parameterCount()) {
		return $DelegatingMethodHandle::invokeWithArguments(arguments);
	}
	...
	return $$nc(asFixedArity())->invokeWithArguments(newArgs);
}

void MethodHandleImpl$AsVarargsCollector::clinit$($Class* clazz) {
...
$Class* MethodHandleImpl$AsVarargsCollector::load$($String* name, bool initialize) {
	$CompoundAttribute asCollectorCachefieldAnnotations$$[] = {
	...
	$FieldInfo fieldInfos$$[] = {
		...
		{"asCollectorCache", "Ljava/lang/invoke/MethodHandle;", nullptr, $PRIVATE, $field(MethodHandleImpl$AsVarargsCollector, asCollectorCache), asCollectorCachefieldAnnotations$$},
		{}
	};
	...
	$loadClass(MethodHandleImpl$AsVarargsCollector, name, initialize, &classInfo$$, MethodHandleImpl$AsVarargsCollector::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(MethodHandleImpl$AsVarargsCollector);
	});
	return class$;
}
