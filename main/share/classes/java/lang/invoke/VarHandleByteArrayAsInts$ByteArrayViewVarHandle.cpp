...
$Class* VarHandleByteArrayAsInts$ByteArrayViewVarHandle::load$($String* name, bool initialize) {
	...
	$loadClass(VarHandleByteArrayAsInts$ByteArrayViewVarHandle, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(VarHandleByteArrayAsInts$ByteArrayViewVarHandle);
	});
	return class$;
}
