...
ProxyClassesDumper* ProxyClassesDumper::getInstance($String* path$renamed) {
	...
	try {
		...
		$var($PrivilegedAction, var$0, $new($ProxyClassesDumper$1, dir));
		$AccessController::doPrivileged(var$0, nullptr, $$new($PermissionArray, {$$new($FilePermission, "<<ALL FILES>>"_s, "read, write"_s)}));
		return $new(ProxyClassesDumper, dir);
	} catch ($InvalidPathException& ex) {
		$$nc($PlatformLogger::getLogger($(ProxyClassesDumper::class$->getName())))->warning($$str({"Path "_s, path, " is not valid - dumping disabled"_s}), ex);
...
$String* ProxyClassesDumper::encodeForFilename($String* className) {
	...
	for (int32_t i = 0; i < len; ++i) {
		...
		if (c <= 31) {
			sb->append(u'%');
			sb->append(ProxyClassesDumper::HEX->get((c >> 4) & 0x0f));
			sb->append(ProxyClassesDumper::HEX->get(c & 0x0f));
		} else {
			int32_t j = 0;
			for (; j < ProxyClassesDumper::BAD_CHARS->length; ++j) {
				if (c == ProxyClassesDumper::BAD_CHARS->get(j)) {
					sb->append(ProxyClassesDumper::REPLACEMENT->get(j));
					break;
				}
			}
...
void ProxyClassesDumper::clinit$($Class* clazz) {
...
$Class* ProxyClassesDumper::load$($String* name, bool initialize) {
	...
	$loadClass(ProxyClassesDumper, name, initialize, &classInfo$$, ProxyClassesDumper::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ProxyClassesDumper);
	});
	return class$;
}
