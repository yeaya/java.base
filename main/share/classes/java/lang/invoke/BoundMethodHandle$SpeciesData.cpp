...
void BoundMethodHandle$SpeciesData::init$($BoundMethodHandle$Specializer* outer, $String* key) {
	$ClassSpecializer$SpeciesData::init$($nc(outer), key);
...
BoundMethodHandle$SpeciesData* BoundMethodHandle$SpeciesData::extendWith(int8_t typeNum) {
	$useLocalObjectStack();
	$var(BoundMethodHandle$SpeciesData, sd, this->extensions->get(typeNum));
	if (sd != nullptr) {
		return sd;
	}
	$init($BoundMethodHandle);
	$var($StringBuilder, var$0, $new($StringBuilder));
	var$0->append($$cast($String, key()));
	var$0->append($$nc($LambdaForm$BasicType::basicType(typeNum))->basicTypeChar());
	$assign(sd, $cast(BoundMethodHandle$SpeciesData, $nc($BoundMethodHandle::SPECIALIZER)->findSpecies($$str(var$0))));
	this->extensions->set(typeNum, sd);
	return sd;
}
...
void BoundMethodHandle$SpeciesData::clinit$($Class* clazz) {
	$load($BoundMethodHandle);
...
$Class* BoundMethodHandle$SpeciesData::load$($String* name, bool initialize) {
	$CompoundAttribute extensionsfieldAnnotations$$[] = {
	...
	$FieldInfo fieldInfos$$[] = {
		...
		{"extensions", "[Ljava/lang/invoke/BoundMethodHandle$SpeciesData;", nullptr, $PRIVATE | $FINAL, $field(BoundMethodHandle$SpeciesData, extensions), extensionsfieldAnnotations$$},
		{}
	};
	...
	$loadClass(BoundMethodHandle$SpeciesData, name, initialize, &classInfo$$, BoundMethodHandle$SpeciesData::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(BoundMethodHandle$SpeciesData);
	});
	return class$;
}
