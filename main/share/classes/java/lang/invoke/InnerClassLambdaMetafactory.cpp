...
void InnerClassLambdaMetafactory::init$($MethodHandles$Lookup* caller, $MethodType* factoryType, $String* interfaceMethodName, $MethodType* interfaceMethodType, $MethodHandle* implementation, $MethodType* dynamicMethodType, bool isSerializable, $ClassArray* altInterfaces, $MethodTypeArray* altMethods) {
	...
	$set(this, implMethodDesc, $$nc(this->implInfo->getMethodType())->toMethodDescriptorString());
	...
	bool var$0 = $Modifier::isProtected(this->implInfo->getModifiers());
	this->useImplMethodHandle = (var$0 && !$VerifyAccess::isSamePackage(this->targetClass, this->implInfo->getDeclaringClass())) || this->implKind == 7;
	...
	if (parameterCount > 0) {
		...
		for (int32_t i = 0; i < parameterCount; ++i) {
			this->argNames->set(i, $$str({"arg$"_s, $$str((i + 1))}));
			this->argDescs->set(i, $($BytecodeDescriptor::unparse($$cast($Class, factoryType->parameterType(i)))));
		}
	} else {
...
$String* InnerClassLambdaMetafactory::lambdaClassName($Class* targetClass) {
	...
	var$0->append($($nc(name)->replace(u'.', u'/')));
	var$0->append("$$Lambda$"_s);
	var$0->append(InnerClassLambdaMetafactory::counter->incrementAndGet());
	return $str(var$0);
}
...
$CallSite* InnerClassLambdaMetafactory::buildCallSite() {
	...
	if ($nc(this->factoryType)->parameterCount() == 0) {
		if (InnerClassLambdaMetafactory::disableEagerInitialization) {
			try {
				return $new($ConstantCallSite, $($nc(this->caller)->findStaticGetter(innerClass, InnerClassLambdaMetafactory::LAMBDA_INSTANCE_FIELD, $$cast($Class, this->factoryType->returnType()))));
			...
			$var($ConstructorArray, ctrs, $cast($ConstructorArray, $AccessController::doPrivileged($$new($InnerClassLambdaMetafactory$1, this, innerClass))));
			if ($nc(ctrs)->length != 1) {
			...
			try {
				$var($Object, inst, $nc(ctrs->get(0))->newInstance($$new($ObjectArray, 0)));
...
$Class* InnerClassLambdaMetafactory::generateInnerClass() {
	...
	if ($nc(this->altInterfaces)->length == 0) {
		...
		$var($Set, itfs, $new($LinkedHashSet, this->altInterfaces->length + 1));
		...
		$assign(interfaceNames, $cast($StringArray, itfs->toArray($$new($StringArray, itfs->size()))));
	}
	this->cw->visit(InnerClassLambdaMetafactory::CLASSFILE_VERSION, 32 + 16 + 4096, this->lambdaClassName$, nullptr, InnerClassLambdaMetafactory::JAVA_LANG_OBJECT, interfaceNames);
	for (int32_t i = 0; i < $nc(this->argDescs)->length; ++i) {
		$var($FieldVisitor, fv, this->cw->visitField(2 + 16, $nc(this->argNames)->get(i), this->argDescs->get(i), nullptr, nullptr));
		$nc(fv)->visitEnd();
	}
	generateConstructor();
	...
	$var($MethodVisitor, mv, this->cw->visitMethod(1, this->interfaceMethodName, $($nc(this->interfaceMethodType)->toMethodDescriptorString()), nullptr, nullptr));
	...
	if (this->altMethods != nullptr) {
		...
		for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
			...
			{
				$assign(mv, this->cw->visitMethod(1, this->interfaceMethodName, $($nc(mt)->toMethodDescriptorString()), nullptr, nullptr));
	...
	this->cw->visitEnd();
	$var($bytes, classBytes, this->cw->toByteArray());
	if (InnerClassLambdaMetafactory::dumper != nullptr) {
		$var($PrivilegedAction, var$0, $new($InnerClassLambdaMetafactory$2, this, classBytes));
		$AccessController::doPrivileged(var$0, nullptr, $$new($PermissionArray, {
			$$new($FilePermission, "<<ALL FILES>>"_s, "read, write"_s),
			$$new($PropertyPermission, "user.dir"_s, "read"_s)
		}));
	}
	try {
...
void InnerClassLambdaMetafactory::generateClassInitializer() {
	$useLocalObjectStack();
	$var($String, lambdaTypeDescriptor, $$sure($Class, $nc(this->factoryType)->returnType())->descriptorString());
	$var($FieldVisitor, fv, this->cw->visitField((2 | 8) | 0x10, InnerClassLambdaMetafactory::LAMBDA_INSTANCE_FIELD, lambdaTypeDescriptor, nullptr, nullptr));
	$nc(fv)->visitEnd();
	$var($MethodVisitor, clinit, this->cw->visitMethod(8, "<clinit>"_s, "()V"_s, nullptr, nullptr));
	$nc(clinit)->visitCode();
	clinit->visitTypeInsn(187, this->lambdaClassName$);
	clinit->visitInsn($Opcodes::DUP);
	if (!InnerClassLambdaMetafactory::$assertionsDisabled && !(this->factoryType->parameterCount() == 0)) {
...
void InnerClassLambdaMetafactory::generateConstructor() {
	$useLocalObjectStack();
	$var($MethodVisitor, ctor, this->cw->visitMethod(2, InnerClassLambdaMetafactory::NAME_CTOR, $($nc(this->constructorType)->toMethodDescriptorString()), nullptr, nullptr));
	$nc(ctor)->visitCode();
	...
	for (int32_t i = 0, lvIndex = 0; i < parameterCount; ++i) {
		ctor->visitVarInsn(25, 0);
		$Class* argType = $cast($Class, this->factoryType->parameterType(i));
...
void InnerClassLambdaMetafactory::generateSerializationFriendlyMethods() {
	$useLocalObjectStack();
	$var($TypeConvertingMethodAdapter, mv, $new($TypeConvertingMethodAdapter, $(this->cw->visitMethod(2 + 16, InnerClassLambdaMetafactory::NAME_METHOD_WRITE_REPLACE, InnerClassLambdaMetafactory::DESCR_METHOD_WRITE_REPLACE, nullptr, nullptr))));
	mv->visitCode();
	mv->visitTypeInsn(187, InnerClassLambdaMetafactory::NAME_SERIALIZED_LAMBDA);
	mv->visitInsn(89);
	mv->visitLdcInsn($($Type::getType(this->targetClass)));
	mv->visitLdcInsn($($$nc($$sure($Class, $nc(this->factoryType)->returnType())->getName())->replace(u'.', u'/')));
	...
	mv->visitLdcInsn($($$nc($nc(this->implInfo->getDeclaringClass())->getName())->replace(u'.', u'/')));
	mv->visitLdcInsn($(this->implInfo->getName()));
	mv->visitLdcInsn($($$nc(this->implInfo->getMethodType())->toMethodDescriptorString()));
	...
	for (int32_t i = 0; i < this->argDescs->length; ++i) {
		mv->visitInsn(89);
		mv->iconst(i);
		mv->visitVarInsn(25, 0);
		mv->visitFieldInsn(180, this->lambdaClassName$, $nc(this->argNames)->get(i), this->argDescs->get(i));
		mv->boxIfTypePrimitive($($Type::getType(this->argDescs->get(i))));
		mv->visitInsn(83);
	}
...
void InnerClassLambdaMetafactory::generateSerializationHostileMethods() {
	$var($MethodVisitor, mv, this->cw->visitMethod(2 + 16, InnerClassLambdaMetafactory::NAME_METHOD_WRITE_OBJECT, InnerClassLambdaMetafactory::DESCR_METHOD_WRITE_OBJECT, nullptr, InnerClassLambdaMetafactory::SER_HOSTILE_EXCEPTIONS));
	$nc(mv)->visitCode();
	...
	mv->visitMaxs(-1, -1);
	mv->visitEnd();
	$assign(mv, this->cw->visitMethod(2 + 16, InnerClassLambdaMetafactory::NAME_METHOD_READ_OBJECT, InnerClassLambdaMetafactory::DESCR_METHOD_READ_OBJECT, nullptr, InnerClassLambdaMetafactory::SER_HOSTILE_EXCEPTIONS));
	$nc(mv)->visitCode();
	mv->visitTypeInsn(187, InnerClassLambdaMetafactory::NAME_NOT_SERIALIZABLE_EXCEPTION);
	mv->visitInsn(89);
	mv->visitLdcInsn("Non-serializable lambda"_s);
...
void InnerClassLambdaMetafactory::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* InnerClassLambdaMetafactory::load$($String* name, bool initialize) {
	...
	$loadClass(InnerClassLambdaMetafactory, name, initialize, &classInfo$$, InnerClassLambdaMetafactory::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(InnerClassLambdaMetafactory);
	});
	return class$;
}
