...
$Class* StackStreamFactory$FrameBuffer::load$($String* name, bool initialize) {
	...
	$loadClass(StackStreamFactory$FrameBuffer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(StackStreamFactory$FrameBuffer);
	});
	return class$;
}
