...
$Class* MethodTypeDesc$$Lambda$descriptorString::load$($String* name, bool initialize) {
	...
	$loadClass(MethodTypeDesc$$Lambda$descriptorString, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MethodTypeDesc$$Lambda$descriptorString);
	});
	return class$;
}
...
$Class* MethodTypeDesc$$Lambda$displayName$1::load$($String* name, bool initialize) {
	...
	$loadClass(MethodTypeDesc$$Lambda$displayName$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MethodTypeDesc$$Lambda$displayName$1);
	});
	return class$;
}
...
$String* MethodTypeDesc::descriptorString() {
	...
	return $String::format("(%s)%s"_s, $$new($ObjectArray, {
		$($$nc($$nc($Stream::of($(parameterArray())))->map($$new(MethodTypeDesc$$Lambda$descriptorString)))->collect($($Collectors::joining()))),
		$($$sure($ClassDesc, returnType())->descriptorString())
	}));
}
...
$String* MethodTypeDesc::displayDescriptor() {
	$useLocalObjectStack();
	return $String::format("(%s)%s"_s, $$new($ObjectArray, {
		$($$nc($$nc($Stream::of($(parameterArray())))->map($$new(MethodTypeDesc$$Lambda$displayName$1)))->collect($($Collectors::joining(","_s)))),
		$($$sure($ClassDesc, returnType())->displayName())
	}));
}
...
$Class* MethodTypeDesc::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.lang.constant.MethodTypeDesc$$Lambda$descriptorString")) {
			return MethodTypeDesc$$Lambda$descriptorString::load$(name, initialize);
		}
		if (name->equals("java.lang.constant.MethodTypeDesc$$Lambda$displayName$1")) {
	...
	$loadClass(MethodTypeDesc, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
