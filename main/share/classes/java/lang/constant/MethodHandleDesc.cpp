...
MethodHandleDesc* MethodHandleDesc::asType($MethodTypeDesc* type) {
	return ($$nc(invocationType())->equals(type)) ? this : $cast(MethodHandleDesc, $new($AsTypeMethodHandleDesc, this, type));
}
...
$Class* MethodHandleDesc::load$($String* name, bool initialize) {
	...
	$loadClass(MethodHandleDesc, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MethodHandleDesc);
	});
	return class$;
}
