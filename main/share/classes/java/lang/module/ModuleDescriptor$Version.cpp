...
int32_t ModuleDescriptor$Version::compareTokens($List* ts1, $List* ts2) {
	...
	for (int32_t i = 0; i < n; ++i) {
		$var($Object, o1, ts1->get(i));
		$var($Object, o2, ts2->get(i));
		...
		int32_t c = $$nc($nc(o1)->toString())->compareTo($($nc(o2)->toString()));
		if (c == 0) {
			continue;
		}
		return c;
	}
	int32_t var$1 = ts1->size();
	$var($List, rest, var$1 > ts2->size() ? ts1 : ts2);
	...
	for (int32_t i = n; i < e; ++i) {
		...
		int32_t var$2 = ts1->size();
		return var$2 - ts2->size();
	}
	return 0;
}
...
$Class* ModuleDescriptor$Version::load$($String* name, bool initialize) {
	...
	$loadClass(ModuleDescriptor$Version, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ModuleDescriptor$Version);
	});
	return class$;
}
