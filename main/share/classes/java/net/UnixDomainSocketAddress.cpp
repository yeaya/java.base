...
$Object* UnixDomainSocketAddress::writeReplace() {
	return $new($UnixDomainSocketAddress$Ser, $($nc(this->path)->toString()));
}
...
$Class* UnixDomainSocketAddress::load$($String* name, bool initialize) {
	...
	$loadClass(UnixDomainSocketAddress, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(UnixDomainSocketAddress);
	});
	return class$;
}
