	...
	}
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($HttpCookie, cookie, $cast($HttpCookie, i$->next()));
			{
				...
				if (var$0 && (secureLink || !cookie->getSecure())) {
	...
	$var($PlatformLogger, logger, $PlatformLogger::getLogger("java.net.CookieManager"_s));
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($String, headerKey, $cast($String, i$->next()));
			{
				...
				if (!var$0) {
					bool var$1 = headerKey->equalsIgnoreCase("Set-Cookie2"_s);
					var$0 = !(var$1 || headerKey->equalsIgnoreCase("Set-Cookie"_s));
				}
				if (var$0) {
					continue;
				}
				{
					$var($Iterator, i$, $$sure($List, responseHeaders->get(headerKey))->iterator());
					for (; $nc(i$)->hasNext();) {
									...
									$var($HttpCookie, cookie, $cast($HttpCookie, i$->next()));
									{
										...
										if (ports != nullptr) {
											...
											if (ports->isEmpty()) {
												...
												bool var$2 = isInPortList(ports, port);
												if (var$2 && shouldAcceptInternal(uri, cookie)) {
													$nc(this->cookieJar)->add(uri, cookie);
												}
											}
...
$Class* CookieManager::load$($String* name, bool initialize) {
	...
	$loadClass(CookieManager, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CookieManager);
	});
	return class$;
}
