...
void URLPermission$Authority::init$($String* scheme, $String* authority) {
	int32_t at = $nc(authority)->indexOf(u'@');
	if (at == -1) {
...
bool URLPermission$Authority::impliesPortrange(URLPermission$Authority* that) {
	...
	return thisrange->get(0) <= $nc(thatrange)->get(0) && thisrange->get(1) >= thatrange->get(1);
}
...
$Class* URLPermission$Authority::load$($String* name, bool initialize) {
	...
	$loadClass(URLPermission$Authority, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(URLPermission$Authority);
	});
	return class$;
}
