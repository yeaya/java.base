...
void Socket::init$($Proxy* proxy) {
	...
	if (type == $Proxy$Type::SOCKS || type == $Proxy$Type::HTTP) {
		...
		$set(this, impl, (type == $Proxy$Type::SOCKS) ? $cast($SocketImpl, $new($SocksSocketImpl, p, delegate)) : $cast($SocketImpl, $new($HttpConnectSocketImpl, p, delegate, this)));
...
void Socket::init$($String* host, int32_t port) {
	$useLocalObjectStack();
	Socket::init$(host != nullptr ? $$new($InetSocketAddress, host, port) : $$new($InetSocketAddress, $($InetAddress::getByName(nullptr)), port), ($SocketAddress*)nullptr, true);
}

void Socket::init$($InetAddress* address, int32_t port) {
	Socket::init$(address != nullptr ? $$cast($SocketAddress, $new($InetSocketAddress, address, port)) : ($SocketAddress*)nullptr, ($SocketAddress*)nullptr, true);
}

void Socket::init$($String* host, int32_t port, $InetAddress* localAddr, int32_t localPort) {
	$useLocalObjectStack();
	$var($SocketAddress, var$0, host != nullptr ? $new($InetSocketAddress, host, port) : $new($InetSocketAddress, $($InetAddress::getByName(nullptr)), port));
	Socket::init$(var$0, $$new($InetSocketAddress, localAddr, localPort), true);
}

void Socket::init$($InetAddress* address, int32_t port, $InetAddress* localAddr, int32_t localPort) {
	$useLocalObjectStack();
	$var($SocketAddress, var$0, address != nullptr ? $cast($SocketAddress, $new($InetSocketAddress, address, port)) : ($SocketAddress*)nullptr);
	Socket::init$(var$0, $$new($InetSocketAddress, localAddr, localPort), true);
}

void Socket::init$($String* host, int32_t port, bool stream) {
	$useLocalObjectStack();
	Socket::init$(host != nullptr ? $$new($InetSocketAddress, host, port) : $$new($InetSocketAddress, $($InetAddress::getByName(nullptr)), port), ($SocketAddress*)nullptr, stream);
}

void Socket::init$($InetAddress* host, int32_t port, bool stream) {
	$useLocalObjectStack();
	$var($SocketAddress, var$0, host != nullptr ? $cast($SocketAddress, $new($InetSocketAddress, host, port)) : ($SocketAddress*)nullptr);
	Socket::init$(var$0, $$new($InetSocketAddress, 0), stream);
}
...
void Socket::connect($SocketAddress* endpoint, int32_t timeout) {
	...
	$nc(this->impl)->connect(epoint, timeout);
	this->connected = true;
	this->bound = true;
}
...
$InputStream* Socket::getInputStream() {
	...
	if (in == nullptr) {
		...
		if (!$nc(Socket::IN)->compareAndSet($$new($ObjectArray, {this, nullptr, in}))) {
			$assign(in, this->in);
		}
	}
	return in;
...
$OutputStream* Socket::getOutputStream() {
	...
	if (out == nullptr) {
		...
		if (!$nc(Socket::OUT)->compareAndSet($$new($ObjectArray, {this, nullptr, out}))) {
			$assign(out, this->out);
		}
	}
	return out;
...
bool Socket::getTcpNoDelay() {
	...
	return $$sure($Boolean, $$nc(getImpl())->getOption($SocketOptions::TCP_NODELAY))->booleanValue();
}
...
bool Socket::getOOBInline() {
	...
	return $$sure($Boolean, $$nc(getImpl())->getOption($SocketOptions::SO_OOBINLINE))->booleanValue();
}
...
bool Socket::getKeepAlive() {
	...
	return $$sure($Boolean, $$nc(getImpl())->getOption($SocketOptions::SO_KEEPALIVE))->booleanValue();
}
...
$String* Socket::toString() {
	...
	try {
		if (isConnected()) {
			...
			var$0->append($($$nc(getImpl())->getInetAddress()));
			var$0->append(",port="_s);
			var$0->append($$nc(getImpl())->getPort());
			var$0->append(",localport="_s);
			var$0->append($$nc(getImpl())->getLocalPort());
			var$0->append("]"_s);
			return $str(var$0);
		}
	} catch ($SocketException& e) {
...
void Socket::clinit$($Class* clazz) {
...
$Class* Socket::load$($String* name, bool initialize) {
	...
	$NamedAttribute setSocketImplFactorymethodAnnotations$$$namedAttribute[] = {
		{"since", 's', "17"},
		{}
	};
	$CompoundAttribute setSocketImplFactorymethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", setSocketImplFactorymethodAnnotations$$$namedAttribute},
		{}
	};
	$MethodInfo methodInfos$$[] = {
		...
		{"<init>", "(Ljava/lang/String;IZ)V", nullptr, $PUBLIC | $DEPRECATED, $method(Socket, init$, void, $String*, int32_t, bool), "java.io.IOException", nullptr, init$methodAnnotations$$$7},
		{"<init>", "(Ljava/net/InetAddress;IZ)V", nullptr, $PUBLIC | $DEPRECATED, $method(Socket, init$, void, $InetAddress*, int32_t, bool), "java.io.IOException", nullptr, init$methodAnnotations$$$8},
		...
		{"setSocketImplFactory", "(Ljava/net/SocketImplFactory;)V", nullptr, $PUBLIC | $STATIC | $SYNCHRONIZED | $DEPRECATED, $staticMethod(Socket, setSocketImplFactory, void, $SocketImplFactory*), "java.io.IOException", nullptr, setSocketImplFactorymethodAnnotations$$},
	...
	$loadClass(Socket, name, initialize, &classInfo$$, Socket::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Socket);
	});
	return class$;
}
