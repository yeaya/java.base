	...
	while ((l = $nc(t)->lo) >= 0 && (h = t->hi) <= $nc(a)->length) {
		if (h - l > th) {
			...
			if (lt == nullptr) {
				...
				$assign(f, ($assign(rt, ($assignField(t, right, $new(ArrayPrefixHelpers$DoubleCumulateTask, t, fn, a, org, fnc, th, mid, h))))));
				$assign(t, ($assign(lt, ($assignField(t, left, $new(ArrayPrefixHelpers$DoubleCumulateTask, t, fn, a, org, fnc, th, l, mid))))));
			} else {
				double pin = t->in;
				$nc(lt)->in = pin;
