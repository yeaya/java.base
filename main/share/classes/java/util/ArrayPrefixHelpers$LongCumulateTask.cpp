	...
	while ((l = $nc(t)->lo) >= 0 && (h = t->hi) <= $nc(a)->length) {
		if (h - l > th) {
			...
			if (lt == nullptr) {
				...
				$assign(f, ($assign(rt, ($assignField(t, right, $new(ArrayPrefixHelpers$LongCumulateTask, t, fn, a, org, fnc, th, mid, h))))));
				$assign(t, ($assign(lt, ($assignField(t, left, $new(ArrayPrefixHelpers$LongCumulateTask, t, fn, a, org, fnc, th, l, mid))))));
			} else {
				int64_t pin = t->in;
				$nc(lt)->in = pin;
