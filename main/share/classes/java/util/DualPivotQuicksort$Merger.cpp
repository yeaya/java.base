...
void DualPivotQuicksort$Merger::compute() {
	...
	if ($instanceOf($ints, this->dst)) {
		...
		$throwNew($IllegalArgumentException, $$str({"Unknown type of array: "_s, $($nc(this->dst)->getClass()->getName())}));
	}
	propagateCompletion();
}
...
$Class* DualPivotQuicksort$Merger::load$($String* name, bool initialize) {
	...
	$loadClass(DualPivotQuicksort$Merger, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
