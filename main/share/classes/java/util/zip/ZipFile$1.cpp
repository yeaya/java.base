...
$List* ZipFile$1::getManifestAndSignatureRelatedFiles($JarFile* jar) {
	return $nc($cast($ZipFile, jar))->getManifestAndSignatureRelatedFiles();
}

int32_t ZipFile$1::getManifestNum($JarFile* jar) {
	return $nc($cast($ZipFile, jar))->getManifestNum();
}

$String* ZipFile$1::getManifestName($JarFile* jar, bool onlyIfHasSignatureRelatedFiles) {
	return $nc($cast($ZipFile, jar))->getManifestName(onlyIfHasSignatureRelatedFiles);
}

$ints* ZipFile$1::getMetaInfVersions($JarFile* jar) {
	return $nc($cast($ZipFile, jar))->getMetaInfVersions();
}
...
$Class* ZipFile$1::load$($String* name, bool initialize) {
	...
	$loadClass(ZipFile$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ZipFile$1);
	});
	return class$;
}
