...
int32_t ZipUtils::get16($bytes* b, int32_t off) {
	$init(ZipUtils);
	return ($nc(b)->get(off) & 0xff) | (($nc(b)->get(off + 1) & 0xff) << 8);
}

int64_t ZipUtils::get32($bytes* b, int32_t off) {
	$init(ZipUtils);
	int32_t var$0 = get16(b, off);
	return (var$0 | ((int64_t)get16(b, off + 2) << 16)) & (int64_t)0xffffffff;
}
...
int32_t ZipUtils::CH($bytes* b, int32_t n) {
	$init(ZipUtils);
	return $nc(b)->get(n) & 0xff;
}

int32_t ZipUtils::SH($bytes* b, int32_t n) {
	$init(ZipUtils);
	return ($nc(b)->get(n) & 0xff) | (($nc(b)->get(n + 1) & 0xff) << 8);
}
...
void ZipUtils::clinit$($Class* clazz) {
	...
	ZipUtils::byteBufferOffsetOffset = ZipUtils::unsafe->objectFieldOffset($ByteBuffer::class$, "offset"_s);
}
