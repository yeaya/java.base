...
int32_t ZipFile$Source::checkAndAddEntry(int32_t pos, int32_t index) {
	...
	if ($ZipUtils::CENSIG(cen, pos) != 33639248) {
	...
	try {
		...
		int32_t hsh = $mod((hash & 0x7fffffff), this->tablelen);
		int32_t next = $nc(this->table)->get(hsh);
		this->table->set(hsh, index);
		$nc(this->entries)->set(index++, hash);
		this->entries->set(index++, next);
		this->entries->set(index, pos);
	} catch ($Exception& e) {
...
ZipFile$Source* ZipFile$Source::get($File* file, bool toDelete, $ZipCoder* zc) {
	...
	try {
		...
		$throwNew($IOException, ipe);
	}
	$var(ZipFile$Source, src, nullptr);
	$synchronized(ZipFile$Source::files) {
		$assign(src, $cast(ZipFile$Source, ZipFile$Source::files->get(key)));
		if (src != nullptr) {
			++src->refs;
			return src;
		}
	...
	$synchronized(ZipFile$Source::files) {
		if (ZipFile$Source::files->containsKey(key)) {
			src->close();
			$assign(src, $cast(ZipFile$Source, ZipFile$Source::files->get(key)));
			++$nc(src)->refs;
			return src;
		}
		ZipFile$Source::files->put(key, src);
		return src;
	}
}
...
void ZipFile$Source::release(ZipFile$Source* src) {
	...
	$synchronized(ZipFile$Source::files) {
		...
		if (var$0 && --src->refs == 0) {
			ZipFile$Source::files->remove(src->key);
			src->close();
		}
	}
}
...
void ZipFile$Source::init$($ZipFile$Source$Key* key, bool toDelete, $ZipCoder* zc) {
	...
	try {
		...
		this->startsWithLoc = ($ZipUtils::LOCSIG(buf) == 67324752);
	} catch ($IOException& x) {
		try {
			$nc(this->zfile)->close();
...
$ZipFile$Source$End* ZipFile$Source::findEND() {
	...
	for (int64_t pos = ziplen - buf->length; pos >= minPos; pos -= (buf->length - 22)) {
		...
		for (int32_t i = buf->length - 22; i >= 0; --i) {
			if (buf->get(i + 0) == (int8_t)u'P' && buf->get(i + 1) == (int8_t)u'K' && buf->get(i + 2) == (int8_t)(char16_t)0x05 && buf->get(i + 3) == (int8_t)(char16_t)0x06) {
				...
				if (end->endpos + 22 + comlen != ziplen) {
					...
					bool var$1 = var$2 || $ZipUtils::GETSIG(sbuf) != 33639248;
					bool var$0 = var$1 || readFullyAt(sbuf, 0, sbuf->length, locpos) != 4;
					if (var$0 || $ZipUtils::GETSIG(sbuf) != 67324752) {
						continue;
					}
				}
				if (comlen > 0) {
				...
				try {
					...
					if (var$3 || $ZipUtils::GETSIG(loc64) != 117853008) {
						return end;
					}
					...
					if (var$4 || $ZipUtils::GETSIG(end64buf) != 101075792) {
						return end;
					}
...
void ZipFile$Source::initCEN(int32_t knownTotal) {
			...
			if (isManifestName(entryPos + ZipFile$Source::META_INF_LEN, nlen - ZipFile$Source::META_INF_LEN)) {
				...
				if (version > 0) {
					if (metaVersionsSet == nullptr) {
						$assign(metaVersionsSet, $cast($AbstractSet, $new($TreeSet)));
					}
	...
	if (signatureNames != nullptr) {
		...
		for (int32_t j = 0; j < len; ++j) {
			this->signatureMetaNames->set(j, $$sure($Integer, signatureNames->get(j))->intValue());
		}
	}
	if (metaVersionsSet != nullptr) {
		...
		int32_t c = 0;
		{
			...
			for (; $nc(i$)->hasNext();) {
				...
				{
					this->metaVersions->set(c++, $nc(version)->intValue());
				}
			}
		}
	} else {
...
int32_t ZipFile$Source::getEntryPos($String* name, bool addSlash) {
	...
	int32_t idx = $nc(this->table)->get($mod((hsh & 0x7fffffff), this->tablelen));
...
$ZipCoder* ZipFile$Source::zipCoderForPos(int32_t pos) {
	...
	if (($ZipUtils::CENFLG(this->cen, pos) & 0x0800) != 0) {
...
bool ZipFile$Source::isMetaName($bytes* name, int32_t off, int32_t len) {
	...
	bool var$7 = var$8 && (name->get(off++) | 0x20) == u'm';
	bool var$6 = var$7 && (name->get(off++) | 0x20) == u'e';
	bool var$5 = var$6 && (name->get(off++) | 0x20) == u't';
	bool var$4 = var$5 && (name->get(off++) | 0x20) == u'a';
	bool var$3 = var$4 && (name->get(off++)) == u'-';
	bool var$2 = var$3 && (name->get(off++) | 0x20) == u'i';
	bool var$1 = var$2 && (name->get(off++) | 0x20) == u'n';
	bool var$0 = var$1 && (name->get(off++) | 0x20) == u'f';
	return var$0 && (name->get(off)) == u'/';
}

bool ZipFile$Source::isManifestName(int32_t off, int32_t len) {
	$var($bytes, name, this->cen);
	bool var$9 = len == 11 && ($nc(name)->get(off++) | 0x20) == u'm';
	bool var$8 = var$9 && (name->get(off++) | 0x20) == u'a';
	bool var$7 = var$8 && (name->get(off++) | 0x20) == u'n';
	bool var$6 = var$7 && (name->get(off++) | 0x20) == u'i';
	bool var$5 = var$6 && (name->get(off++) | 0x20) == u'f';
	bool var$4 = var$5 && (name->get(off++) | 0x20) == u'e';
	bool var$3 = var$4 && (name->get(off++) | 0x20) == u's';
	bool var$2 = var$3 && (name->get(off++) | 0x20) == u't';
	bool var$1 = var$2 && (name->get(off++)) == u'.';
	bool var$0 = var$1 && (name->get(off++) | 0x20) == u'm';
	return (var$0 && (name->get(off) | 0x20) == u'f');
}
...
bool ZipFile$Source::isSignatureRelated(int32_t off, int32_t len) {
	...
	if ($nc(name)->get(off + len - 3) == u'.') {
		int32_t b1 = name->get(off + len - 2) | 0x20;
		int32_t b2 = name->get(off + len - 1) | 0x20;
		if ((b1 == u'e' && b2 == u'c') || (b1 == u's' && b2 == u'f')) {
			signatureRelated = true;
		}
	} else if (name->get(off + len - 4) == u'.') {
		int32_t b1 = name->get(off + len - 3) | 0x20;
		int32_t b2 = name->get(off + len - 2) | 0x20;
		int32_t b3 = name->get(off + len - 1) | 0x20;
	...
	$init($UTF_8);
	$init($Locale);
	if (!ZipFile$Source::$assertionsDisabled && !(signatureRelated == $SignatureFileVerifier::isBlockOrSF($($$new($String, name, off, len, $UTF_8::INSTANCE)->toUpperCase($Locale::ENGLISH))))) {
...
int32_t ZipFile$Source::getMetaVersion(int32_t off, int32_t len) {
	...
	bool var$7 = var$8 && (name->get(off++) | 0x20) == u'v';
	bool var$6 = var$7 && (name->get(off++) | 0x20) == u'e';
	bool var$5 = var$6 && (name->get(off++) | 0x20) == u'r';
	bool var$4 = var$5 && (name->get(off++) | 0x20) == u's';
	bool var$3 = var$4 && (name->get(off++) | 0x20) == u'i';
	bool var$2 = var$3 && (name->get(off++) | 0x20) == u'o';
	bool var$1 = var$2 && (name->get(off++) | 0x20) == u'n';
	bool var$0 = var$1 && (name->get(off++) | 0x20) == u's';
...
void ZipFile$Source::clinit$($Class* clazz) {
	$load($ZipFile);
...
$Class* ZipFile$Source::load$($String* name, bool initialize) {
	$CompoundAttribute zcfieldAnnotations$$[] = {
	...
	$FieldInfo fieldInfos$$[] = {
		...
		{"zc", "Ljava/util/zip/ZipCoder;", nullptr, $PRIVATE | $FINAL, $field(ZipFile$Source, zc), zcfieldAnnotations$$},
	...
	$loadClass(ZipFile$Source, name, initialize, &classInfo$$, ZipFile$Source::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ZipFile$Source);
	});
	return class$;
}
