...
ArrayDeque$DeqSpliterator* ArrayDeque$DeqSpliterator::trySplit() {
	...
	int32_t i = 0;
	int32_t n = 0;
	return ((n = $ArrayDeque::sub(getFence(), i = this->cursor, $nc(es)->length) >> 1) <= 0) ? (ArrayDeque$DeqSpliterator*)nullptr : $new(ArrayDeque$DeqSpliterator, this->this$0, i, this->cursor = $ArrayDeque::inc(i, n, es->length));
}
...
void ArrayDeque$DeqSpliterator::forEachRemaining($Consumer* action) {
	...
	if (cursor != end) {
		this->cursor = end;
		if ($nc(es)->get(cursor) == nullptr || es->get($ArrayDeque::dec(end, es->length)) == nullptr) {
			$throwNew($ConcurrentModificationException);
		}
		for (int32_t i = cursor, to = (i <= end) ? end : es->length;; i = 0, to = end) {
			for (; i < to; ++i) {
...
$Class* ArrayDeque$DeqSpliterator::load$($String* name, bool initialize) {
	...
	$loadClass(ArrayDeque$DeqSpliterator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ArrayDeque$DeqSpliterator);
	});
	return class$;
}
