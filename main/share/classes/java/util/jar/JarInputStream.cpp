...
$ZipEntry* JarInputStream::getNextEntry() {
	...
	if (this->first == nullptr) {
		...
		$assign(e, this->first);
		$init($JarIndex);
		if ($$nc(this->first->getName())->equalsIgnoreCase($JarIndex::INDEX_NAME)) {
...
$ZipEntry* JarInputStream::createZipEntry($String* name) {
	...
	if (this->man != nullptr) {
		$set(e, attr, this->man->getAttributes(name));
	}
	return e;
}
...
$Class* JarInputStream::load$($String* name, bool initialize) {
	...
	$loadClass(JarInputStream, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
