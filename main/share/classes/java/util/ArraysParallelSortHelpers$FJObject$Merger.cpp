...
void ArraysParallelSortHelpers$FJObject$Merger::compute() {
	...
	while (lb < lf && rb < rf) {
		...
		if ($nc(c)->compare(($assign(al, $nc(a)->get(lb))), ($assign(ar, $nc(a)->get(rb)))) <= 0) {
			++lb;
			$assign(t, al);
		} else {
			++rb;
...
$Class* ArraysParallelSortHelpers$FJObject$Merger::load$($String* name, bool initialize) {
	...
	$loadClass(ArraysParallelSortHelpers$FJObject$Merger, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
