...
$Spliterator* Vector$VectorSpliterator::trySplit() {
	...
	return (lo >= mid) ? ($Spliterator*)nullptr : $cast($Spliterator, $new(Vector$VectorSpliterator, this->this$0, this->array, lo, this->index = mid, this->expectedModCount));
}
...
$Class* Vector$VectorSpliterator::load$($String* name, bool initialize) {
	...
	$loadClass(Vector$VectorSpliterator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Vector$VectorSpliterator);
	});
	return class$;
}
