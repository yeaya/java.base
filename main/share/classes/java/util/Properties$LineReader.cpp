...
int32_t Properties$LineReader::readLine() {
	...
	while (true) {
		...
		c = (fromStream) ? (char16_t)($nc(byteBuf)->get(off++) & 0xff) : $nc(charBuf)->get(off++);
		if (skipWhiteSpace) {
			if (c == u' ' || c == u'\t' || c == u'\f') {
				continue;
			}
...
$Class* Properties$LineReader::load$($String* name, bool initialize) {
	...
	$loadClass(Properties$LineReader, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Properties$LineReader);
	});
	return class$;
}
