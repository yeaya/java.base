...
$Object* Collections$SynchronizedList::set(int32_t index, Object$* element) {
	$synchronized(this->mutex) {
		return $nc(this->list)->set(index, element);
	}
}
...
$Object* Collections$SynchronizedList::readResolve() {
	return ($instanceOf($RandomAccess, this->list) ? $of($new($Collections$SynchronizedRandomAccessList, this->list)) : $of(this));
}
...
$Class* Collections$SynchronizedList::load$($String* name, bool initialize) {
	...
	$loadClass(Collections$SynchronizedList, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
