...
$Class* Vector$$Lambda$lambda$removeAll$0::load$($String* name, bool initialize) {
	...
	$loadClass(Vector$$Lambda$lambda$removeAll$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Vector$$Lambda$lambda$removeAll$0);
	});
	return class$;
}
...
$Class* Vector$$Lambda$lambda$retainAll$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(Vector$$Lambda$lambda$retainAll$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Vector$$Lambda$lambda$retainAll$1$1);
	});
	return class$;
}
...
void Vector::setSize(int32_t newSize) {
	$synchronized(this) {
		...
		for (int32_t to = this->elementCount, i = newSize; i < to; ++i) {
			$nc(es)->set(i, nullptr);
		}
...
int32_t Vector::indexOf(Object$* o, int32_t index) {
	$synchronized(this) {
		if (o == nullptr) {
			...
			for (int32_t i = index; i < this->elementCount; ++i) {
				if ($of(o)->equals($nc(this->elementData$)->get(i))) {
					return i;
				}
			}
		}
...
void Vector::removeAllElements() {
	$synchronized(this) {
		$var($ObjectArray, es, this->elementData$);
		for (int32_t to = this->elementCount, i = this->elementCount = 0; i < to; ++i) {
			$nc(es)->set(i, nullptr);
		}
		++this->modCount;
	}
...
bool Vector::removeAll($Collection* c) {
	...
	return bulkRemove($$new(Vector$$Lambda$lambda$removeAll$0, c));
}

bool Vector::retainAll($Collection* c) {
	$Objects::requireNonNull(c);
	return bulkRemove($$new(Vector$$Lambda$lambda$retainAll$1$1, c));
}
...
void Vector::shiftTailOverGap($ObjectArray* es, int32_t lo, int32_t hi) {
	...
	for (int32_t to = this->elementCount, i = (this->elementCount -= hi - lo); i < to; ++i) {
		$nc(es)->set(i, nullptr);
	}
}
...
void Vector::readObject($ObjectInputStream* in) {
	...
	$var($ObjectArray, data, $cast($ObjectArray, gfields->get("elementData"_s, nullptr)));
	if (count < 0 || data == nullptr || count > data->length) {
...
void Vector::writeObject($ObjectOutputStream* s) {
	...
	fields->put("elementData"_s, data);
	s->writeFields();
}
...
$Class* Vector::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.util.Vector$$Lambda$lambda$removeAll$0")) {
			return Vector$$Lambda$lambda$removeAll$0::load$(name, initialize);
		}
		if (name->equals("java.util.Vector$$Lambda$lambda$retainAll$1$1")) {
	...
	$loadClass(Vector, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
