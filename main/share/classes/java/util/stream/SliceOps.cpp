...
int64_t SliceOps::calcSize(int64_t size, int64_t skip, int64_t limit) {
	$init(SliceOps);
	return size >= 0 ? $Math::max((int64_t)0, $Math::min(size - skip, limit)) : -1;
}
...
void SliceOps::clinit$($Class* clazz) {
...
$Class* SliceOps::load$($String* name, bool initialize) {
	...
	$loadClass(SliceOps, name, initialize, &classInfo$$, SliceOps::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SliceOps);
	});
	return class$;
}
