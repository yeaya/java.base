...
$Class* SliceOps$2$$Lambda$lambda$opEvaluateParallelLazy$0::load$($String* name, bool initialize) {
	...
	$loadClass(SliceOps$2$$Lambda$lambda$opEvaluateParallelLazy$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SliceOps$2$$Lambda$lambda$opEvaluateParallelLazy$0);
	});
	return class$;
}
...
$Spliterator* SliceOps$2::opEvaluateParallelLazy($PipelineHelper* helper, $Spliterator* spliterator) {
	...
	if (size > 0 && $nc(spliterator)->hasCharacteristics($Spliterator::SUBSIZED)) {
		...
		return $cast($StreamSpliterators$SliceSpliterator$OfPrimitive, $new($StreamSpliterators$SliceSpliterator$OfInt, var$0, var$1, $SliceOps::calcSliceFence(this->val$skip, this->val$limit)));
	} else {
		$init($StreamOpFlag);
		if (!$StreamOpFlag::ORDERED->isKnown(helper->getStreamAndOpFlags())) {
			...
			return $$sure($Node, $$new($SliceOps$SliceTask, this, helper, spliterator, $$new(SliceOps$2$$Lambda$lambda$opEvaluateParallelLazy$0), this->val$skip, this->val$limit)->invoke())->spliterator();
		}
	}
}
...
$Class* SliceOps$2::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.util.stream.SliceOps$2$$Lambda$lambda$opEvaluateParallelLazy$0")) {
	...
	$loadClass(SliceOps$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
