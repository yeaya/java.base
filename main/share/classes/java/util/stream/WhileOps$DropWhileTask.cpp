...
$Object* WhileOps$DropWhileTask::doLeaf() {
	...
	bool isChild = !isRoot();
	$init($StreamOpFlag);
	int64_t sizeIfKnown = isChild && this->isOrdered && $StreamOpFlag::SIZED->isPreserved($nc(this->op)->sourceOrOpFlags) ? this->op->exactOutputSizeIfKnown(this->spliterator) : -1;
...
void WhileOps$DropWhileTask::onCompletion($CountedCompleter* caller) {
	...
	if (!isLeaf()) {
		if (this->isOrdered) {
			...
			if (this->index == ($cast(WhileOps$DropWhileTask, this->leftChild))->thisNodeSize) {
...
$Node* WhileOps$DropWhileTask::merge() {
	...
	if ($nc(($cast(WhileOps$DropWhileTask, this->leftChild)))->thisNodeSize == 0) {
		...
		return $cast($Node, $cast(WhileOps$DropWhileTask, this->leftChild)->getLocalResult());
	} else {
		$var($StreamShape, var$0, $nc(this->op)->getOutputShape());
		$var($Node, var$1, $cast($Node, $cast(WhileOps$DropWhileTask, this->leftChild)->getLocalResult()));
		return $Nodes::conc(var$0, var$1, $$cast($Node, $cast(WhileOps$DropWhileTask, this->rightChild)->getLocalResult()));
	}
}

$Node* WhileOps$DropWhileTask::doTruncate($Node* input) {
	return this->isOrdered ? $nc(input)->truncate(this->index, $nc(input)->count(), this->generator) : input;
}

void WhileOps$DropWhileTask::clinit$($Class* clazz) {
	$load($WhileOps);
...
$Class* WhileOps$DropWhileTask::load$($String* name, bool initialize) {
	...
	$loadClass(WhileOps$DropWhileTask, name, initialize, &classInfo$$, WhileOps$DropWhileTask::clinit$, []($Class* clazz) -> $Object* {
