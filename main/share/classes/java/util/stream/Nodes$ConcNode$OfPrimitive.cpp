...
void Nodes$ConcNode$OfPrimitive::copyInto(Object$* array, int32_t offset) {
	...
	$nc($cast($Node$OfPrimitive, this->right))->copyInto(array, offset + (int32_t)$cast($Node$OfPrimitive, this->left)->count());
}
...
$Class* Nodes$ConcNode$OfPrimitive::load$($String* name, bool initialize) {
	...
	$loadClass(Nodes$ConcNode$OfPrimitive, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
