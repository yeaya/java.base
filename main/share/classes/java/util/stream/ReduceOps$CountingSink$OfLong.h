#ifndef _java_util_stream_ReduceOps$CountingSink$OfLong_h_
#define _java_util_stream_ReduceOps$CountingSink$OfLong_h_
//$ class java.util.stream.ReduceOps$CountingSink$OfLong
//$ extends java.util.stream.ReduceOps$CountingSink
//$ implements java.util.stream.Sink$OfLong

#include <java/util/stream/ReduceOps$CountingSink.h>
#include <java/util/stream/Sink$OfLong.h>

namespace java {
	namespace util {
		namespace stream {
			class ReduceOps$AccumulatingSink;
		}
	}
}

namespace java {
	namespace util {
		namespace stream {

class ReduceOps$CountingSink$OfLong : public ::java::util::stream::ReduceOps$CountingSink, public ::java::util::stream::Sink$OfLong {
	$class(ReduceOps$CountingSink$OfLong, $NO_CLASS_INIT, ::java::util::stream::ReduceOps$CountingSink, ::java::util::stream::Sink$OfLong)
public:
	ReduceOps$CountingSink$OfLong();
	virtual ::java::lang::Object0* toObject0$() const override {return (::java::lang::Object0*)(void*)this;}
	using ::java::util::stream::ReduceOps$CountingSink::combine;
	using ::java::util::stream::Sink$OfLong::accept;
	using ::java::util::stream::Sink$OfLong::andThen;
	virtual void accept(int32_t value) override;
	virtual void accept(double value) override;
	virtual void accept(Object$* t) override;
	virtual ::java::util::function::Consumer* andThen(::java::util::function::Consumer* after) override;
	virtual void begin(int64_t size) override;
	virtual bool cancellationRequested() override;
	virtual $Object* clone() override;
	virtual void end() override;
	virtual bool equals(Object$* obj) override;
	virtual void finalize() override;
	virtual int32_t hashCode() override;
	void init$();
	virtual void accept(int64_t t) override;
	virtual void combine(::java::util::stream::ReduceOps$AccumulatingSink* other) override;
	virtual $Object* get() override;
	virtual $String* toString() override;
};

		} // stream
	} // util
} // java

#endif // _java_util_stream_ReduceOps$CountingSink$OfLong_h_