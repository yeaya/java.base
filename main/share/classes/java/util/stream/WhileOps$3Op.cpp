...
public:
	...
	virtual $Object* apply(int32_t x$0) override {
		 return WhileOps$3Op::lambda$opEvaluateParallelLazy$0(x$0);
	}
};
$Class* WhileOps$3Op$$Lambda$lambda$opEvaluateParallelLazy$0::load$($String* name, bool initialize) {
	...
	$loadClass(WhileOps$3Op$$Lambda$lambda$opEvaluateParallelLazy$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WhileOps$3Op$$Lambda$lambda$opEvaluateParallelLazy$0);
	});
	return class$;
}
...
$Spliterator* WhileOps$3Op::opEvaluateParallelLazy($PipelineHelper* helper, $Spliterator* spliterator) {
	...
	if ($StreamOpFlag::ORDERED->isKnown($nc(helper)->getStreamAndOpFlags())) {
		return $$nc(opEvaluateParallel(helper, spliterator, $$new(WhileOps$3Op$$Lambda$lambda$opEvaluateParallelLazy$0)))->spliterator();
	} else {
		return $cast($WhileOps$UnorderedWhileSpliterator, $new($WhileOps$UnorderedWhileSpliterator$OfLong$Dropping, $$cast($Spliterator$OfLong, helper->wrapSpliterator(spliterator)), false, this->val$predicate));
	}
}
...
$Class* WhileOps$3Op::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.util.stream.WhileOps$3Op$$Lambda$lambda$opEvaluateParallelLazy$0")) {
	...
	$loadClass(WhileOps$3Op, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
