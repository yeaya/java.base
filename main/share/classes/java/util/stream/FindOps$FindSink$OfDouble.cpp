...
$Class* FindOps$FindSink$OfDouble$$Lambda$isPresent::load$($String* name, bool initialize) {
	...
	$loadClass(FindOps$FindSink$OfDouble$$Lambda$isPresent, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FindOps$FindSink$OfDouble$$Lambda$isPresent);
	});
	return class$;
}
...
$Class* FindOps$FindSink$OfDouble$$Lambda$OfDouble$1::load$($String* name, bool initialize) {
	...
	$loadClass(FindOps$FindSink$OfDouble$$Lambda$OfDouble$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FindOps$FindSink$OfDouble$$Lambda$OfDouble$1);
	});
	return class$;
}
...
$Object* FindOps$FindSink$OfDouble::get() {
	return this->hasValue ? $OptionalDouble::of($nc($cast($Double, this->value))->doubleValue()) : ($OptionalDouble*)nullptr;
}
...
void FindOps$FindSink$OfDouble::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$init($StreamShape);
	$var($StreamShape, var$0, $StreamShape::DOUBLE_VALUE);
	$var($Object, var$1, $OptionalDouble::empty());
	$var($Predicate, var$2, $new(FindOps$FindSink$OfDouble$$Lambda$isPresent));
	$assignStatic(FindOps$FindSink$OfDouble::OP_FIND_FIRST, $new($FindOps$FindOp, true, var$0, var$1, var$2, $$new(FindOps$FindSink$OfDouble$$Lambda$OfDouble$1)));
	$var($StreamShape, var$3, $StreamShape::DOUBLE_VALUE);
	$var($Object, var$4, $OptionalDouble::empty());
	$var($Predicate, var$5, $new(FindOps$FindSink$OfDouble$$Lambda$isPresent));
	$assignStatic(FindOps$FindSink$OfDouble::OP_FIND_ANY, $new($FindOps$FindOp, false, var$3, var$4, var$5, $$new(FindOps$FindSink$OfDouble$$Lambda$OfDouble$1)));
}
...
$Class* FindOps$FindSink$OfDouble::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.util.stream.FindOps$FindSink$OfDouble$$Lambda$isPresent")) {
			return FindOps$FindSink$OfDouble$$Lambda$isPresent::load$(name, initialize);
		}
		if (name->equals("java.util.stream.FindOps$FindSink$OfDouble$$Lambda$OfDouble$1")) {
	...
	$loadClass(FindOps$FindSink$OfDouble, name, initialize, &classInfo$$, FindOps$FindSink$OfDouble::clinit$, []($Class* clazz) -> $Object* {
