...
int32_t WhileOps$UnorderedWhileSpliterator::characteristics() {
	return $nc(this->s)->characteristics() & ~($Spliterator::SIZED | $Spliterator::SUBSIZED);
}
...
$Class* WhileOps$UnorderedWhileSpliterator::load$($String* name, bool initialize) {
	...
	$loadClass(WhileOps$UnorderedWhileSpliterator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WhileOps$UnorderedWhileSpliterator);
	});
	return class$;
}
