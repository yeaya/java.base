...
void AbstractShortCircuitTask::compute() {
	...
	while (($assign(result, $nc(sr)->get())) == nullptr) {
		...
		if (sizeEstimate <= sizeThreshold || ($assign(ls, $nc(rs)->trySplit())) == nullptr) {
			$assign(result, task->doLeaf());
			break;
		}
		...
		$set(task, leftChild, $assign(leftChild, $cast(AbstractShortCircuitTask, task->makeChild(ls))));
...
$Object* AbstractShortCircuitTask::getLocalResult() {
	if (isRoot()) {
		...
		return (answer == nullptr) ? getEmptyResult() : answer;
	} else {
		return $AbstractTask::getLocalResult();
	}
}
...
bool AbstractShortCircuitTask::taskCanceled() {
	...
	if (!cancel) {
		...
		for (; !cancel && parent != nullptr; $assign(parent, $cast(AbstractShortCircuitTask, parent->getParent()))) {
...
void AbstractShortCircuitTask::cancelLaterNodes() {
	...
	for (; parent != nullptr; $assign(node, parent), $assign(parent, $cast(AbstractShortCircuitTask, parent->getParent()))) {
...
$Class* AbstractShortCircuitTask::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractShortCircuitTask, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
