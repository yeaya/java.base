...
bool StreamOpFlag::isStreamFlag() {
	...
	return $$sure($Integer, $nc(this->maskTable)->get($StreamOpFlag$Type::STREAM))->intValue() > 0;
}
...
bool StreamOpFlag::canSet($StreamOpFlag$Type* t) {
	return ($$sure($Integer, $nc(this->maskTable)->get(t))->intValue() & StreamOpFlag::SET_BITS) > 0;
}
...
int32_t StreamOpFlag::createMask($StreamOpFlag$Type* t) {
	...
	int32_t mask = 0;
	{
		...
		for (int32_t len$ = arr$->length, i$ = 0; i$ < len$; ++i$) {
			...
			{
				mask |= $sl($$sure($Integer, $nc($nc(flag)->maskTable)->get(t))->intValue(), $nc(flag)->bitPosition);
			}
		}
	}
	return mask;
...
int32_t StreamOpFlag::getMask(int32_t flags) {
	$init(StreamOpFlag);
	return (flags == 0) ? StreamOpFlag::FLAG_MASK : ~((flags | ((StreamOpFlag::FLAG_MASK_IS & flags) << 1)) | ((StreamOpFlag::FLAG_MASK_NOT & flags) >> 1));
}
...
int32_t StreamOpFlag::toStreamFlags(int32_t combOpFlags) {
	$init(StreamOpFlag);
	return (((~combOpFlags) >> 1) & StreamOpFlag::FLAG_MASK_IS) & combOpFlags;
}

int32_t StreamOpFlag::toCharacteristics(int32_t streamFlags) {
	$init(StreamOpFlag);
	return streamFlags & StreamOpFlag::SPLITERATOR_CHARACTERISTICS_MASK;
}

int32_t StreamOpFlag::fromCharacteristics($Spliterator* spliterator) {
	$init(StreamOpFlag);
	int32_t characteristics = $nc(spliterator)->characteristics();
	if ((characteristics & $Spliterator::SORTED) != 0 && spliterator->getComparator() != nullptr) {
		return (characteristics & StreamOpFlag::SPLITERATOR_CHARACTERISTICS_MASK) & ~$Spliterator::SORTED;
	} else {
...
void StreamOpFlag::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$init($StreamOpFlag$Type);
...
$Class* StreamOpFlag::load$($String* name, bool initialize) {
	...
	$loadClass(StreamOpFlag, name, initialize, &classInfo$$, StreamOpFlag::clinit$, []($Class* clazz) -> $Object* {
