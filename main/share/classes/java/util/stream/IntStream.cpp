...
public:
	...
	virtual $Object* apply(int32_t e) override {
		 return IntStream::lambda$mapMulti$0(mapper, e);
	}
...
$Class* IntStream$$Lambda$lambda$mapMulti$0::load$($String* name, bool initialize) {
	...
	$loadClass(IntStream$$Lambda$lambda$mapMulti$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(IntStream$$Lambda$lambda$mapMulti$0);
	});
	return class$;
}
...
$Class* IntStream$$Lambda$close$1::load$($String* name, bool initialize) {
	...
	$loadClass(IntStream$$Lambda$close$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(IntStream$$Lambda$close$1);
	});
	return class$;
}
...
IntStream* IntStream::mapMulti($IntStream$IntMapMultiConsumer* mapper) {
	...
	return flatMap($$new(IntStream$$Lambda$lambda$mapMulti$0, mapper));
}
...
IntStream* IntStream::takeWhile($IntPredicate* predicate) {
	...
	return $cast(IntStream, $$nc($StreamSupport::intStream(var$0, isParallel()))->onClose($$new(IntStream$$Lambda$close$1, this)));
}
...
IntStream* IntStream::of(int32_t t) {
	$init(IntStream);
	return $StreamSupport::intStream($$cast($Spliterator$OfInt, $new($Streams$IntStreamBuilderImpl, t)), false);
}
...
IntStream* IntStream::generate($IntSupplier* s) {
	...
	return $StreamSupport::intStream($$cast($Spliterator$OfInt, $new($StreamSpliterators$InfiniteSupplyingSpliterator$OfInt, $Long::MAX_VALUE, s)), false);
}
...
$Class* IntStream::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.util.stream.IntStream$$Lambda$lambda$mapMulti$0")) {
			return IntStream$$Lambda$lambda$mapMulti$0::load$(name, initialize);
		}
		if (name->equals("java.util.stream.IntStream$$Lambda$close$1")) {
	...
	$loadClass(IntStream, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(IntStream);
	});
	return class$;
}
