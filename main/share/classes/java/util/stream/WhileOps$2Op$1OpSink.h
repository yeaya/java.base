#ifndef _java_util_stream_WhileOps$2Op$1OpSink_h_
#define _java_util_stream_WhileOps$2Op$1OpSink_h_
//$ class java.util.stream.WhileOps$2Op$1OpSink
//$ extends java.util.stream.Sink$ChainedInt
//$ implements java.util.stream.WhileOps$DropWhileSink

#include <java/util/stream/Sink$ChainedInt.h>
#include <java/util/stream/WhileOps$DropWhileSink.h>

namespace java {
	namespace util {
		namespace stream {
			class Sink;
			class WhileOps$2Op;
		}
	}
}

namespace java {
	namespace util {
		namespace stream {

class WhileOps$2Op$1OpSink : public ::java::util::stream::Sink$ChainedInt, public ::java::util::stream::WhileOps$DropWhileSink {
	$class(WhileOps$2Op$1OpSink, $NO_CLASS_INIT, ::java::util::stream::Sink$ChainedInt, ::java::util::stream::WhileOps$DropWhileSink)
public:
	WhileOps$2Op$1OpSink();
	using ::java::util::stream::Sink$ChainedInt::accept;
	using ::java::util::stream::Sink$ChainedInt::andThen;
	virtual void accept(Object$* i) override;
	virtual void accept(int64_t value) override;
	virtual void accept(double value) override;
	virtual ::java::util::function::Consumer* andThen(::java::util::function::Consumer* after) override;
	virtual void begin(int64_t size) override;
	virtual bool cancellationRequested() override;
	virtual $Object* clone() override;
	virtual void end() override;
	virtual bool equals(Object$* obj) override;
	virtual void finalize() override;
	virtual int32_t hashCode() override;
	void init$(::java::util::stream::WhileOps$2Op* this$0, ::java::util::stream::Sink* val$sink, bool val$retainAndCountDroppedElements);
	virtual void accept(int32_t t) override;
	virtual int64_t getDropCount() override;
	virtual $String* toString() override;
	::java::util::stream::WhileOps$2Op* this$0 = nullptr;
	bool val$retainAndCountDroppedElements = false;
	::java::util::stream::Sink* val$sink = nullptr;
	int64_t dropCount = 0;
	bool take = false;
};

		} // stream
	} // util
} // java

#endif // _java_util_stream_WhileOps$2Op$1OpSink_h_