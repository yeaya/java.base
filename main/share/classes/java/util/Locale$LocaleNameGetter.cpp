...
void Locale$LocaleNameGetter::clinit$($Class* clazz) {
	$load($Locale);
...
$Class* Locale$LocaleNameGetter::load$($String* name, bool initialize) {
	...
	$loadClass(Locale$LocaleNameGetter, name, initialize, &classInfo$$, Locale$LocaleNameGetter::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Locale$LocaleNameGetter);
	});
	return class$;
}
