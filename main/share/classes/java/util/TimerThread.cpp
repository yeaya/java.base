	...
	while (true) {
		try {
			...
			$synchronized(this->queue) {
				...
				if (!taskFired) {
					$nc(this->queue)->wait(executionTime - currentTime);
				}
			}
			if (taskFired) {
				task->run();
...
$Class* TimerThread::load$($String* name, bool initialize) {
	...
	$loadClass(TimerThread, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TimerThread);
	});
	return class$;
}
