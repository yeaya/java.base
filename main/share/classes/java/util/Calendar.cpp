...
$Class* Calendar$$Lambda$getTimeZone::load$($String* name, bool initialize) {
	...
	$loadClass(Calendar$$Lambda$getTimeZone, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Calendar$$Lambda$getTimeZone);
	});
	return class$;
}
...
$TimeZone* Calendar::defaultTimeZone($Locale* l) {
	...
	return shortTZID != nullptr ? $cast($TimeZone, $$nc($$nc($TimeZoneNameUtility::convertLDMLShortID(shortTZID))->map($$new(Calendar$$Lambda$getTimeZone)))->orElse(defaultTZ)) : defaultTZ;
}
...
Calendar* Calendar::createCalendar($TimeZone* zone, $Locale* aLocale) {
	...
	if (cal == nullptr) {
		bool var$1 = aLocale->getLanguage() == "th"_s;
		if (var$1 && aLocale->getCountry() == "TH"_s) {
			$assign(cal, $new($BuddhistCalendar, zone, aLocale));
		} else {
			bool var$3 = aLocale->getVariant() == "JP"_s;
			bool var$2 = var$3 && aLocale->getLanguage() == "ja"_s;
			if (var$2 && aLocale->getCountry() == "JP"_s) {
...
void Calendar::setTime($Date* date) {
	$Objects::requireNonNull(date, "date must not be null"_s);
...
void Calendar::setTimeInMillis(int64_t millis) {
	if (this->time == millis && this->isTimeSet && this->areFieldsSet && this->areAllFieldsSet && ($instanceOf($ZoneInfo, this->zone)) && !$cast($ZoneInfo, this->zone)->isDirty()) {
		return;
	}
	this->time = millis;
	this->isTimeSet = true;
...
void Calendar::clear() {
	for (int32_t i = 0; i < $nc(this->fields)->length;) {
		int32_t var$0 = i;
		$nc(this->stamp)->set(var$0, this->fields->set(i, 0));
...
$String* Calendar::getDisplayName(int32_t field, int32_t style, $Locale* locale) {
	...
	if (var$0 || isNarrowFormatStyle(style) || field == Calendar::ERA && (style & Calendar::SHORT) == Calendar::SHORT) {
...
$StringArray* Calendar::getFieldStrings(int32_t field, int32_t style, $DateFormatSymbols* symbols) {
	...
	case Calendar::ERA:
		...
		break;
	case Calendar::MONTH:
		$assign(var$0, (baseStyle == Calendar::LONG) ? $nc(symbols)->getMonths() : $nc(symbols)->getShortMonths());
		break;
	case Calendar::DAY_OF_WEEK:
		$assign(var$0, (baseStyle == Calendar::LONG) ? $nc(symbols)->getWeekdays() : $nc(symbols)->getShortWeekdays());
		break;
	case Calendar::AM_PM:
...
int32_t Calendar::selectFields() {
	...
	int32_t dowStamp = this->stamp->get(Calendar::DAY_OF_WEEK);
	int32_t monthStamp = this->stamp->get(Calendar::MONTH);
	int32_t domStamp = this->stamp->get(Calendar::DAY_OF_MONTH);
	int32_t womStamp = aggregateStamp(this->stamp->get(Calendar::WEEK_OF_MONTH), dowStamp);
	int32_t dowimStamp = aggregateStamp(this->stamp->get(Calendar::DAY_OF_WEEK_IN_MONTH), dowStamp);
	int32_t doyStamp = this->stamp->get(Calendar::DAY_OF_YEAR);
	int32_t woyStamp = aggregateStamp(this->stamp->get(Calendar::WEEK_OF_YEAR), dowStamp);
	...
	if (bestStamp == Calendar::UNSET) {
		womStamp = this->stamp->get(Calendar::WEEK_OF_MONTH);
		dowimStamp = $Math::max(this->stamp->get(Calendar::DAY_OF_WEEK_IN_MONTH), dowStamp);
		woyStamp = this->stamp->get(Calendar::WEEK_OF_YEAR);
		bestStamp = $Math::max($Math::max(womStamp, dowimStamp), woyStamp);
		if (bestStamp == Calendar::UNSET) {
			bestStamp = (domStamp = monthStamp);
		}
	}
	if (bestStamp == domStamp || (bestStamp == womStamp && this->stamp->get(Calendar::WEEK_OF_MONTH) >= this->stamp->get(Calendar::WEEK_OF_YEAR)) || (bestStamp == dowimStamp && this->stamp->get(Calendar::DAY_OF_WEEK_IN_MONTH) >= this->stamp->get(Calendar::WEEK_OF_YEAR))) {
		...
		if (bestStamp == domStamp) {
			...
			if (womStamp == dowimStamp) {
				if (this->stamp->get(Calendar::WEEK_OF_MONTH) >= this->stamp->get(Calendar::DAY_OF_WEEK_IN_MONTH)) {
				...
				if (this->stamp->get(Calendar::DAY_OF_WEEK_IN_MONTH) != Calendar::UNSET) {
					fieldMask |= Calendar::DAY_OF_WEEK_IN_MONTH_MASK;
				}
			}
		}
	...
	int32_t hourOfDayStamp = this->stamp->get(Calendar::HOUR_OF_DAY);
	int32_t hourStamp = aggregateStamp(this->stamp->get(Calendar::HOUR), this->stamp->get(Calendar::AM_PM));
	bestStamp = (hourStamp > hourOfDayStamp) ? hourStamp : hourOfDayStamp;
	if (bestStamp == Calendar::UNSET) {
		bestStamp = $Math::max(this->stamp->get(Calendar::HOUR), this->stamp->get(Calendar::AM_PM));
	}
	if (bestStamp != Calendar::UNSET) {
		if (bestStamp == hourOfDayStamp) {
			...
			if (this->stamp->get(Calendar::AM_PM) != Calendar::UNSET) {
				fieldMask |= Calendar::AM_PM_MASK;
			}
		}
	}
	if (this->stamp->get(Calendar::MINUTE) != Calendar::UNSET) {
		fieldMask |= Calendar::MINUTE_MASK;
	}
	if (this->stamp->get(Calendar::SECOND) != Calendar::UNSET) {
		fieldMask |= Calendar::SECOND_MASK;
	}
	if (this->stamp->get(Calendar::MILLISECOND) != Calendar::UNSET) {
		fieldMask |= Calendar::MILLISECOND_MASK;
	}
	if (this->stamp->get(Calendar::ZONE_OFFSET) >= Calendar::MINIMUM_USER_STAMP) {
		fieldMask |= Calendar::ZONE_OFFSET_MASK;
	}
	if (this->stamp->get(Calendar::DST_OFFSET) >= Calendar::MINIMUM_USER_STAMP) {
...
bool Calendar::isStandaloneStyle(int32_t style) {
	return (style & Calendar::STANDALONE_MASK) != 0;
}
...
bool Calendar::equals(Object$* obj) {
	...
	try {
		...
		return var$0 && ($instanceOf($ZoneInfo, this->zone) ? this->zone->equals(that->zone) : $nc(this->zone)->equals($(that->getTimeZone())));
	} catch ($Exception& e) {
	}
	return false;
}
...
int32_t Calendar::hashCode() {
	int32_t otheritems = (((this->lenient ? 1 : 0) | (this->firstDayOfWeek << 1)) | (this->minimalDaysInFirstWeek << 4)) | ($nc(this->zone)->hashCode() << 7);
...
$Object* Calendar::clone() {
	try {
		...
		for (int32_t i = 0; i < Calendar::FIELD_COUNT; ++i) {
			other->fields->set(i, $nc(this->fields)->get(i));
			other->stamp->set(i, $nc(this->stamp)->get(i));
			other->isSet$->set(i, $nc(this->isSet$)->get(i));
		}
		if (!this->sharedZone) {
...
$String* Calendar::getFieldName(int32_t field) {
	$init(Calendar);
	return Calendar::FIELD_NAME->get(field);
}
...
$String* Calendar::toString() {
	...
	buffer->append(",zone="_s)->append(this->zone);
	...
	for (int32_t i = 0; i < Calendar::FIELD_COUNT; ++i) {
		buffer->append(u',');
		appendValue(buffer, Calendar::FIELD_NAME->get(i), isSet(i), (int64_t)$nc(this->fields)->get(i));
	}
	buffer->append(u']');
...
void Calendar::setWeekCountData($Locale* desiredLocale) {
	$var($ints, data, $cast($ints, Calendar::cachedLocaleData->get(desiredLocale)));
	if (data == nullptr) {
		...
		Calendar::cachedLocaleData->putIfAbsent(desiredLocale, data);
	}
...
int64_t Calendar::getMillisOf(Calendar* calendar) {
	...
	$var(Calendar, cal, $cast(Calendar, calendar->clone()));
...
void Calendar::adjustStamp() {
	...
	for (;;) {
		...
		for (int32_t i = 0; i < $nc(this->stamp)->length; ++i) {
			if (this->stamp->get(i) == min) {
				this->stamp->set(i, newStamp);
			}
		}
		++newStamp;
		if (min == max) {
...
void Calendar::invalidateWeekFields() {
	if ($nc(this->stamp)->get(Calendar::WEEK_OF_MONTH) != Calendar::COMPUTED && this->stamp->get(Calendar::WEEK_OF_YEAR) != Calendar::COMPUTED) {
		return;
	}
	...
	if (this->stamp->get(Calendar::WEEK_OF_MONTH) == Calendar::COMPUTED) {
		int32_t weekOfMonth = cal->get(Calendar::WEEK_OF_MONTH);
		if ($nc(this->fields)->get(Calendar::WEEK_OF_MONTH) != weekOfMonth) {
			this->fields->set(Calendar::WEEK_OF_MONTH, weekOfMonth);
		}
	}
	if (this->stamp->get(Calendar::WEEK_OF_YEAR) == Calendar::COMPUTED) {
		int32_t weekOfYear = cal->get(Calendar::WEEK_OF_YEAR);
		if ($nc(this->fields)->get(Calendar::WEEK_OF_YEAR) != weekOfYear) {
			this->fields->set(Calendar::WEEK_OF_YEAR, weekOfYear);
		}
	}
}
...
void Calendar::writeObject($ObjectOutputStream* stream) {
	$synchronized(this) {
		...
		if ($instanceOf($ZoneInfo, this->zone)) {
			$var($SimpleTimeZone, stz, $cast($ZoneInfo, this->zone)->getLastRuleInstance());
			if (stz == nullptr) {
				int32_t var$0 = this->zone->getRawOffset();
				$assign(stz, $new($SimpleTimeZone, var$0, $(this->zone->getID())));
			}
...
void Calendar::readObject($ObjectInputStream* stream) {
	...
	if (this->serialVersionOnStream >= 2) {
		...
		for (int32_t i = 0; i < Calendar::FIELD_COUNT; ++i) {
			this->stamp->set(i, $nc(this->isSet$)->get(i) ? Calendar::COMPUTED : Calendar::UNSET);
		}
	}
	this->serialVersionOnStream = Calendar::currentSerialVersion;
	...
	try {
		...
		$assign(zi, $cast($ZoneInfo, $AccessController::doPrivileged($$new($Calendar$1, this, input), $Calendar$CalendarAccessControlContext::INSTANCE)));
...
void Calendar::clinit$($Class* clazz) {
...
$Class* Calendar::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.util.Calendar$$Lambda$getTimeZone")) {
	...
	$loadClass(Calendar, name, initialize, &classInfo$$, Calendar::clinit$, []($Class* clazz) -> $Object* {
