...
$ObjectArray* AbstractCollection::toArray($ObjectArray* a) {
	...
	$var($ObjectArray, r, $nc(a)->length >= size ? a : $cast($ObjectArray, $1Array::newInstance($of(a)->getClass()->getComponentType(), size)));
	$var($Iterator, it, iterator());
...
$Class* AbstractCollection::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractCollection, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AbstractCollection);
	});
	return class$;
}
