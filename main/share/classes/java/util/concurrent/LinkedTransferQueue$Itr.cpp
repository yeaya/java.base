	...
	$var($LinkedTransferQueue$Node, pred, pred$renamed);
	{
		...
		for (; p != nullptr;) {
			...
			if (var$0) {
				...
				var$0 = !this->this$0->tryCasSuccessor(pred, var$1, $assign(c, p));
			}
			if (var$0) {
				$assign(pred, p);
...
$Class* LinkedTransferQueue$Itr::load$($String* name, bool initialize) {
	...
	$loadClass(LinkedTransferQueue$Itr, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LinkedTransferQueue$Itr);
	});
	return class$;
}
