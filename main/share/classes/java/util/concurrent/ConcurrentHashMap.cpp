...
int32_t ConcurrentHashMap::spread(int32_t h) {
	...
	return (h ^ ((int32_t)((uint32_t)h >> 16))) & ConcurrentHashMap::HASH_BITS;
}
...
int32_t ConcurrentHashMap::size() {
	int64_t n = sumCount();
	return ((n < 0) ? 0 : (n > (int64_t)$Integer::MAX_VALUE) ? $Integer::MAX_VALUE : (int32_t)n);
}
...
$Object* ConcurrentHashMap::get(Object$* key) {
	...
	if (var$0 && ($assign(e, tabAt(tab, (n - 1) & h))) != nullptr) {
		if ((eh = $nc(e)->hash) == h) {
			bool var$2 = $equals($assign(ek, e->key), key);
			if (var$2 || (ek != nullptr && $of(key)->equals(ek))) {
				return e->val;
			}
		} else if (eh < 0) {
			return ($assign(p, e->find(h, key))) != nullptr ? $cast($Object, $nc(p)->val) : ($Object*)nullptr;
		}
		while (($assign(e, $nc(e)->next)) != nullptr) {
			bool var$3 = $nc(e)->hash == h;
			if (var$3) {
				bool var$4 = $equals($assign(ek, e->key), key);
				var$3 = var$4 || (ek != nullptr && $of(key)->equals(ek));
			}
			if (var$3) {
				return e->val;
			}
...
$Object* ConcurrentHashMap::putVal(Object$* key, Object$* value, bool onlyIfAbsent) {
	...
	int32_t binCount = 0;
	{
		...
		for (;;) {
			...
			if (tab == nullptr || (n = tab->length) == 0) {
			...
			} else if (($assign(f, tabAt(tab, i = (n - 1) & hash))) == nullptr) {
				...
				bool var$0 = onlyIfAbsent && fh == hash;
				if (var$0) {
					bool var$1 = $equals($assign(fk, f->key), key);
					var$0 = var$1 || (fk != nullptr && $of(key)->equals(fk));
				}
				if (var$0 && ($assign(fv, f->val)) != nullptr) {
					return fv;
				} else {
						...
						if (tabAt(tab, i) == f) {
							if (fh >= 0) {
								binCount = 1;
								{
									...
									for (;; ++binCount) {
										$var($Object, ek, nullptr);
										bool var$2 = $nc(e)->hash == hash;
										if (var$2) {
											bool var$3 = $equals($assign(ek, e->key), key);
											var$2 = var$3 || (ek != nullptr && $of(key)->equals(ek));
										}
										if (var$2) {
											$assign(oldVal, e->val);
											if (!onlyIfAbsent) {
								...
								$var($ConcurrentHashMap$Node, p, nullptr);
								binCount = 2;
								if (($assign(p, $cast($ConcurrentHashMap$TreeBin, f)->putTreeVal(hash, key, value))) != nullptr) {
...
$Object* ConcurrentHashMap::remove(Object$* key) {
	return replaceNode(key, nullptr, nullptr);
}
...
$Object* ConcurrentHashMap::replaceNode(Object$* key, Object$* value, Object$* cv) {
	...
	int32_t hash = spread($nc($of(key))->hashCode());
	{
		...
		for (;;) {
			...
			int32_t i = 0;
			int32_t fh = 0;
			bool var$0 = tab == nullptr || (n = tab->length) == 0;
			if (var$0 || ($assign(f, tabAt(tab, i = (n - 1) & hash))) == nullptr) {
				break;
			} else if ((fh = $nc(f)->hash) == ConcurrentHashMap::MOVED) {
				$assign(tab, helpTransfer(tab, f));
			} else {
						...
						if (fh >= 0) {
							validated = true;
							{
								...
								for (;;) {
									...
									if (var$1) {
										$var($Object, ev, e->val);
										if (cv == nullptr || $equals(cv, ev) || (ev != nullptr && $of(cv)->equals(ev))) {
											$assign(oldVal, ev);
											if (value != nullptr) {
												$set(e, val, value);
							...
							if (var$3 && ($assign(p, $nc(r)->findTreeNode(hash, key, nullptr))) != nullptr) {
								...
								if (cv == nullptr || $equals(cv, pv) || (pv != nullptr && $of(cv)->equals(pv))) {
									$assign(oldVal, pv);
									if (value != nullptr) {
										$set(p, val, value);
...
void ConcurrentHashMap::clear() {
	...
	while (tab != nullptr && i < tab->length) {
		...
		if (f == nullptr) {
			...
			$synchronized(f) {
				if (tabAt(tab, i) == f) {
					$var($ConcurrentHashMap$Node, p, fh >= 0 ? f : ($instanceOf($ConcurrentHashMap$TreeBin, f)) ? $cast($ConcurrentHashMap$Node, $cast($ConcurrentHashMap$TreeBin, f)->first) : ($ConcurrentHashMap$Node*)nullptr);
					while (p != nullptr) {
						--delta;
						$assign(p, p->next);
					}
...
int32_t ConcurrentHashMap::hashCode() {
	...
	if (($assign(t, this->table)) != nullptr) {
		$var($ConcurrentHashMap$Traverser, it, $new($ConcurrentHashMap$Traverser, t, $nc(t)->length, 0, $nc(t)->length));
		{
			$var($ConcurrentHashMap$Node, p, nullptr);
			for (; ($assign(p, it->advance())) != nullptr;) {
				int32_t var$0 = $nc($nc(p)->key)->hashCode();
				h += var$0 ^ $nc(p->val)->hashCode();
			}
		}
	}
	return h;
	...
	if (!$equals(o, this)) {
		...
		$var($ConcurrentHashMap$Traverser, it, $new($ConcurrentHashMap$Traverser, t, f, 0, f));
		{
			...
			for (; ($assign(p, it->advance())) != nullptr;) {
				...
				if (v == nullptr || (!$equals(v, val) && !v->equals(val))) {
					return false;
				}
			}
		}
		{
			...
			for (; $nc(i$)->hasNext();) {
				$var($Map$Entry, e, $cast($Map$Entry, i$->next()));
				{
					...
					bool var$1 = var$2 || ($assign(mv, e->getValue())) == nullptr;
					bool var$0 = var$1 || ($assign(v, get(mk))) == nullptr;
					if (var$0 || (!$equals(mv, v) && !$nc(mv)->equals(v))) {
						return false;
					}
				}
			}
...
void ConcurrentHashMap::writeObject($ObjectOutputStream* s) {
	...
	$nc(streamFields)->put("segments"_s, segments);
...
void ConcurrentHashMap::readObject($ObjectInputStream* s) {
	...
	for (;;) {
		...
		if (k != nullptr && v != nullptr) {
			$assign(p, $new($ConcurrentHashMap$Node, spread(k->hashCode()), k, v, p));
			++size;
		} else {
			break;
		}
				...
				if ($nc(first)->hash < 0) {
					...
					for ($assign(q, first); q != nullptr; $assign(q, q->next)) {
						...
						if (var$0) {
							...
							var$0 = var$1 || (qk != nullptr && $nc(k)->equals(qk));
						}
						if (var$0) {
							insertAtFront = false;
							break;
					...
					if (insertAtFront && binCount >= ConcurrentHashMap::TREEIFY_THRESHOLD) {
						...
						for ($assign(q, p); q != nullptr; $assign(q, q->next)) {
							$var($ConcurrentHashMap$TreeNode, t, $new($ConcurrentHashMap$TreeNode, q->hash, q->key, q->val, nullptr, nullptr));
...
$Object* ConcurrentHashMap::replace(Object$* key, Object$* value) {
	...
	return replaceNode(key, value, nullptr);
}

$Object* ConcurrentHashMap::getOrDefault(Object$* key, Object$* defaultValue) {
	$var($Object, v, nullptr);
	return ($assign(v, get(key))) == nullptr ? $of(defaultValue) : v;
}
...
$Object* ConcurrentHashMap::computeIfAbsent(Object$* key, $Function* mappingFunction) {
	...
	int32_t binCount = 0;
	{
		...
		for (;;) {
			...
			if (tab == nullptr || (n = tab->length) == 0) {
				$assign(tab, initTable());
			} else if (($assign(f, tabAt(tab, i = (n - 1) & h))) == nullptr) {
					...
					$synchronized(f) {
						if (tabAt(tab, i) == f) {
							if (fh >= 0) {
								...
								$var($ConcurrentHashMap$TreeNode, r, nullptr);
								$var($ConcurrentHashMap$TreeNode, p, nullptr);
								bool var$6 = ($assign(r, t->root)) != nullptr;
								if (var$6 && ($assign(p, $nc(r)->findTreeNode(h, key, nullptr))) != nullptr) {
									$assign(val, $nc(p)->val);
				...
				$synchronized(f) {
					if (tabAt(tab, i) == f) {
						if (fh >= 0) {
							...
							$var($Object, pv, (p == nullptr) ? ($Object*)nullptr : $cast($Object, p->val));
				...
				$synchronized(f) {
					if (tabAt(tab, i) == f) {
						if (fh >= 0) {
							...
							$var($ConcurrentHashMap$TreeNode, p, (r == nullptr) ? ($ConcurrentHashMap$TreeNode*)nullptr : r->findTreeNode(h, key, nullptr));
							$assign(val, (p == nullptr) ? $of(value) : $nc(remappingFunction)->apply(p->val, value));
							if (val != nullptr) {
								if (p != nullptr) {
									$set(p, val, val);
								} else {
...
void ConcurrentHashMap::addCount(int64_t x, int32_t check) {
	...
	if (var$0) {
		...
		bool var$4 = cs == nullptr || (m = cs->length - 1) < 0;
		bool var$3 = var$4 || ($assign(c, cs->get($ThreadLocalRandom::getProbe() & m))) == nullptr;
		if (!var$3) {
			int64_t var$5 = ConcurrentHashMap::CELLVALUE;
			int64_t var$6 = v = $nc(c)->value;
			var$3 = !(uncontended = $nc(ConcurrentHashMap::U)->compareAndSetLong(c, var$5, var$6, v + x));
		}
		if (var$3) {
	...
	if (check >= 0) {
		...
		int32_t sc = 0;
		while (true) {
			bool var$7 = s >= (int64_t)(sc = this->sizeCtl) && ($assign(tab, this->table)) != nullptr;
			if (!(var$7 && (n = $nc(tab)->length) < ConcurrentHashMap::MAXIMUM_CAPACITY)) {
				break;
			}
			{
...
$ConcurrentHashMap$NodeArray* ConcurrentHashMap::helpTransfer($ConcurrentHashMap$NodeArray* tab, $ConcurrentHashMap$Node* f) {
	...
	int32_t sc = 0;
	if (tab != nullptr && ($instanceOf($ConcurrentHashMap$ForwardingNode, f)) && ($assign(nextTab, $cast($ConcurrentHashMap$ForwardingNode, f)->nextTable)) != nullptr) {
				...
				if (tabAt(tab, i) == f) {
					...
					if (fh >= 0) {
						...
						}
						{
							...
							for (; p != lastRun; $assign(p, $nc(p)->next)) {
						...
						$assign(ln, (lc <= ConcurrentHashMap::UNTREEIFY_THRESHOLD) ? untreeify(lo) : $cast($ConcurrentHashMap$Node, (hc != 0) ? $new($ConcurrentHashMap$TreeBin, lo) : t));
						$assign(hn, (hc <= ConcurrentHashMap::UNTREEIFY_THRESHOLD) ? untreeify(hi) : $cast($ConcurrentHashMap$Node, (lc != 0) ? $new($ConcurrentHashMap$TreeBin, hi) : t));
						setTabAt(nextTab, i, ln);
						setTabAt(nextTab, i + n, hn);
						setTabAt(tab, i, fwd);
						advance = true;
...
void ConcurrentHashMap::fullAddCount(int64_t x, bool wasUncontended) {
	...
	for (;;) {
		...
		if (var$0 && (n = $nc(cs)->length) > 0) {
			if (($assign(c, cs->get((n - 1) & h))) == nullptr) {
				if (this->cellsBusy == 0) {
					...
					if (this->cellsBusy == 0 && $nc(ConcurrentHashMap::U)->compareAndSetInt(this, ConcurrentHashMap::CELLSBUSY, 0, 1)) {
						...
						try {
							...
							if (var$2 && rs->get(j = (m - 1) & h) == nullptr) {
								rs->set(j, r);
								created = true;
							}
				...
				int64_t var$5 = ConcurrentHashMap::CELLVALUE;
				int64_t var$6 = v = $nc(c)->value;
				if ($nc(ConcurrentHashMap::U)->compareAndSetLong(c, var$5, var$6, v + x)) {
					break;
					...
					try {
						if (this->counterCells == cs) {
							$set(this, counterCells, $cast($ConcurrentHashMap$CounterCellArray, $Arrays::copyOf(cs, n << 1)));
						}
			...
			try {
				if (this->counterCells == cs) {
					...
					rs->set(h & 1, $$new($ConcurrentHashMap$CounterCell, x));
			...
			int64_t var$11 = ConcurrentHashMap::BASECOUNT;
			int64_t var$12 = v = this->baseCount;
			if ($nc(ConcurrentHashMap::U)->compareAndSetLong(this, var$11, var$12, v + x)) {
				break;
			}
		}
	}
			...
			$synchronized(b) {
				if (tabAt(tab, index) == b) {
					...
					$var($ConcurrentHashMap$TreeNode, tl, nullptr);
					{
						...
						for (; e != nullptr; $assign(e, e->next)) {
							$var($ConcurrentHashMap$TreeNode, p, $new($ConcurrentHashMap$TreeNode, e->hash, e->key, e->val, nullptr, nullptr));
...
int32_t ConcurrentHashMap::batchFor(int64_t b) {
	int64_t n = 0;
	if (b == $Long::MAX_VALUE || (n = sumCount()) <= 1 || n < b) {
		return 0;
	}
	int32_t sp = $ForkJoinPool::getCommonPoolParallelism() << 2;
	return (b <= 0 || (n /= b) >= sp) ? sp : (int32_t)n;
}
...
$Object* ConcurrentHashMap::search(int64_t parallelismThreshold, $BiFunction* searchFunction) {
	...
	return $$new($ConcurrentHashMap$SearchMappingsTask, nullptr, var$0, 0, 0, var$1, searchFunction, $$new($AtomicReference))->invoke();
}
...
$Object* ConcurrentHashMap::reduce(int64_t parallelismThreshold, $BiFunction* transformer, $BiFunction* reducer) {
	...
	return $$new($ConcurrentHashMap$MapReduceMappingsTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, reducer)->invoke();
}
...
double ConcurrentHashMap::reduceToDouble(int64_t parallelismThreshold, $ToDoubleBiFunction* transformer, double basis, $DoubleBinaryOperator* reducer) {
	...
	return $$sure($Double, $$new($ConcurrentHashMap$MapReduceMappingsToDoubleTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->doubleValue();
}
...
int64_t ConcurrentHashMap::reduceToLong(int64_t parallelismThreshold, $ToLongBiFunction* transformer, int64_t basis, $LongBinaryOperator* reducer) {
	...
	return $$sure($Long, $$new($ConcurrentHashMap$MapReduceMappingsToLongTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->longValue();
}
...
int32_t ConcurrentHashMap::reduceToInt(int64_t parallelismThreshold, $ToIntBiFunction* transformer, int32_t basis, $IntBinaryOperator* reducer) {
	...
	return $$sure($Integer, $$new($ConcurrentHashMap$MapReduceMappingsToIntTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->intValue();
}
...
$Object* ConcurrentHashMap::searchKeys(int64_t parallelismThreshold, $Function* searchFunction) {
	...
	int32_t var$0 = batchFor(parallelismThreshold);
	$var($ConcurrentHashMap$NodeArray, var$1, this->table);
	return $$new($ConcurrentHashMap$SearchKeysTask, nullptr, var$0, 0, 0, var$1, searchFunction, $$new($AtomicReference))->invoke();
}
...
$Object* ConcurrentHashMap::reduceKeys(int64_t parallelismThreshold, $BiFunction* reducer) {
	...
	return $$new($ConcurrentHashMap$ReduceKeysTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, reducer)->invoke();
}
...
$Object* ConcurrentHashMap::reduceKeys(int64_t parallelismThreshold, $Function* transformer, $BiFunction* reducer) {
	...
	return $$new($ConcurrentHashMap$MapReduceKeysTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, reducer)->invoke();
}
...
double ConcurrentHashMap::reduceKeysToDouble(int64_t parallelismThreshold, $ToDoubleFunction* transformer, double basis, $DoubleBinaryOperator* reducer) {
	...
	return $$sure($Double, $$new($ConcurrentHashMap$MapReduceKeysToDoubleTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->doubleValue();
}
...
int64_t ConcurrentHashMap::reduceKeysToLong(int64_t parallelismThreshold, $ToLongFunction* transformer, int64_t basis, $LongBinaryOperator* reducer) {
	...
	return $$sure($Long, $$new($ConcurrentHashMap$MapReduceKeysToLongTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->longValue();
}
...
int32_t ConcurrentHashMap::reduceKeysToInt(int64_t parallelismThreshold, $ToIntFunction* transformer, int32_t basis, $IntBinaryOperator* reducer) {
	...
	return $$sure($Integer, $$new($ConcurrentHashMap$MapReduceKeysToIntTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->intValue();
}
...
$Object* ConcurrentHashMap::searchValues(int64_t parallelismThreshold, $Function* searchFunction) {
	...
	int32_t var$0 = batchFor(parallelismThreshold);
	$var($ConcurrentHashMap$NodeArray, var$1, this->table);
	return $$new($ConcurrentHashMap$SearchValuesTask, nullptr, var$0, 0, 0, var$1, searchFunction, $$new($AtomicReference))->invoke();
}
...
$Object* ConcurrentHashMap::reduceValues(int64_t parallelismThreshold, $BiFunction* reducer) {
	...
	return $$new($ConcurrentHashMap$ReduceValuesTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, reducer)->invoke();
}
...
$Object* ConcurrentHashMap::reduceValues(int64_t parallelismThreshold, $Function* transformer, $BiFunction* reducer) {
	...
	return $$new($ConcurrentHashMap$MapReduceValuesTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, reducer)->invoke();
}
...
double ConcurrentHashMap::reduceValuesToDouble(int64_t parallelismThreshold, $ToDoubleFunction* transformer, double basis, $DoubleBinaryOperator* reducer) {
	...
	return $$sure($Double, $$new($ConcurrentHashMap$MapReduceValuesToDoubleTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->doubleValue();
}
...
int64_t ConcurrentHashMap::reduceValuesToLong(int64_t parallelismThreshold, $ToLongFunction* transformer, int64_t basis, $LongBinaryOperator* reducer) {
	...
	return $$sure($Long, $$new($ConcurrentHashMap$MapReduceValuesToLongTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->longValue();
}
...
int32_t ConcurrentHashMap::reduceValuesToInt(int64_t parallelismThreshold, $ToIntFunction* transformer, int32_t basis, $IntBinaryOperator* reducer) {
	...
	return $$sure($Integer, $$new($ConcurrentHashMap$MapReduceValuesToIntTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->intValue();
}
...
$Object* ConcurrentHashMap::searchEntries(int64_t parallelismThreshold, $Function* searchFunction) {
	...
	int32_t var$0 = batchFor(parallelismThreshold);
	$var($ConcurrentHashMap$NodeArray, var$1, this->table);
	return $$new($ConcurrentHashMap$SearchEntriesTask, nullptr, var$0, 0, 0, var$1, searchFunction, $$new($AtomicReference))->invoke();
}
...
$Object* ConcurrentHashMap::reduceEntries(int64_t parallelismThreshold, $Function* transformer, $BiFunction* reducer) {
	...
	return $$new($ConcurrentHashMap$MapReduceEntriesTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, reducer)->invoke();
}
...
double ConcurrentHashMap::reduceEntriesToDouble(int64_t parallelismThreshold, $ToDoubleFunction* transformer, double basis, $DoubleBinaryOperator* reducer) {
	...
	return $$sure($Double, $$new($ConcurrentHashMap$MapReduceEntriesToDoubleTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->doubleValue();
}
...
int64_t ConcurrentHashMap::reduceEntriesToLong(int64_t parallelismThreshold, $ToLongFunction* transformer, int64_t basis, $LongBinaryOperator* reducer) {
	...
	return $$sure($Long, $$new($ConcurrentHashMap$MapReduceEntriesToLongTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->longValue();
}
...
int32_t ConcurrentHashMap::reduceEntriesToInt(int64_t parallelismThreshold, $ToIntFunction* transformer, int32_t basis, $IntBinaryOperator* reducer) {
	...
	return $$sure($Integer, $$new($ConcurrentHashMap$MapReduceEntriesToIntTask, nullptr, batchFor(parallelismThreshold), 0, 0, this->table, nullptr, transformer, basis, reducer)->invoke())->intValue();
}

void ConcurrentHashMap::clinit$($Class* clazz) {
	$useLocalObjectStack();
	ConcurrentHashMap::LOAD_FACTOR = 0.75f;
	ConcurrentHashMap::NCPU = $$nc($Runtime::getRuntime())->availableProcessors();
	$load($ConcurrentHashMap$Segment);
	...
	ConcurrentHashMap::TRANSFERINDEX = ConcurrentHashMap::U->objectFieldOffset(ConcurrentHashMap::class$, "transferIndex"_s);
	ConcurrentHashMap::BASECOUNT = ConcurrentHashMap::U->objectFieldOffset(ConcurrentHashMap::class$, "baseCount"_s);
	ConcurrentHashMap::CELLSBUSY = ConcurrentHashMap::U->objectFieldOffset(ConcurrentHashMap::class$, "cellsBusy"_s);
	$load($ConcurrentHashMap$CounterCell);
	ConcurrentHashMap::CELLVALUE = ConcurrentHashMap::U->objectFieldOffset($ConcurrentHashMap$CounterCell::class$, "value"_s);
	$load($ConcurrentHashMap$Node);
	ConcurrentHashMap::ABASE = ConcurrentHashMap::U->arrayBaseOffset($getClass($ConcurrentHashMap$NodeArray));
	{
		int32_t scale = ConcurrentHashMap::U->arrayIndexScale($getClass($ConcurrentHashMap$NodeArray));
		if ((scale & (scale - 1)) != 0) {
...
$Class* ConcurrentHashMap::load$($String* name, bool initialize) {
	...
	$loadClass(ConcurrentHashMap, name, initialize, &classInfo$$, ConcurrentHashMap::clinit$, []($Class* clazz) -> $Object* {
