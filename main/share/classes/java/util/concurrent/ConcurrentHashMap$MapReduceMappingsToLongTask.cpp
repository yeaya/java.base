	...
	if (($assign(transformer, this->transformer)) != nullptr && ($assign(reducer, this->reducer)) != nullptr) {
		...
		}
		{
			...
			for (; ($assign(p, advance())) != nullptr;) {
				r = $nc(reducer)->applyAsLong(r, $nc(transformer)->applyAsLong($nc(p)->key, $nc(p)->val));
			}
		}
		this->result = r;
...
$Class* ConcurrentHashMap$MapReduceMappingsToLongTask::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Ljava/util/concurrent/ConcurrentHashMap$BulkTask;III[Ljava/util/concurrent/ConcurrentHashMap$Node;Ljava/util/concurrent/ConcurrentHashMap$MapReduceMappingsToLongTask;Ljava/util/function/ToLongBiFunction;JLjava/util/function/LongBinaryOperator;)V", "(Ljava/util/concurrent/ConcurrentHashMap$BulkTask<TK;TV;*>;III[Ljava/util/concurrent/ConcurrentHashMap$Node<TK;TV;>;Ljava/util/concurrent/ConcurrentHashMap$MapReduceMappingsToLongTask<TK;TV;>;Ljava/util/function/ToLongBiFunction<-TK;-TV;>;JLjava/util/function/LongBinaryOperator;)V", 0, $method(ConcurrentHashMap$MapReduceMappingsToLongTask, init$, void, $ConcurrentHashMap$BulkTask*, int32_t, int32_t, int32_t, $ConcurrentHashMap$NodeArray*, ConcurrentHashMap$MapReduceMappingsToLongTask*, $ToLongBiFunction*, int64_t, $LongBinaryOperator*)},
		{"compute", "()V", nullptr, $PUBLIC | $FINAL, $virtualMethod(ConcurrentHashMap$MapReduceMappingsToLongTask, compute, void)},
		{"getRawResult", "()Ljava/lang/Long;", nullptr, $PUBLIC | $FINAL, $virtualMethod(ConcurrentHashMap$MapReduceMappingsToLongTask, getRawResult, $Object*)},
		{}
	};
	...
	$loadClass(ConcurrentHashMap$MapReduceMappingsToLongTask, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
