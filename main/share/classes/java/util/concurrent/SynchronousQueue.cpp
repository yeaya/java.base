...
void SynchronousQueue::init$(bool fair) {
	$AbstractQueue::init$();
	$set(this, transferer, fair ? $cast($SynchronousQueue$Transferer, $new($SynchronousQueue$TransferQueue)) : $cast($SynchronousQueue$Transferer, $new($SynchronousQueue$TransferStack)));
}
...
$Object* SynchronousQueue::poll() {
	return $nc(this->transferer)->transfer(nullptr, true, 0);
}
...
void SynchronousQueue::clinit$($Class* clazz) {
	{
		$load($LockSupport);
...
$Class* SynchronousQueue::load$($String* name, bool initialize) {
	...
	$loadClass(SynchronousQueue, name, initialize, &classInfo$$, SynchronousQueue::clinit$, []($Class* clazz) -> $Object* {
