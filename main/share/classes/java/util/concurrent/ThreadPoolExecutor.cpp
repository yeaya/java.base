...
int32_t ThreadPoolExecutor::runStateOf(int32_t c) {
	...
	return c & ~ThreadPoolExecutor::COUNT_MASK;
}
...
void ThreadPoolExecutor::advanceRunState(int32_t targetState) {
	for (;;) {
		...
		if (var$0 || this->ctl->compareAndSet(c, ctlOf(targetState, workerCountOf(c)))) {
			break;
		}
	}
}
...
void ThreadPoolExecutor::tryTerminate() {
	...
	for (;;) {
		...
		bool return$4 = false;
		try {
			if (this->ctl->compareAndSet(c, ctlOf(ThreadPoolExecutor::TIDYING, 0))) {
				...
				try {
					...
					this->ctl->set(ctlOf(ThreadPoolExecutor::TERMINATED, 0));
...
$List* ThreadPoolExecutor::drainQueue() {
	...
	$nc(q)->drainTo($cast($AbstractCollection, taskList));
	if (!q->isEmpty()) {
		$var($RunnableArray, arr$, $cast($RunnableArray, q->toArray($$new($RunnableArray, 0))));
...
bool ThreadPoolExecutor::addWorker($Runnable* firstTask, bool core) {
	...
	for (int32_t c = $nc(this->ctl)->get();;) {
		...
		for (;;) {
			if (workerCountOf(c) >= ((core ? $cast(int32_t, this->corePoolSize) : $cast(int32_t, this->maximumPoolSize)) & ThreadPoolExecutor::COUNT_MASK)) {
				return false;
			}
	...
	try {
		while (task != nullptr || ($assign(task, getTask())) != nullptr) {
			...
			if (!var$2) {
				...
				var$2 = var$3 && runStateAtLeast(this->ctl->get(), ThreadPoolExecutor::STOP);
			}
			bool var$1 = var$2;
...
void ThreadPoolExecutor::init$(int32_t corePoolSize, int32_t maximumPoolSize, int64_t keepAliveTime, $TimeUnit* unit, $BlockingQueue* workQueue, $ThreadFactory* threadFactory, $RejectedExecutionHandler* handler) {
	...
	$set(this, termination, this->mainLock->newCondition());
...
void ThreadPoolExecutor::purge() {
	...
	try {
		...
		while ($nc(it)->hasNext()) {
			...
			if ($instanceOf($Future, r) && $cast($Future, r)->isCancelled()) {
				it->remove();
			}
		}
...
$String* ThreadPoolExecutor::toString() {
	...
	try {
		...
		{
			$var($Iterator, i$, this->workers->iterator());
			for (; $nc(i$)->hasNext();) {
				$var($ThreadPoolExecutor$Worker, w, $cast($ThreadPoolExecutor$Worker, i$->next()));
				{
	...
	var$2->append($($AbstractExecutorService::toString()));
	var$2->append("["_s);
	var$2->append(runState);
	var$2->append(", pool size = "_s);
	var$2->append(nworkers);
	var$2->append(", active threads = "_s);
	var$2->append(nactive);
	var$2->append(", queued tasks = "_s);
	var$2->append($nc(this->workQueue)->size());
	var$2->append(", completed tasks = "_s);
	var$2->append(ncompleted);
	var$2->append("]"_s);
	return $str(var$2);
}
...
void ThreadPoolExecutor::clinit$($Class* clazz) {
...
$Class* ThreadPoolExecutor::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"finalize", "()V", nullptr, $PROTECTED | $DEPRECATED, $virtualMethod(ThreadPoolExecutor, finalize, void), nullptr, nullptr, finalizemethodAnnotations$$},
	...
	$loadClass(ThreadPoolExecutor, name, initialize, &classInfo$$, ThreadPoolExecutor::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ThreadPoolExecutor);
	});
	return class$;
}
