...
void AtomicStampedReference::set(Object$* newReference, int32_t newStamp) {
	$var($AtomicStampedReference$Pair, current, this->pair);
	if (!$equals(newReference, $nc(current)->reference) || newStamp != current->stamp) {
...
bool AtomicStampedReference::casPair($AtomicStampedReference$Pair* cmp, $AtomicStampedReference$Pair* val) {
	return $nc(AtomicStampedReference::PAIR)->compareAndSet($$new($ObjectArray, {this, cmp, val}));
}

void AtomicStampedReference::clinit$($Class* clazz) {
...
$Class* AtomicStampedReference::load$($String* name, bool initialize) {
	...
	$loadClass(AtomicStampedReference, name, initialize, &classInfo$$, AtomicStampedReference::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(AtomicStampedReference);
	});
	return class$;
}
