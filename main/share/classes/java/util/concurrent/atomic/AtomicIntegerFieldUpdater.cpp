...
$Class* AtomicIntegerFieldUpdater::load$($String* name, bool initialize) {
	...
	$loadClass(AtomicIntegerFieldUpdater, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AtomicIntegerFieldUpdater);
	});
	return class$;
}
