...
$Class* AtomicLongFieldUpdater::load$($String* name, bool initialize) {
	...
	$loadClass(AtomicLongFieldUpdater, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AtomicLongFieldUpdater);
	});
	return class$;
}
