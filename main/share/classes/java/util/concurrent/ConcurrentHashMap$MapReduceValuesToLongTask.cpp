...
void ConcurrentHashMap$MapReduceValuesToLongTask::compute() {
	...
	if (($assign(transformer, this->transformer)) != nullptr && ($assign(reducer, this->reducer)) != nullptr) {
		...
		for (int32_t i = this->baseIndex, f = 0, h = 0; this->batch > 0 && (h = (int32_t)((uint32_t)((f = this->baseLimit) + i) >> 1)) > i;) {
			addToPendingCount(1);
			$nc(($set(this, rights, $new(ConcurrentHashMap$MapReduceValuesToLongTask, this, $usrAssign(this->batch, 1), this->baseLimit = h, f, this->tab, this->rights, transformer, r, reducer))))->fork();
		}
		{
...
$Class* ConcurrentHashMap$MapReduceValuesToLongTask::load$($String* name, bool initialize) {
	...
	$loadClass(ConcurrentHashMap$MapReduceValuesToLongTask, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
