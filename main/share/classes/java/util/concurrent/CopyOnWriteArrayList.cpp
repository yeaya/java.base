...
$Class* CopyOnWriteArrayList$$Lambda$lambda$removeAll$0::load$($String* name, bool initialize) {
	...
	$loadClass(CopyOnWriteArrayList$$Lambda$lambda$removeAll$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CopyOnWriteArrayList$$Lambda$lambda$removeAll$0);
	});
	return class$;
}
...
$Class* CopyOnWriteArrayList$$Lambda$lambda$retainAll$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(CopyOnWriteArrayList$$Lambda$lambda$retainAll$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CopyOnWriteArrayList$$Lambda$lambda$retainAll$1$1);
	});
	return class$;
}
...
$Class* CopyOnWriteArrayList$$Lambda$lambda$resetLock$2$2::load$($String* name, bool initialize) {
	...
	$loadClass(CopyOnWriteArrayList$$Lambda$lambda$resetLock$2$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CopyOnWriteArrayList$$Lambda$lambda$resetLock$2$2);
	});
	return class$;
}
...
void CopyOnWriteArrayList::init$($Collection* c) {
	...
	if ($nc($of(c))->getClass() == CopyOnWriteArrayList::class$) {
		$assign(es, $cast(CopyOnWriteArrayList, c)->getArray());
	} else {
		$assign(es, c->toArray());
		$load($ArrayList);
	...
	$synchronized(this->lock) {
		...
		if (snapshot != current) {
			...
			for (int32_t i = 0; i < common; ++i) {
				if (!$equals(current->get(i), snapshot->get(i)) && $Objects::equals(e, current->get(i))) {
					return false;
				}
			}
...
bool CopyOnWriteArrayList::removeAll($Collection* c) {
	...
	return bulkRemove($$new(CopyOnWriteArrayList$$Lambda$lambda$removeAll$0, c));
}

bool CopyOnWriteArrayList::retainAll($Collection* c) {
	$Objects::requireNonNull(c);
	return bulkRemove($$new(CopyOnWriteArrayList$$Lambda$lambda$retainAll$1$1, c));
}
...
bool CopyOnWriteArrayList::addAll($Collection* c) {
	$useLocalObjectStack();
	$var($ObjectArray, cs, ($nc($of(c))->getClass() == CopyOnWriteArrayList::class$) ? $cast(CopyOnWriteArrayList, c)->getArray() : c->toArray());
	if ($nc(cs)->length == 0) {
		return false;
	}
	$synchronized(this->lock) {
		...
		if (var$0) {
			...
			$assign(newElements, $Arrays::copyOf(es, len + cs->length));
...
bool CopyOnWriteArrayList::addAll(int32_t index, $Collection* c) {
	...
	$synchronized(this->lock) {
		...
		if (numMoved == 0) {
			...
			$assign(newElements, $new($ObjectArray, len + cs->length));
			$System::arraycopy(es, 0, newElements, 0, index);
...
bool CopyOnWriteArrayList::equals(Object$* o) {
	...
	$var($Iterator, it, $nc(list)->iterator());
	{
		...
		for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
			...
			if (var$0 || !$Objects::equals(element, $(it->next()))) {
				return false;
			}
		}
	}
...
int32_t CopyOnWriteArrayList::hashCodeOfRange($ObjectArray* es, int32_t from, int32_t to) {
	...
	for (int32_t i = from; i < to; ++i) {
		...
		hashCode = 31 * hashCode + (x == nullptr ? 0 : x->hashCode());
	}
	return hashCode;
}
...
void CopyOnWriteArrayList::resetLock() {
	...
	$var($Field, lockField, $cast($Field, $AccessController::doPrivileged($cast($PrivilegedAction, $$new(CopyOnWriteArrayList$$Lambda$lambda$resetLock$2$2)))));
	try {
...
$Class* CopyOnWriteArrayList::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("java.util.concurrent.CopyOnWriteArrayList$$Lambda$lambda$removeAll$0")) {
			return CopyOnWriteArrayList$$Lambda$lambda$removeAll$0::load$(name, initialize);
		}
		if (name->equals("java.util.concurrent.CopyOnWriteArrayList$$Lambda$lambda$retainAll$1$1")) {
			return CopyOnWriteArrayList$$Lambda$lambda$retainAll$1$1::load$(name, initialize);
		}
		if (name->equals("java.util.concurrent.CopyOnWriteArrayList$$Lambda$lambda$resetLock$2$2")) {
	...
	$loadClass(CopyOnWriteArrayList, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
