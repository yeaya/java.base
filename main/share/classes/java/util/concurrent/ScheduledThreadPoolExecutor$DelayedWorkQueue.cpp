...
void ScheduledThreadPoolExecutor$DelayedWorkQueue::setIndex($RunnableScheduledFuture* f, int32_t idx) {
	...
	if ($instanceOf($ScheduledThreadPoolExecutor$ScheduledFutureTask, f)) {
		$cast($ScheduledThreadPoolExecutor$ScheduledFutureTask, f)->heapIndex = idx;
	}
}
...
void ScheduledThreadPoolExecutor$DelayedWorkQueue::siftDown(int32_t k, $RunnableScheduledFuture* key) {
	...
	while (k < half) {
		...
		if (right < this->size$ && $nc(c)->compareTo(this->queue->get(right)) > 0) {
			$assign(c, this->queue->get(child = right));
		}
...
void ScheduledThreadPoolExecutor$DelayedWorkQueue::grow() {
	...
	$set(this, queue, $cast($RunnableScheduledFutureArray, $Arrays::copyOf(this->queue, newCapacity)));
}
...
int32_t ScheduledThreadPoolExecutor$DelayedWorkQueue::indexOf(Object$* x) {
	if (x != nullptr) {
		if ($instanceOf($ScheduledThreadPoolExecutor$ScheduledFutureTask, x)) {
			int32_t i = $cast($ScheduledThreadPoolExecutor$ScheduledFutureTask, x)->heapIndex;
...
$Object* ScheduledThreadPoolExecutor$DelayedWorkQueue::poll() {
	...
	try {
		...
		$init($TimeUnit);
		$assign(var$2, (first == nullptr || first->getDelay($TimeUnit::NANOSECONDS) > 0) ? ($RunnableScheduledFuture*)nullptr : finishPoll(first));
		return$1 = true;
		goto $finally;
	} catch ($Throwable& var$3) {
		$assign(var$0, var$3);
...
$Iterator* ScheduledThreadPoolExecutor$DelayedWorkQueue::iterator() {
	...
	bool return$1 = false;
	try {
		$assign(var$2, $new($ScheduledThreadPoolExecutor$DelayedWorkQueue$Itr, this, $$cast($RunnableScheduledFutureArray, $Arrays::copyOf(this->queue, this->size$))));
		return$1 = true;
		goto $finally;
	} catch ($Throwable& var$3) {
		$assign(var$0, var$3);
...
$Class* ScheduledThreadPoolExecutor$DelayedWorkQueue::load$($String* name, bool initialize) {
	...
	$loadClass(ScheduledThreadPoolExecutor$DelayedWorkQueue, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
