...
void CountedCompleter::addToPendingCount(int32_t delta) {
	$nc(CountedCompleter::PENDING)->getAndAdd($$new($ObjectArray, {this, $$of(delta)}));
}

bool CountedCompleter::compareAndSetPendingCount(int32_t expected, int32_t count) {
	return $nc(CountedCompleter::PENDING)->compareAndSet($$new($ObjectArray, {this, $$of(expected), $$of(count)}));
}

bool CountedCompleter::weakCompareAndSetPendingCount(int32_t expected, int32_t count) {
	return $nc(CountedCompleter::PENDING)->weakCompareAndSet($$new($ObjectArray, {this, $$of(expected), $$of(count)}));
}
...
void CountedCompleter::quietlyCompleteRoot() {
	...
	$var(CountedCompleter, p, nullptr);
	for (;;) {
		if (($assign(p, $nc(a)->completer)) == nullptr) {
			a->quietlyComplete();
			return;
		}
		$assign(a, p);
...
void CountedCompleter::clinit$($Class* clazz) {
...
$Class* CountedCompleter::load$($String* name, bool initialize) {
	...
	$loadClass(CountedCompleter, name, initialize, &classInfo$$, CountedCompleter::clinit$, []($Class* clazz) -> $Object* {
