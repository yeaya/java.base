...
void StampedLock$ReaderNode::clinit$($Class* clazz) {
	$init($StampedLock);
...
$Class* StampedLock$ReaderNode::load$($String* name, bool initialize) {
	...
	$loadClass(StampedLock$ReaderNode, name, initialize, &classInfo$$, StampedLock$ReaderNode::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(StampedLock$ReaderNode);
	});
	return class$;
}
