...
int32_t AbstractQueuedSynchronizer::acquire($AbstractQueuedSynchronizer$Node* node$renamed, int32_t arg, bool shared, bool interruptible, bool timed, int64_t time) {
	...
	for (;;) {
		bool var$0 = !first && ($assign(pred, (node == nullptr) ? ($AbstractQueuedSynchronizer$Node*)nullptr : $cast($AbstractQueuedSynchronizer$Node, node->prev))) != nullptr;
...
void AbstractQueuedSynchronizer::clinit$($Class* clazz) {
	...
	AbstractQueuedSynchronizer::HEAD = AbstractQueuedSynchronizer::U->objectFieldOffset(AbstractQueuedSynchronizer::class$, "head"_s);
	AbstractQueuedSynchronizer::TAIL = AbstractQueuedSynchronizer::U->objectFieldOffset(AbstractQueuedSynchronizer::class$, "tail"_s);
	{
		$load($LockSupport);
...
$Class* AbstractQueuedSynchronizer::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractQueuedSynchronizer, name, initialize, &classInfo$$, AbstractQueuedSynchronizer::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(AbstractQueuedSynchronizer);
	});
	return class$;
}
