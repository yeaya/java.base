...
bool Pattern$First::match($Matcher* matcher, int32_t i, $CharSequence* seq) {
	if ($instanceOf($Pattern$BnM, this->atom)) {
		bool var$0 = this->atom->match(matcher, i, seq);
	...
	for (;;) {
		...
		if ($nc(this->atom)->match(matcher, i, seq)) {
			return $nc(this->next)->match(matcher, matcher->last, seq);
		}
...
$Class* Pattern$First::load$($String* name, bool initialize) {
	...
	$loadClass(Pattern$First, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Pattern$First);
	});
	return class$;
}
