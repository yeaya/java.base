...
int32_t TimSort::countRunAndMakeAscending($ObjectArray* a, int32_t lo, int32_t hi, $Comparator* c) {
	...
	if ($nc(c)->compare($nc(a)->get(runHi++), $nc(a)->get(lo)) < 0) {
...
$ObjectArray* TimSort::ensureCapacity(int32_t minCapacity) {
	if (this->tmpLen < minCapacity) {
		...
		$var($ObjectArray, newArray, $cast($ObjectArray, $1Array::newInstance($nc(this->a)->getClass()->getComponentType(), newSize)));
		$set(this, tmp, newArray);
		this->tmpLen = newSize;
		this->tmpBase = 0;
	}
...
void TimSort::clinit$($Class* clazz) {
...
$Class* TimSort::load$($String* name, bool initialize) {
	...
	$loadClass(TimSort, name, initialize, &classInfo$$, TimSort::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(TimSort);
	});
	return class$;
}
