...
bool Provider$ServiceKey::equals(Object$* obj) {
	...
	return var$0 && $nc(this->algorithm)->equals(other->algorithm);
}
...
$Class* Provider$ServiceKey::load$($String* name, bool initialize) {
	...
	$loadClass(Provider$ServiceKey, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Provider$ServiceKey);
	});
	return class$;
}
