...
void SecureClassLoader::init$($ClassLoader* parent) {
	...
	$set(this, pdcache, $cast($AbstractMap, $new($ConcurrentHashMap, 11)));
}
...
void SecureClassLoader::clinit$($Class* clazz) {
...
$Class* SecureClassLoader::load$($String* name, bool initialize) {
	...
	$loadClass(SecureClassLoader, name, initialize, &classInfo$$, SecureClassLoader::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SecureClassLoader);
	});
	return class$;
}
