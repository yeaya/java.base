...
AlgorithmParameterGenerator* AlgorithmParameterGenerator::getInstance($String* algorithm) {
	...
	try {
		...
		return $new(AlgorithmParameterGenerator, $cast($AlgorithmParameterGeneratorSpi, $nc(objs)->get(0)), $cast($Provider, $nc(objs)->get(1)), algorithm);
...
$Class* AlgorithmParameterGenerator::load$($String* name, bool initialize) {
	...
	$loadClass(AlgorithmParameterGenerator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AlgorithmParameterGenerator);
	});
	return class$;
}
