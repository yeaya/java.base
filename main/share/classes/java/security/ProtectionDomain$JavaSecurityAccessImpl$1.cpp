...
void ProtectionDomain$JavaSecurityAccessImpl$1::put($ProtectionDomain* pd, $PermissionCollection* pc) {
	$nc(this->map)->put((pd == nullptr ? ($ProtectionDomain$Key*)nullptr : pd->key), pc);
}

$PermissionCollection* ProtectionDomain$JavaSecurityAccessImpl$1::get($ProtectionDomain* pd) {
	return pd == nullptr ? $cast($PermissionCollection, $nc(this->map)->get(nullptr)) : $cast($PermissionCollection, $nc(this->map)->get(pd->key));
}
...
$Class* ProtectionDomain$JavaSecurityAccessImpl$1::load$($String* name, bool initialize) {
	...
	$loadClass(ProtectionDomain$JavaSecurityAccessImpl$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ProtectionDomain$JavaSecurityAccessImpl$1);
	});
	return class$;
}
