...
void AbstractInsnNode::acceptAnnotations($MethodVisitor* methodVisitor) {
	...
	if (this->visibleTypeAnnotations != nullptr) {
		for (int32_t i = 0, n = this->visibleTypeAnnotations->size(); i < n; ++i) {
			...
			$nc(typeAnnotation)->accept($($nc(methodVisitor)->visitInsnAnnotation($nc(typeAnnotation)->typeRef, $nc(typeAnnotation)->typePath, $nc(typeAnnotation)->desc, true)));
		}
	}
	if (this->invisibleTypeAnnotations != nullptr) {
		for (int32_t i = 0, n = this->invisibleTypeAnnotations->size(); i < n; ++i) {
			...
			$nc(typeAnnotation)->accept($($nc(methodVisitor)->visitInsnAnnotation($nc(typeAnnotation)->typeRef, $nc(typeAnnotation)->typePath, $nc(typeAnnotation)->desc, false)));
		}
	}
}
...
$LabelNodeArray* AbstractInsnNode::clone($List* labels, $Map* clonedLabels) {
	...
	for (int32_t i = 0, n = clones->length; i < n; ++i) {
...
AbstractInsnNode* AbstractInsnNode::cloneAnnotations(AbstractInsnNode* insnNode) {
	...
	if ($nc(insnNode)->visibleTypeAnnotations != nullptr) {
		...
		for (int32_t i = 0, n = insnNode->visibleTypeAnnotations->size(); i < n; ++i) {
			$var($TypeAnnotationNode, sourceAnnotation, $cast($TypeAnnotationNode, insnNode->visibleTypeAnnotations->get(i)));
			$var($TypeAnnotationNode, cloneAnnotation, $new($TypeAnnotationNode, $nc(sourceAnnotation)->typeRef, $nc(sourceAnnotation)->typePath, $nc(sourceAnnotation)->desc));
			sourceAnnotation->accept(cloneAnnotation);
			this->visibleTypeAnnotations->add(cloneAnnotation);
		}
	}
	if (insnNode->invisibleTypeAnnotations != nullptr) {
		$set(this, invisibleTypeAnnotations, $new($ArrayList));
		for (int32_t i = 0, n = insnNode->invisibleTypeAnnotations->size(); i < n; ++i) {
			$var($TypeAnnotationNode, sourceAnnotation, $cast($TypeAnnotationNode, insnNode->invisibleTypeAnnotations->get(i)));
			$var($TypeAnnotationNode, cloneAnnotation, $new($TypeAnnotationNode, $nc(sourceAnnotation)->typeRef, $nc(sourceAnnotation)->typePath, $nc(sourceAnnotation)->desc));
			sourceAnnotation->accept(cloneAnnotation);
...
$Class* AbstractInsnNode::load$($String* name, bool initialize) {
	...
	$loadClass(AbstractInsnNode, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AbstractInsnNode);
	});
	return class$;
}
