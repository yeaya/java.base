...
int32_t Frame::getAbstractTypeFromApiFormat($SymbolTable* symbolTable, Object$* type) {
	...
	if ($instanceOf($Integer, type)) {
		return Frame::CONSTANT_KIND | $cast($Integer, type)->intValue();
...
void Frame::setInputFrameFromDescriptor($SymbolTable* symbolTable, int32_t access, $String* descriptor, int32_t maxLocals) {
	...
	if ((access & $Opcodes::ACC_STATIC) == 0) {
		if ((access & $Constants::ACC_CONSTRUCTOR) == 0) {
			this->inputLocals->set(inputLocalIndex++, Frame::REFERENCE_KIND | $nc(symbolTable)->addType($($nc(symbolTable)->getClassName())));
		} else {
			this->inputLocals->set(inputLocalIndex++, Frame::UNINITIALIZED_THIS);
		}
	}
	{
		...
		for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
			$var($Type, argumentType, arr$->get(i$));
			{
				...
				this->inputLocals->set(inputLocalIndex++, abstractType);
				if (abstractType == Frame::LONG || abstractType == Frame::DOUBLE) {
					this->inputLocals->set(inputLocalIndex++, Frame::TOP);
				}
			}
		}
	}
...
void Frame::setInputFrameFromApiFormat($SymbolTable* symbolTable, int32_t numLocal, $ObjectArray* local, int32_t numStack, $ObjectArray* stack) {
	...
	for (int32_t i = 0; i < numLocal; ++i) {
		...
		$init($Opcodes);
		if ($equals(local->get(i), $Opcodes::LONG) || $equals(local->get(i), $Opcodes::DOUBLE)) {
			this->inputLocals->set(inputLocalIndex++, Frame::TOP);
		}
	}
	...
	for (int32_t i = 0; i < numStack; ++i) {
		this->inputStack->set(inputStackIndex++, getAbstractTypeFromApiFormat(symbolTable, $nc(stack)->get(i)));
		$init($Opcodes);
		if ($equals(stack->get(i), $Opcodes::LONG) || $equals(stack->get(i), $Opcodes::DOUBLE)) {
			this->inputStack->set(inputStackIndex++, Frame::TOP);
		}
	}
...
int32_t Frame::getLocal(int32_t localIndex) {
	if (this->outputLocals == nullptr || localIndex >= this->outputLocals->length) {
		return Frame::LOCAL_KIND | localIndex;
	} else {
		int32_t abstractType = this->outputLocals->get(localIndex);
		if (abstractType == 0) {
			abstractType = (this->outputLocals->set(localIndex, Frame::LOCAL_KIND | localIndex));
		}
		return abstractType;
	}
}
...
void Frame::setLocal(int32_t localIndex, int32_t abstractType) {
	...
	this->outputLocals->set(localIndex, abstractType);
}
...
void Frame::push(int32_t abstractType) {
	...
	this->outputStack->set(this->outputStackTop++, abstractType);
...
int32_t Frame::pop() {
	if (this->outputStackTop > 0) {
		...
		return Frame::STACK_KIND | -(--this->outputStackStart);
	}
}
...
void Frame::addInitializedType(int32_t abstractType) {
	...
	this->initializations->set(this->initializationCount++, abstractType);
}

int32_t Frame::getInitializedType($SymbolTable* symbolTable, int32_t abstractType) {
	$useLocalObjectStack();
	if (abstractType == Frame::UNINITIALIZED_THIS || (abstractType & (Frame::DIM_MASK | Frame::KIND_MASK)) == Frame::UNINITIALIZED_KIND) {
		for (int32_t i = 0; i < this->initializationCount; ++i) {
			int32_t initializedType = $nc(this->initializations)->get(i);
			int32_t dim = initializedType & Frame::DIM_MASK;
			int32_t kind = initializedType & Frame::KIND_MASK;
			int32_t value = initializedType & Frame::VALUE_MASK;
			...
			if (abstractType == initializedType) {
				if (abstractType == Frame::UNINITIALIZED_THIS) {
					return Frame::REFERENCE_KIND | $nc(symbolTable)->addType($($nc(symbolTable)->getClassName()));
				} else {
					return Frame::REFERENCE_KIND | $nc(symbolTable)->addType($nc($($nc(symbolTable)->getType(abstractType & Frame::VALUE_MASK)))->value);
				}
			}
		}
	}
	...
	int32_t abstractType4 = 0;
	{
		...
		case $Opcodes::ASTORE:
			...
			if (arg > 0) {
				...
				} else if ((previousLocalType & Frame::KIND_MASK) == Frame::LOCAL_KIND || (previousLocalType & Frame::KIND_MASK) == Frame::STACK_KIND) {
		...
		case $Opcodes::INVOKEINTERFACE:
			...
			if (opcode != $Opcodes::INVOKESTATIC) {
				abstractType1 = pop();
				if (opcode == $Opcodes::INVOKESPECIAL && $nc(argSymbol->name)->charAt(0) == u'<') {
		...
		case $Opcodes::CHECKCAST:
			$assign(castType, $nc(argSymbol)->value);
			pop();
...
int32_t Frame::getConcreteOutputType(int32_t abstractOutputType, int32_t numStack) {
	int32_t dim = abstractOutputType & Frame::DIM_MASK;
	int32_t kind = abstractOutputType & Frame::KIND_MASK;
	if (kind == Frame::LOCAL_KIND) {
		int32_t concreteOutputType = dim + $nc(this->inputLocals)->get(abstractOutputType & Frame::VALUE_MASK);
		if ((abstractOutputType & Frame::TOP_IF_LONG_OR_DOUBLE_FLAG) != 0 && (concreteOutputType == Frame::LONG || concreteOutputType == Frame::DOUBLE)) {
			concreteOutputType = Frame::TOP;
		}
		return concreteOutputType;
	} else if (kind == Frame::STACK_KIND) {
		int32_t concreteOutputType = dim + $nc(this->inputStack)->get(numStack - (abstractOutputType & Frame::VALUE_MASK));
		if ((abstractOutputType & Frame::TOP_IF_LONG_OR_DOUBLE_FLAG) != 0 && (concreteOutputType == Frame::LONG || concreteOutputType == Frame::DOUBLE)) {
			concreteOutputType = Frame::TOP;
		}
		return concreteOutputType;
...
bool Frame::merge($SymbolTable* symbolTable, Frame* dstFrame, int32_t catchTypeIndex) {
	...
	for (int32_t i = 0; i < numLocal; ++i) {
		...
		if (this->outputLocals != nullptr && i < this->outputLocals->length) {
			int32_t abstractOutputType = this->outputLocals->get(i);
			if (abstractOutputType == 0) {
				concreteOutputType = this->inputLocals->get(i);
			} else {
	...
	if (catchTypeIndex > 0) {
		for (int32_t i = 0; i < numLocal; ++i) {
			frameChanged |= merge(symbolTable, this->inputLocals->get(i), dstFrame->inputLocals, i);
		}
		if (dstFrame->inputStack == nullptr) {
	...
	int32_t numInputStack = this->inputStack->length + this->outputStackStart;
	if (dstFrame->inputStack == nullptr) {
		$set(dstFrame, inputStack, $new($ints, numInputStack + this->outputStackTop));
		frameChanged = true;
	}
	for (int32_t i = 0; i < numInputStack; ++i) {
		int32_t concreteOutputType = this->inputStack->get(i);
		if (this->initializations != nullptr) {
			concreteOutputType = getInitializedType(symbolTable, concreteOutputType);
		}
...
bool Frame::merge($SymbolTable* symbolTable, int32_t sourceType, $ints* dstTypes, int32_t dstIndex) {
	...
	if ((sourceType & ~Frame::DIM_MASK) == Frame::NULL) {
	...
	if ((dstType & Frame::DIM_MASK) != 0 || (dstType & Frame::KIND_MASK) == Frame::REFERENCE_KIND) {
		if (srcType == Frame::NULL) {
			return false;
		} else if ((srcType & (Frame::DIM_MASK | Frame::KIND_MASK)) == (dstType & (Frame::DIM_MASK | Frame::KIND_MASK))) {
			if ((dstType & Frame::KIND_MASK) == Frame::REFERENCE_KIND) {
				mergedType = ((srcType & Frame::DIM_MASK) | Frame::REFERENCE_KIND) | $nc(symbolTable)->addMergedType(srcType & Frame::VALUE_MASK, dstType & Frame::VALUE_MASK);
			} else {
				int32_t mergedDim = Frame::ELEMENT_OF + (srcType & Frame::DIM_MASK);
				mergedType = (mergedDim | Frame::REFERENCE_KIND) | $nc(symbolTable)->addType("java/lang/Object"_s);
			}
		} else if ((srcType & Frame::DIM_MASK) != 0 || (srcType & Frame::KIND_MASK) == Frame::REFERENCE_KIND) {
			int32_t srcDim = srcType & Frame::DIM_MASK;
			if (srcDim != 0 && (srcType & Frame::KIND_MASK) != Frame::REFERENCE_KIND) {
				srcDim = Frame::ELEMENT_OF + srcDim;
			}
			int32_t dstDim = dstType & Frame::DIM_MASK;
			if (dstDim != 0 && (dstType & Frame::KIND_MASK) != Frame::REFERENCE_KIND) {
		...
		mergedType = (srcType & Frame::DIM_MASK) != 0 || (srcType & Frame::KIND_MASK) == Frame::REFERENCE_KIND ? srcType : Frame::TOP;
	} else {
		mergedType = Frame::TOP;
	}
...
void Frame::putAbstractType($SymbolTable* symbolTable, int32_t abstractType, $ByteVector* output) {
	$useLocalObjectStack();
	int32_t arrayDimensions = $sr(abstractType & Frame::DIM_MASK, Frame::DIM_SHIFT);
	if (arrayDimensions == 0) {
		int32_t typeValue = abstractType & Frame::VALUE_MASK;
		switch (abstractType & Frame::KIND_MASK) {
		case Frame::CONSTANT_KIND:
			$nc(output)->putByte(typeValue);
			break;
		case Frame::REFERENCE_KIND:
			$$nc($nc(output)->putByte(Frame::ITEM_OBJECT))->putShort($nc($($nc(symbolTable)->addConstantClass($nc($($nc(symbolTable)->getType(typeValue)))->value)))->index);
			break;
		...
		if ((abstractType & Frame::KIND_MASK) == Frame::REFERENCE_KIND) {
			typeDescriptor->append(u'L')->append($nc($($nc(symbolTable)->getType(abstractType & Frame::VALUE_MASK)))->value)->append(u';');
		} else {
...
$Class* Frame::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "(Ljdk/internal/org/objectweb/asm/Label;)V", nullptr, 0, $method(Frame, init$, void, $Label*)},
		{"accept", "(Ljdk/internal/org/objectweb/asm/MethodWriter;)V", nullptr, $FINAL, $method(Frame, accept, void, $MethodWriter*)},
		{"addInitializedType", "(I)V", nullptr, $PRIVATE, $method(Frame, addInitializedType, void, int32_t)},
		{"copyFrom", "(Ljdk/internal/org/objectweb/asm/Frame;)V", nullptr, $FINAL, $method(Frame, copyFrom, void, Frame*)},
		{"execute", "(IILjdk/internal/org/objectweb/asm/Symbol;Ljdk/internal/org/objectweb/asm/SymbolTable;)V", nullptr, 0, $virtualMethod(Frame, execute, void, int32_t, int32_t, $Symbol*, $SymbolTable*)},
		{"getAbstractTypeFromApiFormat", "(Ljdk/internal/org/objectweb/asm/SymbolTable;Ljava/lang/Object;)I", nullptr, $STATIC, $staticMethod(Frame, getAbstractTypeFromApiFormat, int32_t, $SymbolTable*, Object$*)},
		{"getAbstractTypeFromDescriptor", "(Ljdk/internal/org/objectweb/asm/SymbolTable;Ljava/lang/String;I)I", nullptr, $PRIVATE | $STATIC, $staticMethod(Frame, getAbstractTypeFromDescriptor, int32_t, $SymbolTable*, $String*, int32_t)},
		{"getAbstractTypeFromInternalName", "(Ljdk/internal/org/objectweb/asm/SymbolTable;Ljava/lang/String;)I", nullptr, $STATIC, $staticMethod(Frame, getAbstractTypeFromInternalName, int32_t, $SymbolTable*, $String*)},
		{"getConcreteOutputType", "(II)I", nullptr, $PRIVATE, $method(Frame, getConcreteOutputType, int32_t, int32_t, int32_t)},
		{"getInitializedType", "(Ljdk/internal/org/objectweb/asm/SymbolTable;I)I", nullptr, $PRIVATE, $method(Frame, getInitializedType, int32_t, $SymbolTable*, int32_t)},
		{"getInputStackSize", "()I", nullptr, $FINAL, $method(Frame, getInputStackSize, int32_t)},
		{"getLocal", "(I)I", nullptr, $PRIVATE, $method(Frame, getLocal, int32_t, int32_t)},
		{"merge", "(Ljdk/internal/org/objectweb/asm/SymbolTable;Ljdk/internal/org/objectweb/asm/Frame;I)Z", nullptr, $FINAL, $method(Frame, merge, bool, $SymbolTable*, Frame*, int32_t)},
		{"merge", "(Ljdk/internal/org/objectweb/asm/SymbolTable;I[II)Z", nullptr, $PRIVATE | $STATIC, $staticMethod(Frame, merge, bool, $SymbolTable*, int32_t, $ints*, int32_t)},
		{"pop", "()I", nullptr, $PRIVATE, $method(Frame, pop, int32_t)},
		{"pop", "(I)V", nullptr, $PRIVATE, $method(Frame, pop, void, int32_t)},
		{"pop", "(Ljava/lang/String;)V", nullptr, $PRIVATE, $method(Frame, pop, void, $String*)},
		{"push", "(I)V", nullptr, $PRIVATE, $method(Frame, push, void, int32_t)},
		{"push", "(Ljdk/internal/org/objectweb/asm/SymbolTable;Ljava/lang/String;)V", nullptr, $PRIVATE, $method(Frame, push, void, $SymbolTable*, $String*)},
		{"putAbstractType", "(Ljdk/internal/org/objectweb/asm/SymbolTable;ILjdk/internal/org/objectweb/asm/ByteVector;)V", nullptr, $STATIC, $staticMethod(Frame, putAbstractType, void, $SymbolTable*, int32_t, $ByteVector*)},
		{"setInputFrameFromApiFormat", "(Ljdk/internal/org/objectweb/asm/SymbolTable;I[Ljava/lang/Object;I[Ljava/lang/Object;)V", nullptr, $FINAL, $method(Frame, setInputFrameFromApiFormat, void, $SymbolTable*, int32_t, $ObjectArray*, int32_t, $ObjectArray*)},
		{"setInputFrameFromDescriptor", "(Ljdk/internal/org/objectweb/asm/SymbolTable;ILjava/lang/String;I)V", nullptr, $FINAL, $method(Frame, setInputFrameFromDescriptor, void, $SymbolTable*, int32_t, $String*, int32_t)},
		{"setLocal", "(II)V", nullptr, $PRIVATE, $method(Frame, setLocal, void, int32_t, int32_t)},
		{}
	};
	$ClassInfo classInfo$$ = {
		$ACC_SUPER,
