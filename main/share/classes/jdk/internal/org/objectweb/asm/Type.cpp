...
Type* Type::getObjectType($String* internalName) {
	...
	return $new(Type, var$0, internalName, 0, internalName->length());
}
...
$TypeArray* Type::getArgumentTypes($String* methodDescriptor) {
	...
	while ($nc(methodDescriptor)->charAt(currentOffset) != u')') {
		...
		if (methodDescriptor->charAt(currentOffset++) == u'L') {
			int32_t semiColumnOffset = methodDescriptor->indexOf(u';', currentOffset);
...
Type* Type::getReturnType($String* methodDescriptor) {
	$init(Type);
	int32_t var$0 = getReturnTypeOffset(methodDescriptor);
	return getTypeInternal(methodDescriptor, var$0, $nc(methodDescriptor)->length());
}
...
void Type::appendDescriptor($StringBuilder* stringBuilder) {
	if (this->sort == Type::OBJECT) {
		$nc(stringBuilder)->append(this->valueBuffer, this->valueBegin - 1, this->valueEnd + 1);
	} else if (this->sort == Type::INTERNAL) {
		$nc(stringBuilder)->append(u'L')->append(this->valueBuffer, this->valueBegin, this->valueEnd)->append(u';');
	} else {
		$nc(stringBuilder)->append(this->valueBuffer, this->valueBegin, this->valueEnd);
	}
}
...
bool Type::equals(Object$* object) {
	...
	if ((this->sort == Type::INTERNAL ? Type::OBJECT : this->sort) != ($nc(other)->sort == Type::INTERNAL ? Type::OBJECT : other->sort)) {
		return false;
	}
	int32_t begin = this->valueBegin;
	int32_t end = this->valueEnd;
	int32_t otherBegin = other->valueBegin;
	int32_t otherEnd = other->valueEnd;
	if (end - begin != otherEnd - otherBegin) {
		return false;
	}
	for (int32_t i = begin, j = otherBegin; i < end; ++i, ++j) {
...
int32_t Type::hashCode() {
	...
	if (this->sort >= Type::ARRAY) {
		for (int32_t i = this->valueBegin, end = this->valueEnd; i < end; ++i) {
