...
void Handler::init$(Handler* handler, $Label* startPc, $Label* endPc) {
	Handler::init$(startPc, endPc, $nc(handler)->handlerPc, $nc(handler)->catchType, $nc(handler)->catchTypeDescriptor);
	$set(this, nextHandler, handler->nextHandler);
}
...
Handler* Handler::removeRange(Handler* firstHandler, $Label* start, $Label* end) {
	if (firstHandler == nullptr) {
		...
		$set(firstHandler, nextHandler, removeRange(firstHandler->nextHandler, start, end));
	}
	...
	int32_t rangeEnd = end == nullptr ? $Integer::MAX_VALUE : end->bytecodeOffset;
