...
void FieldWriter::putFieldInfo($ByteVector* output) {
	...
	if (this->constantValueIndex != 0) {
		$init($Constants);
		$$nc($$nc(output->putShort(this->symbolTable->addConstantUtf8($Constants::CONSTANT_VALUE)))->putInt(2))->putShort(this->constantValueIndex);
	}
...
$Class* FieldWriter::load$($String* name, bool initialize) {
	...
	$loadClass(FieldWriter, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FieldWriter);
	});
	return class$;
}
