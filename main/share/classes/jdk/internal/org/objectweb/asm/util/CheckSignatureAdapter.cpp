...
$SignatureVisitor* CheckSignatureAdapter::visitClassBound() {
	...
	return $new(CheckSignatureAdapter, CheckSignatureAdapter::TYPE_SIGNATURE, this->signatureVisitor == nullptr ? ($SignatureVisitor*)nullptr : $(this->signatureVisitor->visitClassBound()));
}
...
$SignatureVisitor* CheckSignatureAdapter::visitInterfaceBound() {
	...
	return $new(CheckSignatureAdapter, CheckSignatureAdapter::TYPE_SIGNATURE, this->signatureVisitor == nullptr ? ($SignatureVisitor*)nullptr : $(this->signatureVisitor->visitInterfaceBound()));
}
...
$SignatureVisitor* CheckSignatureAdapter::visitSuperclass() {
	...
	return $new(CheckSignatureAdapter, CheckSignatureAdapter::TYPE_SIGNATURE, this->signatureVisitor == nullptr ? ($SignatureVisitor*)nullptr : $(this->signatureVisitor->visitSuperclass()));
}
...
$SignatureVisitor* CheckSignatureAdapter::visitInterface() {
	...
	return $new(CheckSignatureAdapter, CheckSignatureAdapter::TYPE_SIGNATURE, this->signatureVisitor == nullptr ? ($SignatureVisitor*)nullptr : $(this->signatureVisitor->visitInterface()));
}
...
$SignatureVisitor* CheckSignatureAdapter::visitParameterType() {
	...
	return $new(CheckSignatureAdapter, CheckSignatureAdapter::TYPE_SIGNATURE, this->signatureVisitor == nullptr ? ($SignatureVisitor*)nullptr : $(this->signatureVisitor->visitParameterType()));
}
...
$SignatureVisitor* CheckSignatureAdapter::visitReturnType() {
	...
	$var(CheckSignatureAdapter, checkSignatureAdapter, $new(CheckSignatureAdapter, CheckSignatureAdapter::TYPE_SIGNATURE, this->signatureVisitor == nullptr ? ($SignatureVisitor*)nullptr : $(this->signatureVisitor->visitReturnType())));
...
$SignatureVisitor* CheckSignatureAdapter::visitExceptionType() {
	...
	return $new(CheckSignatureAdapter, CheckSignatureAdapter::TYPE_SIGNATURE, this->signatureVisitor == nullptr ? ($SignatureVisitor*)nullptr : $(this->signatureVisitor->visitExceptionType()));
}
...
void CheckSignatureAdapter::visitBaseType(char16_t descriptor) {
	...
	} else if ("ZCBSIFJD"_s->indexOf(descriptor) == -1) {
...
$SignatureVisitor* CheckSignatureAdapter::visitArrayType() {
	...
	$set(this, state, $CheckSignatureAdapter$State::SIMPLE_TYPE);
	return $new(CheckSignatureAdapter, CheckSignatureAdapter::TYPE_SIGNATURE, this->signatureVisitor == nullptr ? ($SignatureVisitor*)nullptr : $(this->signatureVisitor->visitArrayType()));
}
...
$SignatureVisitor* CheckSignatureAdapter::visitTypeArgument(char16_t wildcard) {
	...
	if ("+-="_s->indexOf(wildcard) == -1) {
		$throwNew($IllegalArgumentException, "Wildcard must be one of +-="_s);
	}
	return $new(CheckSignatureAdapter, CheckSignatureAdapter::TYPE_SIGNATURE, this->signatureVisitor == nullptr ? ($SignatureVisitor*)nullptr : $(this->signatureVisitor->visitTypeArgument(wildcard)));
}
...
void CheckSignatureAdapter::clinit$($Class* clazz) {
	...
	$assignStatic(CheckSignatureAdapter::VISIT_INTERFACE_BOUND_STATES, $EnumSet::of($CheckSignatureAdapter$State::FORMAL, $CheckSignatureAdapter$State::BOUND));
...
$Class* CheckSignatureAdapter::load$($String* name, bool initialize) {
	...
	$loadClass(CheckSignatureAdapter, name, initialize, &classInfo$$, CheckSignatureAdapter::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(CheckSignatureAdapter);
	});
	return class$;
}
