...
$MethodVisitor* StaticInitMerger::visitMethod(int32_t access, $String* name, $String* descriptor, $String* signature, $StringArray* exceptions) {
	...
	if ("<clinit>"_s->equals(name)) {
		...
		var$0->append(this->renamedClinitMethodPrefix);
		var$0->append(this->numClinitMethods++);
		$var($String, newName, $str(var$0));
...
$Class* StaticInitMerger::load$($String* name, bool initialize) {
	...
	$loadClass(StaticInitMerger, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(StaticInitMerger);
	});
	return class$;
}
