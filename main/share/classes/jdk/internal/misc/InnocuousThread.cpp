...
$String* InnocuousThread::newName() {
	...
	return $str({"InnocuousThread-"_s, $$str(InnocuousThread::threadNumber->getAndIncrement())});
}
...
$Thread* InnocuousThread::newThread($String* name, $Runnable* target, int32_t priority) {
	...
	return $cast($Thread, $AccessController::doPrivileged($$new($InnocuousThread$1, name, target, priority)));
}
...
$Thread* InnocuousThread::newSystemThread($String* name, $Runnable* target, int32_t priority) {
	...
	return $cast($Thread, $AccessController::doPrivileged($$new($InnocuousThread$2, name, target, priority)));
}
...
void InnocuousThread::init$($ThreadGroup* group, $Runnable* target, $String* name, $ClassLoader* tccl) {
	...
	InnocuousThread::UNSAFE->putReferenceRelease(this, InnocuousThread::CONTEXTCLASSLOADER, tccl);
}
...
void InnocuousThread::eraseThreadLocals() {
	...
	InnocuousThread::UNSAFE->putReference(this, InnocuousThread::INHERITABLE_THREAD_LOCALS, nullptr);
}
...
void InnocuousThread::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$beforeCallerSensitive();
	$assignStatic(InnocuousThread::threadNumber, $new($AtomicInteger, 1));
	{
		try {
			...
			InnocuousThread::INHERITABLE_THREAD_LOCALS = InnocuousThread::UNSAFE->objectFieldOffset(tk, "inheritableThreadLocals"_s);
			InnocuousThread::INHERITEDACCESSCONTROLCONTEXT = InnocuousThread::UNSAFE->objectFieldOffset(tk, "inheritedAccessControlContext"_s);
			InnocuousThread::CONTEXTCLASSLOADER = InnocuousThread::UNSAFE->objectFieldOffset(tk, "contextClassLoader"_s);
			int64_t tg = InnocuousThread::UNSAFE->objectFieldOffset(tk, "group"_s);
			int64_t gp = InnocuousThread::UNSAFE->objectFieldOffset(gk, "parent"_s);
			$var($ThreadGroup, group, $cast($ThreadGroup, InnocuousThread::UNSAFE->getReference($($Thread::currentThread()), tg)));
			while (group != nullptr) {
				$var($ThreadGroup, parent, $cast($ThreadGroup, InnocuousThread::UNSAFE->getReference(group, gp)));
				if (parent == nullptr) {
					break;
				}
				$assign(group, parent);
			...
			if ($System::getSecurityManager() == nullptr) {
				...
				$assignStatic(InnocuousThread::INNOCUOUSTHREADGROUP, $cast($ThreadGroup, $AccessController::doPrivileged($$new($InnocuousThread$3, root))));
			}
		} catch ($Exception& e) {
			$throwNew($Error, e);
		}
...
$Class* InnocuousThread::load$($String* name, bool initialize) {
	...
	$loadClass(InnocuousThread, name, initialize, &classInfo$$, InnocuousThread::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(InnocuousThread);
	});
	return class$;
}
