...
void VM::saveProperties($Map* props) {
	...
	bool var$0 = s == nullptr || s->isEmpty();
	if (var$0 || s->equals("-1"_s)) {
	...
	try {
		VM::classFileMajorVersion = $($Integer::valueOf($(s->substring(0, index))))->intValue();
		VM::classFileMinorVersion = $($Integer::valueOf($(s->substring(index + 1, s->length()))))->intValue();
...
$Thread$State* VM::toThreadState(int32_t threadStatus) {
	$init(VM);
	if ((threadStatus & VM::JVMTI_THREAD_STATE_RUNNABLE) != 0) {
		$init($Thread$State);
		return $Thread$State::RUNNABLE;
	} else if ((threadStatus & VM::JVMTI_THREAD_STATE_BLOCKED_ON_MONITOR_ENTER) != 0) {
		$init($Thread$State);
		return $Thread$State::BLOCKED;
	} else if ((threadStatus & VM::JVMTI_THREAD_STATE_WAITING_INDEFINITELY) != 0) {
		$init($Thread$State);
		return $Thread$State::WAITING;
	} else if ((threadStatus & VM::JVMTI_THREAD_STATE_WAITING_WITH_TIMEOUT) != 0) {
		$init($Thread$State);
		return $Thread$State::TIMED_WAITING;
	} else if ((threadStatus & VM::JVMTI_THREAD_STATE_TERMINATED) != 0) {
		$init($Thread$State);
		return $Thread$State::TERMINATED;
	} else if ((threadStatus & VM::JVMTI_THREAD_STATE_ALIVE) == 0) {
		$init($Thread$State);
...
$ClassLoader* VM::latestUserDefinedLoader0() {
	$init(VM);
	$prepareNativeStatic(latestUserDefinedLoader0, $ClassLoader*);
	$var($ClassLoader, $ret, $invokeNativeStaticObject());
	$finishNativeStatic();
	return $ret;
}
...
int64_t VM::getuid() {
	$init(VM);
	$prepareNativeStatic(getuid, int64_t);
...
int64_t VM::geteuid() {
	$init(VM);
	$prepareNativeStatic(geteuid, int64_t);
	int64_t $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
int64_t VM::getgid() {
	$init(VM);
	$prepareNativeStatic(getgid, int64_t);
	int64_t $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
int64_t VM::getegid() {
	$init(VM);
	$prepareNativeStatic(getegid, int64_t);
	int64_t $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
int64_t VM::getNanoTimeAdjustment(int64_t offsetInSeconds) {
	$init(VM);
	$prepareNativeStatic(getNanoTimeAdjustment, int64_t, int64_t offsetInSeconds);
	int64_t $ret = $invokeNativeStatic(offsetInSeconds);
	$finishNativeStatic();
	return $ret;
}
...
$StringArray* VM::getRuntimeArguments() {
	$init(VM);
	$prepareNativeStatic(getRuntimeArguments, $StringArray*);
...
void VM::initialize() {
	$init(VM);
	$prepareNativeStatic(initialize, void);
	$invokeNativeStatic();
	$finishNativeStatic();
}
...
$Class* VM::load$($String* name, bool initialize) {
	...
	$loadClass(VM, name, initialize, &classInfo$$, VM::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(VM);
	});
	return class$;
}
