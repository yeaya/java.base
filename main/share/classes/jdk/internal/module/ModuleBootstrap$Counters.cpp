		...
		int64_t currentTime = $System::nanoTime();
		{
			...
			for (; $nc(i$)->hasNext();) {
				$var($Map$Entry, e, $cast($Map$Entry, i$->next()));
				{
					...
					int64_t value = $$sure($Long, e->getValue())->longValue();
...
void ModuleBootstrap$Counters::clinit$($Class* clazz) {
	{
		...
		if (s == nullptr) {
			...
			ModuleBootstrap$Counters::PRINT_COUNTERS = s->equals("debug"_s);
...
$Class* ModuleBootstrap$Counters::load$($String* name, bool initialize) {
	...
	$loadClass(ModuleBootstrap$Counters, name, initialize, &classInfo$$, ModuleBootstrap$Counters::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ModuleBootstrap$Counters);
	});
	return class$;
}
