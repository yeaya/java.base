	...
	for (int32_t i = 1; i < count; ++i) {
		int32_t tag = in->readUnsignedByte();
		{
			...
			case ModuleInfo$ConstantPool::CONSTANT_Utf8:
				...
				this->pool->set(i, $$new($ModuleInfo$ConstantPool$ValueEntry, tag, svalue));
				break;
			...
			case ModuleInfo$ConstantPool::CONSTANT_String:
				...
				this->pool->set(i, $$new($ModuleInfo$ConstantPool$IndexEntry, tag, index));
				break;
			case ModuleInfo$ConstantPool::CONSTANT_Double:
				dvalue = in->readDouble();
				this->pool->set(i, $$new($ModuleInfo$ConstantPool$ValueEntry, tag, $($Double::valueOf(dvalue))));
				++i;
				break;
			...
			case ModuleInfo$ConstantPool::CONSTANT_NameAndType:
				...
				this->pool->set(i, $$new($ModuleInfo$ConstantPool$Index2Entry, tag, index1, index2));
				break;
			case ModuleInfo$ConstantPool::CONSTANT_MethodHandle:
				refKind = in->readUnsignedByte();
				index = in->readUnsignedShort();
				this->pool->set(i, $$new($ModuleInfo$ConstantPool$Index2Entry, tag, refKind, index));
				break;
			...
			case ModuleInfo$ConstantPool::CONSTANT_Float:
				fvalue = in->readFloat();
				this->pool->set(i, $$new($ModuleInfo$ConstantPool$ValueEntry, tag, $($Float::valueOf(fvalue))));
				break;
			case ModuleInfo$ConstantPool::CONSTANT_Integer:
				ivalue = in->readInt();
				this->pool->set(i, $$new($ModuleInfo$ConstantPool$ValueEntry, tag, $($Integer::valueOf(ivalue))));
				break;
			case ModuleInfo$ConstantPool::CONSTANT_Long:
				lvalue = in->readLong();
				this->pool->set(i, $$new($ModuleInfo$ConstantPool$ValueEntry, tag, $($Long::valueOf(lvalue))));
				++i;
				break;
			default:
...
$String* ModuleInfo$ConstantPool::getClassName(int32_t index) {
	$useLocalObjectStack();
	checkIndex(index);
	$var($ModuleInfo$ConstantPool$Entry, e, this->pool->get(index));
	...
	$var($String, value, getUtf8($cast($ModuleInfo$ConstantPool$IndexEntry, e)->index));
...
$String* ModuleInfo$ConstantPool::getUtf8(int32_t index) {
	...
	return $cast($String, $cast($ModuleInfo$ConstantPool$ValueEntry, e)->value);
}
...
$Class* ModuleInfo$ConstantPool::load$($String* name, bool initialize) {
	...
	$loadClass(ModuleInfo$ConstantPool, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ModuleInfo$ConstantPool);
	});
	return class$;
}
