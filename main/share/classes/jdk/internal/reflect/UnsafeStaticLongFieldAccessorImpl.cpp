...
void UnsafeStaticLongFieldAccessorImpl::set(Object$* obj, Object$* value) {
	...
	if ($instanceOf($Byte, value)) {
		$init($UnsafeFieldAccessorImpl);
		$nc($UnsafeFieldAccessorImpl::unsafe)->putLong(this->base, this->fieldOffset, $cast($Byte, value)->byteValue());
		return;
	}
	if ($instanceOf($Short, value)) {
		$init($UnsafeFieldAccessorImpl);
		$nc($UnsafeFieldAccessorImpl::unsafe)->putLong(this->base, this->fieldOffset, $cast($Short, value)->shortValue());
		return;
	}
	if ($instanceOf($Character, value)) {
		$init($UnsafeFieldAccessorImpl);
		$nc($UnsafeFieldAccessorImpl::unsafe)->putLong(this->base, this->fieldOffset, $cast($Character, value)->charValue());
		return;
	}
	if ($instanceOf($Integer, value)) {
		$init($UnsafeFieldAccessorImpl);
		$nc($UnsafeFieldAccessorImpl::unsafe)->putLong(this->base, this->fieldOffset, $cast($Integer, value)->intValue());
		return;
	}
	if ($instanceOf($Long, value)) {
		$init($UnsafeFieldAccessorImpl);
		$nc($UnsafeFieldAccessorImpl::unsafe)->putLong(this->base, this->fieldOffset, $cast($Long, value)->longValue());
		return;
	}
	throwSetIllegalArgumentException(value);
}
...
$Class* UnsafeStaticLongFieldAccessorImpl::load$($String* name, bool initialize) {
	...
	$loadClass(UnsafeStaticLongFieldAccessorImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
