...
void UnsafeStaticBooleanFieldAccessorImpl::set(Object$* obj, Object$* value) {
	...
	if ($instanceOf($Boolean, value)) {
		$init($UnsafeFieldAccessorImpl);
		$nc($UnsafeFieldAccessorImpl::unsafe)->putBoolean(this->base, this->fieldOffset, $cast($Boolean, value)->booleanValue());
		return;
	}
	throwSetIllegalArgumentException(value);
}
...
$Class* UnsafeStaticBooleanFieldAccessorImpl::load$($String* name, bool initialize) {
	...
	$loadClass(UnsafeStaticBooleanFieldAccessorImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
