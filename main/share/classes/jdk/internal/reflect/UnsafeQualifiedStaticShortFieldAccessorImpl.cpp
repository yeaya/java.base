...
void UnsafeQualifiedStaticShortFieldAccessorImpl::set(Object$* obj, Object$* value) {
	...
	if ($instanceOf($Byte, value)) {
		$init($UnsafeFieldAccessorImpl);
		$nc($UnsafeFieldAccessorImpl::unsafe)->putShortVolatile(this->base, this->fieldOffset, $cast($Byte, value)->byteValue());
		return;
	}
	if ($instanceOf($Short, value)) {
		$init($UnsafeFieldAccessorImpl);
		$nc($UnsafeFieldAccessorImpl::unsafe)->putShortVolatile(this->base, this->fieldOffset, $cast($Short, value)->shortValue());
		return;
	}
	throwSetIllegalArgumentException(value);
}
...
$Class* UnsafeQualifiedStaticShortFieldAccessorImpl::load$($String* name, bool initialize) {
	...
	$loadClass(UnsafeQualifiedStaticShortFieldAccessorImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
