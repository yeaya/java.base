...
$Class* BuiltinClassLoader$$Lambda$lambda$findResourceOnClassPath$0::load$($String* name, bool initialize) {
	...
	$loadClass(BuiltinClassLoader$$Lambda$lambda$findResourceOnClassPath$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BuiltinClassLoader$$Lambda$lambda$findResourceOnClassPath$0);
	});
	return class$;
}
...
$Class* BuiltinClassLoader$$Lambda$lambda$findResourcesOnClassPath$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(BuiltinClassLoader$$Lambda$lambda$findResourcesOnClassPath$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BuiltinClassLoader$$Lambda$lambda$findResourcesOnClassPath$1$1);
	});
	return class$;
}
...
$Class* BuiltinClassLoader$$Lambda$lambda$findClassInModuleOrNull$2$2::load$($String* name, bool initialize) {
	...
	$loadClass(BuiltinClassLoader$$Lambda$lambda$findClassInModuleOrNull$2$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(BuiltinClassLoader$$Lambda$lambda$findClassInModuleOrNull$2$2);
	});
	return class$;
}
...
void BuiltinClassLoader::init$($String* name, BuiltinClassLoader* parent, $URLClassPath* ucp) {
	$SecureClassLoader::init$(name, parent == nullptr || parent == $ClassLoaders::bootLoader() ? ($ClassLoader*)nullptr : $cast($ClassLoader, parent));
	$set(this, parent, parent);
	$set(this, ucp, ucp);
	$set(this, nameToModule, $cast($AbstractMap, $new($ConcurrentHashMap, 32)));
...
$List* BuiltinClassLoader::findMiscResource($String* name) {
	...
	$var($Map, map, (ref != nullptr) ? $cast($Map, ref->get()) : ($Map*)nullptr);
	if (map == nullptr) {
		if ($VM::isModuleSystemInited()) {
			$assign(map, $cast($AbstractMap, $new($ConcurrentHashMap)));
			$set(this, resourceCache, $new($SoftReference, map));
		}
	} else {
		$var($List, urls, $cast($List, map->get(name)));
		if (urls != nullptr) {
			return urls;
		}
	}
	...
	try {
		$assign(urls, $cast($List, $AccessController::doPrivileged($$new($BuiltinClassLoader$2, this, name))));
...
$URL* BuiltinClassLoader::findResource($ModuleReference* mref, $String* name) {
	...
	if ($System::getSecurityManager() == nullptr) {
		...
		try {
			$assign(u, $cast($URI, $AccessController::doPrivileged($$new($BuiltinClassLoader$3, this, mref, name))));
		} catch ($PrivilegedActionException& pae) {
			$throw($$cast($IOException, pae->getCause()));
		}
	}
...
$URL* BuiltinClassLoader::findResourceOnClassPath($String* name) {
	...
	if (hasClassPath()) {
		if ($System::getSecurityManager() == nullptr) {
			...
			$var($PrivilegedAction, pa, $new(BuiltinClassLoader$$Lambda$lambda$findResourceOnClassPath$0, this, name));
...
$Enumeration* BuiltinClassLoader::findResourcesOnClassPath($String* name) {
	...
	if (hasClassPath()) {
		if ($System::getSecurityManager() == nullptr) {
			...
			$assign(pa, $new(BuiltinClassLoader$$Lambda$lambda$findResourcesOnClassPath$1$1, this, name));
		...
		if (c == nullptr) {
			...
			if (loadedModule != nullptr) {
				...
				if (loader == this) {
					...
					c = $nc(loader)->loadClassOrNull(cn);
				}
			} else {
...
$Class* BuiltinClassLoader::defineClass($String* cn, $BuiltinClassLoader$LoadedModule* loadedModule) {
	...
	try {
		...
		if ($instanceOf($ModulePatcher$PatchedModuleReader, reader)) {
			$var($Resource, r, $cast($ModulePatcher$PatchedModuleReader, reader)->findResource(rn));
			if (r != nullptr) {
...
$ModuleReader* BuiltinClassLoader::moduleReaderFor($ModuleReference* mref) {
	...
	if (reader == nullptr) {
		...
		$assign(reader, $cast($ModuleReader, this->moduleToReader->computeIfAbsent(mref, create)));
	}
	return reader;
}
...
void BuiltinClassLoader::clinit$($Class* clazz) {
	$useLocalObjectStack();
	$beforeCallerSensitive();
	...
	}
	{
		...
		if (archivedClassLoaders != nullptr) {
			...
			$assignStatic(BuiltinClassLoader::packageToModule$, $cast($AbstractMap, $new($ConcurrentHashMap, 1024)));
		}
	}
}
...
$Class* BuiltinClassLoader::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("jdk.internal.loader.BuiltinClassLoader$$Lambda$lambda$findResourceOnClassPath$0")) {
			return BuiltinClassLoader$$Lambda$lambda$findResourceOnClassPath$0::load$(name, initialize);
		}
		if (name->equals("jdk.internal.loader.BuiltinClassLoader$$Lambda$lambda$findResourcesOnClassPath$1$1")) {
			return BuiltinClassLoader$$Lambda$lambda$findResourcesOnClassPath$1$1::load$(name, initialize);
		}
		if (name->equals("jdk.internal.loader.BuiltinClassLoader$$Lambda$lambda$findClassInModuleOrNull$2$2")) {
			return BuiltinClassLoader$$Lambda$lambda$findClassInModuleOrNull$2$2::load$(name, initialize);
		}
	}
	$CompoundAttribute ucpfieldAnnotations$$[] = {
	...
	$FieldInfo fieldInfos$$[] = {
		...
		{"ucp", "Ljdk/internal/loader/URLClassPath;", nullptr, $PRIVATE, $field(BuiltinClassLoader, ucp), ucpfieldAnnotations$$},
	...
	$loadClass(BuiltinClassLoader, name, initialize, &classInfo$$, BuiltinClassLoader::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(BuiltinClassLoader);
	});
	return class$;
}
