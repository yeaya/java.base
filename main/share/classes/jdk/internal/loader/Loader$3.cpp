...
$Object* Loader$3::nextElement() {
	if ($nc(this->iterator)->hasNext()) {
		return $cast($URL, this->iterator->next());
	} else {
		return $cast($URL, $nc(this->val$e)->nextElement());
	}
}
...
$Class* Loader$3::load$($String* name, bool initialize) {
	...
	$loadClass(Loader$3, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Loader$3);
	});
	return class$;
}
