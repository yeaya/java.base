...
$FileAttributeView* JrtFileAttributeView::get($JrtPath* path, $Class* type, $LinkOptionArray* options) {
	...
	if (type == $BasicFileAttributeView::class$) {
		return $cast($FileAttributeView, $new(JrtFileAttributeView, path, false, options));
	}
	if (type == JrtFileAttributeView::class$) {
		return $cast($FileAttributeView, $new(JrtFileAttributeView, path, true, options));
	}
	return nullptr;
}
...
void JrtFileAttributeView::setAttribute($JrtPath* path, $String* attribute$renamed, Object$* value) {
	...
	try {
		...
		} else if (id == $JrtFileAttributeView$AttrID::lastAccessTime) {
			$nc(path)->setTimes(nullptr, $cast($FileTime, value), nullptr);
		} else if (id == $JrtFileAttributeView$AttrID::creationTime) {
...
$Object* JrtFileAttributeView::attribute($JrtFileAttributeView$AttrID* id, $JrtFileAttributes* jrtfas, bool isJrtView) {
	...
	case 2:
		return $nc(jrtfas)->creationTime();
	case 3:
		return $nc(jrtfas)->lastAccessTime();
	case 4:
		return $nc(jrtfas)->lastModifiedTime();
	case 5:
...
$Class* JrtFileAttributeView::load$($String* name, bool initialize) {
	...
	$loadClass(JrtFileAttributeView, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(JrtFileAttributeView);
	});
	return class$;
}
