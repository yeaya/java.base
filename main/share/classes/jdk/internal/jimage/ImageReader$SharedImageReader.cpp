...
$Class* ImageReader$SharedImageReader$$Lambda$lambda$handlePackages$0::load$($String* name, bool initialize) {
	...
	$loadClass(ImageReader$SharedImageReader$$Lambda$lambda$handlePackages$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ImageReader$SharedImageReader$$Lambda$lambda$handlePackages$0);
	});
	return class$;
}
...
$Class* ImageReader$SharedImageReader$$Lambda$lambda$handleModulesSubTree$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(ImageReader$SharedImageReader$$Lambda$lambda$handleModulesSubTree$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ImageReader$SharedImageReader$$Lambda$lambda$handleModulesSubTree$1$1);
	});
	return class$;
}
...
$ImageReader* ImageReader$SharedImageReader::open($Path* imagePath, $ByteOrder* byteOrder) {
	...
	$synchronized(ImageReader$SharedImageReader::OPEN_FILES) {
		$var(ImageReader$SharedImageReader, reader, $cast(ImageReader$SharedImageReader, ImageReader$SharedImageReader::OPEN_FILES->get(imagePath)));
		if (reader == nullptr) {
			$assign(reader, $new(ImageReader$SharedImageReader, imagePath, byteOrder));
			ImageReader$SharedImageReader::OPEN_FILES->put(imagePath, reader);
...
void ImageReader$SharedImageReader::close($ImageReader* image) {
	...
	$synchronized(ImageReader$SharedImageReader::OPEN_FILES) {
		...
		if (this->openers->isEmpty()) {
			...
			if (!ImageReader$SharedImageReader::OPEN_FILES->remove($(this->getImagePath()), this)) {
...
$ImageReader$Node* ImageReader$SharedImageReader::handlePackages($String* name, $ImageLocation* loc) {
	...
	if ($nc(name)->equals("/packages"_s)) {
		visitLocation(loc, $$new(ImageReader$SharedImageReader$$Lambda$lambda$handlePackages$0, this));
...
$ImageReader$Node* ImageReader$SharedImageReader::handleModulesSubTree($String* name, $ImageLocation* loc) {
	...
	visitLocation(loc, $$new(ImageReader$SharedImageReader$$Lambda$lambda$handleModulesSubTree$1$1, this, dir));
...
void ImageReader$SharedImageReader::clinit$($Class* clazz) {
	$load($ImageReader);
...
$Class* ImageReader$SharedImageReader::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("jdk.internal.jimage.ImageReader$SharedImageReader$$Lambda$lambda$handlePackages$0")) {
			return ImageReader$SharedImageReader$$Lambda$lambda$handlePackages$0::load$(name, initialize);
		}
		if (name->equals("jdk.internal.jimage.ImageReader$SharedImageReader$$Lambda$lambda$handleModulesSubTree$1$1")) {
	...
	$loadClass(ImageReader$SharedImageReader, name, initialize, &classInfo$$, ImageReader$SharedImageReader::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ImageReader$SharedImageReader);
	});
	return class$;
}
