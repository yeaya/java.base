...
ImageStream* ImageStream::align(int32_t alignment) {
	int32_t padding = (getSize() - 1) & (($sl(1, alignment)) - 1);
...
void ImageStream::ensure(int32_t needs) {
	...
	if (needs > $nc(this->buffer)->remaining()) {
		...
		int32_t newSize = needs <= $nc(bytes)->length ? bytes->length << 1 : position + needs;
...
int32_t ImageStream::get() {
	return $nc(this->buffer)->get() & 0xff;
}
...
ImageStream* ImageStream::put(ImageStream* stream) {
	...
	put(var$0, 0, stream->buffer->position());
	return this;
}
...
$bytes* ImageStream::toArray() {
	...
	return $Arrays::copyOf(var$0, this->buffer->position());
}
...
$Class* ImageStream::load$($String* name, bool initialize) {
	...
	$loadClass(ImageStream, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ImageStream);
	});
	return class$;
}
