...
$Enum* SimpleConsoleLogger::logLevel($PlatformLogger$Level* level) {
	return this->usePlatformLevel ? $cast($Enum, level) : $cast($Enum, $nc(level)->systemLevel());
}

$Enum* SimpleConsoleLogger::logLevel($System$Logger$Level* level) {
	return this->usePlatformLevel ? $cast($Enum, $PlatformLogger::toPlatformLevel(level)) : $cast($Enum, level);
}
...
$String* SimpleConsoleLogger::getCallerInfo() {
	...
	if ($nc(frame)->isPresent()) {
		...
		var$0->append($($$sure($StackWalker$StackFrame, frame->get())->getClassName()));
		var$0->append(" "_s);
		var$0->append($($$sure($StackWalker$StackFrame, frame->get())->getMethodName()));
		return $str(var$0);
	} else {
		return this->name;
	}
...
void SimpleConsoleLogger::publish($String* callerInfo, $Enum* level, $String* msg$renamed, $ObjectArray* params) {
	...
	$assign(msg, params == nullptr || params->length == 0 ? msg : $SimpleConsoleLogger$Formatting::formatMessage(msg, params));
	$$nc(outputStream())->print($(format(level, msg, nullptr, callerInfo)));
}
...
void SimpleConsoleLogger::clinit$($Class* clazz) {
...
$Class* SimpleConsoleLogger::load$($String* name, bool initialize) {
	...
	$loadClass(SimpleConsoleLogger, name, initialize, &classInfo$$, SimpleConsoleLogger::clinit$, []($Class* clazz) -> $Object* {
