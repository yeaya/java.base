...
$String* Normalizer2::normalize($CharSequence* src) {
	...
	if ($instanceOf($String, src)) {
		...
		if (spanLength != 0) {
			$var($StringBuilder, sb, $$new($StringBuilder, src->length())->append(src, 0, spanLength));
			return $$nc(normalizeSecondAndAppend(sb, $(src->subSequence(spanLength, src->length()))))->toString();
		}
	}
...
$Class* Normalizer2::load$($String* name, bool initialize) {
	...
	$loadClass(Normalizer2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Normalizer2);
	});
	return class$;
}
