...
void Trie::init$($InputStream* inputStream, $Trie$DataManipulate* dataManipulate) {
	...
	this->m_isLatin1Linear_ = (this->m_options_ & Trie::HEADER_OPTIONS_LATIN1_IS_LINEAR_MASK_) != 0;
...
int32_t Trie::getRawOffset(int32_t offset, char16_t ch) {
	return ($sl($nc(this->m_index_)->get(offset + ($sr(ch, Trie::INDEX_STAGE_1_SHIFT_))), Trie::INDEX_STAGE_2_SHIFT_)) + (ch & Trie::INDEX_STAGE_3_MASK_);
}
...
int32_t Trie::getCodePointOffset(int32_t ch) {
	if (ch < 0) {
		...
		return getSurrogateOffset($UTF16::getLeadSurrogate(ch), (char16_t)(ch & Trie::SURROGATE_MASK_));
	} else {
		return -1;
	}
}
...
bool Trie::isCharTrie() {
	return (this->m_options_ & Trie::HEADER_OPTIONS_DATA_IS_32_BIT_) == 0;
}
...
bool Trie::checkHeader(int32_t signature) {
	...
	if ((this->m_options_ & Trie::HEADER_OPTIONS_SHIFT_MASK_) != Trie::INDEX_STAGE_1_SHIFT_ || (($sr(this->m_options_, Trie::HEADER_OPTIONS_INDEX_SHIFT_)) & Trie::HEADER_OPTIONS_SHIFT_MASK_) != Trie::INDEX_STAGE_2_SHIFT_) {
		return false;
	}
	return true;
}
