...
void StringPrepDataReader::init$($InputStream* inputStream) {
	$set(this, unicodeVersion, $ICUBinary::readHeader(inputStream, StringPrepDataReader::DATA_FORMAT_ID, this));
...
void StringPrepDataReader::read($bytes* idnaBytes, $chars* mappingTable) {
	...
	for (int32_t i = 0; i < $nc(mappingTable)->length; ++i) {
		mappingTable->set(i, this->dataInputStream->readChar());
	}
}
...
bool StringPrepDataReader::isDataVersionAcceptable($bytes* version) {
	return $nc(version)->get(0) == StringPrepDataReader::DATA_FORMAT_VERSION->get(0) && version->get(2) == StringPrepDataReader::DATA_FORMAT_VERSION->get(2) && version->get(3) == StringPrepDataReader::DATA_FORMAT_VERSION->get(3);
}
...
void StringPrepDataReader::clinit$($Class* clazz) {
...
$Class* StringPrepDataReader::load$($String* name, bool initialize) {
	...
	$loadClass(StringPrepDataReader, name, initialize, &classInfo$$, StringPrepDataReader::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(StringPrepDataReader);
	});
	return class$;
}
