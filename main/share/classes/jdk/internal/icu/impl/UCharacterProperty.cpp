...
$VersionInfo* UCharacterProperty::getAge(int32_t codepoint) {
	...
	return $VersionInfo::getInstance(($sr(version, UCharacterProperty::FIRST_NIBBLE_SHIFT_)) & UCharacterProperty::LAST_NIBBLE_MASK_, version & UCharacterProperty::LAST_NIBBLE_MASK_, 0, 0);
}

int32_t UCharacterProperty::getType(int32_t c) {
	return getProperty(c) & UCharacterProperty::TYPE_MASK;
}
...
int32_t UCharacterProperty::getRawSupplementary(char16_t lead, char16_t trail) {
	...
	return ($sl(lead, UCharacterProperty::LEAD_SURROGATE_SHIFT_)) + trail + UCharacterProperty::SURROGATE_OFFSET_;
}
...
int32_t UCharacterProperty::mergeScriptCodeOrIndex(int32_t scriptX) {
	$init(UCharacterProperty);
	return ($sr(scriptX & UCharacterProperty::SCRIPT_HIGH_MASK, UCharacterProperty::SCRIPT_HIGH_SHIFT)) | (scriptX & UCharacterProperty::SCRIPT_LOW_MASK);
}
...
void UCharacterProperty::clinit$($Class* clazz) {
	$init($VersionInfo);
...
$Class* UCharacterProperty::load$($String* name, bool initialize) {
	...
	$loadClass(UCharacterProperty, name, initialize, &classInfo$$, UCharacterProperty::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(UCharacterProperty);
	});
	return class$;
}
