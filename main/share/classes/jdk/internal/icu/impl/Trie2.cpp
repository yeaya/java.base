...
Trie2* Trie2::createFromSerialized($ByteBuffer* bytes) {
	...
	try {
		...
		if ((header->options & Trie2::UTRIE2_OPTIONS_VALUE_BITS_MASK) != 0) {
		...
		This->errorValue = This->index->get(This->data16 + Trie2::UTRIE2_BAD_UTF8_DATA_OFFSET);
		$assign(var$2, This);
		return$1 = true;
		goto $finally;
...
$Class* Trie2::load$($String* name, bool initialize) {
	...
	$loadClass(Trie2, name, initialize, &classInfo$$, Trie2::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Trie2);
	});
	return class$;
}
