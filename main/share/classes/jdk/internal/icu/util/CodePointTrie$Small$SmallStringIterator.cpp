...
bool CodePointTrie$Small$SmallStringIterator::next() {
	...
	char16_t lead = this->s->charAt(this->sIndex++);
	this->c = lead;
	int32_t dataIndex = 0;
	if (!$Character::isSurrogate(lead)) {
		...
		bool var$0 = var$1 && this->sIndex < this->s->length();
		if (var$0 && $Character::isLowSurrogate(trail = this->s->charAt(this->sIndex))) {
			++this->sIndex;
...
bool CodePointTrie$Small$SmallStringIterator::previous() {
	...
	if (!$Character::isSurrogate(trail)) {
		...
		if (var$0 && $Character::isHighSurrogate(lead = this->s->charAt(this->sIndex - 1))) {
			--this->sIndex;
			this->c = $Character::toCodePoint(lead, trail);
			$init($CodePointTrie$Type);
			dataIndex = this->this$0->smallIndex($CodePointTrie$Type::SMALL, this->c);
...
$Class* CodePointTrie$Small$SmallStringIterator::load$($String* name, bool initialize) {
	...
	$loadClass(CodePointTrie$Small$SmallStringIterator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CodePointTrie$Small$SmallStringIterator);
	});
	return class$;
}
