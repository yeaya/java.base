...
CodePointTrie* CodePointTrie::fromBinary($CodePointTrie$Type* type$renamed, $CodePointTrie$ValueWidth* valueWidth$renamed, $ByteBuffer* bytes) {
	...
	try {
		...
		int32_t valueWidthInt = options & CodePointTrie::OPTIONS_VALUE_BITS_MASK;
		...
		if ((options & CodePointTrie::OPTIONS_RESERVED_MASK) != 0) {
		...
		dataLength |= ((options & CodePointTrie::OPTIONS_DATA_LENGTH_MASK) << 4);
		dataNullOffset |= ((options & CodePointTrie::OPTIONS_DATA_NULL_OFFSET_MASK) << 8);
		...
		case 1:
			{
				...
				$init($CodePointTrie$Type);
				$assign(var$2, type == $CodePointTrie$Type::FAST ? $cast(CodePointTrie, $new($CodePointTrie$Fast16, index, data16, highStart, index3NullOffset, dataNullOffset)) : $cast(CodePointTrie, $new($CodePointTrie$Small16, index, data16, highStart, index3NullOffset, dataNullOffset)));
				return$1 = true;
				goto $finally;
			}
		case 2:
			{
				...
				$init($CodePointTrie$Type);
				$assign(var$2, type == $CodePointTrie$Type::FAST ? $cast(CodePointTrie, $new($CodePointTrie$Fast32, index, data32, highStart, index3NullOffset, dataNullOffset)) : $cast(CodePointTrie, $new($CodePointTrie$Small32, index, data32, highStart, index3NullOffset, dataNullOffset)));
				return$1 = true;
				goto $finally;
			}
		case 3:
			{
				...
				$init($CodePointTrie$Type);
				$assign(var$2, type == $CodePointTrie$Type::FAST ? $cast(CodePointTrie, $new($CodePointTrie$Fast8, index, data8, highStart, index3NullOffset, dataNullOffset)) : $cast(CodePointTrie, $new($CodePointTrie$Small8, index, data8, highStart, index3NullOffset, dataNullOffset)));
				return$1 = true;
				goto $finally;
			}
		default:
...
bool CodePointTrie::getRange(int32_t start, $CodePointMap$ValueFilter* filter, $CodePointMap$Range* range) {
	...
	do {
		...
		if (c <= 0x0000ffff && (type == $CodePointTrie$Type::FAST || c <= CodePointTrie::SMALL_MAX)) {
			...
			i3Block = $nc(this->index)->get($nc(this->index)->get(i1) + (($sr(c, CodePointTrie::SHIFT_2)) & CodePointTrie::INDEX_2_MASK));
			if (i3Block == prevI3Block && (c - start) >= CodePointTrie::CP_PER_INDEX_2_ENTRY) {
				if (!CodePointTrie::$assertionsDisabled && !((c & (CodePointTrie::CP_PER_INDEX_2_ENTRY - 1)) == 0)) {
					$throwNew($AssertionError);
				}
			...
			i3 = ($sr(c, CodePointTrie::SHIFT_3)) & CodePointTrie::INDEX_3_MASK;
		...
		do {
			...
			if ((i3Block & 0x8000) == 0) {
				...
				int32_t gi = i3 & 7;
				block = ($sl($nc(this->index)->get(group++), 2 + (2 * gi))) & 0x00030000;
				block |= this->index->get(group + gi);
			}
			if (block == prevBlock && (c - start) >= dataBlockLength) {
				if (!CodePointTrie::$assertionsDisabled && !((c & (dataBlockLength - 1)) == 0)) {
					$throwNew($AssertionError);
				}
				c += dataBlockLength;
			} else {
...
int32_t CodePointTrie::toBinary($OutputStream* os) {
	...
	try {
		...
		int32_t var$0 = (((this->dataLength & 0x000f0000) >> 4) | ((this->dataNullOffset & 0x000f0000) >> 8)) | ($$nc(getType())->ordinal() << 6);
...
int32_t CodePointTrie::fastIndex(int32_t c) {
	return $nc(this->index)->get($sr(c, CodePointTrie::FAST_SHIFT)) + (c & CodePointTrie::FAST_DATA_MASK);
}
...
int32_t CodePointTrie::internalSmallIndex($CodePointTrie$Type* type, int32_t c) {
	...
	int32_t i3Block = $nc(this->index)->get($nc(this->index)->get(i1) + (($sr(c, CodePointTrie::SHIFT_2)) & CodePointTrie::INDEX_2_MASK));
	int32_t i3 = ($sr(c, CodePointTrie::SHIFT_3)) & CodePointTrie::INDEX_3_MASK;
	int32_t dataBlock = 0;
	if ((i3Block & 0x8000) == 0) {
		dataBlock = this->index->get(i3Block + i3);
	} else {
		i3Block = (i3Block & 0x7fff) + (i3 & ~7) + (i3 >> 3);
		i3 &= (uint32_t)7;
		dataBlock = ($sl(this->index->get(i3Block++), 2 + (2 * i3))) & 0x00030000;
		dataBlock |= this->index->get(i3Block + i3);
	}
	return dataBlock + (c & CodePointTrie::SMALL_DATA_MASK);
}

void CodePointTrie::clinit$($Class* clazz) {
...
$Class* CodePointTrie::load$($String* name, bool initialize) {
	$CompoundAttribute datafieldAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute dataLengthfieldAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute highStartfieldAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$FieldInfo fieldInfos$$[] = {
		...
		{"data", "Ljdk/internal/icu/util/CodePointTrie$Data;", nullptr, $PROTECTED | $FINAL | $DEPRECATED, $field(CodePointTrie, data), datafieldAnnotations$$},
		{"dataLength", "I", nullptr, $PROTECTED | $FINAL | $DEPRECATED, $field(CodePointTrie, dataLength), dataLengthfieldAnnotations$$},
		{"highStart", "I", nullptr, $PROTECTED | $FINAL | $DEPRECATED, $field(CodePointTrie, highStart), highStartfieldAnnotations$$},
	...
	$CompoundAttribute fastIndexmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$CompoundAttribute smallIndexmethodAnnotations$$[] = {
		{"Ljava/lang/Deprecated;", nullptr},
		{}
	};
	$MethodInfo methodInfos$$[] = {
		...
		{"cpIndex", "(I)I", nullptr, $PROTECTED | $ABSTRACT | $DEPRECATED, $virtualMethod(CodePointTrie, cpIndex, int32_t, int32_t), nullptr, nullptr, cpIndexmethodAnnotations$$},
		{"fastIndex", "(I)I", nullptr, $PROTECTED | $FINAL | $DEPRECATED, $method(CodePointTrie, fastIndex, int32_t, int32_t), nullptr, nullptr, fastIndexmethodAnnotations$$},
		...
		{"smallIndex", "(Ljdk/internal/icu/util/CodePointTrie$Type;I)I", nullptr, $PROTECTED | $FINAL | $DEPRECATED, $method(CodePointTrie, smallIndex, int32_t, $CodePointTrie$Type*, int32_t), nullptr, nullptr, smallIndexmethodAnnotations$$},
	...
	$loadClass(CodePointTrie, name, initialize, &classInfo$$, CodePointTrie::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(CodePointTrie);
	});
	return class$;
}
