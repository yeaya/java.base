...
$HashMap* SystemProps$Raw::cmdProperties() {
	...
	for (int32_t i = 0; i < vmProps->length;) {
...
$StringArray* SystemProps$Raw::vmProperties() {
	...
	$prepareNativeStatic(vmProperties, $StringArray*);
...
$StringArray* SystemProps$Raw::platformProperties() {
	$init(SystemProps$Raw);
	$prepareNativeStatic(platformProperties, $StringArray*);
	$var($StringArray, $ret, $invokeNativeStaticObject());
	$finishNativeStatic();
	return $ret;
}
...
$Class* SystemProps$Raw::load$($String* name, bool initialize) {
	...
	$loadClass(SystemProps$Raw, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SystemProps$Raw);
	});
	return class$;
}
