...
void PBEParameters::engineInit($bytes* encoded) {
	try {
		...
		$nc(val->data$)->reset();
		$set(this, salt, val->data$->getOctetString());
...
$String* PBEParameters::engineToString() {
	...
	return $str({saltString, LINE_SEP, "    iterationCount:"_s, LINE_SEP, $($Debug::toHexString($($BigInteger::valueOf(this->iCount)))), LINE_SEP});
}
...
$Class* PBEParameters::load$($String* name, bool initialize) {
	...
	$loadClass(PBEParameters, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PBEParameters);
	});
	return class$;
}
