...
void PBES2Core::engineInit(int32_t opmode, $Key* key, $AlgorithmParameterSpec* params, $SecureRandom* random) {
	...
	try {
		...
		if ($instanceOf($PBEKey, key)) {
			$set(this, salt, $cast($PBEKey, key)->getSalt());
			if (this->salt != nullptr && this->salt->length < 8) {
		...
		if (params == nullptr) {
			...
			$var($bytes, specSalt, $cast($PBEParameterSpec, params)->getSalt());
	...
	try {
		try {
			$assign(s, $cast($PBKDF2KeyImpl, this->kdf->engineGenerateSecret(pbeSpec)));
	...
	this->cipher->init(opmode, cipherKey, this->ivSpec, random);
}
...
$Class* PBES2Core::load$($String* name, bool initialize) {
	...
	$loadClass(PBES2Core, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PBES2Core);
	});
	return class$;
}
