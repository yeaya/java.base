...
void OAEPParameters::engineInit($AlgorithmParameterSpec* paramSpec) {
	...
	if ($$nc($nc(pSrc)->getAlgorithm())->equals("PSpecified"_s)) {
		$set(this, p, $cast($PSource$PSpecified, pSrc)->getValue());
	} else {
...
void OAEPParameters::engineInit($bytes* encoded) {
	...
	for (int32_t i = 0; i < $nc(datum)->length; ++i) {
		...
		if ($nc(data)->isContextSpecific((int8_t)0)) {
			...
			$var($DerInputStream, dis, $new($DerInputStream, $(val->getEncodedParams())));
...
void OAEPParameters::clinit$($Class* clazz) {
	$init($KnownOIDs);
...
$Class* OAEPParameters::load$($String* name, bool initialize) {
	...
	$loadClass(OAEPParameters, name, initialize, &classInfo$$, OAEPParameters::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(OAEPParameters);
	});
	return class$;
}
