...
$PublicKey* ConstructKeys::constructPublicKey($bytes* encodedKey, int32_t ofs, int32_t len, $String* encodedKeyAlgorithm) {
	...
	try {
		$var($KeyFactory, keyFactory, $KeyFactory::getInstance(encodedKeyAlgorithm, $($SunJCE::getInstance())));
...
$Class* ConstructKeys::load$($String* name, bool initialize) {
	...
	$loadClass(ConstructKeys, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ConstructKeys);
	});
	return class$;
}
