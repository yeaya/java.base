...
$bytes* KeyProtector::protect($PrivateKey* key) {
	...
	try {
		...
		cipher->engineInit($Cipher::ENCRYPT_MODE, sKey, pbeSpec, nullptr);
	...
	$var($AlgorithmParameters, pbeParams, $AlgorithmParameters::getInstance("PBE"_s, $($SunJCE::getInstance())));
	$nc(pbeParams)->init(pbeSpec);
	$init($KnownOIDs);
	...
	try {
		try {
			...
			if (encrAlg->equals($($KnownOIDs::JAVASOFT_JDKKeyProtector->value()))) {
				...
				$var($PBEWithMD5AndTripleDESCipher, cipher, nullptr);
				$assign(cipher, $new($PBEWithMD5AndTripleDESCipher));
				cipher->engineInit($Cipher::DECRYPT_MODE, sKey, pbeSpec, nullptr);
...
$bytes* KeyProtector::recover($bytes* protectedKey) {
	...
	for (i = 0, j = 0; i < this->password->length; ++i) {
		passwdBytes->set(j++, (int8_t)(this->password->get(i) >> 8));
		passwdBytes->set(j++, (int8_t)this->password->get(i));
	}
