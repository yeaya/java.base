...
void DHKeyPairGenerator::init$() {
	...
	initialize($SecurityProviderConstants::DEF_DH_KEY_SIZE, nullptr);
}
...
$KeyPair* DHKeyPairGenerator::generateKeyPair() {
	...
	$var($BigInteger, g, this->params->getG());
	if (this->lSize <= 0) {
	...
	bool var$0 = false;
	do {
		$assign(x, $new($BigInteger, this->lSize, this->random));
		bool var$1 = x->compareTo($BigInteger::ONE) < 0;
		var$0 = var$1 || (x->compareTo(pMinus2) > 0);
...
$Class* DHKeyPairGenerator::load$($String* name, bool initialize) {
	...
	$loadClass(DHKeyPairGenerator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DHKeyPairGenerator);
	});
	return class$;
}
