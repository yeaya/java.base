...
void DESedeWrapCipher::engineInit(int32_t opmode, $Key* key, $AlgorithmParameterSpec* params, $SecureRandom* random$renamed) {
	...
	if (opmode == $Cipher::WRAP_MODE) {
		...
		if (params == nullptr) {
			...
			$set(this, iv, $cast($IvParameterSpec, params)->getIV());
		} else {
...
$AlgorithmParameters* DESedeWrapCipher::engineGetParameters() {
	...
	if (this->iv != nullptr) {
		...
		try {
			$assign(params, $AlgorithmParameters::getInstance(algo, $($SunJCE::getInstance())));
			$nc(params)->init($$new($IvParameterSpec, this->iv));
...
$bytes* DESedeWrapCipher::engineWrap($Key* key) {
	...
	if ((keyVal == nullptr) || (keyVal->length == 0)) {
	...
	try {
		...
		$System::arraycopy(cks, 0, in, keyVal->length, DESedeWrapCipher::CHECKSUM_LEN);
		$System::arraycopy(this->iv, 0, out, 0, this->iv->length);
		$nc(this->cipher)->encrypt(in, 0, in->length, out, this->iv->length);
		...
		try {
			this->cipher->init(false, $(this->cipherKey->getAlgorithm()), cipherKeyEncoded, DESedeWrapCipher::IV2);
		...
		try {
			this->cipher->init(this->decrypting, $(this->cipherKey->getAlgorithm()), cipherKeyEncoded, this->iv);
		} catch ($InvalidKeyException& ike) {
			$throwNew($RuntimeException, "Internal cipher key is corrupted"_s);
		} catch ($InvalidAlgorithmParameterException& iape) {
			$throwNew($RuntimeException, "Internal cipher IV is invalid"_s);
...
$Key* DESedeWrapCipher::engineUnwrap($bytes* wrappedKey, $String* wrappedKeyAlgorithm, int32_t wrappedKeyType) {
	...
	$var($bytes, buffer, $new($bytes, wrappedKey->length));
	...
	$var($bytes, buffer2, $new($bytes, buffer->length - this->iv->length));
	$var($Throwable, var$0, nullptr);
	$var($Key, var$2, nullptr);
	bool return$1 = false;
	try {
		try {
			this->cipher->init(true, $(this->cipherKey->getAlgorithm()), cipherKeyEncoded, this->iv);
		} catch ($InvalidAlgorithmParameterException& iape) {
		...
		try {
			this->cipher->init(this->decrypting, $(this->cipherKey->getAlgorithm()), cipherKeyEncoded, DESedeWrapCipher::IV2);
		} catch ($InvalidAlgorithmParameterException& iape) {
			$throwNew($InvalidKeyException, "IV in wrapped key is invalid"_s);
		}
...
void DESedeWrapCipher::clinit$($Class* clazz) {
...
$Class* DESedeWrapCipher::load$($String* name, bool initialize) {
	...
	$loadClass(DESedeWrapCipher, name, initialize, &classInfo$$, DESedeWrapCipher::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DESedeWrapCipher);
	});
	return class$;
}
