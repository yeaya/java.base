...
void NTLM$Writer::writeShort(int32_t offset, int32_t number) {
	...
	this->internal->set(offset + 1, (int8_t)(number >> 8));
}

void NTLM$Writer::writeInt(int32_t offset, int32_t number) {
	$nc(this->internal)->set(offset, (int8_t)(number));
	this->internal->set(offset + 1, (int8_t)(number >> 8));
	this->internal->set(offset + 2, (int8_t)(number >> 16));
	this->internal->set(offset + 3, (int8_t)(number >> 24));
}
...
void NTLM$Writer::writeSecurityBuffer(int32_t offset, $String* str, bool unicode) {
	...
	writeSecurityBuffer(offset, str == nullptr ? ($bytes*)nullptr : $(str->getBytes(unicode ? $StandardCharsets::UTF_16LE : $StandardCharsets::ISO_8859_1)));
}
...
void NTLM$Writer::clinit$($Class* clazz) {
	$load($NTLM);
...
$Class* NTLM$Writer::load$($String* name, bool initialize) {
	...
	$loadClass(NTLM$Writer, name, initialize, &classInfo$$, NTLM$Writer::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(NTLM$Writer);
	});
	return class$;
}
