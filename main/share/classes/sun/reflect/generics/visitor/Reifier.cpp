...
void Reifier::clinit$($Class* clazz) {
...
$Class* Reifier::load$($String* name, bool initialize) {
	...
	$loadClass(Reifier, name, initialize, &classInfo$$, Reifier::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Reifier);
	});
	return class$;
}
