...
$AnnotatedType* AnnotatedTypeFactory$AnnotatedArrayTypeImpl::getAnnotatedGenericComponentType() {
	...
	$var($TypeAnnotation$LocationInfo, var$0, $AnnotatedTypeFactory::nestingForType(t, $($$nc(getLocation())->pushArray())));
	$var($TypeAnnotationArray, var$1, getTypeAnnotations());
	$var($TypeAnnotationArray, var$2, getTypeAnnotations());
	return $AnnotatedTypeFactory::buildAnnotatedType(t, var$0, var$1, var$2, $(getDecl()));
}
...
$String* AnnotatedTypeFactory$AnnotatedArrayTypeImpl::toString() {
	...
	$var($AnnotatedType, componentType, $cast($AnnotatedTypeFactory$AnnotatedTypeBaseImpl, this));
	while ($instanceOf($AnnotatedArrayType, componentType)) {
		...
		sb->append($$str({$(annotationsToString($(annotatedArrayType->getAnnotations()), true)), "[]"_s}));
		$assign(componentType, annotatedArrayType->getAnnotatedGenericComponentType());
	}
	sb->insert(0, $($nc(componentType)->toString()));
	return sb->toString();
}
...
bool AnnotatedTypeFactory$AnnotatedArrayTypeImpl::equals(Object$* o) {
	...
	if ($instanceOf($AnnotatedArrayType, o)) {
		...
		if (var$0) {
			$var($Object, var$1, getAnnotatedGenericComponentType());
			var$0 = $Objects::equals(var$1, $(that->getAnnotatedGenericComponentType()));
		}
		return var$0;
	} else {
		return false;
...
int32_t AnnotatedTypeFactory$AnnotatedArrayTypeImpl::hashCode() {
	...
	return var$0 ^ $$nc(getAnnotatedGenericComponentType())->hashCode();
}
...
$Class* AnnotatedTypeFactory$AnnotatedArrayTypeImpl::load$($String* name, bool initialize) {
	...
	$loadClass(AnnotatedTypeFactory$AnnotatedArrayTypeImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
