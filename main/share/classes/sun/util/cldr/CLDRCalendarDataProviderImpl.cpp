...
$Optional* CLDRCalendarDataProviderImpl::retrieveInteger($String* src, $String* region) {
	...
	if (regionIndex >= 0) {
		int32_t start = src->lastIndexOf(u';', regionIndex) + 1;
		return $Optional::of($($Integer::valueOf($Integer::parseInt(src, start, src->indexOf(u':', start), 10))));
	}
	return $Optional::empty();
}

void CLDRCalendarDataProviderImpl::clinit$($Class* clazz) {
	$assignStatic(CLDRCalendarDataProviderImpl::firstDay, $cast($AbstractMap, $new($ConcurrentHashMap)));
	$assignStatic(CLDRCalendarDataProviderImpl::minDays, $cast($AbstractMap, $new($ConcurrentHashMap)));
}
...
$Class* CLDRCalendarDataProviderImpl::load$($String* name, bool initialize) {
	...
	$loadClass(CLDRCalendarDataProviderImpl, name, initialize, &classInfo$$, CLDRCalendarDataProviderImpl::clinit$, []($Class* clazz) -> $Object* {
