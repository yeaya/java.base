...
int32_t PlatformLogger$Level::intValue() {
	return PlatformLogger$Level::LEVEL_VALUES->get(this->ordinal());
}
...
PlatformLogger$Level* PlatformLogger$Level::valueOf(int32_t level) {
	...
	int32_t i = $Arrays::binarySearch(PlatformLogger$Level::LEVEL_VALUES, 0, PlatformLogger$Level::LEVEL_VALUES->length - 2, level);
	return $nc($(values()))->get(i >= 0 ? i : (-i - 1));
}

void PlatformLogger$Level::clinit$($Class* clazz) {
...
$Class* PlatformLogger$Level::load$($String* name, bool initialize) {
	...
	$loadClass(PlatformLogger$Level, name, initialize, &classInfo$$, PlatformLogger$Level::clinit$, []($Class* clazz) -> $Object* {
