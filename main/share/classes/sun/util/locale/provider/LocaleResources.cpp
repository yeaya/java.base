...
$Class* LocaleResources$$Lambda$lambda$getZoneStrings$0::load$($String* name, bool initialize) {
	...
	$loadClass(LocaleResources$$Lambda$lambda$getZoneStrings$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LocaleResources$$Lambda$lambda$getZoneStrings$0);
	});
	return class$;
}
...
$Class* LocaleResources$$Lambda$lambda$getZoneStrings$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(LocaleResources$$Lambda$lambda$getZoneStrings$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LocaleResources$$Lambda$lambda$getZoneStrings$1$1);
	});
	return class$;
}
...
void LocaleResources::init$($ResourceBundleBasedAdapter* adapter, $Locale* locale) {
	...
	$set(this, type, $cast($LocaleProviderAdapter, adapter)->getAdapterType());
}

void LocaleResources::removeEmptyReferences() {
	$useLocalObjectStack();
	$var($Object, ref, nullptr);
	while (($assign(ref, this->referenceQueue->poll())) != nullptr) {
		this->cache->remove($($nc($cast($LocaleResources$ResourceReference, ref))->getCacheKey()));
	}
}
...
$Object* LocaleResources::getBreakIteratorInfo($String* key) {
	...
	$var($LocaleResources$ResourceReference, data, $cast($LocaleResources$ResourceReference, this->cache->get(cacheKey)));
	if (data == nullptr || (($assign(biInfo, data->get())) == nullptr)) {
		$assign(biInfo, $$nc($nc(this->localeData)->getBreakIteratorInfo(this->locale))->getObject(key));
		this->cache->put(cacheKey, $$new($LocaleResources$ResourceReference, cacheKey, biInfo, this->referenceQueue));
	}
	return biInfo;
}
...
$String* LocaleResources::getCalendarData($String* key) {
	...
	removeEmptyReferences();
	$var($LocaleResources$ResourceReference, data, $cast($LocaleResources$ResourceReference, this->cache->get(cacheKey)));
	if (data == nullptr || (($assign(caldata, $cast($String, data->get()))) == nullptr)) {
		...
		this->cache->put(cacheKey, $$new($LocaleResources$ResourceReference, cacheKey, caldata, this->referenceQueue));
	}
	return caldata;
}
...
$String* LocaleResources::getCollationData() {
	...
	removeEmptyReferences();
	$var($LocaleResources$ResourceReference, data, $cast($LocaleResources$ResourceReference, this->cache->get(LocaleResources::COLLATION_DATA_CACHEKEY)));
	if (data == nullptr || (($assign(coldata, $cast($String, data->get()))) == nullptr)) {
		$var($ResourceBundle, rb, $nc(this->localeData)->getCollationData(this->locale));
		if ($nc(rb)->containsKey(key)) {
			$assign(coldata, rb->getString(key));
		}
		this->cache->put(LocaleResources::COLLATION_DATA_CACHEKEY, $$new($LocaleResources$ResourceReference, LocaleResources::COLLATION_DATA_CACHEKEY, coldata, this->referenceQueue));
	}
	return coldata;
}
...
$ObjectArray* LocaleResources::getDecimalFormatSymbolsData() {
	...
	removeEmptyReferences();
	$var($LocaleResources$ResourceReference, data, $cast($LocaleResources$ResourceReference, this->cache->get(LocaleResources::DECIMAL_FORMAT_SYMBOLS_DATA_CACHEKEY)));
	if (data == nullptr || (($assign(dfsdata, $cast($ObjectArray, data->get()))) == nullptr)) {
		...
		this->cache->put(LocaleResources::DECIMAL_FORMAT_SYMBOLS_DATA_CACHEKEY, $$new($LocaleResources$ResourceReference, LocaleResources::DECIMAL_FORMAT_SYMBOLS_DATA_CACHEKEY, dfsdata, this->referenceQueue));
	}
	return dfsdata;
}
...
$String* LocaleResources::getCurrencyName($String* key) {
	...
	if (data != nullptr && (($assign(currencyName, data->get())) != nullptr)) {
		if ($nc(currencyName)->equals(LocaleResources::NULLOBJECT)) {
	...
	if ($nc(olrb)->containsKey(key)) {
		...
		this->cache->put(cacheKey, $$new($LocaleResources$ResourceReference, cacheKey, currencyName, this->referenceQueue));
	}
	return $cast($String, currencyName);
}
...
$String* LocaleResources::getLocaleName($String* key) {
	...
	if (data != nullptr && (($assign(localeName, data->get())) != nullptr)) {
		if ($nc(localeName)->equals(LocaleResources::NULLOBJECT)) {
	...
	if ($nc(olrb)->containsKey(key)) {
		...
		this->cache->put(cacheKey, $$new($LocaleResources$ResourceReference, cacheKey, localeName, this->referenceQueue));
	}
	return $cast($String, localeName);
}
...
$Object* LocaleResources::getTimeZoneNames($String* key) {
	...
	if (var$0 || $Objects::isNull($assign(val, $nc(data)->get()))) {
		...
		if (val != nullptr) {
			this->cache->put(cacheKey, $$new($LocaleResources$ResourceReference, cacheKey, val, this->referenceQueue));
		}
	}
	return val;
}
...
$Set* LocaleResources::getZoneIDs() {
	...
	removeEmptyReferences();
	$var($LocaleResources$ResourceReference, data, $cast($LocaleResources$ResourceReference, this->cache->get(LocaleResources::ZONE_IDS_CACHEKEY)));
	if (data == nullptr || (($assign(zoneIDs, $cast($Set, data->get()))) == nullptr)) {
		$var($TimeZoneNamesBundle, rb, $nc(this->localeData)->getTimeZoneNames(this->locale));
		$assign(zoneIDs, $nc(rb)->keySet());
		this->cache->put(LocaleResources::ZONE_IDS_CACHEKEY, $$new($LocaleResources$ResourceReference, LocaleResources::ZONE_IDS_CACHEKEY, zoneIDs, this->referenceQueue));
	}
	return zoneIDs;
}
...
$StringArray2* LocaleResources::getZoneStrings() {
	...
	$var($Set, tzIds, $new($HashSet, $($Arrays::asList($($TimeZone::getAvailableIDs())))));
	{
	...
	if (this->type == $LocaleProviderAdapter$Type::CLDR) {
		$$nc($$nc(tzIds->stream())->filter($$new(LocaleResources$$Lambda$lambda$getZoneStrings$0)))->forEach($$new(LocaleResources$$Lambda$lambda$getZoneStrings$1$1, keyset, rb, value));
	}
	return $cast($StringArray2, value->toArray($$new($StringArray2, 0)));
}
...
$StringArray* LocaleResources::getCalendarNames($String* key) {
	...
	removeEmptyReferences();
	$var($LocaleResources$ResourceReference, data, $cast($LocaleResources$ResourceReference, this->cache->get(cacheKey)));
	if (data == nullptr || (($assign(names, $cast($StringArray, data->get()))) == nullptr)) {
		$var($ResourceBundle, rb, $nc(this->localeData)->getDateFormatData(this->locale));
		if ($nc(rb)->containsKey(key)) {
			$assign(names, rb->getStringArray(key));
			this->cache->put(cacheKey, $$new($LocaleResources$ResourceReference, cacheKey, names, this->referenceQueue));
		}
	}
	return names;
}
...
$String* LocaleResources::getDateTimePattern(int32_t timeStyle, int32_t dateStyle, $Calendar* cal$renamed) {
	...
	return getDateTimePattern(nullptr, timeStyle, dateStyle, $($nc(cal)->getCalendarType()));
}
...
$String* LocaleResources::getJavaTimeDateTimePattern(int32_t timeStyle, int32_t dateStyle, $String* calType$renamed) {
	...
	if (pattern == nullptr) {
		$assign(pattern, getDateTimePattern(nullptr, timeStyle, dateStyle, calType));
	}
	return pattern;
}
...
$String* LocaleResources::getDateTimePattern($String* prefix, int32_t timeStyle, int32_t dateStyle, $String* calType) {
	...
	if (timeStyle >= 0) {
		...
		if (timePattern == nullptr) {
			$assign(timePattern, getDateTimePattern(nullptr, "TimePatterns"_s, timeStyle, calType));
		}
	}
	if (dateStyle >= 0) {
		if (prefix != nullptr) {
		...
		if (datePattern == nullptr) {
			$assign(datePattern, getDateTimePattern(nullptr, "DatePatterns"_s, dateStyle, calType));
		}
	}
	if (timeStyle >= 0) {
		if (dateStyle >= 0) {
			...
			if (dateTimePattern == nullptr) {
				$assign(dateTimePattern, getDateTimePattern(nullptr, "DateTimePatterns"_s, dateTimeStyle, calType));
			}
...
$StringArray* LocaleResources::getNumberPatterns() {
	...
	removeEmptyReferences();
	$var($LocaleResources$ResourceReference, data, $cast($LocaleResources$ResourceReference, this->cache->get(LocaleResources::NUMBER_PATTERNS_CACHEKEY)));
	if (data == nullptr || (($assign(numberPatterns, $cast($StringArray, data->get()))) == nullptr)) {
		...
		this->cache->put(LocaleResources::NUMBER_PATTERNS_CACHEKEY, $$new($LocaleResources$ResourceReference, LocaleResources::NUMBER_PATTERNS_CACHEKEY, numberPatterns, this->referenceQueue));
	}
...
$StringArray* LocaleResources::getCNPatterns($NumberFormat$Style* formatStyle) {
	...
	$var($LocaleResources$ResourceReference, data, $cast($LocaleResources$ResourceReference, this->cache->get(cacheKey)));
	if (data == nullptr || (($assign(compactNumberPatterns, $cast($StringArray, data->get()))) == nullptr)) {
		$var($ResourceBundle, resource, $nc(this->localeData)->getNumberFormatData(this->locale));
		$assign(compactNumberPatterns, $cast($StringArray, $nc(resource)->getObject($$str({width, ".CompactNumberPatterns"_s}))));
		this->cache->put(cacheKey, $$new($LocaleResources$ResourceReference, cacheKey, compactNumberPatterns, this->referenceQueue));
	}
...
$ResourceBundle* LocaleResources::getJavaTimeFormatData() {
	...
	if ($instanceOf($ParallelListResourceBundle, rb)) {
		this->localeData->setSupplementary($cast($ParallelListResourceBundle, rb));
	}
	return rb;
}
...
$String* LocaleResources::getDateTimePattern($String* prefix, $String* key, int32_t styleIndex, $String* calendarType) {
	...
	$var($String, cacheKey, sb->insert(0, LocaleResources::DATE_TIME_PATTERN)->toString());
	removeEmptyReferences();
	$var($LocaleResources$ResourceReference, data, $cast($LocaleResources$ResourceReference, this->cache->get(cacheKey)));
	$var($Object, value, LocaleResources::NULLOBJECT);
	if (data == nullptr || (($assign(value, data->get())) == nullptr)) {
		...
		if ($nc(r)->containsKey(resourceKey)) {
			...
			if (!LocaleResources::$assertionsDisabled && ! !resourceKey->equals(key)) {
				$throwNew($AssertionError);
			}
		...
		this->cache->put(cacheKey, $$new($LocaleResources$ResourceReference, cacheKey, value, this->referenceQueue));
	}
	...
	return ($nc(styles)->length > 1 ? styles->get(styleIndex) : styles->get(0));
}
...
$StringArray* LocaleResources::getRules() {
	...
	removeEmptyReferences();
	$var($LocaleResources$ResourceReference, data, $cast($LocaleResources$ResourceReference, this->cache->get(LocaleResources::RULES_CACHEKEY)));
	if (data == nullptr || (($assign(rules, $cast($StringArray, data->get()))) == nullptr)) {
		$var($ResourceBundle, rb, $nc(this->localeData)->getDateFormatData(this->locale));
		...
		if (rb->containsKey("DayPeriodRules"_s)) {
			rules->set(1, $(rb->getString("DayPeriodRules"_s)));
		}
		this->cache->put(LocaleResources::RULES_CACHEKEY, $$new($LocaleResources$ResourceReference, LocaleResources::RULES_CACHEKEY, rules, this->referenceQueue));
	}
	return rules;
}
...
void LocaleResources::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	LocaleResources::TRACE_ON = $($Boolean::valueOf($($GetPropertyAction::privilegedGetProperty("locale.resources.debug"_s, "false"_s))))->booleanValue();
}
...
$Class* LocaleResources::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("sun.util.locale.provider.LocaleResources$$Lambda$lambda$getZoneStrings$0")) {
			return LocaleResources$$Lambda$lambda$getZoneStrings$0::load$(name, initialize);
		}
		if (name->equals("sun.util.locale.provider.LocaleResources$$Lambda$lambda$getZoneStrings$1$1")) {
	...
	$loadClass(LocaleResources, name, initialize, &classInfo$$, LocaleResources::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(LocaleResources);
	});
	return class$;
}
