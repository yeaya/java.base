...
$String* TimeZoneNameUtility$TimeZoneNameGetter::examineAliases($TimeZoneNameProvider* tznp, $Locale* locale, $String* requestID, $String* tzid, int32_t style, $Map* aliases) {
	...
	for (; $nc(i$)->hasNext();) {
		...
		if ($$sure($String, $nc(entry)->getValue())->equals(tzid)) {
...
void TimeZoneNameUtility$TimeZoneNameGetter::clinit$($Class* clazz) {
...
$Class* TimeZoneNameUtility$TimeZoneNameGetter::load$($String* name, bool initialize) {
	...
	$loadClass(TimeZoneNameUtility$TimeZoneNameGetter, name, initialize, &classInfo$$, TimeZoneNameUtility$TimeZoneNameGetter::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(TimeZoneNameUtility$TimeZoneNameGetter);
	});
	return class$;
}
