...
bool Bundles$CacheKey::equals(Object$* other) {
	...
	try {
		...
		bool var$0 = $nc(this->locale)->equals(otherEntry->locale);
		return var$0 && $nc(this->name)->equals(otherEntry->name);
...
$String* Bundles$CacheKey::toString() {
	...
	if ($nc(l)->isEmpty()) {
		if (!$$nc(this->locale->getVariant())->isEmpty()) {
			$assign(l, $str({"__"_s, $(this->locale->getVariant())}));
		} else {
			$assign(l, "\"\""_s);
		}
	}
...
$Class* Bundles$CacheKey::load$($String* name, bool initialize) {
	...
	$loadClass(Bundles$CacheKey, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Bundles$CacheKey);
	});
	return class$;
}
