...
bool ParallelListResourceBundle$KeySet::contains(Object$* o) {
	...
	return (this->parent != nullptr) ? this->parent->containsKey($cast($String, o)) : false;
}
...
int32_t ParallelListResourceBundle$KeySet::size() {
	...
	$var($Set, allset, $new($HashSet, this->set));
...
$Class* ParallelListResourceBundle$KeySet::load$($String* name, bool initialize) {
	...
	$loadClass(ParallelListResourceBundle$KeySet, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
