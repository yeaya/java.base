...
void ZoneInfoFile$ZoneOffsetTransitionRule::init$($DataInput* in) {
	...
	this->dom = (int8_t)(((int32_t)((uint32_t)(data & (63 << 22)) >> 22)) - 32);
	this->dow = dowByte == 0 ? -1 : dowByte;
	this->secondOfDay = timeByte == 31 ? in->readInt() : timeByte * 3600;
	this->timeEndOfDay = timeByte == 24;
	this->timeDefinition = (int32_t)((uint32_t)(data & (3 << 12)) >> 12);
...
$Class* ZoneInfoFile$ZoneOffsetTransitionRule::load$($String* name, bool initialize) {
	...
	$loadClass(ZoneInfoFile$ZoneOffsetTransitionRule, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ZoneInfoFile$ZoneOffsetTransitionRule);
	});
	return class$;
}
