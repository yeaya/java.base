...
bool CalendarDate::equals(Object$* obj) {
	...
	bool thatHasZone = that->zoneinfo != nullptr;
	if (hasZone != thatHasZone) {
		return false;
	}
	if (hasZone && !$nc(this->zoneinfo)->equals(that->zoneinfo)) {
		return false;
	}
...
int32_t CalendarDate::hashCode() {
	...
	int32_t zone = this->zoneinfo != nullptr ? this->zoneinfo->hashCode() : 0;
...
$Class* CalendarDate::load$($String* name, bool initialize) {
	...
	$loadClass(CalendarDate, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CalendarDate);
	});
	return class$;
}
