...
void CalendarSystem::initNames() {
	...
	for (int32_t i = 0; i < CalendarSystem::namePairs->length; i += 2) {
		clName->setLength(0);
		$var($String, cl, clName->append(CalendarSystem::PACKAGE_NAME)->append(CalendarSystem::namePairs->get(i + 1))->toString());
		nameMap->put(CalendarSystem::namePairs->get(i), cl);
	}
...
CalendarSystem* CalendarSystem::forName($String* calendarName) {
	...
	$var(CalendarSystem, cs, $cast(CalendarSystem, CalendarSystem::calendars->putIfAbsent(calendarName, cal)));
	return (cs == nullptr) ? cal : cs;
}

void CalendarSystem::clinit$($Class* clazz) {
...
$Class* CalendarSystem::load$($String* name, bool initialize) {
	...
	$loadClass(CalendarSystem, name, initialize, &classInfo$$, CalendarSystem::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(CalendarSystem);
	});
	return class$;
}
