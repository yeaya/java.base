...
$WatchKey* PollingWatchService::register$($Path* path, $WatchEvent$KindArray* events, $WatchEvent$ModifierArray* modifiers) {
		...
		for (int32_t len$ = arr$->length, i$ = 0; i$ < len$; ++i$) {
			$var($WatchEvent$Modifier, modifier, arr$->get(i$));
			{
				...
				if ($nc($ExtendedOptions::SENSITIVITY_HIGH)->matches(modifier)) {
					sensitivity = $$sure($Integer, $ExtendedOptions::SENSITIVITY_HIGH->parameter())->intValue();
				} else if ($nc($ExtendedOptions::SENSITIVITY_MEDIUM)->matches(modifier)) {
					sensitivity = $$sure($Integer, $ExtendedOptions::SENSITIVITY_MEDIUM->parameter())->intValue();
				} else if ($nc($ExtendedOptions::SENSITIVITY_LOW)->matches(modifier)) {
					sensitivity = $$sure($Integer, $ExtendedOptions::SENSITIVITY_LOW->parameter())->intValue();
				} else {
	...
	try {
		...
		return $cast($WatchKey, $AccessController::doPrivileged($$new($PollingWatchService$2, this, path, eventSet, value)));
...
$PollingWatchService$PollingWatchKey* PollingWatchService::doPrivilegedRegister($Path* path, $Set* events, int32_t sensitivityInSeconds) {
	...
	$var($Object, fileKey, attrs->fileKey());
	if (fileKey == nullptr) {
	...
	$synchronized(closeLock()) {
		...
		$synchronized(this->map) {
			$assign(watchKey, $cast($PollingWatchService$PollingWatchKey, this->map->get(fileKey)));
			if (watchKey == nullptr) {
...
void PollingWatchService::implClose() {
	...
	$AccessController::doPrivileged($$new($PollingWatchService$3, this));
}
...
$Class* PollingWatchService::load$($String* name, bool initialize) {
	...
	$loadClass(PollingWatchService, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PollingWatchService);
	});
	return class$;
}
