			...
			try {
				...
				for (; $nc(i$)->hasNext();) {
					$var($Path, entry, $cast($Path, i$->next()));
					{
						...
						$var($Object, var$1, $nc(entry)->getFileName());
...
void PollingWatchService$PollingWatchKey::cancel() {
	...
	$synchronized(this->this$0->map) {
		this->this$0->map->remove($(fileKey()));
	}
	disable();
}
			...
			try {
				...
				for (; $nc(i$)->hasNext();) {
					$var($Path, entry, $cast($Path, i$->next()));
					{
						...
						if (e == nullptr) {
							$var($Object, var$1, entry->getFileName());
							this->entries->put(var$1, $$new($PollingWatchService$CacheEntry, lastModified, this->tickCount));
...
$Class* PollingWatchService$PollingWatchKey::load$($String* name, bool initialize) {
	...
	$loadClass(PollingWatchService$PollingWatchKey, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PollingWatchService$PollingWatchKey);
	});
	return class$;
}
