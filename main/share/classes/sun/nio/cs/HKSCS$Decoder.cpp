...
$CoderResult* HKSCS$Decoder::decodeBufferLoop($ByteBuffer* src, $CharBuffer* dst) {
	...
	try {
		while (src->hasRemaining()) {
			...
			if (outSize == 2) {
				dst->put($Surrogate::high(0x00020000 + c));
...
void HKSCS$Decoder::clinit$($Class* clazz) {
...
$Class* HKSCS$Decoder::load$($String* name, bool initialize) {
	...
	$loadClass(HKSCS$Decoder, name, initialize, &classInfo$$, HKSCS$Decoder::clinit$, []($Class* clazz) -> $Object* {
