...
int32_t GB18030$Encoder::getGB18030($shorts* outerIndex, $StringArray* innerEncoderIndex, char16_t ch) {
	int32_t offset = $nc(outerIndex)->get((ch & 0xff00) >> 8) << 8;
	return $nc($nc(innerEncoderIndex)->get(offset >> 12))->charAt((offset & 0x0fff) + (ch & 0xff));
}
	...
	try {
		while (sp < sl) {
			...
			if ($Character::isSurrogate(c)) {
				if ((condensedKey = this->sgp->parse(c, sa, sp, sl)) < 0) {
...
$CoderResult* GB18030$Encoder::encodeBufferLoop($CharBuffer* src, $ByteBuffer* dst) {
	...
	try {
		while (src->hasRemaining()) {
			...
			int32_t inputSize = 1;
			if ($Character::isSurrogate(c)) {
				if ((condensedKey = this->sgp->parse(c, src)) < 0) {
...
void GB18030$Encoder::clinit$($Class* clazz) {
	$load($GB18030);
...
$Class* GB18030$Encoder::load$($String* name, bool initialize) {
	...
	$loadClass(GB18030$Encoder, name, initialize, &classInfo$$, GB18030$Encoder::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(GB18030$Encoder);
	});
	return class$;
}
