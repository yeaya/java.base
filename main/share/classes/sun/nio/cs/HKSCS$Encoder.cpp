...
int32_t HKSCS$Encoder::encodeChar(char16_t ch) {
	int32_t bb = $nc($nc(this->c2bBmp)->get(ch >> 8))->get(ch & 0xff);
	if (bb == 0x0000fffd) {
...
int32_t HKSCS$Encoder::encodeSupp(int32_t cp) {
	if ((cp & 0x000f0000) != 0x00020000) {
		return 0x0000fffd;
	}
	return $nc($nc(this->c2bSupp)->get((cp >> 8) & 0xff))->get(cp & 0xff);
}
...
void HKSCS$Encoder::initc2b($charArray2* c2b, $StringArray* b2cStr, $String* pua) {
	...
	$useLocalObjectStack();
	int32_t b2Min = 64;
	$Arrays::fill(c2b, HKSCS$Encoder::C2B_UNMAPPABLE);
	for (int32_t b1 = 0; b1 < 256; ++b1) {
		...
		for (int32_t i = 0; i < $nc(s)->length(); ++i) {
			...
			$nc(c2b->get(hi))->set(c & 0xff, (char16_t)((b1 << 8) | (i + b2Min)));
		}
	}
	if (pua != nullptr) {
...
void HKSCS$Encoder::clinit$($Class* clazz) {
...
$Class* HKSCS$Encoder::load$($String* name, bool initialize) {
	...
	$loadClass(HKSCS$Encoder, name, initialize, &classInfo$$, HKSCS$Encoder::clinit$, []($Class* clazz) -> $Object* {
