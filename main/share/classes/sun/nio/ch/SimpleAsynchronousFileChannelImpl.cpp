...
$AsynchronousFileChannel* SimpleAsynchronousFileChannelImpl::open($FileDescriptor* fdo, bool reading, bool writing, $ThreadPool* pool) {
	...
	$var($ExecutorService, executor, (pool == nullptr) ? $SimpleAsynchronousFileChannelImpl$DefaultExecutorHolder::defaultExecutor : pool->executor());
...
void SimpleAsynchronousFileChannelImpl::close() {
	...
	SimpleAsynchronousFileChannelImpl::nd->close(this->fdObj);
}
...
int64_t SimpleAsynchronousFileChannelImpl::size() {
	...
	try {
		...
		try {
			begin();
			do {
				n = SimpleAsynchronousFileChannelImpl::nd->size(this->fdObj);
...
$AsynchronousFileChannel* SimpleAsynchronousFileChannelImpl::truncate(int64_t size) {
	...
	try {
		...
		try {
			...
			if (size < n && isOpen()) {
				do {
					n = SimpleAsynchronousFileChannelImpl::nd->truncate(this->fdObj, size);
				} while ((n == $IOStatus::INTERRUPTED) && isOpen());
			}
			$assign(var$5, this);
			return$4 = true;
...
void SimpleAsynchronousFileChannelImpl::force(bool metaData) {
	...
	try {
		...
		try {
			begin();
			do {
				n = SimpleAsynchronousFileChannelImpl::nd->force(this->fdObj, metaData);
			} while ((n == $IOStatus::INTERRUPTED) && isOpen());
...
$Future* SimpleAsynchronousFileChannelImpl::implLock(int64_t position, int64_t size, bool shared, Object$* attachment, $CompletionHandler* handler) {
	...
	if (fli == nullptr) {
		...
		$Invoker::invokeIndirectly(handler, attachment, nullptr, exc, this->executor$);
		return nullptr;
	}
...
$FileLock* SimpleAsynchronousFileChannelImpl::tryLock(int64_t position, int64_t size, bool shared) {
	...
	try {
		...
		int32_t n = 0;
		do {
			n = SimpleAsynchronousFileChannelImpl::nd->lock(this->fdObj, false, position, size, shared);
...
void SimpleAsynchronousFileChannelImpl::implRelease($FileLockImpl* fli) {
	...
	SimpleAsynchronousFileChannelImpl::nd->release(var$0, var$1, fli->size());
}
...
$Future* SimpleAsynchronousFileChannelImpl::implRead($ByteBuffer* dst, int64_t position, Object$* attachment, $CompletionHandler* handler) {
	...
	bool var$0 = !isOpen();
	if (var$0 || (dst->remaining() == 0)) {
		$var($Throwable, exc, (isOpen()) ? ($Throwable*)nullptr : $cast($Throwable, $new($ClosedChannelException)));
		if (handler == nullptr) {
			return $CompletedFuture::withResult($($Integer::valueOf(0)), exc);
		}
		$Invoker::invokeIndirectly(handler, attachment, $($Integer::valueOf(0)), exc, this->executor$);
		return nullptr;
	}
	$var($PendingFuture, result, (handler == nullptr) ? $new($PendingFuture, this) : ($PendingFuture*)nullptr);
...
void SimpleAsynchronousFileChannelImpl::clinit$($Class* clazz) {
...
$Class* SimpleAsynchronousFileChannelImpl::load$($String* name, bool initialize) {
	...
	$loadClass(SimpleAsynchronousFileChannelImpl, name, initialize, &classInfo$$, SimpleAsynchronousFileChannelImpl::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SimpleAsynchronousFileChannelImpl);
	});
	return class$;
}
