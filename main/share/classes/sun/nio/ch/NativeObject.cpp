...
NativeObject* NativeObject::getObject(int32_t offset) {
	...
	case 8:
		...
		break;
	case 4:
		newAddress = $nc(NativeObject::unsafe)->getInt(offset + this->address$) & -1;
		break;
	default:
...
void NativeObject::putObject(int32_t offset, NativeObject* ob) {
	...
	case 8:
		...
		break;
	case 4:
		putInt(offset, (int32_t)($nc(ob)->address$ & -1));
		break;
	default:
		$throwNew($InternalError, "Address size not supported"_s);
	}
...
$ByteOrder* NativeObject::byteOrder() {
	...
	try {
		NativeObject::unsafe->putLong(a, (int64_t)0x0102030405060708);
		int8_t b = NativeObject::unsafe->getByte(a);
		switch (b) {
		case 1:
			$init($ByteOrder);
		...
		NativeObject::unsafe->freeMemory(a);
	}
	if (var$0 != nullptr) {
		$throw(var$0);
	}
...
void NativeObject::clinit$($Class* clazz) {
...
$Class* NativeObject::load$($String* name, bool initialize) {
	...
	$loadClass(NativeObject, name, initialize, &classInfo$$, NativeObject::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(NativeObject);
	});
	return class$;
}
