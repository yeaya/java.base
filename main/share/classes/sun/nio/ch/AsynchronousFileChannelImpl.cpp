...
void AsynchronousFileChannelImpl::begin() {
	$$nc(this->closeLock->readLock())->lock();
	if (this->closed) {
...
void AsynchronousFileChannelImpl::end() {
	$$nc(this->closeLock->readLock())->unlock();
}
...
$FileLockImpl* AsynchronousFileChannelImpl::addToFileLockTable(int64_t position, int64_t size, bool shared) {
	...
	try {
		...
		$assign(fli, $new($FileLockImpl, this, position, size, shared));
...
$Class* AsynchronousFileChannelImpl::load$($String* name, bool initialize) {
	...
	$loadClass(AsynchronousFileChannelImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AsynchronousFileChannelImpl);
	});
	return class$;
}
