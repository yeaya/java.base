...
void FileChannelImpl$Unmapper::unmap() {
	...
	if ($nc(this->fd)->valid()) {
		try {
			FileChannelImpl$Unmapper::nd->close(this->fd);
...
void FileChannelImpl$Unmapper::clinit$($Class* clazz) {
...
$Class* FileChannelImpl$Unmapper::load$($String* name, bool initialize) {
	...
	$loadClass(FileChannelImpl$Unmapper, name, initialize, &classInfo$$, FileChannelImpl$Unmapper::clinit$, []($Class* clazz) -> $Object* {
