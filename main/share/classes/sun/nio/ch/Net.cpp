...
$InetSocketAddress* Net::checkAddress($SocketAddress* sa) {
	...
	$var($InetAddress, addr, isa->getAddress());
...
void Net::translateToSocketException($Exception* x) {
	...
	if ($instanceOf($ClosedChannelException, x)) {
		...
		$assign(nx, $new($SocketException, $(x->getMessage())));
	}
	if (nx != x) {
		$nc(nx)->initCause(x);
	}
...
$InetAddress* Net::anyLocalAddress($ProtocolFamily* family) {
	...
	} else if ($equals(family, $StandardProtocolFamily::INET6)) {
...
$Inet4Address* Net::anyInet4Address($NetworkInterface* interf) {
	...
	return $cast($Inet4Address, $AccessController::doPrivileged($$new($Net$2, interf)));
}

int32_t Net::inet4AsInt($InetAddress* ia) {
	$init(Net);
	if ($instanceOf($Inet4Address, ia)) {
		$var($bytes, addr, ia->getAddress());
		int32_t address = $nc(addr)->get(3) & 0xff;
...
$bytes* Net::inet6AsByteArray($InetAddress* ia) {
	...
	if ($instanceOf($Inet4Address, ia)) {
		$var($bytes, ip4address, ia->getAddress());
...
void Net::setSocketOption($FileDescriptor* fd, $ProtocolFamily* family, $SocketOption* name, Object$* value$renamed) {
	...
	if ($nc(Net::extendedOptions)->isOptionSupported(name)) {
		Net::extendedOptions->setOption(fd, name, value);
		return;
	}
	...
	int32_t var$0 = $nc(key)->level();
	setIntOption0(fd, mayNeedConversion, var$0, key->name(), arg, isIPv6);
}
...
$Object* Net::getSocketOption($FileDescriptor* fd, $ProtocolFamily* family, $SocketOption* name) {
	...
	$Class* type = $nc(name)->type();
	if ($nc(Net::extendedOptions)->isOptionSupported(name)) {
		return Net::extendedOptions->getOption(fd, name);
	}
	if (type != $Integer::class$ && type != $Boolean::class$) {
		$throwNew($AssertionError, $of("Should not reach here"_s));
	}
	...
	bool mayNeedConversion = (family == Net::UNSPEC);
	int32_t var$0 = $nc(key)->level();
	int32_t value = getIntOption0(fd, mayNeedConversion, var$0, key->name());
	if (type == $Integer::class$) {
...
bool Net::isIPv6Available0() {
	$init(Net);
	$prepareNativeStatic(isIPv6Available0, bool);
...
int32_t Net::isExclusiveBindAvailable() {
	$init(Net);
	$prepareNativeStatic(isExclusiveBindAvailable, int32_t);
...
bool Net::shouldSetBothIPv4AndIPv6Options0() {
	$init(Net);
	$prepareNativeStatic(shouldSetBothIPv4AndIPv6Options0, bool);
	bool $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
bool Net::canIPv6SocketJoinIPv4Group0() {
	$init(Net);
	$prepareNativeStatic(canIPv6SocketJoinIPv4Group0, bool);
	bool $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
bool Net::canJoin6WithIPv4Group0() {
	$init(Net);
	$prepareNativeStatic(canJoin6WithIPv4Group0, bool);
	bool $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
bool Net::canUseIPv6OptionsWithIPv4LocalAddress0() {
	$init(Net);
	$prepareNativeStatic(canUseIPv6OptionsWithIPv4LocalAddress0, bool);
	bool $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
int32_t Net::socket0(bool preferIPv6, bool stream, bool reuse, bool fastLoopback) {
	$init(Net);
	$prepareNativeStatic(socket0, int32_t, bool preferIPv6, bool stream, bool reuse, bool fastLoopback);
	int32_t $ret = $invokeNativeStatic(preferIPv6, stream, reuse, fastLoopback);
	$finishNativeStatic();
	return $ret;
}
...
void Net::bind0($FileDescriptor* fd, bool preferIPv6, bool useExclBind, $InetAddress* addr, int32_t port) {
	$init(Net);
	$prepareNativeStatic(bind0, void, $FileDescriptor* fd, bool preferIPv6, bool useExclBind, $InetAddress* addr, int32_t port);
...
void Net::listen($FileDescriptor* fd, int32_t backlog) {
	$init(Net);
	$prepareNativeStatic(listen, void, $FileDescriptor* fd, int32_t backlog);
...
int32_t Net::connect($ProtocolFamily* family, $FileDescriptor* fd, $SocketAddress* remote) {
	...
	$var($InetAddress, var$0, $nc(isa)->getAddress());
	return connect(family, fd, var$0, isa->getPort());
}

int32_t Net::connect0(bool preferIPv6, $FileDescriptor* fd, $InetAddress* remote, int32_t remotePort) {
	$init(Net);
	$prepareNativeStatic(connect0, int32_t, bool preferIPv6, $FileDescriptor* fd, $InetAddress* remote, int32_t remotePort);
	int32_t $ret = $invokeNativeStatic(preferIPv6, fd, remote, remotePort);
	$finishNativeStatic();
	return $ret;
}
...
int32_t Net::accept($FileDescriptor* fd, $FileDescriptor* newfd, $InetSocketAddressArray* isaa) {
	$init(Net);
	$prepareNativeStatic(accept, int32_t, $FileDescriptor* fd, $FileDescriptor* newfd, $InetSocketAddressArray* isaa);
	int32_t $ret = $invokeNativeStatic(fd, newfd, isaa);
	$finishNativeStatic();
	return $ret;
}
...
void Net::shutdown($FileDescriptor* fd, int32_t how) {
	$init(Net);
	$prepareNativeStatic(shutdown, void, $FileDescriptor* fd, int32_t how);
...
int32_t Net::localPort($FileDescriptor* fd) {
	$init(Net);
	$prepareNativeStatic(localPort, int32_t, $FileDescriptor* fd);
...
$InetAddress* Net::localInetAddress($FileDescriptor* fd) {
	$init(Net);
	$prepareNativeStatic(localInetAddress, $InetAddress*, $FileDescriptor* fd);
	$var($InetAddress, $ret, $invokeNativeStaticObject(fd));
	$finishNativeStatic();
	return $ret;
}
...
int32_t Net::remotePort($FileDescriptor* fd) {
	$init(Net);
	$prepareNativeStatic(remotePort, int32_t, $FileDescriptor* fd);
	int32_t $ret = $invokeNativeStatic(fd);
	$finishNativeStatic();
	return $ret;
}
...
$InetAddress* Net::remoteInetAddress($FileDescriptor* fd) {
	$init(Net);
	$prepareNativeStatic(remoteInetAddress, $InetAddress*, $FileDescriptor* fd);
	$var($InetAddress, $ret, $invokeNativeStaticObject(fd));
	$finishNativeStatic();
	return $ret;
}
...
int32_t Net::getIntOption0($FileDescriptor* fd, bool mayNeedConversion, int32_t level, int32_t opt) {
	$init(Net);
	$prepareNativeStatic(getIntOption0, int32_t, $FileDescriptor* fd, bool mayNeedConversion, int32_t level, int32_t opt);
	int32_t $ret = $invokeNativeStatic(fd, mayNeedConversion, level, opt);
	$finishNativeStatic();
	return $ret;
}
...
void Net::setIntOption0($FileDescriptor* fd, bool mayNeedConversion, int32_t level, int32_t opt, int32_t arg, bool isIPv6) {
	$init(Net);
	$prepareNativeStatic(setIntOption0, void, $FileDescriptor* fd, bool mayNeedConversion, int32_t level, int32_t opt, int32_t arg, bool isIPv6);
...
int32_t Net::poll($FileDescriptor* fd, int32_t events, int64_t timeout) {
	$init(Net);
	$prepareNativeStatic(poll, int32_t, $FileDescriptor* fd, int32_t events, int64_t timeout);
	int32_t $ret = $invokeNativeStatic(fd, events, timeout);
	$finishNativeStatic();
	return $ret;
}
...
bool Net::pollConnect($FileDescriptor* fd, int64_t timeout) {
	$init(Net);
	$prepareNativeStatic(pollConnect, bool, $FileDescriptor* fd, int64_t timeout);
	bool $ret = $invokeNativeStatic(fd, timeout);
	$finishNativeStatic();
	return $ret;
}
...
int32_t Net::available($FileDescriptor* fd) {
	$init(Net);
	$prepareNativeStatic(available, int32_t, $FileDescriptor* fd);
	int32_t $ret = $invokeNativeStatic(fd);
	$finishNativeStatic();
	return $ret;
}
...
int32_t Net::sendOOB($FileDescriptor* fd, int8_t data) {
	$init(Net);
	$prepareNativeStatic(sendOOB, int32_t, $FileDescriptor* fd, int8_t data);
	int32_t $ret = $invokeNativeStatic(fd, data);
	$finishNativeStatic();
	return $ret;
}
...
bool Net::discardOOB($FileDescriptor* fd) {
	$init(Net);
	$prepareNativeStatic(discardOOB, bool, $FileDescriptor* fd);
...
int32_t Net::joinOrDrop4(bool join, $FileDescriptor* fd, int32_t group, int32_t interf, int32_t source) {
	$init(Net);
	$prepareNativeStatic(joinOrDrop4, int32_t, bool join, $FileDescriptor* fd, int32_t group, int32_t interf, int32_t source);
	int32_t $ret = $invokeNativeStatic(join, fd, group, interf, source);
	$finishNativeStatic();
	return $ret;
}
...
int32_t Net::blockOrUnblock4(bool block, $FileDescriptor* fd, int32_t group, int32_t interf, int32_t source) {
	$init(Net);
	$prepareNativeStatic(blockOrUnblock4, int32_t, bool block, $FileDescriptor* fd, int32_t group, int32_t interf, int32_t source);
	int32_t $ret = $invokeNativeStatic(block, fd, group, interf, source);
	$finishNativeStatic();
	return $ret;
}
...
int32_t Net::joinOrDrop6(bool join, $FileDescriptor* fd, $bytes* group, int32_t index, $bytes* source) {
	$init(Net);
	$prepareNativeStatic(joinOrDrop6, int32_t, bool join, $FileDescriptor* fd, $bytes* group, int32_t index, $bytes* source);
	int32_t $ret = $invokeNativeStatic(join, fd, group, index, source);
	$finishNativeStatic();
	return $ret;
}
...
int32_t Net::blockOrUnblock6(bool block, $FileDescriptor* fd, $bytes* group, int32_t index, $bytes* source) {
	$init(Net);
	$prepareNativeStatic(blockOrUnblock6, int32_t, bool block, $FileDescriptor* fd, $bytes* group, int32_t index, $bytes* source);
	int32_t $ret = $invokeNativeStatic(block, fd, group, index, source);
	$finishNativeStatic();
	return $ret;
}
...
void Net::setInterface4($FileDescriptor* fd, int32_t interf) {
	$init(Net);
	$prepareNativeStatic(setInterface4, void, $FileDescriptor* fd, int32_t interf);
...
int32_t Net::getInterface4($FileDescriptor* fd) {
	$init(Net);
	$prepareNativeStatic(getInterface4, int32_t, $FileDescriptor* fd);
	int32_t $ret = $invokeNativeStatic(fd);
	$finishNativeStatic();
	return $ret;
}
...
void Net::setInterface6($FileDescriptor* fd, int32_t index) {
	$init(Net);
	$prepareNativeStatic(setInterface6, void, $FileDescriptor* fd, int32_t index);
...
int32_t Net::getInterface6($FileDescriptor* fd) {
	$init(Net);
	$prepareNativeStatic(getInterface6, int32_t, $FileDescriptor* fd);
	int32_t $ret = $invokeNativeStatic(fd);
	$finishNativeStatic();
	return $ret;
}
...
int16_t Net::pollinValue() {
	$init(Net);
	$prepareNativeStatic(pollinValue, int16_t);
	int16_t $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
int16_t Net::polloutValue() {
	$init(Net);
	$prepareNativeStatic(polloutValue, int16_t);
	int16_t $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
int16_t Net::pollerrValue() {
	$init(Net);
	$prepareNativeStatic(pollerrValue, int16_t);
	int16_t $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
int16_t Net::pollhupValue() {
	$init(Net);
	$prepareNativeStatic(pollhupValue, int16_t);
	int16_t $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
int16_t Net::pollnvalValue() {
	$init(Net);
	$prepareNativeStatic(pollnvalValue, int16_t);
	int16_t $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}
...
int16_t Net::pollconnValue() {
	$init(Net);
	$prepareNativeStatic(pollconnValue, int16_t);
	int16_t $ret = $invokeNativeStatic();
	$finishNativeStatic();
	return $ret;
}

void Net::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(Net::UNSPEC, $new($Net$1));
	{
		try {
			...
			if (!Net::$assertionsDisabled && !($instanceOf($Inet4Address, Net::anyLocalInet4Address) && Net::anyLocalInet4Address->isAnyLocalAddress())) {
				$throwNew($AssertionError);
			}
			$assignStatic(Net::anyLocalInet6Address, $InetAddress::getByAddress($$new($bytes, 16)));
			if (!Net::$assertionsDisabled && !($instanceOf($Inet6Address, Net::anyLocalInet6Address) && Net::anyLocalInet6Address->isAnyLocalAddress())) {
				$throwNew($AssertionError);
			}
			$assignStatic(Net::inet4LoopbackAddress$, Net::inet4FromInt(0x7f000001));
			if (!Net::$assertionsDisabled && !($instanceOf($Inet4Address, Net::inet4LoopbackAddress$) && Net::inet4LoopbackAddress$->isLoopbackAddress())) {
				$throwNew($AssertionError);
			}
			$var($bytes, bytes, $new($bytes, 16));
			bytes->set(15, 1);
			$assignStatic(Net::inet6LoopbackAddress$, $InetAddress::getByAddress(bytes));
			if (!Net::$assertionsDisabled && !($instanceOf($Inet6Address, Net::inet6LoopbackAddress$) && Net::inet6LoopbackAddress$->isLoopbackAddress())) {
				$throwNew($AssertionError);
			}
		} catch ($Exception& e) {
...
$Class* Net::load$($String* name, bool initialize) {
	...
	$loadClass(Net, name, initialize, &classInfo$$, Net::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Net);
	});
	return class$;
}
