...
void SimpleAsynchronousFileChannelImpl$2::run() {
	...
	try {
		try {
			this->this$0->begin();
			do {
				n = $IOUtil::read(this->this$0->fdObj, this->val$dst, this->val$position, $SimpleAsynchronousFileChannelImpl::nd);
...
$Class* SimpleAsynchronousFileChannelImpl$2::load$($String* name, bool initialize) {
	...
	$loadClass(SimpleAsynchronousFileChannelImpl$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SimpleAsynchronousFileChannelImpl$2);
	});
	return class$;
}
