...
bool VerifyAccess::isMemberAccessible($Class* refc, $Class* defc, int32_t mods, $Class* lookupClass, $Class* prevLookupClass, int32_t allowedModes) {
	...
	if (!VerifyAccess::$assertionsDisabled && !((allowedModes & ~((((VerifyAccess::ALL_ACCESS_MODES | VerifyAccess::PACKAGE_ALLOWED) | VerifyAccess::MODULE_ALLOWED) | VerifyAccess::UNCONDITIONAL_ALLOWED) | VerifyAccess::ORIGINAL_ALLOWED)) == 0)) {
	...
	}
	{
		...
		switch (mods & VerifyAccess::ALL_ACCESS_MODES) {
		case 1:
			if (!VerifyAccess::$assertionsDisabled && !((allowedModes & 1) != 0 || (allowedModes & VerifyAccess::UNCONDITIONAL_ALLOWED) != 0)) {
				$throwNew($AssertionError);
			}
			return true;
		case 4:
			...
			if ((allowedModes & VerifyAccess::PROTECTED_OR_PACKAGE_ALLOWED) != 0 && isSamePackage(defc, lookupClass)) {
				return true;
			}
			if ((allowedModes & 4) == 0) {
				return false;
			}
			if ((mods & 8) != 0 && !isRelatedClass(refc, lookupClass)) {
				return false;
			}
			if ((allowedModes & 4) != 0 && isSubClass(lookupClass, defc)) {
				return true;
			}
			return false;
		case VerifyAccess::PACKAGE_ONLY:
			...
			return ((allowedModes & VerifyAccess::PACKAGE_ALLOWED) != 0 && isSamePackage(defc, lookupClass));
		case 2:
			canAccess = ((allowedModes & 2) != 0 && $Reflection::areNestMates(defc, lookupClass));
...
bool VerifyAccess::isClassAccessible($Class* refc, $Class* lookupClass, $Class* prevLookupClass, int32_t allowedModes) {
	...
	if ((allowedModes & VerifyAccess::PACKAGE_ALLOWED) != 0 && isSamePackage(lookupClass, refc)) {
		return true;
	}
	...
	if ($Modifier::isPublic(mods)) {
		...
		if ((allowedModes & VerifyAccess::UNCONDITIONAL_ALLOWED) != 0) {
			return $nc(refModule)->isExported($(refc->getPackageName()));
		}
		if (lookupModule == refModule && prevLookupClass == nullptr) {
			if ((allowedModes & VerifyAccess::MODULE_ALLOWED) != 0) {
				return true;
			}
			if (!VerifyAccess::$assertionsDisabled && !((allowedModes & 1) != 0)) {
				$throwNew($AssertionError);
			}
			return $nc(refModule)->isExported($(refc->getPackageName()));
		}
		$var($Module, prevLookupModule, prevLookupClass != nullptr ? prevLookupClass->getModule() : ($Module*)nullptr);
...
bool VerifyAccess::isModuleAccessible($Class* refc, $Module* m1, $Module* m2) {
	...
	if ($Modifier::isPublic(mods)) {
		...
		if (var$0 && (m2 == nullptr || m2->canRead(refModule))) {
			$var($String, pn, refc->getPackageName());
			bool var$1 = $nc(refModule)->isExported(pn, m1);
			if (var$1 && (m2 == nullptr || refModule->isExported(pn, m2))) {
				return true;
			}
		}
	}
...
bool VerifyAccess::isTypeVisible($Class* type, $Class* refc) {
	...
	$var($ClassLoader, typeLoader, type->getClassLoader());
	...
	$Class* res = $cast($Class, $AccessController::doPrivileged($$new($VerifyAccess$1, name, refcLoader)));
	return (type == res);
}
...
bool VerifyAccess::isTypeVisible($MethodType* type, $Class* refc) {
	...
	for (int32_t n = 0, max = type->parameterCount(); n < max; ++n) {
...
bool VerifyAccess::loadersAreRelated($ClassLoader* loader1, $ClassLoader* loader2, bool loader1MustBeParent) {
	...
	}
	{
		...
		for (; scan2 != nullptr; $assign(scan2, scan2->getParent())) {
			if (scan2 == loader1) {
				return true;
			}
		}
	...
	}
	{
		...
		for (; scan1 != nullptr; $assign(scan1, scan1->getParent())) {
			if (scan1 == loader2) {
				return true;
			}
		}
...
void VerifyAccess::clinit$($Class* clazz) {
...
$Class* VerifyAccess::load$($String* name, bool initialize) {
	...
	$loadClass(VerifyAccess, name, initialize, &classInfo$$, VerifyAccess::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(VerifyAccess);
	});
	return class$;
}
