...
void MeteredStream::clinit$($Class* clazz) {
...
$Class* MeteredStream::load$($String* name, bool initialize) {
	...
	$loadClass(MeteredStream, name, initialize, &classInfo$$, MeteredStream::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(MeteredStream);
	});
	return class$;
}
