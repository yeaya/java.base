...
bool DigestAuthentication::isAuthorizationStale($String* header) {
	...
	if (s == nullptr || !s->equals("true"_s)) {
		return false;
	}
	$var($String, newNonce, p->findValue("nonce"_s));
	if (newNonce == nullptr || newNonce->isEmpty()) {
		return false;
	}
...
bool DigestAuthentication::setHeaders($HttpURLConnection* conn, $HeaderParser* p, $String* raw) {
	...
	this->params->setOpaque($(p->findValue("opaque"_s)));
	this->params->setQop($(p->findValue("qop"_s)));
	$var($String, uri, ""_s);
	...
	if (this->type == $AuthenticationInfo::PROXY_AUTHENTICATION && $nc(conn)->tunnelState() == $HttpURLConnection$TunnelState::SETUP) {
		...
		try {
			$assign(uri, $nc(conn)->getRequestURI());
		} catch ($IOException& e) {
		}
		$assign(method, $nc(conn)->getMethod());
	}
	if (this->params->nonce == nullptr || this->authMethod == nullptr || this->pw == nullptr || this->realm == nullptr) {
		return false;
	}
	if ($nc(this->authMethod)->length() >= 1) {
		$var($StringBuilder, var$0, $new($StringBuilder));
		var$0->append($Character::toUpperCase(this->authMethod->charAt(0)));
		var$0->append($($(this->authMethod->substring(1))->toLowerCase()));
		$set(this, authMethod, $str(var$0));
	}
	$var($String, algorithm, p->findValue("algorithm"_s));
...
$String* DigestAuthentication::getHeaderValueImpl($String* uri, $String* method) {
	...
	$var($String, opaque, this->params->getOpaque());
	$var($String, cnonce, this->params->getCnonce());
	$var($String, nonce, this->params->getNonce());
	$var($String, algorithm, this->params->getAlgorithm());
	this->params->incrementNC();
	int32_t nccount = this->params->getNCCount();
	$var($String, ncstring, nullptr);
	if (nccount != -1) {
		$assign(ncstring, $$nc($Integer::toHexString(nccount))->toLowerCase());
		int32_t len = ncstring->length();
		if (len < 8) {
			$assign(ncstring, $str({DigestAuthentication::zeroPad->get(len), ncstring}));
		}
	}
	try {
		$assign(response, computeDigest(true, $(this->pw->getUserName()), passwd, this->realm, method, uri, nonce, cnonce, ncstring));
	...
	$var($String, value, $str({this->authMethod, " username=\""_s, $(this->pw->getUserName()), "\", realm=\""_s, this->realm, "\", nonce=\""_s, nonce, ncfield, ", uri=\""_s, uri, "\", response=\""_s, response, "\""_s, algoS}));
	if (opaque != nullptr) {
...
void DigestAuthentication::checkResponse($String* header, $String* method, $String* uri) {
	$useLocalObjectStack();
	$var($chars, passwd, $nc(this->pw)->getPassword());
	$var($String, username, this->pw->getUserName());
	bool qop = $nc(this->params)->authQop();
	$var($String, opaque, this->params->getOpaque());
	$var($String, cnonce, this->params->cnonce);
	$var($String, nonce, this->params->getNonce());
	$var($String, algorithm, this->params->getAlgorithm());
	int32_t nccount = this->params->getNCCount();
	$var($String, ncstring, nullptr);
...
$String* DigestAuthentication::computeDigest(bool isRequest, $String* userName, $chars* password, $String* realm, $String* connMethod, $String* requestURI, $String* nonceString, $String* cnonce, $String* ncValue) {
	...
	if (md5sess) {
		if (($assign(HashA1, this->params->getCachedHA1())) == nullptr) {
...
$String* DigestAuthentication::encode($String* src, $chars* passwd, $MessageDigest* md) {
	$useLocalObjectStack();
	$init($ISO_8859_1);
	$nc(md)->update($($nc(src)->getBytes($ISO_8859_1::INSTANCE)));
	if (passwd != nullptr) {
	...
	for (int32_t i = 0; i < digest->length; ++i) {
		int32_t hashchar = (((int32_t)((uint32_t)digest->get(i) >> 4)) & 0x0f);
		res->append(DigestAuthentication::charArray->get(hashchar));
		hashchar = (digest->get(i) & 0x0f);
		res->append(DigestAuthentication::charArray->get(hashchar));
	}
	return res->toString();
}

void DigestAuthentication::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	{
		$var($Boolean, b, $cast($Boolean, $AccessController::doPrivileged($$new($DigestAuthentication$1))));
		DigestAuthentication::delimCompatFlag = (b == nullptr) ? false : b->booleanValue();
	}
...
$Class* DigestAuthentication::load$($String* name, bool initialize) {
	...
	$loadClass(DigestAuthentication, name, initialize, &classInfo$$, DigestAuthentication::clinit$, []($Class* clazz) -> $Object* {
