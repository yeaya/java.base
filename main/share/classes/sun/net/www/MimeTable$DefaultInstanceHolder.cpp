...
$MimeTable* MimeTable$DefaultInstanceHolder::getDefaultInstance() {
	...
	return $cast($MimeTable, $AccessController::doPrivileged($$new($MimeTable$DefaultInstanceHolder$1)));
}

void MimeTable$DefaultInstanceHolder::clinit$($Class* clazz) {
...
$Class* MimeTable$DefaultInstanceHolder::load$($String* name, bool initialize) {
	...
	$loadClass(MimeTable$DefaultInstanceHolder, name, initialize, &classInfo$$, MimeTable$DefaultInstanceHolder::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(MimeTable$DefaultInstanceHolder);
	});
	return class$;
}
