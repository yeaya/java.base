...
void InetAddressCachePolicy::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	{
		$var($Integer, tmp, $cast($Integer, $AccessController::doPrivileged($$new($InetAddressCachePolicy$1))));
		if (tmp != nullptr) {
		...
		$assign(tmp, $cast($Integer, $AccessController::doPrivileged($$new($InetAddressCachePolicy$2))));
		if (tmp != nullptr) {
...
$Class* InetAddressCachePolicy::load$($String* name, bool initialize) {
	...
	$loadClass(InetAddressCachePolicy, name, initialize, &classInfo$$, InetAddressCachePolicy::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(InetAddressCachePolicy);
	});
	return class$;
}
