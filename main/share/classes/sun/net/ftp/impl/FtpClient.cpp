...
$Class* FtpClient$$Lambda$lambda$openPassiveDataConnection$0::load$($String* name, bool initialize) {
	...
	$loadClass(FtpClient$$Lambda$lambda$openPassiveDataConnection$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FtpClient$$Lambda$lambda$openPassiveDataConnection$0);
	});
	return class$;
}
...
$Class* FtpClient$$Lambda$lambda$openPassiveDataConnection$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(FtpClient$$Lambda$lambda$openPassiveDataConnection$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FtpClient$$Lambda$lambda$openPassiveDataConnection$1$1);
	});
	return class$;
}
...
$Class* FtpClient$$Lambda$getHostAddress$2::load$($String* name, bool initialize) {
	...
	$loadClass(FtpClient$$Lambda$getHostAddress$2, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FtpClient$$Lambda$getHostAddress$2);
	});
	return class$;
}
...
$Class* FtpClient$$Lambda$equalsIgnoreCase$3::load$($String* name, bool initialize) {
	...
	$loadClass(FtpClient$$Lambda$equalsIgnoreCase$3, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FtpClient$$Lambda$equalsIgnoreCase$3);
	});
	return class$;
}
...
$Class* FtpClient$$Lambda$getLocalHost$4::load$($String* name, bool initialize) {
	...
	$loadClass(FtpClient$$Lambda$getLocalHost$4, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FtpClient$$Lambda$getLocalHost$4);
	});
	return class$;
}
...
$Class* FtpClient$$Lambda$lambda$privilegedGetAllByName$2$5::load$($String* name, bool initialize) {
	...
	$loadClass(FtpClient$$Lambda$lambda$privilegedGetAllByName$2$5, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FtpClient$$Lambda$lambda$privilegedGetAllByName$2$5);
	});
	return class$;
}
...
void FtpClient::getTransferName() {
	...
	int32_t e = response->lastIndexOf(u')');
	if (i >= 0) {
		i += 17;
...
int32_t FtpClient::readServerResponse() {
	...
	while (true) {
		...
		if ($nc(FtpClient::logger)->isLoggable($PlatformLogger$Level::FINEST)) {
			FtpClient::logger->finest($$str({"Server ["_s, this->serverAddr, "] --> "_s, response}));
		}
		if (response->isEmpty()) {
			code = -1;
		} else {
...
void FtpClient::sendServer($String* cmd) {
	...
	$init($PlatformLogger$Level);
	if ($nc(FtpClient::logger)->isLoggable($PlatformLogger$Level::FINEST)) {
		FtpClient::logger->finest($$str({"Server ["_s, this->serverAddr, "] <-- "_s, cmd}));
	}
}
...
bool FtpClient::readReply() {
	...
	bool var$0 = this->lastReplyCode->isPositiveCompletion();
	if (var$0 || this->lastReplyCode->isPositiveIntermediate()) {
...
$Socket* FtpClient::openPassiveDataConnection($String* cmd) {
	...
	if (issueCommand("EPSV ALL"_s)) {
		...
		int32_t var$0 = $Integer::parseInt($(m->group(3)));
		port = var$0 + ($Integer::parseInt($(m->group(2))) << 8);
		$var($String, s, $$nc(m->group(1))->replace(u',', u'.'));
	...
	if (this->proxy != nullptr) {
		...
		if (this->proxy->type() == $Proxy$Type::SOCKS) {
			$var($PrivilegedAction, pa, $new(FtpClient$$Lambda$lambda$openPassiveDataConnection$0, this));
	...
	$var($PrivilegedAction, pa, $new(FtpClient$$Lambda$lambda$openPassiveDataConnection$1$1, this));
	...
	if (this->useCrypto) {
		try {
			$var($String, var$1, $nc(dest)->getHostName());
			$assign(s, $nc(this->sslFact)->createSocket(s, var$1, dest->getPort(), true));
		} catch ($Exception& e) {
	...
	if (!issueCommand(cmd)) {
		...
		$var($String, var$2, $str({cmd, ":"_s, $(getResponseString())}));
		$throwNew($FtpProtocolException, var$2, $(getLastReplyCode()));
	}
	return s;
}
...
$InetSocketAddress* FtpClient::validatePasvAddress(int32_t port, $String* s, $InetAddress* address) {
	...
	if ($nc(serverAddress)->equals(s)) {
		...
		bool var$0 = address->isLoopbackAddress();
		if (var$0 && $nc(s)->startsWith("127."_s)) {
			return $new($InetSocketAddress, s, port);
		...
		} else if ($nc(s)->startsWith("127."_s)) {
	...
	bool var$1 = $IPAddressUtil::isIPv4LiteralAddress(hostName);
	if (!(var$1 || $IPAddressUtil::isIPv6LiteralAddress(hostName))) {
		$var($InetAddressArray, names, privilegedGetAllByName(hostName));
		$var($String, resAddress, $cast($String, $$nc($$nc($$nc($$nc($Arrays::stream(names))->map($$new(FtpClient$$Lambda$getHostAddress$2)))->filter($$new(FtpClient$$Lambda$equalsIgnoreCase$3, $nc(s))))->findFirst())->orElse(nullptr)));
...
$InetAddress* FtpClient::privilegedLocalHost() {
	...
	$useLocalObjectStack();
	$beforeCallerSensitive();
	$var($PrivilegedExceptionAction, action, $new(FtpClient$$Lambda$getLocalHost$4));
	try {
...
$InetAddressArray* FtpClient::privilegedGetAllByName($String* hostName) {
	...
	$useLocalObjectStack();
	$beforeCallerSensitive();
	$var($PrivilegedExceptionAction, pAction, $new(FtpClient$$Lambda$lambda$privilegedGetAllByName$2$5, hostName));
	try {
...
$Socket* FtpClient::openDataConnection($String* cmd) {
	...
	$init($Proxy$Type);
	if (this->proxy != nullptr && this->proxy->type() == $Proxy$Type::SOCKS) {
	...
	try {
		...
		if ($nc(myAddress)->isAnyLocalAddress()) {
			$assign(myAddress, this->server->getLocalAddress());
		}
		$var($StringBuilder, var$2, $new($StringBuilder));
		var$2->append("EPRT |"_s);
		var$2->append(($instanceOf($Inet6Address, myAddress)) ? "2"_s : "1"_s);
		var$2->append("|"_s);
		var$2->append($($nc(myAddress)->getHostAddress()));
		var$2->append("|"_s);
		var$2->append(portSocket->getLocalPort());
		var$2->append("|"_s);
		$assign(portCmd, $str(var$2));
		bool var$3 = !issueCommand(portCmd);
		if (var$3 || !issueCommand(cmd)) {
			$assign(portCmd, "PORT "_s);
			$var($bytes, addr, $nc(myAddress)->getAddress());
			for (int32_t i = 0; i < $nc(addr)->length; ++i) {
				$assign(portCmd, $str({portCmd, $$str((addr->get(i) & 0xff)), ","_s}));
			}
			$var($StringBuilder, var$4, $new($StringBuilder));
			var$4->append(portCmd);
			var$4->append(portSocket->getLocalPort());
			var$4->append(8);
			var$4->append(255);
			var$4->append(","_s);
			var$4->append(portSocket->getLocalPort());
			var$4->append(255);
			$assign(portCmd, $str(var$4));
	...
	if (this->useCrypto) {
		try {
			$var($String, var$6, $nc(this->serverAddr)->getHostName());
			$assign(clientSocket, $nc(this->sslFact)->createSocket(clientSocket, var$6, this->serverAddr->getPort(), true));
		} catch ($Exception& ex) {
...
void FtpClient::tryConnect($InetSocketAddress* dest, int32_t timeout) {
	...
	try {
		$set(this, out, $new($PrintStream, $$new($BufferedOutputStream, $($nc(this->server)->getOutputStream())), true, FtpClient::encoding));
...
$SocketAddress* FtpClient::getServerAddress() {
	return this->server == nullptr ? ($SocketAddress*)nullptr : this->server->getRemoteSocketAddress();
}
...
$FtpClient* FtpClient::login($String* user, $chars* password) {
	...
	for (int32_t i = 0; i < $nc(this->serverResponse)->size(); ++i) {
		$assign(l, $cast($String, this->serverResponse->elementAt(i)));
		if (l != nullptr) {
...
$FtpClient* FtpClient::login($String* user, $chars* password, $String* account) {
	...
	if (this->serverResponse != nullptr) {
		$var($Iterator, i$, this->serverResponse->iterator());
		for (; $nc(i$)->hasNext();) {
...
$FtpClient* FtpClient::changeDirectory($String* remoteDirectory) {
	if (remoteDirectory == nullptr || remoteDirectory->isEmpty()) {
...
$String* FtpClient::getWorkingDirectory() {
	...
	return answ->substring(5, answ->lastIndexOf(u'\"'));
}
...
$List* FtpClient::getFeatures() {
	...
	for (int32_t i = 1; i < $nc(resp)->size() - 1; ++i) {
		...
		features->add($($nc(s)->substring(1, $nc(s)->length() - 1)));
	}
	return features;
}
...
$FtpClient* FtpClient::reInit() {
	...
	if (this->useCrypto) {
		if ($instanceOf($SSLSocket, this->server)) {
			$var($SSLSession, session, $cast($SSLSocket, this->server)->getSession());
...
int64_t FtpClient::getSize($String* path) {
	...
	if (this->lastReplyCode == $FtpReplyCode::FILE_STATUS) {
		...
		$assign(s, $nc(s)->substring(4, $nc(s)->length() - 1));
...
$Date* FtpClient::getLastModified($String* path) {
	...
	if (this->lastReplyCode == $FtpReplyCode::FILE_STATUS) {
		$var($String, s, getResponseString());
		return parseRfc3659TimeValue($($nc(s)->substring(4, $nc(s)->length() - 1)));
	}
	return nullptr;
}
...
$FtpClient* FtpClient::startSecureSession() {
	...
	try {
		...
		$assign(s, $nc(this->sslFact)->createSocket(var$0, var$1, this->serverAddr->getPort(), true));
...
$String* FtpClient::getHelp($String* cmd) {
	...
	$var($Vector, resp, getResponseStrings());
	if ($nc(resp)->size() == 1) {
		return $$sure($String, resp->get(0))->substring(4);
	}
	$var($StringBuilder, sb, $new($StringBuilder));
	for (int32_t i = 1; i < resp->size() - 1; ++i) {
		sb->append($($$sure($String, resp->get(i))->substring(3)));
	}
	return sb->toString();
}
...
void FtpClient::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(FtpClient::linkp, $Pattern::compile("(\\p{Print}+) \\-\\> (\\p{Print}+)$"_s));
	{
		...
		$var($StringArray, acceptPasvAddress, $new($StringArray, {nullptr}));
		$var($String, enc, $cast($String, $AccessController::doPrivileged($$new($FtpClient$1, acceptPasvAddress, vals))));
		if (vals->get(0) == 0) {
		...
		for (int32_t i = 0; i < $nc(FtpClient::patStrings)->length; ++i) {
			$nc(FtpClient::patterns)->set(i, $($Pattern::compile(FtpClient::patStrings->get(i))));
		}
...
$Class* FtpClient::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("sun.net.ftp.impl.FtpClient$$Lambda$lambda$openPassiveDataConnection$0")) {
			return FtpClient$$Lambda$lambda$openPassiveDataConnection$0::load$(name, initialize);
		}
		if (name->equals("sun.net.ftp.impl.FtpClient$$Lambda$lambda$openPassiveDataConnection$1$1")) {
			return FtpClient$$Lambda$lambda$openPassiveDataConnection$1$1::load$(name, initialize);
		}
		if (name->equals("sun.net.ftp.impl.FtpClient$$Lambda$getHostAddress$2")) {
			return FtpClient$$Lambda$getHostAddress$2::load$(name, initialize);
		}
		if (name->equals("sun.net.ftp.impl.FtpClient$$Lambda$equalsIgnoreCase$3")) {
			return FtpClient$$Lambda$equalsIgnoreCase$3::load$(name, initialize);
		}
		if (name->equals("sun.net.ftp.impl.FtpClient$$Lambda$getLocalHost$4")) {
			return FtpClient$$Lambda$getLocalHost$4::load$(name, initialize);
		}
		if (name->equals("sun.net.ftp.impl.FtpClient$$Lambda$lambda$privilegedGetAllByName$2$5")) {
	...
	$loadClass(FtpClient, name, initialize, &classInfo$$, FtpClient::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(FtpClient);
	});
	return class$;
}
