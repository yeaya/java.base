...
FtpClientProvider* FtpClientProvider::provider() {
	...
	$synchronized(FtpClientProvider::lock) {
		...
		return $cast(FtpClientProvider, $AccessController::doPrivileged($$new($FtpClientProvider$1)));
	}
}

void FtpClientProvider::clinit$($Class* clazz) {
...
$Class* FtpClientProvider::load$($String* name, bool initialize) {
	...
	$loadClass(FtpClientProvider, name, initialize, &classInfo$$, FtpClientProvider::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(FtpClientProvider);
	});
	return class$;
}
