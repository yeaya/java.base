...
void DisabledAlgorithmConstraints$DenyAfterConstraint::init$($String* algo, int32_t year, int32_t month, int32_t day) {
	...
	if ($DisabledAlgorithmConstraints::debug != nullptr) {
		$DisabledAlgorithmConstraints::debug->println($$str({"DenyAfterConstraint read in as:  year "_s, $$str(year), ", month = "_s, $$str(month), ", day = "_s, $$str(day)}));
	}
	...
	if (var$0 || year < c->getActualMinimum($Calendar::YEAR)) {
		$throwNew($IllegalArgumentException, $$str({"Invalid year given in constraint: "_s, $$str(year)}));
	}
	bool var$1 = (month - 1) > c->getActualMaximum($Calendar::MONTH);
	if (var$1 || (month - 1) < c->getActualMinimum($Calendar::MONTH)) {
		$throwNew($IllegalArgumentException, $$str({"Invalid month given in constraint: "_s, $$str(month)}));
	}
	bool var$2 = day > c->getActualMaximum($Calendar::DAY_OF_MONTH);
	if (var$2 || day < c->getActualMinimum($Calendar::DAY_OF_MONTH)) {
		$throwNew($IllegalArgumentException, $$str({"Invalid Day of Month given in constraint: "_s, $$str(day)}));
	}
	$set(this, denyAfterDate, c->getTime());
	if ($DisabledAlgorithmConstraints::debug != nullptr) {
		$DisabledAlgorithmConstraints::debug->println($$str({"DenyAfterConstraint date set to: "_s, $(DisabledAlgorithmConstraints$DenyAfterConstraint::dateFormat->format(this->denyAfterDate))}));
	}
}
...
void DisabledAlgorithmConstraints$DenyAfterConstraint::permits($ConstraintsParameters* cp) {
	...
	if (!$nc(this->denyAfterDate)->after(currentDate)) {
		...
		var$0->append("denyAfter constraint check failed: "_s);
		var$0->append(this->algorithm);
		var$0->append(" used with Constraint date: "_s);
		var$0->append($(DisabledAlgorithmConstraints$DenyAfterConstraint::dateFormat->format(this->denyAfterDate)));
		var$0->append("; params date: "_s);
		var$0->append($(DisabledAlgorithmConstraints$DenyAfterConstraint::dateFormat->format(currentDate)));
...
bool DisabledAlgorithmConstraints$DenyAfterConstraint::permits($Key* key) {
	...
	$init($DisabledAlgorithmConstraints);
	if ($DisabledAlgorithmConstraints::debug != nullptr) {
		$DisabledAlgorithmConstraints::debug->println($$str({"DenyAfterConstraints.permits(): "_s, this->algorithm}));
	}
	return $nc(this->denyAfterDate)->after($$new($Date));
}

void DisabledAlgorithmConstraints$DenyAfterConstraint::clinit$($Class* clazz) {
...
$Class* DisabledAlgorithmConstraints$DenyAfterConstraint::load$($String* name, bool initialize) {
	...
	$loadClass(DisabledAlgorithmConstraints$DenyAfterConstraint, name, initialize, &classInfo$$, DisabledAlgorithmConstraints$DenyAfterConstraint::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(DisabledAlgorithmConstraints$DenyAfterConstraint);
	});
	return class$;
}
