...
void DisabledAlgorithmConstraints$Constraints::init$($String* propertyName, $List* constraintArray) {
	...
	$set(this, constraintsMap, $new($HashMap));
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			{
				if (constraintEntry == nullptr || constraintEntry->isEmpty()) {
					continue;
				}
				...
				if ($DisabledAlgorithmConstraints::debug != nullptr) {
					$DisabledAlgorithmConstraints::debug->println($$str({"Constraints: "_s, constraintEntry}));
				}
				int32_t space = constraintEntry->indexOf(u' ');
				$var($String, algorithm, $AlgorithmDecomposer::hashName((space > 0 ? $(constraintEntry->substring(0, space)) : constraintEntry)));
				$init($Locale);
				$var($Object, var$0, $nc(algorithm)->toUpperCase($Locale::ENGLISH));
					...
					for (int32_t len$ = arr$->length, i$ = 0; i$ < len$; ++i$) {
						$var($String, entry, arr$->get(i$));
						{
							...
							if (entry->startsWith("keySize"_s)) {
								if ($DisabledAlgorithmConstraints::debug != nullptr) {
									$DisabledAlgorithmConstraints::debug->println($$str({"Constraints set to keySize: "_s, entry}));
								}
								...
								$var($DisabledAlgorithmConstraints$Constraint$Operator, var$1, $DisabledAlgorithmConstraints$Constraint$Operator::of($(tokens->nextToken())));
								$assign(c, $new($DisabledAlgorithmConstraints$KeySizeConstraint, algorithm, var$1, $Integer::parseInt($(tokens->nextToken()))));
							} else if (entry->equalsIgnoreCase("jdkCA"_s)) {
								if ($DisabledAlgorithmConstraints::debug != nullptr) {
									$DisabledAlgorithmConstraints::debug->println("Constraints set to jdkCA."_s);
								}
								if (jdkCALimit) {
								...
								bool var$2 = entry->startsWith("denyAfter"_s);
								$init($DisabledAlgorithmConstraints$Constraints$Holder);
								if (var$2 && $nc(($assign(matcher, $nc($DisabledAlgorithmConstraints$Constraints$Holder::DENY_AFTER_PATTERN)->matcher(entry))))->matches()) {
									if ($DisabledAlgorithmConstraints::debug != nullptr) {
										$DisabledAlgorithmConstraints::debug->println("Constraints set to denyAfter"_s);
									}
									if (denyAfterLimit) {
									...
									int32_t month = $Integer::parseInt($(matcher->group(2)));
									int32_t day = $Integer::parseInt($(matcher->group(3)));
									...
									if ($DisabledAlgorithmConstraints::debug != nullptr) {
										$DisabledAlgorithmConstraints::debug->println($$str({"Constraints usage length is "_s, $$str(s->length)}));
									}
								} else {
									$throwNew($IllegalArgumentException, $$str({"Error in security property. Constraint unknown: "_s, entry}));
								}
							...
							if (lastConstraint == nullptr) {
								...
								$set(lastConstraint, nextConstraint, c);
							}
	...
	}
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			if (!$nc(constraint)->permits(key)) {
				$init($DisabledAlgorithmConstraints);
				if ($DisabledAlgorithmConstraints::debug != nullptr) {
					$DisabledAlgorithmConstraints::debug->println($$str({"Constraints: failed key sizeconstraint check "_s, $$str($KeyUtil::getKeySize(key))}));
				}
				return false;
			}
		}
	...
	}
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			if (!$nc(constraint)->permits(aps)) {
				$init($DisabledAlgorithmConstraints);
				if ($DisabledAlgorithmConstraints::debug != nullptr) {
					$DisabledAlgorithmConstraints::debug->println($$str({"Constraints: failed algorithm parameters constraint check "_s, aps}));
				}
				return false;
			}
		}
...
void DisabledAlgorithmConstraints$Constraints::permits($String* algorithm, $ConstraintsParameters* cp) {
	...
	$init($DisabledAlgorithmConstraints);
	if ($DisabledAlgorithmConstraints::debug != nullptr) {
		$DisabledAlgorithmConstraints::debug->println($$str({"Constraints.permits(): "_s, algorithm, ", "_s, $($nc($of(cp))->toString())}));
	}
...
$Class* DisabledAlgorithmConstraints$Constraints::load$($String* name, bool initialize) {
	...
	$loadClass(DisabledAlgorithmConstraints$Constraints, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DisabledAlgorithmConstraints$Constraints);
	});
	return class$;
}
