...
void DisabledAlgorithmConstraints$jdkCAConstraint::permits($ConstraintsParameters* cp) {
	...
	if ($DisabledAlgorithmConstraints::debug != nullptr) {
		$DisabledAlgorithmConstraints::debug->println($$str({"jdkCAConstraints.permits(): "_s, this->algorithm}));
	}
...
$Class* DisabledAlgorithmConstraints$jdkCAConstraint::load$($String* name, bool initialize) {
	...
	$loadClass(DisabledAlgorithmConstraints$jdkCAConstraint, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DisabledAlgorithmConstraints$jdkCAConstraint);
	});
	return class$;
}
