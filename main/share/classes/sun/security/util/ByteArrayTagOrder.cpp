...
int32_t ByteArrayTagOrder::compare($bytes* bytes1, $bytes* bytes2) {
	return ($nc(bytes1)->get(0) | 0x20) - ($nc(bytes2)->get(0) | 0x20);
}
...
$Class* ByteArrayTagOrder::load$($String* name, bool initialize) {
	...
	$loadClass(ByteArrayTagOrder, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ByteArrayTagOrder);
	});
	return class$;
}
