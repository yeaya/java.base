...
void PSSParameters::engineInit($bytes* encoded) {
	...
	$var($MGF1ParameterSpec, mgfSpec, $cast($MGF1ParameterSpec, $PSSParameterSpec::DEFAULT->getMGFParameters()));
	int32_t saltLength = $PSSParameterSpec::DEFAULT->getSaltLength();
	int32_t trailerField = $PSSParameterSpec::DEFAULT->getTrailerField();
	...
	$var($DerValueArray, datum, der->getSequence(4));
	{
		...
		for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
			...
			if ($nc(d)->isContextSpecific((int8_t)0)) {
				...
				$var($AlgorithmId, params, $AlgorithmId::parse($$new($DerValue, $(val->getEncodedParams()))));
...
$Class* PSSParameters::load$($String* name, bool initialize) {
	...
	$loadClass(PSSParameters, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PSSParameters);
	});
	return class$;
}
