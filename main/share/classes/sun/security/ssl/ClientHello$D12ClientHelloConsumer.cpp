...
void ClientHello$D12ClientHelloConsumer::consume($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message) {
	...
	$var($HelloCookieManager, hcm, $nc(shc->sslContext)->getHelloCookieManager($ProtocolVersion::DTLS10));
	if (!shc->isResumption && !$nc(hcm)->isCookieValid(shc, clientHello, clientHello->cookie)) {
		$init($SSLHandshake);
...
$Class* ClientHello$D12ClientHelloConsumer::load$($String* name, bool initialize) {
	...
	$loadClass(ClientHello$D12ClientHelloConsumer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ClientHello$D12ClientHelloConsumer);
	});
	return class$;
}
