...
$Class* CertSignAlgsExtension$CHCertSignatureSchemesProducer::load$($String* name, bool initialize) {
	...
	$loadClass(CertSignAlgsExtension$CHCertSignatureSchemesProducer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CertSignAlgsExtension$CHCertSignatureSchemesProducer);
	});
	return class$;
}
