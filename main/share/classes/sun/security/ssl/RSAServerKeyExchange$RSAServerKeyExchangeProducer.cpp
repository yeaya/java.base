...
$bytes* RSAServerKeyExchange$RSAServerKeyExchangeProducer::produce($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message) {
	...
	} else if (!"RSA"_s->equals($($nc(x509Possession->popPrivateKey)->getAlgorithm()))) {
		$init($Alert);
	...
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
		$SSLLogger::fine("Produced RSA ServerKeyExchange handshake message"_s, $$new($ObjectArray, {skem}));
	}
...
$Class* RSAServerKeyExchange$RSAServerKeyExchangeProducer::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PRIVATE, $method(RSAServerKeyExchange$RSAServerKeyExchangeProducer, init$, void)},
		{"produce", "(Lsun/security/ssl/ConnectionContext;Lsun/security/ssl/SSLHandshake$HandshakeMessage;)[B", nullptr, $PUBLIC, $virtualMethod(RSAServerKeyExchange$RSAServerKeyExchangeProducer, produce, $bytes*, $ConnectionContext*, $SSLHandshake$HandshakeMessage*), "java.io.IOException"},
		{}
	};
	...
	$loadClass(RSAServerKeyExchange$RSAServerKeyExchangeProducer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(RSAServerKeyExchange$RSAServerKeyExchangeProducer);
	});
	return class$;
}
