...
$String* Alert$Level::nameOf(int8_t level) {
	...
	return $str({"UNKNOWN ALERT LEVEL ("_s, $$str((level & 0xff)), ")"_s});
}

void Alert$Level::clinit$($Class* clazz) {
...
$Class* Alert$Level::load$($String* name, bool initialize) {
	...
	$loadClass(Alert$Level, name, initialize, &classInfo$$, Alert$Level::clinit$, []($Class* clazz) -> $Object* {
