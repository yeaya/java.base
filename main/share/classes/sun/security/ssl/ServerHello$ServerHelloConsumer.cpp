...
void ServerHello$ServerHelloConsumer::consume($ConnectionContext* context, $ByteBuffer* message) {
	...
	if (!chc->handshakeConsumers->isEmpty()) {
		chc->handshakeConsumers->remove($($Byte::valueOf($SSLHandshake::HELLO_VERIFY_REQUEST->id)));
	}
	if (!chc->handshakeConsumers->isEmpty()) {
		$init($Alert);
	...
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
		$SSLLogger::fine("Consuming ServerHello handshake message"_s, $$new($ObjectArray, {shm}));
	}
...
void ServerHello$ServerHelloConsumer::onHelloRetryRequest($ClientHandshakeContext* chc, $ServerHello$ServerHelloMessage* helloRetryRequest) {
	...
	$init($SSLLogger);
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
		$SSLLogger::fine($$str({"Negotiated protocol version: "_s, serverVersion->name$}), $$new($ObjectArray, 0));
	}
...
void ServerHello$ServerHelloConsumer::onServerHello($ClientHandshakeContext* chc, $ServerHello$ServerHelloMessage* serverHello) {
	...
	if (!$nc(chc->conContext)->isNegotiated) {
		...
		$nc(chc->conContext->outputRecord)->setVersion(chc->negotiatedProtocol);
	}
	$init($SSLLogger);
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
	...
	if ($nc(serverHello->serverRandom)->isVersionDowngrade(chc)) {
		$init($Alert);
		$throw($(chc->conContext->fatal($Alert::ILLEGAL_PARAMETER, "A potential protocol version downgrade attack"_s)));
	}
...
$Class* ServerHello$ServerHelloConsumer::load$($String* name, bool initialize) {
	...
	$loadClass(ServerHello$ServerHelloConsumer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ServerHello$ServerHelloConsumer);
	});
	return class$;
}
