...
$KeyStore* TrustStoreManager$TrustAnchorManager::loadKeyStore($TrustStoreManager$TrustStoreDescriptor* descriptor) {
	...
	if (!"NONE"_s->equals($nc(descriptor)->storeName) && descriptor->storeFile == nullptr) {
		$init($SSLLogger);
		if ($SSLLogger::isOn$ && $SSLLogger::isOn("trustmanager"_s)) {
	...
	$var($KeyStore, ks, nullptr);
	if ($nc(descriptor->storeProvider)->isEmpty()) {
	...
	if (!$nc(descriptor->storePassword)->isEmpty()) {
		$assign(password, descriptor->storePassword->toCharArray());
	}
	if (!"NONE"_s->equals(descriptor->storeName)) {
		try {
			$var($FileInputStream, fis, $cast($FileInputStream, $AccessController::doPrivileged($$new($OpenFileInputStreamAction, descriptor->storeFile))));
			$var($Throwable, var$0, nullptr);
			try {
				try {
			...
			$init($SSLLogger);
			if ($SSLLogger::isOn$ && $SSLLogger::isOn("trustmanager"_s)) {
				$SSLLogger::fine($$str({"Not available key store: "_s, descriptor->storeName}), $$new($ObjectArray, 0));
			}
			return nullptr;
		}
	} else {
...
$Class* TrustStoreManager$TrustAnchorManager::load$($String* name, bool initialize) {
	...
	$loadClass(TrustStoreManager$TrustAnchorManager, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TrustStoreManager$TrustAnchorManager);
	});
	return class$;
}
