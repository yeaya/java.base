...
$String* Alert$AlertMessage::toString() {
	...
	$var($ObjectArray, messageFields, $new($ObjectArray, {
		$($Alert$Level::nameOf(this->level)),
		$($Alert::nameOf(this->id))
	}));
...
$Class* Alert$AlertMessage::load$($String* name, bool initialize) {
	...
	$loadClass(Alert$AlertMessage, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Alert$AlertMessage);
	});
	return class$;
}
