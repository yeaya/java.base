...
$SSLKeyDerivation* ECDHKeyExchange$ECDHEXDHKAGenerator::createKeyDerivation($HandshakeContext* context) {
	...
	$var($PrivateKey, var$0, $nc(namedGroupPossession)->getPrivateKey());
	return $new($KAKeyDerivation, alg, context, var$0, $($nc(namedGroupCredentials)->getPublicKey()));
}
...
$Class* ECDHKeyExchange$ECDHEXDHKAGenerator::load$($String* name, bool initialize) {
	...
	$loadClass(ECDHKeyExchange$ECDHEXDHKAGenerator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ECDHKeyExchange$ECDHEXDHKAGenerator);
	});
	return class$;
}
