...
void ProtocolVersion::init$($String* $enum$name, int32_t $enum$ordinal, int32_t id, $String* name, bool isDTLS) {
	...
	$set(this, name$, name);
	this->isDTLS = isDTLS;
	this->major = (int8_t)(((int32_t)((uint32_t)id >> 8)) & 0xff);
	this->minor = (int8_t)(id & 0xff);
...
$String* ProtocolVersion::nameOf(int32_t id) {
	$init(ProtocolVersion);
	return nameOf((int8_t)(((int32_t)((uint32_t)id >> 8)) & 0xff), (int8_t)(id & 0xff));
}
...
bool ProtocolVersion::isNegotiable(int8_t major, int8_t minor, bool isDTLS, bool allowSSL20Hello) {
	$init(ProtocolVersion);
	int32_t v = ((major & 0xff) << 8) | (minor & 0xff);
	if (isDTLS) {
...
$List* ProtocolVersion::namesOf($StringArray* protocolNames) {
	$init(ProtocolVersion);
	$useLocalObjectStack();
	if (protocolNames == nullptr || protocolNames->length == 0) {
...
void ProtocolVersion::clinit$($Class* clazz) {
...
$Class* ProtocolVersion::load$($String* name, bool initialize) {
	...
	$loadClass(ProtocolVersion, name, initialize, &classInfo$$, ProtocolVersion::clinit$, []($Class* clazz) -> $Object* {
