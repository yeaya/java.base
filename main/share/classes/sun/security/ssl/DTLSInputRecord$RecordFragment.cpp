...
int32_t DTLSInputRecord$RecordFragment::compareTo(DTLSInputRecord$RecordFragment* o) {
	...
	if (this->contentType == $ContentType::CHANGE_CIPHER_SPEC->id) {
		...
		} else if ((this->recordEpoch == o->recordEpoch) && (o->contentType == $ContentType::HANDSHAKE->id)) {
			return 1;
		}
	} else if ($nc(o)->contentType == $ContentType::CHANGE_CIPHER_SPEC->id) {
	...
	return compareToSequence($nc(o)->recordEpoch, $nc(o)->recordSeq);
}
...
$Class* DTLSInputRecord$RecordFragment::load$($String* name, bool initialize) {
	...
	$loadClass(DTLSInputRecord$RecordFragment, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DTLSInputRecord$RecordFragment);
	});
	return class$;
}
