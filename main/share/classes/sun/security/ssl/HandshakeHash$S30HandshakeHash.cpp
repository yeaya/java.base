...
$MessageDigest* HandshakeHash$S30HandshakeHash::cloneMd5() {
	...
	if ($instanceOf($Cloneable, this->mdMD5)) {
		try {
			$assign(md5Clone, $cast($MessageDigest, this->mdMD5->clone()));
		...
		$nc(md5Clone)->update($(this->md5->archived()));
	}
	return md5Clone;
}
...
$MessageDigest* HandshakeHash$S30HandshakeHash::cloneSha() {
	...
	if ($instanceOf($Cloneable, this->mdSHA)) {
		try {
			$assign(shaClone, $cast($MessageDigest, this->mdSHA->clone()));
		} catch ($CloneNotSupportedException& ex) {
			$throwNew($RuntimeException, "MessageDigest does no support clone operation"_s);
		}
	} else {
		...
		$nc(shaClone)->update($(this->sha->archived()));
	}
	return shaClone;
}
...
void HandshakeHash$S30HandshakeHash::updateDigest($MessageDigest* md, $bytes* pad1, $bytes* pad2, $SecretKey* masterSecret) {
	$init(HandshakeHash$S30HandshakeHash);
	$useLocalObjectStack();
	$var($bytes, keyBytes, "RAW"_s->equals($($nc(masterSecret)->getFormat())) ? masterSecret->getEncoded() : ($bytes*)nullptr);
	if (keyBytes != nullptr) {
...
void HandshakeHash$S30HandshakeHash::digestKey($MessageDigest* md, $SecretKey* key) {
	...
	try {
		if ($instanceOf($MessageDigestSpi2, md)) {
			$cast($MessageDigestSpi2, md)->engineUpdate(key);
		} else {
...
void HandshakeHash$S30HandshakeHash::clinit$($Class* clazz) {
...
$Class* HandshakeHash$S30HandshakeHash::load$($String* name, bool initialize) {
	...
	$loadClass(HandshakeHash$S30HandshakeHash, name, initialize, &classInfo$$, HandshakeHash$S30HandshakeHash::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(HandshakeHash$S30HandshakeHash);
	});
	return class$;
}
