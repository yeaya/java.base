...
$Class* SSLCipher$NullReadCipherGenerator$NullReadCipher::load$($String* name, bool initialize) {
	...
	$loadClass(SSLCipher$NullReadCipherGenerator$NullReadCipher, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SSLCipher$NullReadCipherGenerator$NullReadCipher);
	});
	return class$;
}
