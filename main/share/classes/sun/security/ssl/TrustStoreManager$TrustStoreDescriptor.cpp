...
TrustStoreManager$TrustStoreDescriptor* TrustStoreManager$TrustStoreDescriptor::createInstance() {
	...
	return $cast(TrustStoreManager$TrustStoreDescriptor, $AccessController::doPrivileged($$new($TrustStoreManager$TrustStoreDescriptor$1)));
}
...
bool TrustStoreManager$TrustStoreDescriptor::equals(Object$* obj) {
	...
	if ($instanceOf(TrustStoreManager$TrustStoreDescriptor, obj)) {
		...
		bool var$1 = (this->lastModified == that->lastModified) && $Objects::equals(this->storeName, that->storeName);
...
int32_t TrustStoreManager$TrustStoreDescriptor::hashCode() {
	int32_t result = 17;
	if (this->storeName != nullptr && !this->storeName->isEmpty()) {
		result = 31 * result + this->storeName->hashCode();
	}
	if (this->storeType != nullptr && !this->storeType->isEmpty()) {
		result = 31 * result + this->storeType->hashCode();
	}
	if (this->storeProvider != nullptr && !this->storeProvider->isEmpty()) {
...
void TrustStoreManager$TrustStoreDescriptor::clinit$($Class* clazz) {
	$init($File);
...
$Class* TrustStoreManager$TrustStoreDescriptor::load$($String* name, bool initialize) {
	...
	$loadClass(TrustStoreManager$TrustStoreDescriptor, name, initialize, &classInfo$$, TrustStoreManager$TrustStoreDescriptor::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(TrustStoreManager$TrustStoreDescriptor);
	});
	return class$;
}
