...
$bytes* CertificateVerify$S30CertificateVerifyProducer::produce($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message) {
	...
	$var($CertificateVerify$S30CertificateVerifyMessage, cvm, $new($CertificateVerify$S30CertificateVerifyMessage, chc, x509Possession));
	$init($SSLLogger);
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
...
$Class* CertificateVerify$S30CertificateVerifyProducer::load$($String* name, bool initialize) {
	...
	$loadClass(CertificateVerify$S30CertificateVerifyProducer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CertificateVerify$S30CertificateVerifyProducer);
	});
	return class$;
}
