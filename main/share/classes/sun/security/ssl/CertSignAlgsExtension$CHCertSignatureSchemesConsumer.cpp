...
void CertSignAlgsExtension$CHCertSignatureSchemesConsumer::consume($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message, $ByteBuffer* buffer) {
	...
	$nc(shc->handshakeExtensions)->put($SSLExtension::CH_SIGNATURE_ALGORITHMS_CERT, spec);
}
...
$Class* CertSignAlgsExtension$CHCertSignatureSchemesConsumer::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PRIVATE, $method(CertSignAlgsExtension$CHCertSignatureSchemesConsumer, init$, void)},
		{"consume", "(Lsun/security/ssl/ConnectionContext;Lsun/security/ssl/SSLHandshake$HandshakeMessage;Ljava/nio/ByteBuffer;)V", nullptr, $PUBLIC, $virtualMethod(CertSignAlgsExtension$CHCertSignatureSchemesConsumer, consume, void, $ConnectionContext*, $SSLHandshake$HandshakeMessage*, $ByteBuffer*), "java.io.IOException"},
		{}
	};
	...
	$loadClass(CertSignAlgsExtension$CHCertSignatureSchemesConsumer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CertSignAlgsExtension$CHCertSignatureSchemesConsumer);
	});
	return class$;
}
