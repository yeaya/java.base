...
void AlpnExtension$SHAlpnConsumer::consume($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message, $ByteBuffer* buffer) {
	...
	if (requestedAlps == nullptr || requestedAlps->applicationProtocols == nullptr || requestedAlps->applicationProtocols->isEmpty()) {
		$init($Alert);
	...
	$set(chc, applicationProtocol, $cast($String, spec->applicationProtocols->get(0)));
...
$Class* AlpnExtension$SHAlpnConsumer::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PRIVATE, $method(AlpnExtension$SHAlpnConsumer, init$, void)},
		{"consume", "(Lsun/security/ssl/ConnectionContext;Lsun/security/ssl/SSLHandshake$HandshakeMessage;Ljava/nio/ByteBuffer;)V", nullptr, $PUBLIC, $virtualMethod(AlpnExtension$SHAlpnConsumer, consume, void, $ConnectionContext*, $SSLHandshake$HandshakeMessage*, $ByteBuffer*), "java.io.IOException"},
		{}
	};
	...
	$loadClass(AlpnExtension$SHAlpnConsumer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AlpnExtension$SHAlpnConsumer);
	});
	return class$;
}
