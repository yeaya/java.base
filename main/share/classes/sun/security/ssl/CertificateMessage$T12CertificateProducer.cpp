...
$bytes* CertificateMessage$T12CertificateProducer::onProduceCertificate($ServerHandshakeContext* shc, $SSLHandshake$HandshakeMessage* message) {
	...
	$var($CertificateMessage$T12CertificateMessage, cm, $new($CertificateMessage$T12CertificateMessage, shc, x509Possession->popCerts));
	$init($SSLLogger);
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
		$SSLLogger::fine("Produced server Certificate handshake message"_s, $$new($ObjectArray, {cm}));
	}
...
$bytes* CertificateMessage$T12CertificateProducer::onProduceCertificate($ClientHandshakeContext* chc, $SSLHandshake$HandshakeMessage* message) {
	...
	if (x509Possession->popCerts != nullptr && x509Possession->popCerts->length != 0) {
		chc->handshakeSession->setLocalCertificates(x509Possession->popCerts);
	} else {
		chc->handshakeSession->setLocalCertificates(nullptr);
	}
	$var($CertificateMessage$T12CertificateMessage, cm, $new($CertificateMessage$T12CertificateMessage, chc, x509Possession->popCerts));
	$init($SSLLogger);
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
		$SSLLogger::fine("Produced client Certificate handshake message"_s, $$new($ObjectArray, {cm}));
	}
...
$Class* CertificateMessage$T12CertificateProducer::load$($String* name, bool initialize) {
	...
	$loadClass(CertificateMessage$T12CertificateProducer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CertificateMessage$T12CertificateProducer);
	});
	return class$;
}
