...
$Ciphertext* SSLEngineOutputRecord$HandshakeFragment::acquireCiphertext($ByteBuffer* dstBuf) {
	...
	$var($SSLEngineOutputRecord$RecordMemo, memo, $cast($SSLEngineOutputRecord$RecordMemo, this->handshakeMemos->getFirst()));
	...
	if (this->this$0->packetSize > 0) {
		...
		fragLen = $nc(memo->encodeCipher)->calculateFragmentSize(fragLen, 5);
	} else {
	...
	int32_t dstContent = dstPos + 5 + $nc(memo->encodeCipher)->getExplicitNonceSize();
	dstBuf->position(dstContent);
	if (hsMemo != nullptr) {
		int32_t remainingFragLen = fragLen;
		while ((remainingFragLen > 0) && !this->handshakeMemos->isEmpty()) {
			...
			if (hsMemo->acquireOffset == 0) {
				...
				dstBuf->put((int8_t)((memoFragLen >> 16) & 0xff));
				dstBuf->put((int8_t)((memoFragLen >> 8) & 0xff));
				dstBuf->put((int8_t)(memoFragLen & 0xff));
				remainingFragLen -= 4;
			}
			...
			if (hsMemo->acquireOffset == memoFragLen) {
				...
				if ((remainingFragLen > chipLen) && !this->handshakeMemos->isEmpty()) {
					$var($SSLEngineOutputRecord$RecordMemo, rm, $cast($SSLEngineOutputRecord$RecordMemo, this->handshakeMemos->getFirst()));
...
$Class* SSLEngineOutputRecord$HandshakeFragment::load$($String* name, bool initialize) {
	...
	$loadClass(SSLEngineOutputRecord$HandshakeFragment, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SSLEngineOutputRecord$HandshakeFragment);
	});
	return class$;
}
