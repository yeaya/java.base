...
$Class* MaxFragExtension$SHMaxFragmentLengthProducer::load$($String* name, bool initialize) {
	...
	$loadClass(MaxFragExtension$SHMaxFragmentLengthProducer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MaxFragExtension$SHMaxFragmentLengthProducer);
	});
	return class$;
}
