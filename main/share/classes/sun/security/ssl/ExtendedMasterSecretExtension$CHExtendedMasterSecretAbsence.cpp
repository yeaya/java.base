...
void ExtendedMasterSecretExtension$CHExtendedMasterSecretAbsence::absent($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message) {
	...
	if ($nc(shc->negotiatedProtocol)->useTLS10PlusSpec() && !$SSLConfiguration::allowLegacyMasterSecret) {
		$init($Alert);
	...
	if (shc->isResumption && shc->resumingSession != nullptr) {
		...
		} else if (!$SSLConfiguration::allowLegacyResumption) {
			$init($Alert);
			$throw($($nc(shc->conContext)->fatal($Alert::HANDSHAKE_FAILURE, "Missing Extended Master Secret extension on session resumption"_s)));
		} else {
...
$Class* ExtendedMasterSecretExtension$CHExtendedMasterSecretAbsence::load$($String* name, bool initialize) {
	...
	$loadClass(ExtendedMasterSecretExtension$CHExtendedMasterSecretAbsence, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ExtendedMasterSecretExtension$CHExtendedMasterSecretAbsence);
	});
	return class$;
}
