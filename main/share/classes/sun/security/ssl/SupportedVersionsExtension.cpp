...
void SupportedVersionsExtension::clinit$($Class* clazz) {
...
$Class* SupportedVersionsExtension::load$($String* name, bool initialize) {
	...
	$loadClass(SupportedVersionsExtension, name, initialize, &classInfo$$, SupportedVersionsExtension::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SupportedVersionsExtension);
	});
	return class$;
}
