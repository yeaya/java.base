...
DHKeyExchange$DHECredentials* DHKeyExchange$DHECredentials::valueOf($NamedGroup* ng, $bytes* encodedPublic) {
	...
	if (encodedPublic == nullptr || encodedPublic->length == 0) {
		return nullptr;
	}
	$var($DHParameterSpec, params, $cast($DHParameterSpec, ng->keAlgParamSpec));
...
$Class* DHKeyExchange$DHECredentials::load$($String* name, bool initialize) {
	...
	$loadClass(DHKeyExchange$DHECredentials, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DHKeyExchange$DHECredentials);
	});
	return class$;
}
