...
$bytes* CertificateVerify$T10CertificateVerifyProducer::produce($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message) {
	...
	$var($CertificateVerify$T10CertificateVerifyMessage, cvm, $new($CertificateVerify$T10CertificateVerifyMessage, chc, x509Possession));
	$init($SSLLogger);
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
...
$Class* CertificateVerify$T10CertificateVerifyProducer::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PRIVATE, $method(CertificateVerify$T10CertificateVerifyProducer, init$, void)},
		{"produce", "(Lsun/security/ssl/ConnectionContext;Lsun/security/ssl/SSLHandshake$HandshakeMessage;)[B", nullptr, $PUBLIC, $virtualMethod(CertificateVerify$T10CertificateVerifyProducer, produce, $bytes*, $ConnectionContext*, $SSLHandshake$HandshakeMessage*), "java.io.IOException"},
		{}
	};
	...
	$loadClass(CertificateVerify$T10CertificateVerifyProducer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CertificateVerify$T10CertificateVerifyProducer);
	});
	return class$;
}
