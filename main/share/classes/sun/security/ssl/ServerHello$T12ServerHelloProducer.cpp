...
$bytes* ServerHello$T12ServerHelloProducer::produce($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message) {
	...
	if (!$nc(shc)->isResumption || shc->resumingSession == nullptr) {
		...
		$init($ClientAuthType);
		if ((ke != nullptr) && (shc->sslConfig->clientAuthType != $ClientAuthType::CLIENT_AUTH_NONE) && !$nc(shc->negotiatedCipherSuite)->isAnonymous()) {
		...
		if (shc->statelessResumption) {
			$set(shc, resumingSession, $new($SSLSessionImpl, shc->resumingSession, ($nc($nc(clientHello)->sessionId)->length() == 0) ? $$new($SessionId, true, $($nc(shc->sslContext)->getSecureRandom())) : $$new($SessionId, $(clientHello->sessionId->getId()))));
		}
	...
	$var($SessionId, var$2, $nc(shc->handshakeSession)->getSessionId());
	$var($CipherSuite, var$3, shc->negotiatedCipherSuite);
	$var($ServerHello$ServerHelloMessage, shm, $new($ServerHello$ServerHelloMessage, shc, var$1, var$2, var$3, $$new($RandomCookie, shc), clientHello));
	...
	if (shc->isResumption && shc->resumingSession != nullptr) {
		...
		if (kdg == nullptr) {
			...
			$set(shc, handshakeKeyDerivation, kdg->createKeyDerivation(shc, $(shc->resumingSession->getMasterSecret())));
		}
		...
		for (; $nc(i$)->hasNext();) {
			$CipherSuite* cs = $cast($CipherSuite, i$->next());
			{
				...
				if (shc->sslConfig->clientAuthType == $ClientAuthType::CLIENT_AUTH_REQUIRED) {
					...
					if (($nc(cs)->keyExchange == $CipherSuite$KeyExchange::K_DH_ANON) || (cs->keyExchange == $CipherSuite$KeyExchange::K_ECDH_ANON)) {
						continue;
					}
				}
				$var($SSLKeyExchange, ke, $SSLKeyExchange::valueOf($nc(cs)->keyExchange, shc->negotiatedProtocol));
				if (ke == nullptr) {
					continue;
				}
				$init($CryptoPrimitive);
				if (!$nc($ServerHandshakeContext::legacyAlgorithmConstraints)->permits($($EnumSet::of($CryptoPrimitive::KEY_AGREEMENT)), cs->name$, nullptr)) {
					legacySuites->add(cs);
					continue;
				}
				...
				$init($SSLLogger);
				if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
					$SSLLogger::fine($$str({"use cipher suite "_s, cs->name$}), $$new($ObjectArray, 0));
				}
			...
			$CipherSuite* cs = $cast($CipherSuite, i$->next());
			{
				...
				if (ke != nullptr) {
					...
					if ((hcds != nullptr) && (hcds->length != 0)) {
						$init($SSLLogger);
						if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
							$SSLLogger::warning($$str({"use legacy cipher suite "_s, cs->name$}), $$new($ObjectArray, 0));
						}
						return $new($ServerHello$T12ServerHelloProducer$KeyExchangeProperties, cs, ke, hcds);
					}
				}
...
$Class* ServerHello$T12ServerHelloProducer::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PRIVATE, $method(ServerHello$T12ServerHelloProducer, init$, void)},
		{"chooseCipherSuite", "(Lsun/security/ssl/ServerHandshakeContext;Lsun/security/ssl/ClientHello$ClientHelloMessage;)Lsun/security/ssl/ServerHello$T12ServerHelloProducer$KeyExchangeProperties;", nullptr, $PRIVATE | $STATIC, $staticMethod(ServerHello$T12ServerHelloProducer, chooseCipherSuite, $ServerHello$T12ServerHelloProducer$KeyExchangeProperties*, $ServerHandshakeContext*, $ClientHello$ClientHelloMessage*), "java.io.IOException"},
		{"produce", "(Lsun/security/ssl/ConnectionContext;Lsun/security/ssl/SSLHandshake$HandshakeMessage;)[B", nullptr, $PUBLIC, $virtualMethod(ServerHello$T12ServerHelloProducer, produce, $bytes*, $ConnectionContext*, $SSLHandshake$HandshakeMessage*), "java.io.IOException"},
		{}
	};
	...
	$loadClass(ServerHello$T12ServerHelloProducer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ServerHello$T12ServerHelloProducer);
	});
	return class$;
}
