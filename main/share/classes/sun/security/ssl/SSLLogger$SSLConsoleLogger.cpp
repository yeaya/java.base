...
void SSLLogger$SSLConsoleLogger::log($System$Logger$Level* level, $ResourceBundle* rb, $String* message, $Throwable* thrwbl) {
	...
	if (isLoggable(level)) {
		try {
			$var($String, formatted, $SSLLogger$SSLSimpleFormatter::format(this, level, message, $$new($ObjectArray, {thrwbl})));
...
$Class* SSLLogger$SSLConsoleLogger::load$($String* name, bool initialize) {
	...
	$loadClass(SSLLogger$SSLConsoleLogger, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SSLLogger$SSLConsoleLogger);
	});
	return class$;
}
