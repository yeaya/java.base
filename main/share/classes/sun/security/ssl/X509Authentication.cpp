...
$Map$EntryArray* X509Authentication::getHandshakeProducers($HandshakeContext* handshakeContext) {
	...
	if (!$nc($nc(handshakeContext)->negotiatedProtocol)->useTLS13PlusSpec()) {
		$init($SSLHandshake);
		return ($new($Map$EntryArray, {$$new($AbstractMap$SimpleImmutableEntry, $($Byte::valueOf($SSLHandshake::CERTIFICATE->id)), $SSLHandshake::CERTIFICATE)}));
	}
	return ($new($Map$EntryArray, 0));
}

void X509Authentication::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* X509Authentication::load$($String* name, bool initialize) {
	...
	$loadClass(X509Authentication, name, initialize, &classInfo$$, X509Authentication::clinit$, []($Class* clazz) -> $Object* {
