...
$KeyManagerArray* SSLContextImpl$DefaultManagersHolder::getKeyManagers() {
	...
	$AccessController::doPrivileged($$new($SSLContextImpl$DefaultManagersHolder$1, props));
	...
	bool var$0 = SSLContextImpl$DefaultManagersHolder::P11KEYSTORE->equals(defaultKeyStoreType);
	if (var$0 && !SSLContextImpl$DefaultManagersHolder::NONE->equals(defaultKeyStore)) {
	...
	try {
		...
		if (var$2 && !SSLContextImpl$DefaultManagersHolder::NONE->equals(defaultKeyStore)) {
			$assign(fs, $cast($FileInputStream, $AccessController::doPrivileged($$new($SSLContextImpl$DefaultManagersHolder$2, defaultKeyStore))));
		}
	...
	if (SSLContextImpl$DefaultManagersHolder::P11KEYSTORE->equals(defaultKeyStoreType)) {
...
void SSLContextImpl$DefaultManagersHolder::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(SSLContextImpl$DefaultManagersHolder::P11KEYSTORE, "PKCS11"_s);
	{
		...
		try {
			...
			$init($SSLLogger);
			if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,defaultctx"_s)) {
				$SSLLogger::warning("Failed to load default trust managers"_s, $$new($ObjectArray, {e}));
			}
		}
		...
		if (reserved == nullptr) {
			try {
				...
				$init($SSLLogger);
				if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,defaultctx"_s)) {
					$SSLLogger::warning("Failed to load default key managers"_s, $$new($ObjectArray, {e}));
				}
			}
		}
		if (reserved != nullptr) {
...
$Class* SSLContextImpl$DefaultManagersHolder::load$($String* name, bool initialize) {
	...
	$loadClass(SSLContextImpl$DefaultManagersHolder, name, initialize, &classInfo$$, SSLContextImpl$DefaultManagersHolder::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SSLContextImpl$DefaultManagersHolder);
	});
	return class$;
}
