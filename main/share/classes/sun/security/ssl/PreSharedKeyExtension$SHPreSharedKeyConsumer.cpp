...
void PreSharedKeyExtension$SHPreSharedKeyConsumer::consume($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message, $ByteBuffer* buffer) {
	...
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
		$SSLLogger::fine("Received pre_shared_key extension: "_s, $$new($ObjectArray, {shPsk}));
	}
	if (shPsk->selectedIdentity != 0) {
		$init($Alert);
		$throw($($nc(chc->conContext)->fatal($Alert::ILLEGAL_PARAMETER, "Selected identity index is not in correct range."_s)));
	}
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
		$SSLLogger::fine("Resuming session: "_s, $$new($ObjectArray, {chc->resumingSession}));
	}
}
...
$Class* PreSharedKeyExtension$SHPreSharedKeyConsumer::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PRIVATE, $method(PreSharedKeyExtension$SHPreSharedKeyConsumer, init$, void)},
		{"consume", "(Lsun/security/ssl/ConnectionContext;Lsun/security/ssl/SSLHandshake$HandshakeMessage;Ljava/nio/ByteBuffer;)V", nullptr, $PUBLIC, $virtualMethod(PreSharedKeyExtension$SHPreSharedKeyConsumer, consume, void, $ConnectionContext*, $SSLHandshake$HandshakeMessage*, $ByteBuffer*), "java.io.IOException"},
		{}
	};
	...
	$loadClass(PreSharedKeyExtension$SHPreSharedKeyConsumer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PreSharedKeyExtension$SHPreSharedKeyConsumer);
	});
	return class$;
}
