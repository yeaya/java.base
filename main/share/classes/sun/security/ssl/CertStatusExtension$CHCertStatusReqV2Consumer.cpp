...
void CertStatusExtension$CHCertStatusReqV2Consumer::consume($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message, $ByteBuffer* buffer) {
	...
	$nc(shc->handshakeExtensions)->put($SSLExtension::CH_STATUS_REQUEST_V2, spec);
	if (!shc->isResumption) {
		$init($SSLHandshake);
...
$Class* CertStatusExtension$CHCertStatusReqV2Consumer::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PRIVATE, $method(CertStatusExtension$CHCertStatusReqV2Consumer, init$, void)},
		{"consume", "(Lsun/security/ssl/ConnectionContext;Lsun/security/ssl/SSLHandshake$HandshakeMessage;Ljava/nio/ByteBuffer;)V", nullptr, $PUBLIC, $virtualMethod(CertStatusExtension$CHCertStatusReqV2Consumer, consume, void, $ConnectionContext*, $SSLHandshake$HandshakeMessage*, $ByteBuffer*), "java.io.IOException"},
		{}
	};
	...
	$loadClass(CertStatusExtension$CHCertStatusReqV2Consumer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CertStatusExtension$CHCertStatusReqV2Consumer);
	});
	return class$;
}
