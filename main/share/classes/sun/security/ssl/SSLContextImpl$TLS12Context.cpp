...
void SSLContextImpl$TLS12Context::clinit$($Class* clazz) {
	{
...
$Class* SSLContextImpl$TLS12Context::load$($String* name, bool initialize) {
	...
	$loadClass(SSLContextImpl$TLS12Context, name, initialize, &classInfo$$, SSLContextImpl$TLS12Context::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(SSLContextImpl$TLS12Context);
	});
	return class$;
}
