...
bool OutputRecord::seqNumIsHuge() {
	...
	bool return$1 = false;
	try {
		var$2 = ($nc(this->writeCipher)->authenticator != nullptr) && this->writeCipher->authenticator->seqNumIsHuge();
		return$1 = true;
		goto $finally;
...
void OutputRecord::changeWriteCiphers($SSLCipher$SSLWriteCipher* writeCipher, int8_t keyUpdateRequest) {
	...
	try {
		...
		$var($bytes, hm, $cast($bytes, OutputRecord::HANDSHAKE_MESSAGE_KEY_UPDATE->clone()));
...
int64_t OutputRecord::d10Encrypt($SSLCipher$SSLWriteCipher* encCipher, int8_t contentType, $ByteBuffer* destination, int32_t headerOffset, int32_t dstLim, int32_t headerSize, $ProtocolVersion* protocolVersion) {
	...
	destination->put(headerOffset + 4, sequenceNumber->get(1));
	destination->put(headerOffset + 5, sequenceNumber->get(2));
	destination->put(headerOffset + 6, sequenceNumber->get(3));
	destination->put(headerOffset + 7, sequenceNumber->get(4));
	destination->put(headerOffset + 8, sequenceNumber->get(5));
	destination->put(headerOffset + 9, sequenceNumber->get(6));
	destination->put(headerOffset + 10, sequenceNumber->get(7));
...
int64_t OutputRecord::t13Encrypt($SSLCipher$SSLWriteCipher* encCipher, int8_t contentType, $ByteBuffer* destination, int32_t headerOffset, int32_t dstLim, int32_t headerSize, $ProtocolVersion* protocolVersion) {
	...
	$var($bytes, sequenceNumber, $nc(encCipher->authenticator)->sequenceNumber());
	encCipher->encrypt(contentType, destination);
	int32_t fragLen = $nc(destination)->limit() - headerOffset - headerSize;
	destination->put(headerOffset, contentType);
...
int64_t OutputRecord::t13Encrypt($SSLCipher$SSLWriteCipher* encCipher, int8_t contentType, int32_t headerSize) {
	...
	this->buf->set(1, $nc(pv)->major);
	this->buf->set(2, pv->minor);
	this->buf->set(3, (int8_t)((fragLen >> 8) & 0xff));
	this->buf->set(4, (int8_t)(fragLen & 0xff));
	return $Authenticator::toLong(sequenceNumber);
}
...
int64_t OutputRecord::t10Encrypt($SSLCipher$SSLWriteCipher* encCipher, int8_t contentType, int32_t headerSize) {
	...
	int32_t fragLen = this->count - headerSize;
	$nc(this->buf)->set(0, contentType);
	this->buf->set(1, $nc(this->protocolVersion)->major);
	this->buf->set(2, this->protocolVersion->minor);
	this->buf->set(3, (int8_t)((fragLen >> 8) & 0xff));
	this->buf->set(4, (int8_t)(fragLen & 0xff));
	return $Authenticator::toLong(sequenceNumber);
}
...
$ByteBuffer* OutputRecord::encodeV2ClientHello($bytes* fragment, int32_t offset, int32_t length) {
	...
	int32_t v3CSLen = ((fragment->get(v3CSLenOffset) & 0xff) << 8) + (fragment->get(v3CSLenOffset + 1) & 0xff);
	...
	dstBuf->position(0);
	dstBuf->put((int8_t)(0x80 | (((int32_t)((uint32_t)msgLen >> 8)) & 0xff)));
	dstBuf->put((int8_t)(msgLen & 0xff));
	$init($SSLHandshake);
	...
	dstBuf->put((int8_t)(v2CSLen & 0xff));
	dstBuf->put((int8_t)0);
	dstBuf->put((int8_t)0);
	dstBuf->put((int8_t)0);
...
int32_t OutputRecord::V3toV2CipherSuite($ByteBuffer* dstBuf, int8_t byte1, int8_t byte2) {
	...
	dstBuf->put(byte1);
	dstBuf->put(byte2);
	if (((byte2 & 0xff) > 0x0a) || (OutputRecord::V3toV2CipherMap1->get(byte2) == -1)) {
		return 3;
	}
	dstBuf->put((int8_t)OutputRecord::V3toV2CipherMap1->get(byte2));
	dstBuf->put((int8_t)0);
	dstBuf->put((int8_t)OutputRecord::V3toV2CipherMap3->get(byte2));
	return 6;
}

void OutputRecord::clinit$($Class* clazz) {
...
$Class* OutputRecord::load$($String* name, bool initialize) {
	...
	$loadClass(OutputRecord, name, initialize, &classInfo$$, OutputRecord::clinit$, []($Class* clazz) -> $Object* {
