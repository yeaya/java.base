...
void ClientHello$T12ClientHelloConsumer::consume($ConnectionContext* context, $SSLHandshake$HandshakeMessage* message) {
	...
	if ($nc($nc(shc)->conContext)->isNegotiated) {
		...
		if ($ServerHandshakeContext::rejectClientInitiatedRenego && !shc->kickstartMessageDelivered) {
			$init($Alert);
			$throw($(shc->conContext->fatal($Alert::HANDSHAKE_FAILURE, "Client initiated renegotiation is not allowed"_s)));
		}
	}
	$init($SSLExtension);
	...
	if ($nc(clientHello->sessionId)->length() != 0 || shc->statelessResumption) {
		...
		if (shc->statelessResumption) {
			...
			$assign(previous, $nc(cache)->get($(clientHello->sessionId->getId())));
		}
		...
		if (resumingSession) {
			$ProtocolVersion* sessionProtocol = $nc(previous)->getProtocolVersion();
		...
		if (resumingSession) {
			$CipherSuite* suite = $nc(previous)->getSuite();
		...
		if (resumingSession && identityAlg != nullptr) {
			$var($String, sessionIdentityAlg, $nc(previous)->getIdentificationProtocol());
	...
	$set(shc, clientHelloRandom, clientHello->clientRandom);
	$init($SSLHandshake);
	$var($SSLExtensionArray, extTypes, $nc(shc->sslConfig)->getExclusiveExtensions($SSLHandshake::CLIENT_HELLO, $($List::of($SSLExtension::CH_SESSION_TICKET))));
...
$Class* ClientHello$T12ClientHelloConsumer::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PRIVATE, $method(ClientHello$T12ClientHelloConsumer, init$, void)},
		{"consume", "(Lsun/security/ssl/ConnectionContext;Lsun/security/ssl/SSLHandshake$HandshakeMessage;)V", nullptr, $PUBLIC, $virtualMethod(ClientHello$T12ClientHelloConsumer, consume, void, $ConnectionContext*, $SSLHandshake$HandshakeMessage*), "java.io.IOException"},
		{}
	};
	...
	$loadClass(ClientHello$T12ClientHelloConsumer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ClientHello$T12ClientHelloConsumer);
	});
	return class$;
}
