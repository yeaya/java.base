...
void X509KeyManagerImpl$KeyType::init$($String* algorithm) {
	int32_t k = $nc(algorithm)->indexOf(u'_');
	if (k == -1) {
...
$Class* X509KeyManagerImpl$KeyType::load$($String* name, bool initialize) {
	...
	$loadClass(X509KeyManagerImpl$KeyType, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(X509KeyManagerImpl$KeyType);
	});
	return class$;
}
