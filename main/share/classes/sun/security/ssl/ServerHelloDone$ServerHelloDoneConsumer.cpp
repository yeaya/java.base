...
void ServerHelloDone$ServerHelloDoneConsumer::consume($ConnectionContext* context, $ByteBuffer* message) {
	...
	if ($SSLLogger::isOn$ && $SSLLogger::isOn("ssl,handshake"_s)) {
		$SSLLogger::fine("Consuming ServerHelloDone handshake message"_s, $$new($ObjectArray, {shdm}));
	}
	$nc(chc->handshakeProducers)->put($($Byte::valueOf($SSLHandshake::CLIENT_KEY_EXCHANGE->id)), $SSLHandshake::CLIENT_KEY_EXCHANGE);
	chc->handshakeProducers->put($($Byte::valueOf($SSLHandshake::FINISHED->id)), $SSLHandshake::FINISHED);
...
$Class* ServerHelloDone$ServerHelloDoneConsumer::load$($String* name, bool initialize) {
	...
	$loadClass(ServerHelloDone$ServerHelloDoneConsumer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ServerHelloDone$ServerHelloDoneConsumer);
	});
	return class$;
}
