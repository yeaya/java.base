...
void TrustManagerFactoryImpl::engineInit($KeyStore* ks) {
	...
	if (ks == nullptr) {
		try {
			$set(this, trustManager, getInstance($($TrustStoreManager::getTrustedCerts())));
		} catch ($SecurityException& se) {
			$init($SSLLogger);
			if ($SSLLogger::isOn$ && $SSLLogger::isOn("trustmanager"_s)) {
				$SSLLogger::fine("SunX509: skip default keystore"_s, $$new($ObjectArray, {se}));
			}
		} catch ($Error& err) {
			$init($SSLLogger);
			if ($SSLLogger::isOn$ && $SSLLogger::isOn("trustmanager"_s)) {
				$SSLLogger::fine("SunX509: skip default keystore"_s, $$new($ObjectArray, {err}));
			}
			$throw(err);
		} catch ($RuntimeException& re) {
			$init($SSLLogger);
			if ($SSLLogger::isOn$ && $SSLLogger::isOn("trustmanager"_s)) {
				$SSLLogger::fine("SunX509: skip default keystor"_s, $$new($ObjectArray, {re}));
			}
			$throw(re);
		} catch ($Exception& e) {
			$init($SSLLogger);
			if ($SSLLogger::isOn$ && $SSLLogger::isOn("trustmanager"_s)) {
				$SSLLogger::fine("SunX509: skip default keystore"_s, $$new($ObjectArray, {e}));
			}
			$throwNew($KeyStoreException, "problem accessing trust store"_s, e);
		}
	} else {
		$set(this, trustManager, getInstance($($TrustStoreUtil::getTrustedCerts(ks))));
	}
...
$TrustManagerArray* TrustManagerFactoryImpl::engineGetTrustManagers() {
	...
	return $new($TrustManagerArray, {this->trustManager});
}

$FileInputStream* TrustManagerFactoryImpl::getFileInputStream($File* file) {
	$init(TrustManagerFactoryImpl);
	$beforeCallerSensitive();
	return $cast($FileInputStream, $AccessController::doPrivileged($$new($TrustManagerFactoryImpl$1, file)));
}
...
$Class* TrustManagerFactoryImpl::load$($String* name, bool initialize) {
	...
	$loadClass(TrustManagerFactoryImpl, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TrustManagerFactoryImpl);
	});
	return class$;
}
