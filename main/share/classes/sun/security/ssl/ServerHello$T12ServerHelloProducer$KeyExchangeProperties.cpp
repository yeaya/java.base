...
$Class* ServerHello$T12ServerHelloProducer$KeyExchangeProperties::load$($String* name, bool initialize) {
	...
	$loadClass(ServerHello$T12ServerHelloProducer$KeyExchangeProperties, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ServerHello$T12ServerHelloProducer$KeyExchangeProperties);
	});
	return class$;
}
