...
$Set* SSLAlgorithmDecomposer::decomposes($SSLCipher* bulkCipher) {
	...
	$init($SSLAlgorithmDecomposer$1);
	switch ($nc($SSLAlgorithmDecomposer$1::$SwitchMap$sun$security$ssl$SSLCipher)->get((bulkCipher)->ordinal())) {
	case 1:
...
$Set* SSLAlgorithmDecomposer::decompose($String* algorithm) {
	...
	if (var$0 || algorithm->startsWith("TLS_"_s)) {
...
$Class* SSLAlgorithmDecomposer::load$($String* name, bool initialize) {
	...
	$loadClass(SSLAlgorithmDecomposer, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SSLAlgorithmDecomposer);
	});
	return class$;
}
