...
void Resources_zh_HK::clinit$($Class* clazz) {
	...
	$assignStatic(Resources_zh_HK::contents, $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			...
			u"使用 \"keytool -command_name -help\" 取得 command_name 的用法"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			"provider.argument"_s,
			u"提供者引數"_s
		}),
		$$new($ObjectArray, {
			"provider.class.name"_s,
			u"提供者類別名稱"_s
		}),
		$$new($ObjectArray, {
			"provider.name"_s,
		...
		$$new($ObjectArray, {
			"if.alias.not.specified.destalias.srckeypass.and.destkeypass.must.not.be.specified"_s,
			u"如果未指定別名，則不能指定 destalias、srckeypass 及 destkeypass"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			"Certificate.fingerprint.SHA1."_s,
			u"憑證指紋 (SHA1): "_s
		}),
		$$new($ObjectArray, {
			"Keystore.type."_s,
		...
		$$new($ObjectArray, {
			"PKCS.10.Certificate.Request.Version.1.0.Subject.s.Public.Key.s.format.s.key."_s,
