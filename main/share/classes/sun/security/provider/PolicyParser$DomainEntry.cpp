...
void PolicyParser$DomainEntry::add($PolicyParser$KeyStoreEntry* entry) {
	...
	if (!this->entries->containsKey(keystoreName)) {
		...
		$var($ObjectArray, source, $new($ObjectArray, {keystoreName}));
...
$String* PolicyParser$DomainEntry::toString() {
	...
	if (this->entries != nullptr) {
		$var($Iterator, i$, $$nc(this->entries->values())->iterator());
		for (; $nc(i$)->hasNext();) {
...
$Class* PolicyParser$DomainEntry::load$($String* name, bool initialize) {
	...
	$loadClass(PolicyParser$DomainEntry, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PolicyParser$DomainEntry);
	});
	return class$;
}
