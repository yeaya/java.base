...
void SHA::resetHashes() {
	...
	this->state->set(4, (int32_t)0xc3d2e1f0);
}
...
void SHA::implCompress0($bytes* buf, int32_t ofs) {
	...
	for (int32_t t = 16; t <= 79; ++t) {
		...
		this->W->set(t, (temp << 1) | ((int32_t)((uint32_t)temp >> 31)));
	}
	...
	for (int32_t i = 0; i < 20; ++i) {
		int32_t temp = ((a << 5) | ($usr(a, 32 - 5))) + ((b & c) | ((~b) & d)) + e + $nc(this->W)->get(i) + SHA::round1_kt;
		e = d;
		d = c;
		c = ((b << 30) | ($usr(b, 32 - 30)));
		b = a;
	...
	for (int32_t i = 40; i < 60; ++i) {
		int32_t temp = ((a << 5) | ($usr(a, 32 - 5))) + (((b & c) | (b & d)) | (c & d)) + e + $nc(this->W)->get(i) + SHA::round3_kt;
		e = d;
		d = c;
		c = ((b << 30) | ($usr(b, 32 - 30)));
		b = a;
...
$Class* SHA::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(SHA, init$, void)},
		{"clone", "()Ljava/lang/Object;", nullptr, $PUBLIC, $virtualMethod(SHA, clone, $Object*), "java.lang.CloneNotSupportedException"},
		{"implCompress", "([BI)V", nullptr, 0, $virtualMethod(SHA, implCompress, void, $bytes*, int32_t)},
		{"implCompress0", "([BI)V", nullptr, $PRIVATE, $method(SHA, implCompress0, void, $bytes*, int32_t), nullptr, nullptr, implCompress0methodAnnotations$$},
		{"implCompressCheck", "([BI)V", nullptr, $PRIVATE, $method(SHA, implCompressCheck, void, $bytes*, int32_t)},
		{"implDigest", "([BI)V", nullptr, 0, $virtualMethod(SHA, implDigest, void, $bytes*, int32_t)},
		{"implReset", "()V", nullptr, 0, $virtualMethod(SHA, implReset, void)},
		{"resetHashes", "()V", nullptr, $PRIVATE, $method(SHA, resetHashes, void)},
		{}
	};
	$ClassInfo classInfo$$ = {
	...
	$loadClass(SHA, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $of($alloc(SHA));
	});
	return class$;
}
