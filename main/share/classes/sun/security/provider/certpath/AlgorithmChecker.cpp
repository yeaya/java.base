...
void AlgorithmChecker::init$($TrustAnchor* anchor, $AlgorithmConstraints* constraints, $Date* date, $String* variant) {
	...
	$set(this, constraints, constraints == nullptr ? $cast($AlgorithmConstraints, AlgorithmChecker::certPathDefaultConstraints) : constraints);
	$set(this, date, date);
	$init($Validator);
...
void AlgorithmChecker::check($Certificate* cert, $Collection* unresolvedCritExts) {
	...
	$var($PublicKey, currPubKey, cert->getPublicKey());
	...
	if ($instanceOf($DisabledAlgorithmConstraints, this->constraints)) {
		$cast($DisabledAlgorithmConstraints, this->constraints)->permits(currSigAlg, currSigAlgParams, cp);
	} else {
		...
		if (!this->constraints->permits(primitives, currPubKey)) {
			...
			var$0->append("Algorithm constraints check failed on key "_s);
			var$0->append($($nc(currPubKey)->getAlgorithm()));
			var$0->append(" with size of "_s);
			var$0->append($KeyUtil::getKeySize(currPubKey));
			var$0->append("bits"_s);
			$init($CertPathValidatorException$BasicReason);
	...
	if (!this->constraints->permits(AlgorithmChecker::SIGNATURE_PRIMITIVE_SET, currSigAlg, this->prevPubKey, currSigAlgParams)) {
		$init($CertPathValidatorException$BasicReason);
		$throwNew($CertPathValidatorException, $$str({"Algorithm constraints check failed on signature algorithm: "_s, currSigAlg}), nullptr, nullptr, -1, $CertPathValidatorException$BasicReason::ALGORITHM_CONSTRAINED);
	}
	if ($PKIX::isDSAPublicKeyWithoutParams(currPubKey)) {
		...
		try {
			...
			$var($DSAPublicKeySpec, ks, $new($DSAPublicKeySpec, y, var$1, var$2, $(params->getG())));
...
void AlgorithmChecker::check($PublicKey* key, $AlgorithmId* algorithmId, $String* variant, $TrustAnchor* anchor) {
	...
	$nc(AlgorithmChecker::certPathDefaultConstraints)->permits(var$0, var$1, $$new($CertPathConstraintsParameters, key, variant, anchor));
}

void AlgorithmChecker::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* AlgorithmChecker::load$($String* name, bool initialize) {
	...
	$loadClass(AlgorithmChecker, name, initialize, &classInfo$$, AlgorithmChecker::clinit$, []($Class* clazz) -> $Object* {
