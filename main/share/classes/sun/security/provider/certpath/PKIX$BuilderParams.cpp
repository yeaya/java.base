...
$List* PKIX$BuilderParams::certStores() {
	...
	if (this->stores == nullptr) {
		$set(this, stores, $new($ArrayList, $($nc(this->params$)->getCertStores())));
		...
		for (; $nc(i$)->hasNext();) {
			...
			try {
				...
				if ($PKIX::debug != nullptr) {
					$PKIX::debug->println($$str({"BuilderParams.getTargetSubjectDN: non-fatal exception retrieving certs: "_s, e}));
					e->printStackTrace();
				}
			}
		}
...
$Class* PKIX$BuilderParams::load$($String* name, bool initialize) {
	...
	$loadClass(PKIX$BuilderParams, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PKIX$BuilderParams);
	});
	return class$;
}
