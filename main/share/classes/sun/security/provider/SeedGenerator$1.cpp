...
$Object* SeedGenerator$1::run() {
	...
	try {
		$var($Properties, p, $System::getProperties());
		{
			...
			for (; $nc(i$)->hasNext();) {
				$var($String, s, $cast($String, i$->next()));
				{
					...
					this->val$md->update($($$nc(p->getProperty(s))->getBytes()));
				}
			}
		}
		...
		$nc(this->val$md)->update((int8_t)ex->hashCode());
	}
	...
	$assign(memBytes, $SeedGenerator::longToByteArray(rt->freeMemory()));
	this->val$md->update(memBytes, 0, $nc(memBytes)->length);
	return nullptr;
}
...
$Class* SeedGenerator$1::load$($String* name, bool initialize) {
	...
	$loadClass(SeedGenerator$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SeedGenerator$1);
	});
	return class$;
}
