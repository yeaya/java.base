...
using $1DSAPublicKey = ::sun::security::provider::DSAPublicKey;
...
$PublicKey* DSAKeyFactory::engineGeneratePublic($KeySpec* keySpec) {
	...
	try {
		if ($instanceOf($DSAPublicKeySpec, keySpec)) {
			...
			$var($BigInteger, var$0, dsaPubKeySpec->getY());
			$var($BigInteger, var$1, dsaPubKeySpec->getP());
			$var($BigInteger, var$2, dsaPubKeySpec->getQ());
			return $cast($X509Key, $new($DSAPublicKeyImpl, var$0, var$1, var$2, $(dsaPubKeySpec->getG())));
		} else if ($instanceOf($X509EncodedKeySpec, keySpec)) {
			return $cast($X509Key, $new($DSAPublicKeyImpl, $($cast($X509EncodedKeySpec, keySpec)->getEncoded())));
		} else {
...
$PrivateKey* DSAKeyFactory::engineGeneratePrivate($KeySpec* keySpec) {
	...
	try {
		if ($instanceOf($DSAPrivateKeySpec, keySpec)) {
			...
			$var($BigInteger, var$0, dsaPrivKeySpec->getX());
			$var($BigInteger, var$1, dsaPrivKeySpec->getP());
			$var($BigInteger, var$2, dsaPrivKeySpec->getQ());
			return $cast($PKCS8Key, $new($DSAPrivateKey, var$0, var$1, var$2, $(dsaPrivKeySpec->getG())));
			...
			bool return$4 = false;
			try {
				$assign(var$5, $cast($PKCS8Key, $new($DSAPrivateKey, encoded)));
				return$4 = true;
				goto $finally;
	...
	try {
		if ($instanceOf($DSAPublicKey, key)) {
			...
			if ($nc(keySpec)->isAssignableFrom(dsaPubKeySpec)) {
				$var($DSAPublicKey, dsaPubKey, $cast($DSAPublicKey, key));
				$assign(params, dsaPubKey->getParams());
			...
			if ($nc(keySpec)->isAssignableFrom(dsaPrivKeySpec)) {
				...
				$assign(params, dsaPrivKey->getParams());
...
$Class* DSAKeyFactory::load$($String* name, bool initialize) {
	...
	$loadClass(DSAKeyFactory, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(DSAKeyFactory);
	});
	return class$;
}
