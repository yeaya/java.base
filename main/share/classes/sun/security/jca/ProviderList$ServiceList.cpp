...
$Provider$Service* ProviderList$ServiceList::tryGet(int32_t index) {
	...
	if ($ProviderList::preferredPropList != nullptr && this->preferredList == nullptr) {
		$set(this, preferredList, $ProviderList::preferredPropList->getAll(this));
	}
	while (true) {
		if ((index == 0) && (this->firstService != nullptr)) {
			return this->firstService;
		} else if ((this->services != nullptr) && (this->services->size() > index)) {
			return $cast($Provider$Service, this->services->get(index));
		}
		if (this->providerIndex >= $nc(this->this$0->configs)->length) {
			return nullptr;
		}
		if (this->preferredList != nullptr && this->preferredIndex < this->preferredList->size()) {
			$var($ProviderList$PreferredEntry, entry, $cast($ProviderList$PreferredEntry, this->preferredList->get(this->preferredIndex++)));
			$assign(p, this->this$0->getProvider($nc(entry)->provider));
			if (p == nullptr) {
				if ($ProviderList::debug != nullptr) {
					$ProviderList::debug->println($$str({"No provider found with name: "_s, entry->provider}));
				}
				continue;
			}
		} else {
		...
		if (this->type != nullptr) {
			...
			for (; $nc(i$)->hasNext();) {
				...
				{
					$var($Provider$Service, s, $nc(p)->getService($nc(id)->type, $nc(id)->algorithm));
					if (s != nullptr) {
						addService(s);
					}
				}
...
$Class* ProviderList$ServiceList::load$($String* name, bool initialize) {
	...
	$loadClass(ProviderList$ServiceList, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
