...
void URIName::init$($String* name) {
	...
	$set(this, host, this->uri->getHost());
	if (this->host != nullptr) {
		if (this->host->charAt(0) == u'[') {
			$var($String, ipV6Host, this->host->substring(1, this->host->length() - 1));
			try {
...
int32_t URIName::constrains($GeneralNameInterface* inputName) {
	...
	if (inputName == nullptr) {
		...
		$var($String, otherHost, $cast(URIName, inputName)->getHost());
...
$Class* URIName::load$($String* name, bool initialize) {
	...
	$loadClass(URIName, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(URIName);
	});
	return class$;
}
