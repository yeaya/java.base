...
void CertificateSerialNumber::clinit$($Class* clazz) {
...
$Class* CertificateSerialNumber::load$($String* name, bool initialize) {
	...
	$loadClass(CertificateSerialNumber, name, initialize, &classInfo$$, CertificateSerialNumber::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(CertificateSerialNumber);
	});
	return class$;
}
