...
$ObjectArray2* launcher_fr::getContents() {
	...
	return $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			...
			u"Erreur : impossible de trouver ou de charger la classe principale {0}\nCausé par : {1}: {2}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error2"_s,
			u"Erreur : la méthode principale n\'\'est pas {0} dans la classe {1}, définissez la méthode principale comme suit :\n   public static void main(String[] args)"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error3"_s,
			u"Erreur : la méthode principale doit renvoyer une valeur de type void dans la classe {0}, \ndéfinissez la méthode principale comme suit :\n   public static void main(String[] args)"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error4"_s,
			u"Erreur : la méthode principale est introuvable dans la classe {0}, définissez la méthode principale comme suit :\n   public static void main(String[] args)\nou une classe d\'\'applications JavaFX doit étendre {1}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error5"_s,
			u"Erreur : des composants d\'exécution JavaFX obligatoires pour exécuter cette application sont manquants."_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error6"_s,
			"Erreur : LinkageError lors du chargement de la classe principale {0}\n\t{1}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error7"_s,
			u"Erreur : impossible d\'\'initialiser la classe principale {0}\nCausé par : {1}: {2}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.init.error"_s,
			"erreur d\'initialisation"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.jar.error1"_s,
			"Erreur : une erreur inattendue est survenue lors de la tentative d\'\'ouverture du fichier {0}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.jar.error2"_s,
			"fichier manifeste introuvable dans {0}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.jar.error3"_s,
			"aucun attribut manifest principal dans {0}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.jar.error4"_s,
			"erreur lors du chargement de l\'\'agent Java dans {0}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.javafx.error1"_s,
			u"Erreur : la signature de la méthode launchApplication JavaFX est incorrecte, la\nméthode doit être déclarée statique et renvoyer une valeur de type void"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error1"_s,
			"le module {0} n\'\'a pas d\'\'attribut MainClass, utilisez -m <module>/<main-class>"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error2"_s,
			"Erreur : impossible de trouver ou charger la classe principale {0} dans le module {1}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error3"_s,
			"Erreur : impossible de charger la classe principale {0} dans le module {1}\n\t{2}"_s
		}),
		$$new($ObjectArray, {
...
$Class* launcher_fr::load$($String* name, bool initialize) {
	...
	$loadClass(launcher_fr, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(launcher_fr);
	});
	return class$;
}
