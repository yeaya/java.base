...
$ObjectArray2* launcher_sv::getContents() {
	...
	return $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			...
			"Fel: kunde inte hitta eller ladda huvudklassen {0}\nOrsakades av: {1}: {2}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error2"_s,
			u"Fel: Huvudmetoden är inte {0} i klassen {1}, definiera huvudmetoden som:\n   public static void main(String[] args)"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error3"_s,
			u"Fel: Huvudmetoden måste returnera ett värde av typen void i klassen {0}, \ndefiniera huvudmetoden som:\n   public static void main(String[] args)"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error4"_s,
			u"Fel: Huvudmetoden finns inte i klassen {0}, definiera huvudmetoden som:\n   public static void main(String[] args)\neller så måste en JavaFX-applikationsklass utöka {1}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error5"_s,
			u"Fel: JavaFX-exekveringskomponenter saknas, och de krävs för att kunna köra den här applikationen"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error6"_s,
			u"Fel: LinkageError inträffade vid laddning av huvudklassen {0}\n\t{1}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.cls.error7"_s,
			"Fel: Kan inte initiera huvudklassen {0}\nOrsakades av: {1}: {2}"_s
		}),
		$$new($ObjectArray, {
		...
		$$new($ObjectArray, {
			...
			u"Fel: Ett oväntat fel inträffade när filen {0} skulle öppnas"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.jar.error2"_s,
			"manifest finns inte i {0}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.jar.error3"_s,
			"inget huvudmanifestattribut i {0}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.jar.error4"_s,
			"fel vid laddning av java-agenten i {0}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.javafx.error1"_s,
			u"Fel: JavaFX launchApplication-metoden har fel signatur, den \nmåste ha deklarerats som statisk och returnera ett värde av typen void"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error1"_s,
			u"modulen {0} har inget MainClass-attribut, använd -m <module>/<main-class>"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error2"_s,
			"Fel: kunde inte hitta eller ladda huvudklassen {0} i modulen {1}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error3"_s,
			"Fel: Kan inte ladda huvudklassen {0} i modulen {1}\n\t{2}"_s
		}),
		$$new($ObjectArray, {
...
$Class* launcher_sv::load$($String* name, bool initialize) {
	...
	$loadClass(launcher_sv, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(launcher_sv);
	});
	return class$;
}
