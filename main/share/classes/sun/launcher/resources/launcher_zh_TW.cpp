...
$ObjectArray2* launcher_zh_TW::getContents() {
	...
	return $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			...
			u"載入 {0} 中的 Java 代理程式時發生錯誤"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.javafx.error1"_s,
			u"錯誤: JavaFX launchApplication 方法的簽章錯誤，它\n必須宣告為靜態並傳回 void 類型的值"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error1"_s,
			u"模組 {0} 不含 MainClass 屬性，請使用 -m <module>/<main-class>"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error2"_s,
			u"錯誤: 找不到或無法載入模組 {1} 中的主要類別 {0}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error3"_s,
			u"錯誤: 無法載入模組 {1} 中的主要類別 {0}\n\t{2}"_s
		}),
		$$new($ObjectArray, {
...
$Class* launcher_zh_TW::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(launcher_zh_TW, init$, void)},
		{"getContents", "()[[Ljava/lang/Object;", nullptr, $PROTECTED | $FINAL, $virtualMethod(launcher_zh_TW, getContents, $ObjectArray2*)},
		{}
	};
	$ClassInfo classInfo$$ = {
	...
	$loadClass(launcher_zh_TW, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(launcher_zh_TW);
	});
	return class$;
}
