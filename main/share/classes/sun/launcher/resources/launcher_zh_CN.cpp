...
$ObjectArray2* launcher_zh_CN::getContents() {
	...
	return $new($ObjectArray2, {
		...
		$$new($ObjectArray, {
			...
			u"在 {0} 中加载 Java 代理时出错"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.javafx.error1"_s,
			u"错误: JavaFX launchApplication 方法具有错误的签名, 必须\n将方法声明为静态方法并返回空类型的值"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error1"_s,
			u"模块 {0} 不具有 ModuleMainClass 属性，请使用 -m <模块>/<主类>"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error2"_s,
			u"错误: 在模块 {1} 中找不到或无法加载主类 {0}"_s
		}),
		$$new($ObjectArray, {
			"java.launcher.module.error3"_s,
			u"错误: 无法将主类 {0} 加载到模块 {1} 中\n\t{2}"_s
		}),
		$$new($ObjectArray, {
