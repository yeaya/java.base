...
void Subject$ClassSet::populateSet() {
	...
	while ($nc(iterator)->hasNext()) {
		...
		if (this->which == $Subject::PRIV_CREDENTIAL_SET) {
			$assign(next, $AccessController::doPrivileged($$new($Subject$ClassSet$1, this, iterator)));
		} else {
			$assign(next, iterator->next());
		}
		if ($nc(this->c)->isAssignableFrom($nc(next)->getClass())) {
			if (this->which != $Subject::PRIV_CREDENTIAL_SET) {
				$nc(this->set)->add(next);
			} else {
				$var($SecurityManager, sm, $System::getSecurityManager());
				if (sm != nullptr) {
					$var($String, var$0, next->getClass()->getName());
...
bool Subject$ClassSet::add(Object$* o) {
	...
	if (!$nc(this->c)->isAssignableFrom($nc($of(o))->getClass())) {
		...
		$var($ObjectArray, source, $new($ObjectArray, {$(this->c->toString())}));
...
$Class* Subject$ClassSet::load$($String* name, bool initialize) {
	...
	$loadClass(Subject$ClassSet, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
