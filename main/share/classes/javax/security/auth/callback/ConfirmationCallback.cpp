...
void ConfirmationCallback::init$(int32_t messageType, $StringArray* options, int32_t defaultOption) {
	if (messageType < ConfirmationCallback::INFORMATION || messageType > ConfirmationCallback::ERROR || options == nullptr || options->length == 0 || defaultOption < 0 || defaultOption >= options->length) {
		$throwNew($IllegalArgumentException);
	}
	...
	$set(this, options, $cast($StringArray, options->clone()));
	this->defaultOption = defaultOption;
}

void ConfirmationCallback::init$($String* prompt, int32_t messageType, int32_t optionType, int32_t defaultOption) {
	if (prompt == nullptr || prompt->isEmpty() || messageType < ConfirmationCallback::INFORMATION || messageType > ConfirmationCallback::ERROR || optionType < ConfirmationCallback::YES_NO_OPTION || optionType > ConfirmationCallback::OK_CANCEL_OPTION) {
		$throwNew($IllegalArgumentException);
	}
	switch (optionType) {
	case ConfirmationCallback::YES_NO_OPTION:
...
void ConfirmationCallback::init$($String* prompt, int32_t messageType, $StringArray* options, int32_t defaultOption) {
	if (prompt == nullptr || prompt->isEmpty() || messageType < ConfirmationCallback::INFORMATION || messageType > ConfirmationCallback::ERROR || options == nullptr || options->length == 0 || defaultOption < 0 || defaultOption >= options->length) {
		$throwNew($IllegalArgumentException);
	}
	for (int32_t i = 0; i < $nc(options)->length; ++i) {
		if (options->get(i) == nullptr || $nc(options->get(i))->isEmpty()) {
...
$StringArray* ConfirmationCallback::getOptions() {
	return this->options == nullptr ? ($StringArray*)nullptr : $cast($StringArray, this->options->clone());
}
...
$Class* ConfirmationCallback::load$($String* name, bool initialize) {
	...
	$loadClass(ConfirmationCallback, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
