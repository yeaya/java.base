...
$Class* LoginContext$$Lambda$lambda$invoke$0::load$($String* name, bool initialize) {
	...
	$loadClass(LoginContext$$Lambda$lambda$invoke$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LoginContext$$Lambda$lambda$invoke$0);
	});
	return class$;
}
...
void LoginContext::init($String* name) {
	...
	if (this->config == nullptr) {
		$set(this, config, $cast($Configuration, $AccessController::doPrivileged($$new($LoginContext$1, this))));
	}
	...
	if (entries == nullptr) {
		...
		$assign(entries, this->config->getAppConfigurationEntry(LoginContext::OTHER));
		if (entries == nullptr) {
	...
	for (int32_t i = 0; i < entries->length; ++i) {
		...
		this->moduleStack->set(i, $$new($LoginContext$ModuleInfo, $$new($AppConfigurationEntry, var$0, var$1, $($nc(entries->get(i))->getOptions())), nullptr));
	}
	$set(this, contextClassLoader, $cast($ClassLoader, $AccessController::doPrivileged($$new($LoginContext$2, this))));
}
...
void LoginContext::loadDefaultCallbackHandler() {
	...
	try {
		...
		$set(this, callbackHandler, $cast($CallbackHandler, $AccessController::doPrivileged($$new($LoginContext$3, this, finalLoader))));
...
void LoginContext::invokePriv($String* methodName) {
	...
	$beforeCallerSensitive();
	try {
		$AccessController::doPrivileged($$new($LoginContext$4, this, methodName), this->creatorAcc);
	} catch ($PrivilegedActionException& pae) {
...
void LoginContext::invoke($String* methodName) {
	...
	for (int32_t i = this->moduleIndex; i < $nc(this->moduleStack)->length; ++i, ++this->moduleIndex) {
		try {
			if ($nc(this->moduleStack->get(i))->module == nullptr) {
				$var($String, name, $nc($nc(this->moduleStack->get(i))->entry)->getLoginModuleName());
				$var($ServiceLoader, sc, $cast($ServiceLoader, $AccessController::doPrivileged($cast($PrivilegedAction, $$new(LoginContext$$Lambda$lambda$invoke$0, this)))));
				{
					$var($Iterator, i$, $nc(sc)->iterator());
					for (; $nc(i$)->hasNext();) {
						$var($LoginModule, m, $cast($LoginModule, i$->next()));
						if ($$nc($nc(m)->getClass()->getName())->equals(name)) {
							$set($nc(this->moduleStack->get(i)), module, m);
							if (LoginContext::debug != nullptr) {
								LoginContext::debug->println($$str({name, " loaded as a service"_s}));
							}
							break;
						}
					}
				...
				if ($nc(this->moduleStack->get(i))->module == nullptr) {
					try {
						...
						$set($nc(this->moduleStack->get(i)), module, $cast($LoginModule, tmp));
						if (LoginContext::debug != nullptr) {
							LoginContext::debug->println($$str({name, " loaded via reflection"_s}));
						}
					} catch ($ClassNotFoundException& e) {
						$throwNew($LoginException, $$str({"No LoginModule found for "_s, name}));
					}
				}
				$nc($nc(this->moduleStack->get(i))->module)->initialize(this->subject, this->callbackHandler, this->state, $($nc(this->moduleStack->get(i))->entry->getOptions()));
			}
			bool status = false;
			{
			...
			if (status == true) {
				...
				if (var$0 && $nc($nc($nc(this->moduleStack)->get(i))->entry)->getControlFlag() == $AppConfigurationEntry$LoginModuleControlFlag::SUFFICIENT && this->firstRequiredError == nullptr) {
					clearState();
					if (LoginContext::debug != nullptr) {
						LoginContext::debug->println($$str({methodName, " SUFFICIENT success"_s}));
					}
					return;
				}
				if (LoginContext::debug != nullptr) {
					LoginContext::debug->println($$str({methodName, " success"_s}));
				}
				this->success = true;
			} else if (LoginContext::debug != nullptr) {
				LoginContext::debug->println($$str({methodName, " ignored"_s}));
			}
		} catch ($Exception& ite) {
			...
			if ($instanceOf($PendingException, ite) && $nc(methodName)->equals(LoginContext::LOGIN_METHOD)) {
				...
				if (LoginContext::debug != nullptr) {
					LoginContext::debug->println("original security exception with detail msg replaced by new exception with empty detail msg"_s);
					LoginContext::debug->println($$str({"original security exception: "_s, $(ite->toString())}));
				}
			} else {
			...
			if ($nc($nc($nc(this->moduleStack)->get(i))->entry)->getControlFlag() == $AppConfigurationEntry$LoginModuleControlFlag::REQUISITE) {
				if (LoginContext::debug != nullptr) {
					LoginContext::debug->println($$str({methodName, " REQUISITE failure"_s}));
				}
				bool var$2 = $nc(methodName)->equals(LoginContext::ABORT_METHOD);
				if (var$2 || methodName->equals(LoginContext::LOGOUT_METHOD)) {
			...
			} else if ($nc(this->moduleStack->get(i))->entry->getControlFlag() == $AppConfigurationEntry$LoginModuleControlFlag::REQUIRED) {
				if (LoginContext::debug != nullptr) {
					LoginContext::debug->println($$str({methodName, " REQUIRED failure"_s}));
				}
				if (this->firstRequiredError == nullptr) {
					$set(this, firstRequiredError, le);
				}
				...
				if (LoginContext::debug != nullptr) {
					LoginContext::debug->println($$str({methodName, " OPTIONAL failure"_s}));
				}
...
void LoginContext::clinit$($Class* clazz) {
...
$Class* LoginContext::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("javax.security.auth.login.LoginContext$$Lambda$lambda$invoke$0")) {
	...
	$loadClass(LoginContext, name, initialize, &classInfo$$, LoginContext::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(LoginContext);
	});
	return class$;
}
