...
$StringArray* SSLParameters::clone($StringArray* s) {
	return (s == nullptr) ? ($StringArray*)nullptr : $cast($StringArray, s->clone());
}
		...
		if (!serverNames->isEmpty()) {
			$set(this, sniNames, $new($LinkedHashMap, serverNames->size()));
			{
				...
				for (; $nc(i$)->hasNext();) {
					...
					if (this->sniNames->put($($Integer::valueOf($nc(serverName)->getType())), serverName) != nullptr) {
						$throwNew($IllegalArgumentException, $$str({"Duplicated server name of type "_s, $$str(serverName->getType())}));
					}
				}
			}
		} else {
		...
		if (!matchers->isEmpty()) {
			$set(this, sniMatchers, $new($HashMap, matchers->size()));
			{
				...
				for (; $nc(i$)->hasNext();) {
					...
					if (this->sniMatchers->put($($Integer::valueOf($nc(matcher)->getType())), matcher) != nullptr) {
						$throwNew($IllegalArgumentException, $$str({"Duplicated server name of type "_s, $$str(matcher->getType())}));
					}
				}
			}
		} else {
...
$Class* SSLParameters::load$($String* name, bool initialize) {
	...
	$loadClass(SSLParameters, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SSLParameters);
	});
	return class$;
}
