...
$String* TrustManagerFactory::getDefaultAlgorithm() {
	...
	$assign(type, $cast($String, $AccessController::doPrivileged($$new($TrustManagerFactory$1))));
	if (type == nullptr) {
...
TrustManagerFactory* TrustManagerFactory::getInstance($String* algorithm) {
	...
	return $new(TrustManagerFactory, $cast($TrustManagerFactorySpi, $nc(instance)->impl), $nc(instance)->provider, algorithm);
}
...
$Class* TrustManagerFactory::load$($String* name, bool initialize) {
	...
	$loadClass(TrustManagerFactory, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TrustManagerFactory);
	});
	return class$;
}
