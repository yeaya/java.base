...
$String* SSLEngineResult::toString() {
	...
	var$0->append(this->status);
	var$0->append(" HandshakeStatus = "_s);
	var$0->append(this->handshakeStatus);
	var$0->append("\nbytesConsumed = "_s);
	var$0->append(this->bytesConsumed$);
	var$0->append(" bytesProduced = "_s);
	var$0->append(this->bytesProduced$);
	var$0->append(this->sequenceNumber$ == -1 ? ""_s : $$str({" sequenceNumber = "_s, $($Long::toUnsignedString(this->sequenceNumber$))}));
	return ($str(var$0));
}
...
$Class* SSLEngineResult::load$($String* name, bool initialize) {
	...
	$loadClass(SSLEngineResult, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SSLEngineResult);
	});
	return class$;
}
