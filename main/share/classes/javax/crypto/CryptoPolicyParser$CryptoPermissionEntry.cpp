...
bool CryptoPolicyParser$CryptoPermissionEntry::equals(Object$* obj) {
	...
	} else if (!this->cryptoPermission->equals($nc(that)->cryptoPermission)) {
		return false;
	}
	if (this->alg == nullptr) {
	...
	} else if (!this->alg->equalsIgnoreCase($nc(that)->alg)) {
		return false;
	}
	...
	if (this->algParamSpec == nullptr) {
		if (that->algParamSpec != nullptr) {
			return false;
		}
	} else if (!this->algParamSpec->equals(that->algParamSpec)) {
		return false;
	}
	return true;
}
...
$Class* CryptoPolicyParser$CryptoPermissionEntry::load$($String* name, bool initialize) {
	...
	$loadClass(CryptoPolicyParser$CryptoPermissionEntry, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(CryptoPolicyParser$CryptoPermissionEntry);
	});
	return class$;
}
