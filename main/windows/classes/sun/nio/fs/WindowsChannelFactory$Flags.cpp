		...
		for (; $nc(i$)->hasNext();) {
			$var($OpenOption, option, $cast($OpenOption, i$->next()));
			{
				if ($instanceOf($StandardOpenOption, option)) {
					...
					switch ($nc($WindowsChannelFactory$2::$SwitchMap$java$nio$file$StandardOpenOption)->get($cast($StandardOpenOption, option)->ordinal())) {
					case 1:
						flags->read = true;
						break;
					case 2:
...
$Class* WindowsChannelFactory$Flags::load$($String* name, bool initialize) {
	...
	$loadClass(WindowsChannelFactory$Flags, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WindowsChannelFactory$Flags);
	});
	return class$;
}
