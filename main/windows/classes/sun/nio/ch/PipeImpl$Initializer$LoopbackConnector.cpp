	...
	try {
		...
		try {
			...
			if ($instanceOf($UnixDomainSocketAddress, sa)) {
				$var($Path, path, $cast($UnixDomainSocketAddress, sa)->getPath());
...
$Class* PipeImpl$Initializer$LoopbackConnector::load$($String* name, bool initialize) {
	...
	$loadClass(PipeImpl$Initializer$LoopbackConnector, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(PipeImpl$Initializer$LoopbackConnector);
	});
	return class$;
}
