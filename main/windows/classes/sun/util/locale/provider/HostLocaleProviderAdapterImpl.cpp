...
bool HostLocaleProviderAdapterImpl::isSupportedCalendarLocale($Locale* locale) {
	...
	if (calid <= 0 || calid >= HostLocaleProviderAdapterImpl::calIDToLDML->length) {
		return false;
	}
	$var($String, requestedCalType, $nc(locale)->getUnicodeLocaleType("ca"_s));
	$var($String, nativeCalType, $nc(HostLocaleProviderAdapterImpl::calIDToLDML->get(calid))->replaceFirst("_.*"_s, ""_s));
...
bool HostLocaleProviderAdapterImpl::isSupportedNativeDigitLocale($Locale* locale) {
	...
	if ($nc(HostLocaleProviderAdapterImpl::supportedLocaleSet)->contains(base)) {
		if (numtype == nullptr || numtype->equals("latn"_s)) {
			return true;
...
$Locale* HostLocaleProviderAdapterImpl::getCalendarLocale($Locale* locale) {
	...
	if (calid > 0 && calid < HostLocaleProviderAdapterImpl::calIDToLDML->length) {
		...
		$var($StringArray, caltype, $nc(HostLocaleProviderAdapterImpl::calIDToLDML->get(calid))->split("_"_s));
		if (caltype->length > 1) {
...
int32_t HostLocaleProviderAdapterImpl::getCalendarIDFromLDMLType($String* ldmlType) {
	$init(HostLocaleProviderAdapterImpl);
	for (int32_t i = 0; i < HostLocaleProviderAdapterImpl::calIDToLDML->length; ++i) {
		if ($nc(HostLocaleProviderAdapterImpl::calIDToLDML->get(i))->startsWith(ldmlType)) {
			return i;
		}
	}
	return -1;
...
bool HostLocaleProviderAdapterImpl::initialize() {
	$init(HostLocaleProviderAdapterImpl);
	$prepareNativeStatic(initialize, bool);
...
$String* HostLocaleProviderAdapterImpl::getDateTimePattern(int32_t dateStyle, int32_t timeStyle, $String* langTag) {
	$init(HostLocaleProviderAdapterImpl);
	$prepareNativeStatic(getDateTimePattern, $String*, int32_t dateStyle, int32_t timeStyle, $String* langTag);
...
int32_t HostLocaleProviderAdapterImpl::getCalendarID($String* langTag) {
	$init(HostLocaleProviderAdapterImpl);
	$prepareNativeStatic(getCalendarID, int32_t, $String* langTag);
	int32_t $ret = $invokeNativeStatic(langTag);
	$finishNativeStatic();
	return $ret;
}
...
$String* HostLocaleProviderAdapterImpl::getNumberPattern(int32_t numberStyle, $String* langTag) {
	$init(HostLocaleProviderAdapterImpl);
	$prepareNativeStatic(getNumberPattern, $String*, int32_t numberStyle, $String* langTag);
	$var($String, $ret, $invokeNativeStaticObject(numberStyle, langTag));
	$finishNativeStatic();
	return $ret;
}
...
bool HostLocaleProviderAdapterImpl::isNativeDigit($String* langTag) {
	$init(HostLocaleProviderAdapterImpl);
	$prepareNativeStatic(isNativeDigit, bool, $String* langTag);
	bool $ret = $invokeNativeStatic(langTag);
	$finishNativeStatic();
	return $ret;
}
...
int32_t HostLocaleProviderAdapterImpl::getCalendarDataValue($String* langTag, int32_t type) {
	$init(HostLocaleProviderAdapterImpl);
	$prepareNativeStatic(getCalendarDataValue, int32_t, $String* langTag, int32_t type);
...
$StringArray* HostLocaleProviderAdapterImpl::getCalendarDisplayStrings($String* langTag, int32_t calid, int32_t field, int32_t style) {
	$init(HostLocaleProviderAdapterImpl);
	$prepareNativeStatic(getCalendarDisplayStrings, $StringArray*, $String* langTag, int32_t calid, int32_t field, int32_t style);
	$var($StringArray, $ret, $invokeNativeStaticObject(langTag, calid, field, style));
	$finishNativeStatic();
	return $ret;
}
	...
	$assignStatic(HostLocaleProviderAdapterImpl::decimalFormatSymbolsCache, $new($ConcurrentHashMap));
	{
		...
		if (HostLocaleProviderAdapterImpl::initialize()) {
			...
			if (!$nc(formatLocale)->equals(displayLocale)) {
				...
				tmpSet->addAll($(c->getCandidateLocales(""_s, l)));
			}
		} else {
