...
bool ProcessEnvironment$StringValues::equals(Object$* o) {
	return $instanceOf(ProcessEnvironment$StringValues, o) && $nc(this->c)->equals($cast(ProcessEnvironment$StringValues, o)->c);
}
...
$Class* ProcessEnvironment$StringValues::load$($String* name, bool initialize) {
	...
	$loadClass(ProcessEnvironment$StringValues, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ProcessEnvironment$StringValues);
	});
	return class$;
}
