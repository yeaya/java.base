...
void UnixFileAttributeViews$Posix::setAttribute($String* attribute, Object$* value) {
	...
	if (attribute->equals(UnixFileAttributeViews$Posix::OWNER_NAME)) {
		setOwner($cast($UserPrincipal, value));
		return;
	}
	if (attribute->equals(UnixFileAttributeViews$Posix::GROUP_NAME)) {
...
void UnixFileAttributeViews$Posix::addRequestedPosixAttributes($PosixFileAttributes* attrs, $AbstractBasicFileAttributeView$AttributesBuilder* builder) {
	...
	if (builder->match(UnixFileAttributeViews$Posix::OWNER_NAME)) {
		builder->add(UnixFileAttributeViews$Posix::OWNER_NAME, $($nc(attrs)->owner()));
	}
	if (builder->match(UnixFileAttributeViews$Posix::GROUP_NAME)) {
...
$UserPrincipal* UnixFileAttributeViews$Posix::getOwner() {
	return $$sure($UnixFileAttributes, readAttributes())->owner();
}
...
void UnixFileAttributeViews$Posix::clinit$($Class* clazz) {
...
$Class* UnixFileAttributeViews$Posix::load$($String* name, bool initialize) {
	...
	$loadClass(UnixFileAttributeViews$Posix, name, initialize, &classInfo$$, UnixFileAttributeViews$Posix::clinit$, []($Class* clazz) -> $Object* {
