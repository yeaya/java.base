...
$Path* UnixDirectoryStream$UnixDirectoryIterator::readNextEntry() {
	...
	for (;;) {
		...
		if (!isSelfOrParent(nameAsBytes)) {
			...
			try {
				if (this->this$0->filter == nullptr || this->this$0->filter->accept(entry)) {
					return entry;
				}
...
void UnixDirectoryStream$UnixDirectoryIterator::clinit$($Class* clazz) {
...
$Class* UnixDirectoryStream$UnixDirectoryIterator::load$($String* name, bool initialize) {
	...
	$loadClass(UnixDirectoryStream$UnixDirectoryIterator, name, initialize, &classInfo$$, UnixDirectoryStream$UnixDirectoryIterator::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(UnixDirectoryStream$UnixDirectoryIterator);
	});
	return class$;
}
