...
void ParsingTest::main($StringArray* argv) {
	...
	check((int64_t)0x00007048860ddf79, "xx123456789012345yy"_s, 2, 17, 10);
...
$Class* ParsingTest::load$($String* name, bool initialize) {
	...
	$loadClass(ParsingTest, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ParsingTest);
	});
	return class$;
}
