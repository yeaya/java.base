...
void ImplicitStringConcatMany::main($StringArray* args) {
	...
	for (int32_t c = 0; c < 300; ++c) {
		sb->append($($String::format("s%03d"_s, $$new($ObjectArray, {$($Integer::valueOf(c))}))));
	}
...
void ImplicitStringConcatMany::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* ImplicitStringConcatMany::load$($String* name, bool initialize) {
	...
	$loadClass(ImplicitStringConcatMany, name, initialize, &classInfo$$, ImplicitStringConcatMany::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ImplicitStringConcatMany);
	});
	return class$;
}
