...
void NulDevice::main($StringArray* args) {
	...
	}
	{
		...
		$var($OutputStream, os, $Files::newOutputStream(path, $$new($OpenOptionArray, {$StandardOpenOption::WRITE})));
	...
	{
		$var($FileChannel, ch, $FileChannel::open(path, $$new($OpenOptionArray, {$StandardOpenOption::READ})));
	...
	}
	{
		...
		try {
			try {
				...
				int32_t nw = $$sure($Integer, $$nc(ch->write($($ByteBuffer::wrap($$new($bytes, {(int8_t)2}))), 0))->get())->intValue();
				if (nw != 1) {
					$throwNew($RuntimeException, $$str({"Wrote "_s, $$str(nw), " bytes, not one"_s}));
				}
				$var($ByteBuffer, buf, $ByteBuffer::allocate(10));
				int32_t nr = $$sure($Integer, $$nc(ch->read(buf, 0))->get())->intValue();
				if (nr != -1) {
					$throwNew($RuntimeException, $$str({"Read returns "_s, $$str(nr), " not -1"_s}));
				}
			} catch ($Throwable& t$) {
...
$Class* NulDevice::load$($String* name, bool initialize) {
	...
	$loadClass(NulDevice, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(NulDevice);
	});
	return class$;
}
