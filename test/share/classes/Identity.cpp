...
void Identity::fail($String* msg) {
	$init(Identity);
	Identity::failed->set(true);
	Identity::done->countDown();
...
void Identity::main($StringArray* args) {
	$init(Identity);
	$useLocalObjectStack();
	int32_t groupCount = 3 + Identity::rand->nextInt(8);
		...
		try {
			try {
				...
				for (int32_t i = 0; i < groupCount; ++i) {
					...
					if (Identity::rand->nextBoolean()) {
						int32_t nThreads = 1 + Identity::rand->nextInt(10);
						...
						$assign(group, $AsynchronousChannelGroup::withCachedThreadPool(pool, Identity::rand->nextInt(5)));
					}
					groups->set(i, group);
...
void Identity::clinit$($Class* clazz) {
...
$Class* Identity::load$($String* name, bool initialize) {
	...
	$loadClass(Identity, name, initialize, &classInfo$$, Identity::clinit$, []($Class* clazz) -> $Object* {
