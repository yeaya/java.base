...
void LargeFileAvailable::createLargeFile(int64_t filesize, $File* file) {
	...
	$Files::delete$($($nc(file)->toPath()));
	{
		...
		$var($FileChannel, fc, $FileChannel::open($(file->toPath()), $$new($OpenOptionArray, {
		...
		try {
			try {
				...
				$init($TimeUnit);
				$System::out->printf("  Wrote large file in %d ns (%d ms) %n"_s, $$new($ObjectArray, {
					$($Long::valueOf(t1 - t0)),
					$($Long::valueOf($TimeUnit::NANOSECONDS->toMillis(t1 - t0)))
				}));
				if (rc != 1) {
...
$Class* LargeFileAvailable::load$($String* name, bool initialize) {
	...
	$loadClass(LargeFileAvailable, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(LargeFileAvailable);
	});
	return class$;
}
