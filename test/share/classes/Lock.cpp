...
void Lock::testLockProtocol($File* file, $Lock$LockWorkerMirror* worker) {
	...
	$var($AsynchronousFileChannel, ch, $AsynchronousFileChannel::open($(file->toPath()), $$new($OpenOptionArray, {
...
void Lock::testAsyncClose($File* file, $Lock$LockWorkerMirror* worker) {
	...
	for (int32_t i = 0; i < 100; ++i) {
		...
		try {
			$init($TimeUnit);
			$nc(result)->get(Lock::rand->nextInt(100), $TimeUnit::MILLISECONDS);
			$throwNew($RuntimeException, "Timeout expected"_s);
		} catch ($TimeoutException& x) {
		}
		ch->close();
...
$Lock$LockWorkerMirror* Lock::startLockWorker() {
	...
	int32_t port = $$cast($InetSocketAddress, $nc(ssc)->getLocalAddress())->getPort();
	...
	$Lock$IOHandler::handle($(p->getErrorStream()));
...
void Lock::runLockWorker(int32_t port) {
	$init(Lock);
	$useLocalObjectStack();
	$var($SocketChannel, sc, $SocketChannel::open($$new($InetSocketAddress, port)));
...
$Class* Lock::load$($String* name, bool initialize) {
	...
	$loadClass(Lock, name, initialize, &classInfo$$, Lock::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Lock);
	});
	return class$;
}
