...
void TryLock::test1(bool shared, bool trylock) {
	...
	try {
		try {
			if (trylock) {
				$assign(fl, $nc(fc)->tryLock(0, $nc(fc)->size(), shared));
			} else {
				$assign(fl, $nc(fc)->lock(0, $nc(fc)->size(), shared));
			}
			if (!shared) {
...
$Class* TryLock::load$($String* name, bool initialize) {
	...
	$loadClass(TryLock, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TryLock);
	});
	return class$;
}
