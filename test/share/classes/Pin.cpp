...
void Pin::main($StringArray* args) {
	...
	$System::err->println("Allowing SoftReferences to be enqueued."_s);
	$System::gc();
	$Thread::sleep(1000);
	$System::err->println("Invoking get() on SoftReferences."_s);
	for (int32_t i = 0; i < Pin::NUM_BLOCKS; ++i) {
		$assign(block, $cast($bytes, $nc(blocks->get(i))->get()));
	}
	$assign(block, nullptr);
	$System::err->println("Forcing desperate garbage collection..."_s);
	...
	try {
		...
		$System::err->println("Got OutOfMemoryError, as expected."_s);
	}
	int32_t emptyCount = 0;
	int32_t fullCount = 0;
	$System::err->print("Examining contents of array:"_s);
	for (int32_t i = 0; i < Pin::NUM_BLOCKS; ++i) {
		$assign(block, $cast($bytes, $nc(blocks->get(i))->get()));
		if (block == nullptr) {
			++emptyCount;
	...
	$System::err->println($$str({" "_s, $$str(emptyCount), " empty, "_s, $$str(fullCount), " full."_s}));
	if (emptyCount == 0) {
...
$Class* Pin::load$($String* name, bool initialize) {
	...
	$loadClass(Pin, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Pin);
	});
	return class$;
}
