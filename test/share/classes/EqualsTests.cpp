...
void EqualsTests::main($StringArray* argv) {
	...
	$var($BigDecimalArray2, testValues, $new($BigDecimalArray2, {
		...
		$$new($BigDecimalArray, {
			$($BigDecimal::valueOf($Math::round($Math::pow(2, 10)))),
			$$new($BigDecimal, "1024"_s)
		}),
		$$new($BigDecimalArray, {
			$$new($BigDecimal, "1020"_s),
			$($BigDecimal::valueOf($Math::pow(2, 11)))
		}),
		$$new($BigDecimalArray, {
	...
	bool expected = $nc($Boolean::TRUE)->booleanValue();
	{
		...
		for (int32_t len$ = arr$->length, i$ = 0; i$ < len$; ++i$) {
			...
			{
				failures += equalsTest($nc(testValuePair)->get(0), $nc(testValuePair)->get(1), expected);
				expected = !expected;
			}
		}
	}
...
$Class* EqualsTests::load$($String* name, bool initialize) {
	...
	$loadClass(EqualsTests, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(EqualsTests);
	});
	return class$;
}
