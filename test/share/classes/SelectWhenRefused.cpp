	...
	try {
		try {
			...
			if (n > 0) {
				...
				$$sure($DatagramChannel, $$nc($DatagramChannel::open())->bind(refuser))->close();
...
$Class* SelectWhenRefused::load$($String* name, bool initialize) {
	...
	$loadClass(SelectWhenRefused, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SelectWhenRefused);
	});
	return class$;
}
