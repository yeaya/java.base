...
void InheritIOEHandle::main($StringArray* args) {
	...
	$InheritIOEHandle$APP* app = ($nc(args)->length > 0) ? $InheritIOEHandle$APP::valueOf(args->get(0)) : $InheritIOEHandle$APP::A;
...
void InheritIOEHandle::clinit$($Class* clazz) {
...
$Class* InheritIOEHandle::load$($String* name, bool initialize) {
	...
	$loadClass(InheritIOEHandle, name, initialize, &classInfo$$, InheritIOEHandle::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(InheritIOEHandle);
	});
	return class$;
}
