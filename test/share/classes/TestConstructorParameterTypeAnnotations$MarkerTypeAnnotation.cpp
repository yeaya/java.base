...
$Class* TestConstructorParameterTypeAnnotations$MarkerTypeAnnotation::load$($String* name, bool initialize) {
	...
	$loadClass(TestConstructorParameterTypeAnnotations$MarkerTypeAnnotation, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TestConstructorParameterTypeAnnotations$MarkerTypeAnnotation);
	});
	return class$;
}
