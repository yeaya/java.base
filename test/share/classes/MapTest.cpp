			...
			try {
				try {
					...
					if (result != expectedResult) {
						...
						MapTest::err->println($$str({"I got "_s, $$str(result)}));
...
void MapTest::testExceptions() {
	...
	$useLocalObjectStack();
	{
		...
		$var($FileChannel, fc, $FileChannel::open($($nc(MapTest::blah)->toPath()), $$new($OpenOptionArray, {$StandardOpenOption::READ})));
		$var($Throwable, var$0, nullptr);
		try {
			try {
				testExceptions(fc);
				...
				checkException(fc, $FileChannel$MapMode::READ_WRITE, -1, fc->size(), $$new($ClassArray, {
				...
				checkException(fc, $FileChannel$MapMode::PRIVATE, 0, fc->size(), $$new($ClassArray, {$NonWritableChannelException::class$}));
				checkException(fc, $FileChannel$MapMode::PRIVATE, -1, fc->size(), $$new($ClassArray, {
					$NonWritableChannelException::class$,
					$IllegalArgumentException::class$
				}));
	...
	{
		$var($FileChannel, fc, $FileChannel::open($($nc(MapTest::blah)->toPath()), $$new($OpenOptionArray, {$StandardOpenOption::WRITE})));
		...
		try {
			try {
				...
				$load($IllegalArgumentException);
				checkException(fc, $FileChannel$MapMode::READ_ONLY, -1, fc->size(), $$new($ClassArray, {
...
void MapTest::testExceptions($FileChannel* fc) {
	...
	$init($FileChannel$MapMode);
	$load($IllegalArgumentException);
	checkException(fc, $FileChannel$MapMode::READ_ONLY, -1, fc->size(), $$new($ClassArray, {$IllegalArgumentException::class$}));
	checkException(fc, nullptr, -1, fc->size(), $$new($ClassArray, {
		$IllegalArgumentException::class$,
...
void MapTest::clinit$($Class* clazz) {
...
$Class* MapTest::load$($String* name, bool initialize) {
	...
	$loadClass(MapTest, name, initialize, &classInfo$$, MapTest::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(MapTest);
	});
	return class$;
}
