	...
	$var($StringBuilder, b, $new($StringBuilder));
	{
		...
		for (int32_t len$ = arr$->length, i$ = 0; i$ < len$; ++i$) {
			...
			if (c < 128) {
				...
				b->append("\\u"_s)->append($($String::format("%04X"_s, $$new($ObjectArray, {$($Integer::valueOf((int32_t)c))}))));
			}
		}
	}
	return b->toString();
...
void ImplicitStringConcatShapesTestGen::main($StringArray* args) {
	$useLocalObjectStack();
	$var($PrintWriter, pw, $new($PrintWriter, $System::out));
	...
	$var($List, l, $new($ArrayList, $(values->keySet())));
	{
		...
		for (; $nc(i$)->hasNext();) {
			$var($String, l1, $cast($String, i$->next()));
			{
				lines->add($($String::format("test(\"%s\", \"\" + %s);"_s, $$new($ObjectArray, {
					$(escapeToUnicode($$cast($String, values->get(l1)))),
					l1
				}))));
				lines->add($($String::format("test(\"%s\", \"prefix\" + %s);"_s, $$new($ObjectArray, {
					$(escapeToUnicode($$str({"prefix"_s, $$cast($String, values->get(l1))}))),
					l1
				}))));
				lines->add($($String::format("test(\"%s\", %s + \"suffix\");"_s, $$new($ObjectArray, {
					$(escapeToUnicode($$str({$$cast($String, values->get(l1)), "suffix"_s}))),
					l1
				}))));
				lines->add($($String::format("test(\"%s\", \"prefix\" + %s + \"suffix\");"_s, $$new($ObjectArray, {
					$(escapeToUnicode($$str({"prefix"_s, $$cast($String, values->get(l1)), "suffix"_s}))),
					l1
				}))));
			}
		}
			...
			$var($String, l1, $cast($String, i$->next()));
			{
				...
				for (; $nc(i$)->hasNext();) {
					$var($String, l2, $cast($String, i$->next()));
					{
						...
						var$0->append($$cast($String, values->get(l1)));
						var$0->append($$cast($String, values->get(l2)));
						lines->add($($String::format("test(\"%s\", \"\" + %s + %s);"_s, $$new($ObjectArray, {
							$(escapeToUnicode($$str(var$0))),
							l1,
							l2
						}))));
						$var($StringBuilder, var$1, $new($StringBuilder));
						var$1->append($$cast($String, values->get(l1)));
						var$1->append($$cast($String, values->get(l2)));
						var$1->append("suffix"_s);
						lines->add($($String::format("test(\"%s\", \"\" + %s + %s + \"suffix\");"_s, $$new($ObjectArray, {
							$(escapeToUnicode($$str(var$1))),
							l1,
							l2
						}))));
						$var($StringBuilder, var$2, $new($StringBuilder));
						var$2->append("prefix"_s);
						var$2->append($$cast($String, values->get(l1)));
						var$2->append("suffix1"_s);
						var$2->append($$cast($String, values->get(l2)));
						var$2->append("suffix2"_s);
						lines->add($($String::format("test(\"%s\", \"prefix\" + %s + \"suffix1\" + %s + \"suffix2\");"_s, $$new($ObjectArray, {
							$(escapeToUnicode($$str(var$2))),
							l1,
							l2
						}))));
					}
...
$String* ImplicitStringConcatShapesTestGen::typeValue($String* type, int32_t i) {
	{
		...
		case 12:
		case 13:
			return $nc(type)->replace("Minus"_s, ""_s);
		case 14:
		case 15:
		case 16:
		case 17:
...
$String* ImplicitStringConcatShapesTestGen::initValue($String* type, int32_t i) {
	$useLocalObjectStack();
	{
		...
		case 7:
			return $String::valueOf(i * 1000000);
		case 8:
			return $str({$($String::valueOf(i * 1000000000)), "L"_s});
		case 9:
			return $str({$($String::valueOf(-i * 1000000000)), "L"_s});
		case 10:
...
$Class* ImplicitStringConcatShapesTestGen::load$($String* name, bool initialize) {
	...
	$loadClass(ImplicitStringConcatShapesTestGen, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ImplicitStringConcatShapesTestGen);
	});
	return class$;
}
