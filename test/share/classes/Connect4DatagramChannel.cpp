...
$Class* Connect4DatagramChannel$$Lambda$lambda$wait$1::load$($String* name, bool initialize) {
	...
	$loadClass(Connect4DatagramChannel$$Lambda$lambda$wait$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Connect4DatagramChannel$$Lambda$lambda$wait$1);
	});
	return class$;
}
...
$Class* Connect4DatagramChannel$$Lambda$lambda$wait$0$1::load$($String* name, bool initialize) {
	...
	$loadClass(Connect4DatagramChannel$$Lambda$lambda$wait$0$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Connect4DatagramChannel$$Lambda$lambda$wait$0$1);
	});
	return class$;
}
...
void Connect4DatagramChannel::wait($CompletableFutureArray* futures) {
	...
	$$nc($Stream::of(futures))->forEach($$new(Connect4DatagramChannel$$Lambda$lambda$wait$1, future));
	$nc(future)->join();
}
...
$SocketAddress* Connect4DatagramChannel::toConnectAddress($SocketAddress* address) {
	...
	if ($instanceOf($InetSocketAddress, address)) {
		...
		if ($$nc(inet->getAddress())->isAnyLocalAddress()) {
...
void Connect4DatagramChannel::lambda$wait$1($CompletableFuture* future, $CompletableFuture* f) {
	$init(Connect4DatagramChannel);
	$nc(f)->exceptionally($$new(Connect4DatagramChannel$$Lambda$lambda$wait$0$1, future));
}
...
void Connect4DatagramChannel::clinit$($Class* clazz) {
...
$Class* Connect4DatagramChannel::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("Connect4DatagramChannel$$Lambda$lambda$wait$1")) {
			return Connect4DatagramChannel$$Lambda$lambda$wait$1::load$(name, initialize);
		}
		if (name->equals("Connect4DatagramChannel$$Lambda$lambda$wait$0$1")) {
	...
	$loadClass(Connect4DatagramChannel, name, initialize, &classInfo$$, Connect4DatagramChannel::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Connect4DatagramChannel);
	});
	return class$;
}
