...
void MethodHandleConstants::openBuf() {
	...
	$System::setOut($$new($PrintStream, MethodHandleConstants::buf));
}
...
void MethodHandleConstants::clinit$($Class* clazz) {
...
$Class* MethodHandleConstants::load$($String* name, bool initialize) {
	...
	$loadClass(MethodHandleConstants, name, initialize, &classInfo$$, MethodHandleConstants::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(MethodHandleConstants);
	});
	return class$;
}
