...
int32_t MockSimplePeriod::compareTo(MockSimplePeriod* otherPeriod) {
	$useLocalObjectStack();
	if ($nc(this->unit)->equals($($nc(otherPeriod)->getUnit())) == false) {
		$var($StringBuilder, var$0, $new($StringBuilder));
		var$0->append("Units cannot be compared: "_s);
		var$0->append(this->unit);
		var$0->append(" and "_s);
		var$0->append($(otherPeriod->getUnit()));
...
bool MockSimplePeriod::equals(Object$* obj) {
	...
	if ($instanceOf(MockSimplePeriod, obj)) {
		...
		return this->amount == other->amount && $nc(this->unit)->equals(other->unit);
	}
	return false;
}

int32_t MockSimplePeriod::hashCode() {
	return $nc(this->unit)->hashCode() ^ (int32_t)(this->amount ^ ((int64_t)((uint64_t)this->amount >> 32)));
}
...
void MockSimplePeriod::clinit$($Class* clazz) {
	$init($ChronoUnit);
...
$Class* MockSimplePeriod::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		...
		{"compareTo", "(Ltck/java/time/MockSimplePeriod;)I", nullptr, $PUBLIC, $method(MockSimplePeriod, compareTo, int32_t, MockSimplePeriod*)},
		...
		{"of", "(JLjava/time/temporal/TemporalUnit;)Ltck/java/time/MockSimplePeriod;", nullptr, $PUBLIC | $STATIC, $staticMethod(MockSimplePeriod, of, MockSimplePeriod*, int64_t, $TemporalUnit*)},
	...
	$loadClass(MockSimplePeriod, name, initialize, &classInfo$$, MockSimplePeriod::clinit$, []($Class* clazz) -> $Object* {
