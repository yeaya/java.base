...
$Class* AdaptorConcurrentIO$$Lambda$lambda$testConcurrentSendReceive$0::load$($String* name, bool initialize) {
	...
	$loadClass(AdaptorConcurrentIO$$Lambda$lambda$testConcurrentSendReceive$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AdaptorConcurrentIO$$Lambda$lambda$testConcurrentSendReceive$0);
	});
	return class$;
}
...
void AdaptorConcurrentIO::main($StringArray* args) {
	...
	testConcurrentSendReceive(60000);
}
	...
	$useLocalObjectStack();
	{
		...
		try {
			try {
				...
				try {
					$var($Future, result, $nc(pool)->submit($$new(AdaptorConcurrentIO$$Lambda$lambda$testConcurrentSendReceive$0, s)));
					$Thread::sleep(200);
...
$Class* AdaptorConcurrentIO::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("AdaptorConcurrentIO$$Lambda$lambda$testConcurrentSendReceive$0")) {
	...
	$loadClass(AdaptorConcurrentIO, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AdaptorConcurrentIO);
	});
	return class$;
}
