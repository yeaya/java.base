...
void Bug6683975::test(int32_t style) {
	...
	if (!$nc(str_th)->equals(str_th_TH)) {
		...
		$System::err->println($$str({"\tth: "_s, str_th}));
		$System::err->println($$str({"\tth_TH: "_s, str_th_TH}));
	}
	$var($Date, date, $new($Date, 2008 - 1900, $Calendar::SEPTEMBER, 30, 8, 0, 0));
	$assign(str_th, df_th->format(date));
	if (!$nc($nc(Bug6683975::expected_th)->get(style))->equals(str_th)) {
		...
		$System::err->println($$str({"\tExpected: "_s, Bug6683975::expected_th->get(style)}));
		$System::err->println($$str({"\tGot: "_s, str_th}));
	}
	$assign(str_th_TH, df_th_TH->format(date));
	if (!$nc($nc(Bug6683975::expected_th_TH)->get(style))->equals(str_th_TH)) {
		...
		$System::err->println($$str({"\tExpected: "_s, Bug6683975::expected_th_TH->get(style)}));
		$System::err->println($$str({"\tGot: "_s, str_th_TH}));
	}
}
...
void Bug6683975::clinit$($Class* clazz) {
...
$Class* Bug6683975::load$($String* name, bool initialize) {
	...
	$loadClass(Bug6683975, name, initialize, &classInfo$$, Bug6683975::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Bug6683975);
	});
	return class$;
}
