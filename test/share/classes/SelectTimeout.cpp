...
$Class* SelectTimeout$$Lambda$lambda$test$0::load$($String* name, bool initialize) {
	...
	$loadClass(SelectTimeout$$Lambda$lambda$test$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SelectTimeout$$Lambda$lambda$test$0);
	});
	return class$;
}
...
bool SelectTimeout::test(int64_t timeout) {
	...
	$var($Thread, t, $new($Thread, $$new(SelectTimeout$$Lambda$lambda$test$0, selector, timeout, isTimedOut, theException)));
	t->start();
	t->join(SelectTimeout::SLEEP_MILLIS);
	bool result = false;
	if (theException->get() == nullptr) {
		if (timeout > SelectTimeout::SLEEP_MILLIS && isTimedOut->get()) {
			$nc($System::err)->printf("Test timed out early with timeout %d%n"_s, $$new($ObjectArray, {$($Long::valueOf(timeout))}));
			result = false;
		} else {
			$nc($System::out)->printf("Test succeeded with timeout %d%n"_s, $$new($ObjectArray, {$($Long::valueOf(timeout))}));
			result = true;
		}
	} else {
		$nc($System::err)->printf("Test failed with timeout %d%n"_s, $$new($ObjectArray, {$($Long::valueOf(timeout))}));
		$$sure($Exception, theException->get())->printStackTrace();
		result = false;
	}
	t->interrupt();
	$nc(selector)->close();
...
$Class* SelectTimeout::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("SelectTimeout$$Lambda$lambda$test$0")) {
	...
	$loadClass(SelectTimeout, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(SelectTimeout);
	});
	return class$;
}
