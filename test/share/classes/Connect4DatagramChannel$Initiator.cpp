...
void Connect4DatagramChannel$Initiator::run() {
	...
	try {
		try {
			...
			$Connect4DatagramChannel::log->println($$str({"Initiator attempting to write to Responder at "_s, $($nc(this->connectSocketAddress)->toString())}));
			this->dc->write(bb);
			try {
				int32_t port = $$nc(this->dc->socket())->getLocalPort();
				...
				$Connect4DatagramChannel::log->println($$str({"Testing if Initiator throws AlreadyConnectedException"_s, $(otherAddress->toString())}));
				this->dc->send(bb, otherAddress);
...
$Class* Connect4DatagramChannel$Initiator::load$($String* name, bool initialize) {
	...
	$loadClass(Connect4DatagramChannel$Initiator, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
