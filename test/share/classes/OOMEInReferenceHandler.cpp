...
void OOMEInReferenceHandler::main($StringArray* args) {
	...
	$var($ThreadArray, threads, $new($ThreadArray, $nc(tg)->activeCount()));
...
$Class* OOMEInReferenceHandler::load$($String* name, bool initialize) {
	...
	$loadClass(OOMEInReferenceHandler, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(OOMEInReferenceHandler);
	});
	return class$;
}
