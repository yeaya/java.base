...
$Class* AnnotationTypeRuntimeAssumptionTest$AltClassLoader::findClass($String* name) {
	...
	if ($nc(name)->endsWith("_v1"_s)) {
		...
		$var($String, altPath, $(altName->replace(u'.', u'/'))->concat(".class"_s));
		try {
			...
			try {
				try {
					if (is != nullptr) {
						...
						var$2 = defineClass(name, bytes, 0, bytes->length);
						return$1 = true;
						goto $finally;
					} else {
...
$Class* AnnotationTypeRuntimeAssumptionTest$AltClassLoader::load$($String* name, bool initialize) {
	...
	$loadClass(AnnotationTypeRuntimeAssumptionTest$AltClassLoader, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AnnotationTypeRuntimeAssumptionTest$AltClassLoader);
	});
	return class$;
}
