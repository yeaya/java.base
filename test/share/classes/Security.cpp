...
$Class* Security$$Lambda$lambda$testPolicy1$0::load$($String* name, bool initialize) {
	...
	$loadClass(Security$$Lambda$lambda$testPolicy1$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Security$$Lambda$lambda$testPolicy1$0);
	});
	return class$;
}
...
$Class* Security$$Lambda$lambda$testPolicy1$1$1::load$($String* name, bool initialize) {
	...
	$loadClass(Security$$Lambda$lambda$testPolicy1$1$1, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Security$$Lambda$lambda$testPolicy1$1$1);
	});
	return class$;
}
...
void Security::call($Security$Command* r, $Class* expectedException) {
	...
	try {
		...
		threw = true;
		if (!($nc(expectedException)->isAssignableFrom(t->getClass()))) {
			$throwNew($RuntimeException, $$str({"wrong exception type thrown "_s, $(t->toString())}));
		}
	}
	if (expectedException != nullptr && !threw) {
		$throwNew($RuntimeException, $("% was expected"_s->formatted($$new($ObjectArray, {$(expectedException->getName())}))));
	}
}
...
void Security::testPolicy1() {
	...
	try {
		...
		try {
			try {
				{
					$var($SocketChannel, client, $SocketChannel::open($StandardProtocolFamily::UNIX));
					$var($Throwable, var$2, nullptr);
					try {
						try {
							call($$new(Security$$Lambda$lambda$testPolicy1$0, server, saddr), Security::SE);
							call($$new(Security$$Lambda$lambda$testPolicy1$1$1, client, saddr), Security::SE);
						} catch ($Throwable& t$) {
							if (client != nullptr) {
								try {
									client->close();
			...
			try {
				{
					...
					try {
						try {
							call($$new(Security$$Lambda$lambda$testPolicy1$0, server, saddr), nullptr);
							call($$new(Security$$Lambda$lambda$testPolicy1$1$1, client, saddr), nullptr);
							{
				...
				$nc(s1)->bind(saddr);
				{
					...
					try {
						try {
							...
							if ($$nc($$nc($$sure($UnixDomainSocketAddress, s1->getLocalAddress())->getPath())->toString())->length() != 0) {
								$throwNew($RuntimeException, "address should have been empty"_s);
							}
							if ($$nc($$nc($$sure($UnixDomainSocketAddress, s2->getLocalAddress())->getPath())->toString())->length() != 0) {
								$throwNew($RuntimeException, "address should have been empty"_s);
							}
						} catch ($Throwable& t$) {
							if (s2 != nullptr) {
...
$Class* Security::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("Security$$Lambda$lambda$testPolicy1$0")) {
			return Security$$Lambda$lambda$testPolicy1$0::load$(name, initialize);
		}
		if (name->equals("Security$$Lambda$lambda$testPolicy1$1$1")) {
