...
void Encode::init$() {
	...
	$var($String, toEncode, $cstr({0x00010000, ' ', 0x00010401, ' '}));
	...
	$var($InputStreamReader, reader, $new($InputStreamReader, bais, "8859_1"_s));
	$var($String, url, $str({"http://localhost:"_s, $($Integer::toString(this->ss->getLocalPort())), "/missing.nothtml"_s}));
								...
								$var($String, req, in->readLine());
								{
									...
									try {
										try {
											...
											try {
												try {
													$var($PrintStream, out, $new($PrintStream, bos));
...
$Class* Encode::load$($String* name, bool initialize) {
	...
	$loadClass(Encode, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Encode);
	});
	return class$;
}
