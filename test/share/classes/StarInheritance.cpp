...
void StarInheritance::test($MethodArray* ma, $ArrayList* expect) {
	...
	var$0->append("Test "_s);
	var$0->append(StarInheritance::n++);
	$nc($System::out)->println($$str(var$0));
	if ($nc(expect)->size() != $nc(ma)->length) {
		$nc($System::err)->println($$str({"  found methods: "_s, $($Arrays::asList(ma))}));
		$System::err->println($$str({"  expected locations: "_s, expect}));
		$throwNew($RuntimeException, $$str({"found = "_s, $$str(ma->length), "; expected = "_s, $$str(expect->size())}));
	}
	for (int32_t i = 0; i < ma->length; ++i) {
		$var($Method, m, ma->get(i));
		$System::out->println($$str({"  "_s, $($nc(m)->toString())}));
		int32_t n = 0;
		bool var$1 = $$nc(m->getName())->equals("m"_s);
		if (var$1 && (n = expect->indexOf(m->getDeclaringClass())) != -1) {
			expect->remove(n);
		} else {
...
void StarInheritance::main($StringArray* args) {
	...
	test(var$0, $$new($ArrayList, $($Arrays::asList(l1))));
	$load($A2);
	$var($ClassArray, l2, $new($ClassArray, {$A2::class$}));
	$var($MethodArray, var$1, $A2::class$->getMethods());
	test(var$1, $$new($ArrayList, $($Arrays::asList(l2))));
	$load($B3);
	$load($C3);
	...
	test(var$2, $$new($ArrayList, $($Arrays::asList(l3))));
	$load($B4);
	$var($ClassArray, l4, $new($ClassArray, {$B4::class$}));
	$load($A4);
	$var($MethodArray, var$3, $A4::class$->getMethods());
	test(var$3, $$new($ArrayList, $($Arrays::asList(l4))));
}

void StarInheritance::clinit$($Class* clazz) {
	StarInheritance::n = 1;
}
...
$Class* StarInheritance::load$($String* name, bool initialize) {
	...
	$loadClass(StarInheritance, name, initialize, &classInfo$$, StarInheritance::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(StarInheritance);
	});
	return class$;
}
