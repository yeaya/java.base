...
$Class* WithParameters$Foo::load$($String* name, bool initialize) {
	...
	$ParameterAnnotation quxmethodParamAnnotations$$[] = {
	...
	$loadClass(WithParameters$Foo, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WithParameters$Foo);
	});
	return class$;
}
