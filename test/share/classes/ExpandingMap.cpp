...
void ExpandingMap::main($StringArray* args) {
	...
	int32_t maximumFileSize = 300000000;
	...
	while (currentPosition + currentSize < maximumFileSize) {
		...
		while (currentSize + inc > maximumMapSize) {
			if (currentSize < maximumMapSize) {
				$System::out->format("map %d -> %d\n"_s, $$new($ObjectArray, {
					...
					$($Integer::valueOf((currentPosition + maximumMapSize)))
				}));
		...
		if (currentSize > 0) {
			$System::out->format("map %d -> %d\n"_s, $$new($ObjectArray, {
				$($Integer::valueOf(currentPosition)),
				$($Integer::valueOf((currentPosition + currentSize)))
			}));
...
$Class* ExpandingMap::load$($String* name, bool initialize) {
	...
	$loadClass(ExpandingMap, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ExpandingMap);
	});
	return class$;
}
