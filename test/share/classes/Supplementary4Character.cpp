...
void Supplementary4Character::constantError($String* name, int32_t value, int32_t expectedValue) {
	...
	var$0->append("Character."_s);
	var$0->append(name);
	var$0->append(" has a wrong value: got "_s);
	var$0->append($(toHexString(value)));
	var$0->append(", expected "_s);
	var$0->append($(toHexString(expectedValue)));
...
int32_t Supplementary4Character::test01($chars* cu) {
	...
	for (int32_t i = 0; i <= $Character::MAX_VALUE; ++i) {
		...
		if (n != 1 || $nc(cu)->get(index) != i) {
			$throwNew($RuntimeException, $$str({"wrong toChars(int, char[], int) result for BMP: len="_s, $$str(n), ", cu["_s, $$str(index), "]="_s, $(toHexString($nc(cu)->get(index)))}));
		}
	...
	for (int32_t i = $Character::MAX_VALUE / 2; i <= $Character::MAX_VALUE; ++i) {
		...
		for (int32_t j = $Character::MAX_VALUE / 2; j <= $Character::MAX_VALUE; ++j) {
			...
			if ($Character::isSurrogatePair(hi, lo) != isSurrogatePair) {
				$var($StringBuilder, var$0, $new($StringBuilder));
				var$0->append("wrong surrogate pair test for hi="_s);
				var$0->append($(toHexString(hi)));
				var$0->append(", lo="_s);
				var$0->append($(toHexString(lo)));
				$throwNew($RuntimeException, $$str(var$0));
			}
			if (isSurrogatePair) {
				...
				if (cp != supplementary) {
					...
					var$1->append("wrong code point: got "_s);
					var$1->append($(toHexString(cp)));
					var$1->append(", expected="_s);
					var$1->append($(toHexString(supplementary)));
					$throwNew($RuntimeException, $$str(var$1));
				}
				$var($chars, u, $Character::toChars(cp));
				if ($nc(u)->length != 2 || u->get(0) != hi || u->get(1) != lo) {
				...
				if (n != 2 || $nc(cu)->get(index) != hi || $nc(cu)->get(index + 1) != lo) {
					...
					var$2->append("wrong toChars(int, char[], int) result for supplementary: len="_s);
					var$2->append(n);
					var$2->append(", cu["_s);
					var$2->append(index);
					var$2->append("]="_s);
					var$2->append($(toHexString($nc(cu)->get(index))));
					var$2->append(", cu["_s);
					var$2->append(index);
					var$2->append(1);
					var$2->append("]="_s);
					var$2->append($(toHexString(cu->get(index + 1))));
...
void Supplementary4Character::test02($CharSequence* cs) {
	...
	for (int32_t i = 0; i < $nc(cs)->length(); i += $Character::charCount(ch)) {
		...
		if (ch != cp) {
			$var($StringBuilder, var$0, $new($StringBuilder));
			var$0->append("wrong codePointAt(CharSequence, "_s);
			var$0->append(i);
			var$0->append(") value: got "_s);
			var$0->append($(toHexString(ch)));
			var$0->append(", expected "_s);
			var$0->append($(toHexString(cp)));
			$throwNew($RuntimeException, $$str(var$0));
		}
		++cp;
	...
	--cp;
	for (int32_t i = cs->length(); i > 0; i -= $Character::charCount(ch)) {
		ch = $Character::codePointBefore(cs, i);
		if (ch != cp) {
			$var($StringBuilder, var$1, $new($StringBuilder));
			var$1->append("codePointBefore(CharSequence, "_s);
			var$1->append(i);
			var$1->append(") returned "_s);
			var$1->append($(toHexString(ch)));
			var$1->append(", expected "_s);
			var$1->append($(toHexString(cp)));
			$throwNew($RuntimeException, $$str(var$1));
		}
...
void Supplementary4Character::test03($chars* a) {
	...
	for (int32_t i = 0; i < $nc(a)->length; i += $Character::charCount(ch)) {
		...
		if (ch != cp) {
			$var($StringBuilder, var$0, $new($StringBuilder));
			var$0->append("codePointAt(char[], "_s);
			var$0->append(i);
		...
		if (x != a->get(i)) {
			$throwNew($RuntimeException, $($String::format("codePointAt(char[], %d, %d) returned 0x%04x, expected 0x%04x\n"_s, $$new($ObjectArray, {
				...
				$($Integer::valueOf(i + 1)),
				$($Integer::valueOf(x)),
				$($Integer::valueOf((int32_t)a->get(i)))
			}))));
		}
		++cp;
		if (cp == Supplementary4Character::MIN_HIGH) {
	...
	--cp;
	for (int32_t i = a->length; i > 0; i -= $Character::charCount(ch)) {
		ch = $Character::codePointBefore(a, i);
		if (ch != cp) {
			$var($StringBuilder, var$1, $new($StringBuilder));
			var$1->append("codePointBefore(char[], "_s);
			var$1->append(i);
			var$1->append(") returned "_s);
			var$1->append($(toHexString(ch)));
			var$1->append(", expected "_s);
		...
		if (x != a->get(i - 1)) {
			$throwNew($RuntimeException, $($String::format("codePointAt(char[], %d, %d) returned 0x%04x, expected 0x%04x\n"_s, $$new($ObjectArray, {
				...
				$($Integer::valueOf(i - 1)),
				$($Integer::valueOf(x)),
				$($Integer::valueOf((int32_t)a->get(i - 1)))
			}))));
		}
		--cp;
		if (cp == Supplementary4Character::MAX_LOW) {
...
void Supplementary4Character::test04($String* str) {
	...
	for (int32_t i = 0; i <= length; i += 99, length -= 29999) {
		int32_t n = $Character::codePointCount(str, i, length);
	...
	length = str->length();
	int32_t n = $Character::codePointCount(str, 0, 0);
...
void Supplementary4Character::testUnpaired() {
	...
	testCodePoint($cstr({0x00010000, 0xdc00}), $$new($ints, {
		0x00010000,
		0x0000dc00
	}));
	testCodePoint($cstr({0xd800, 0x00010000}), $$new($ints, {
		0x0000d800,
		0x00010000
	}));
	testCodePoint($cstr({0xd800, 0xd800, 0xd800, 0x00010000, 0xdc00, 0xdc00, 0xdc00}), $$new($ints, {
		0x0000d800,
		0x0000d800,
		0x0000d800,
		0x00010000,
...
void Supplementary4Character::testCodePoint($String* str, $ints* codepoints) {
	...
	for (int32_t i = 0; i < $nc(str)->length(); i += $Character::charCount(c)) {
		...
		if (c != $nc(codepoints)->get(j++)) {
			$var($StringBuilder, var$0, $new($StringBuilder));
			var$0->append("codePointAt(CharSequence, "_s);
			var$0->append(i);
			var$0->append(") returned "_s);
			var$0->append($(toHexString(c)));
			var$0->append(", expected "_s);
			var$0->append($(toHexString(codepoints->get(j - 1))));
			$throwNew($RuntimeException, $$str(var$0));
		}
	}
	...
	j = codepoints->length;
	for (int32_t i = str->length(); i > 0; i -= $Character::charCount(c)) {
		...
		if (c != codepoints->get(--j)) {
			...
			var$1->append(i);
			var$1->append(") returned "_s);
			var$1->append($(toHexString(c)));
			var$1->append(", expected "_s);
			var$1->append($(toHexString(codepoints->get(j))));
			$throwNew($RuntimeException, $$str(var$1));
		}
	}
	if (j != 0) {
	...
	for (int32_t i = 0; i < a->length; i += $Character::charCount(c)) {
		...
		if (c != codepoints->get(j++)) {
			$var($StringBuilder, var$2, $new($StringBuilder));
			var$2->append("codePointAt(char[], "_s);
			var$2->append(i);
			var$2->append(") returned "_s);
			var$2->append($(toHexString(c)));
			var$2->append(", expected "_s);
			var$2->append($(toHexString(codepoints->get(j - 1))));
			$throwNew($RuntimeException, $$str(var$2));
		}
	}
	...
	for (int32_t i = a->length; i > 0; i -= $Character::charCount(c)) {
		...
		if (c != codepoints->get(--j)) {
			...
			var$3->append("codePointBefore(char[], "_s);
			var$3->append(i);
			var$3->append(") returned "_s);
			var$3->append($(toHexString(c)));
			var$3->append(", expected "_s);
			var$3->append($(toHexString(codepoints->get(j))));
	...
	for (int32_t i = 0; i < codepoints->length; ++i) {
		...
		for (int32_t k = 0; k < $nc(a)->length; ++k) {
			if (str->charAt(j++) != a->get(k)) {
				...
				var$4->append("toChars(int) returned "_s);
				var$4->append($(toHexString("result"_s, a)));
				var$4->append(" from codepoint="_s);
				var$4->append($(toHexString(codepoints->get(i))));
	...
	if (!str->equals(s)) {
		...
		var$5->append("toChars(int, char[], int) returned "_s);
		var$5->append($(toHexString("dst"_s, $(s->toCharArray()))));
		var$5->append(", expected "_s);
		var$5->append($(toHexString("data"_s, $(str->toCharArray()))));
...
void Supplementary4Character::testExceptions01($CharSequence* cs) {
	...
	callCodePoint(Supplementary4Character::At, cs, cs->length() * 3, $IndexOutOfBoundsException::class$);
	...
	callCodePoint(Supplementary4Character::Before, cs, cs->length() + 1, $IndexOutOfBoundsException::class$);
	...
	int32_t var$0 = cs->length();
	callOffsetByCodePoints(cs, var$0, -cs->length() * 2, $IndexOutOfBoundsException::class$);
...
void Supplementary4Character::testExceptions02($chars* a) {
	...
	callCodePoint(Supplementary4Character::At, a, a->length * 3, $IndexOutOfBoundsException::class$);
	...
	callCodePoint(Supplementary4Character::Before, a, a->length + 1, $IndexOutOfBoundsException::class$);
	...
	callCodePoint(Supplementary4Character::At, a, 0, a->length + 1, $IndexOutOfBoundsException::class$);
	...
	callCodePoint(Supplementary4Character::At, a, a->length, 1, $IndexOutOfBoundsException::class$);
	callCodePoint(Supplementary4Character::At, a, a->length * 3, 1, $IndexOutOfBoundsException::class$);
	...
	callCodePoint(Supplementary4Character::Before, a, 2, a->length, $IndexOutOfBoundsException::class$);
	...
	callCodePoint(Supplementary4Character::Before, a, a->length + 1, a->length - 1, $IndexOutOfBoundsException::class$);
...
$Class* Supplementary4Character::load$($String* name, bool initialize) {
	...
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(Supplementary4Character, init$, void)},
		{"callCodePoint", "(ZLjava/lang/CharSequence;ILjava/lang/Class;)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, callCodePoint, void, bool, $CharSequence*, int32_t, $Class*)},
		{"callCodePoint", "(Z[CILjava/lang/Class;)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, callCodePoint, void, bool, $chars*, int32_t, $Class*)},
		{"callCodePoint", "(Z[CIILjava/lang/Class;)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, callCodePoint, void, bool, $chars*, int32_t, int32_t, $Class*)},
		{"callCodePointCount", "(Ljava/lang/Object;IILjava/lang/Class;)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, callCodePointCount, void, Object$*, int32_t, int32_t, $Class*)},
		{"callOffsetByCodePoints", "(Ljava/lang/CharSequence;IILjava/lang/Class;)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, callOffsetByCodePoints, void, $CharSequence*, int32_t, int32_t, $Class*)},
		{"callOffsetByCodePoints", "([CIIIILjava/lang/Class;)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, callOffsetByCodePoints, void, $chars*, int32_t, int32_t, int32_t, int32_t, $Class*)},
		{"callToChars1", "(ILjava/lang/Class;)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, callToChars1, void, int32_t, $Class*)},
		{"callToChars3", "(I[CILjava/lang/Class;)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, callToChars3, void, int32_t, $chars*, int32_t, $Class*)},
		{"callToString", "(ILjava/lang/Class;)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, callToString, void, int32_t, $Class*)},
		{"checkCodePointCount", "(Ljava/lang/Object;II)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, checkCodePointCount, void, Object$*, int32_t, int32_t)},
		{"checkNewIndex", "(Ljava/lang/Object;III)V", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, checkNewIndex, void, Object$*, int32_t, int32_t, int32_t)},
		{"codePointCount", "(Ljava/lang/CharSequence;)I", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, codePointCount, int32_t, $CharSequence*)},
		{"constantError", "(Ljava/lang/String;II)V", nullptr, $STATIC, $staticMethod(Supplementary4Character, constantError, void, $String*, int32_t, int32_t)},
		{"getType", "(Ljava/lang/Object;)Ljava/lang/String;", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, getType, $String*, Object$*)},
		{"main", "([Ljava/lang/String;)V", nullptr, $PUBLIC | $STATIC, $staticMethod(Supplementary4Character, main, void, $StringArray*)},
		{"test00", "()V", nullptr, $STATIC, $staticMethod(Supplementary4Character, test00, void)},
		{"test01", "([C)I", nullptr, $STATIC, $staticMethod(Supplementary4Character, test01, int32_t, $chars*)},
		{"test02", "(Ljava/lang/CharSequence;)V", nullptr, $STATIC, $staticMethod(Supplementary4Character, test02, void, $CharSequence*)},
		{"test03", "([C)V", nullptr, $STATIC, $staticMethod(Supplementary4Character, test03, void, $chars*)},
		{"test04", "(Ljava/lang/String;)V", nullptr, $STATIC, $staticMethod(Supplementary4Character, test04, void, $String*)},
		{"test05", "(Ljava/lang/String;)V", nullptr, $STATIC, $staticMethod(Supplementary4Character, test05, void, $String*)},
		{"test06", "()V", nullptr, $STATIC, $staticMethod(Supplementary4Character, test06, void)},
		{"testCodePoint", "(Ljava/lang/String;[I)V", nullptr, $STATIC, $staticMethod(Supplementary4Character, testCodePoint, void, $String*, $ints*)},
		{"testConstants", "()V", nullptr, $STATIC, $staticMethod(Supplementary4Character, testConstants, void)},
		{"testExceptions00", "()V", nullptr, $STATIC, $staticMethod(Supplementary4Character, testExceptions00, void)},
		{"testExceptions01", "(Ljava/lang/CharSequence;)V", nullptr, $STATIC, $staticMethod(Supplementary4Character, testExceptions01, void, $CharSequence*)},
		{"testExceptions02", "([C)V", nullptr, $STATIC, $staticMethod(Supplementary4Character, testExceptions02, void, $chars*)},
		{"testUnpaired", "()V", nullptr, $STATIC, $staticMethod(Supplementary4Character, testUnpaired, void)},
		{"toHexString", "(I)Ljava/lang/String;", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, toHexString, $String*, int32_t)},
		{"toHexString", "(Ljava/lang/String;[C)Ljava/lang/String;", nullptr, $PRIVATE | $STATIC, $staticMethod(Supplementary4Character, toHexString, $String*, $String*, $chars*)},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $ACC_SUPER,
	...
	$loadClass(Supplementary4Character, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Supplementary4Character);
	});
	return class$;
}
