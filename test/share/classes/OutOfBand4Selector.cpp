...
void OutOfBand4Selector::test($Socket* s, bool urgentExpected, int32_t bytesBefore, int32_t bytesAfter, $Selector* sel) {
	...
	for (;;) {
		int32_t n = $nc(sel)->select(2000);
		if (n == 0) {
			if (bytesRead == bytesExpected) {
				$nc($System::out)->format("Selector wakeups %d\tSpurious reads %d%n"_s, $$new($ObjectArray, {
					$($Integer::valueOf(spuriousWakeups)),
					$($Integer::valueOf(spuriousReads))
				}));
				return;
			}
		...
		$nc(bb)->clear();
		n = $$sure($SocketChannel, $nc(key)->channel())->read(bb);
		if (n == 0) {
			if (++spuriousReads >= 3) {
...
$Class* OutOfBand4Selector::load$($String* name, bool initialize) {
	...
	$loadClass(OutOfBand4Selector, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(OutOfBand4Selector);
	});
	return class$;
}
