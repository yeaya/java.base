...
bool Foo4ExternalizableBlockData::equals(Object$* other) {
	if ($instanceOf(Foo4ExternalizableBlockData, other)) {
		...
		return ((this->x == f->x) && (this->y == f->y) && ((this->obj != nullptr) ? this->obj->equals(f->obj) : (f->obj == nullptr)));
	}
	return false;
}
...
$Class* Foo4ExternalizableBlockData::load$($String* name, bool initialize) {
	...
	$loadClass(Foo4ExternalizableBlockData, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Foo4ExternalizableBlockData);
	});
	return class$;
}
