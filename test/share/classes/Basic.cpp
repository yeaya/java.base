...
void Basic::main($StringArray* args) {
	...
	int32_t N = 1000;
	{
		...
		for (; $nc(i$)->hasNext();) {
			...
			for (int32_t i = 0; i < N; ++i) {
				int64_t value = Basic::rand->nextLong();
	...
	}
	{
		...
		for (; $nc(i$)->hasNext();) {
			$TimeUnit* unit = $cast($TimeUnit, i$->next());
			{
				...
				$var($FileTime, ft, $FileTime::from(value, unit));
				$var($Instant, instant, $nc(ft)->toInstant());
				if ($nc(unit)->compareTo($TimeUnit::SECONDS) < 0) {
					eqTime(value, unit, instant);
				} else if (!$nc(instant)->equals($Instant::MIN)) {
					$throwNew($RuntimeException, "should overflow to MIN"_s);
				}
				value = $Long::MAX_VALUE;
				$assign(ft, $FileTime::from(value, unit));
				$assign(instant, $nc(ft)->toInstant());
				if (unit->compareTo($TimeUnit::SECONDS) < 0) {
					eqTime(value, unit, instant);
				} else if (!$nc(instant)->equals($Instant::MAX)) {
					$throwNew($RuntimeException, "should overflow to MAX"_s);
				}
			}
		}
	}
	int64_t MAX_SECOND = (int64_t)0x00701cd2fa9578ff;
	for (int32_t i = 0; i < N; ++i) {
		int64_t v = Basic::rand->nextLong();
		int64_t secs = $mod(v, MAX_SECOND);
		$var($Instant, instant, $Instant::ofEpochSecond(secs, Basic::rand->nextInt(1000000000)));
		$var($FileTime, ft, $FileTime::from(instant));
		bool var$0 = !$$nc($nc(ft)->toInstant())->equals(instant);
		if (var$0 || ft->to($TimeUnit::SECONDS) != secs) {
	...
	ts((int64_t)9999999999, $TimeUnit::NANOSECONDS, "1970-01-01T00:00:09.999999999Z"_s);
	...
	ts((int64_t)-9999999999, $TimeUnit::NANOSECONDS, "1969-12-31T23:59:50.000000001Z"_s);
	ts((int64_t)-62135596799999, $TimeUnit::MILLISECONDS, "0001-01-01T00:00:00.001Z"_s);
	ts((int64_t)-62135596800000, $TimeUnit::MILLISECONDS, "0001-01-01T00:00:00Z"_s);
	ts((int64_t)0xffffc77cedd327ff, $TimeUnit::MILLISECONDS, "-0001-12-31T23:59:59.999Z"_s);
	ts((int64_t)253402300799999, $TimeUnit::MILLISECONDS, "9999-12-31T23:59:59.999Z"_s);
	ts((int64_t)0xfffea8895c1143ff, $TimeUnit::MILLISECONDS, "-9999-12-31T23:59:59.999Z"_s);
	ts((int64_t)-11644473600000000, $TimeUnit::MICROSECONDS, "1601-01-01T00:00:00Z"_s);
	...
	try {
		time->compareTo(($FileTime*)nullptr);
		$throwNew($RuntimeException, "NullPointerException expected"_s);
	} catch ($NullPointerException& npe) {
	}
...
void Basic::eqTime(int64_t value, $TimeUnit* unit, $Instant* instant) {
	...
	if (var$0 || (int32_t)nanos != instant->getNano()) {
...
$Class* Basic::load$($String* name, bool initialize) {
	...
	$loadClass(Basic, name, initialize, &classInfo$$, Basic::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(Basic);
	});
	return class$;
}
