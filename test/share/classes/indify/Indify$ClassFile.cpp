...
void Indify$ClassFile::readFrom($DataInputStream* in) {
	...
	this->pool->readFrom(in);
	this->Code_index = this->pool->stringIndex("Code"_s, false);
...
void Indify$ClassFile::writeTo($DataOutputStream* out) {
	...
	$Indify::writeOutputs(out, $$new($ObjectArray, {
		$($Integer::valueOf(this->magic)),
		$($Integer::valueOf(this->version)),
		this->pool,
		$($Short::valueOf(this->access)),
		$($Short::valueOf(this->thisc)),
		$($Short::valueOf(this->superc)),
		this->interfaces,
		this->fields,
		this->methods,
		this->attrs$
...
$String* Indify$ClassFile::nameString() {
	return this->pool->getString(7, this->thisc);
}
...
$Class* Indify$ClassFile::load$($String* name, bool initialize) {
	...
	$loadClass(Indify$ClassFile, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
