...
void Indify$Pool::init$() {
	...
	$set(this, strings, $cast($AbstractMap, $new($TreeMap)));
}
...
$Object* Indify$Pool::get(int32_t index) {
	return $cast($Indify$Constant, $Indify$CountedList::get((char16_t)index));
}

$String* Indify$Pool::getString(int8_t tag, int16_t index) {
	$$sure($Indify$Constant, get(index))->checkTag(tag);
...
$String* Indify$Pool::getString(int16_t index) {
	...
	if ($instanceOf($Short, v)) {
		$assign(v, $nc($($$sure($Indify$Constant, get($cast($Short, v)->shortValue()))->checkTag(1)))->item);
	}
...
int32_t Indify$Pool::stringIndex($String* name, bool createIfNotFound) {
	...
	return $nc($(addConstant(1, name)))->index;
}

$ShortArray* Indify$Pool::getMemberRef(int16_t index) {
	$useLocalObjectStack();
	$var($ShortArray, cls_nnt, $$sure($Indify$Constant, get(index))->itemIndexes());
	$var($ShortArray, name_type, $$sure($Indify$Constant, get($nc($nc(cls_nnt)->get(1))->shortValue()))->itemIndexes());
...
$Class* Indify$Pool::load$($String* name, bool initialize) {
	...
	$loadClass(Indify$Pool, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
