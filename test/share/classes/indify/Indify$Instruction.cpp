...
Indify$Instruction* Indify$Instruction::init(int32_t pc) {
	this->pc = pc;
	this->bc = $nc(this->codeBase)->get(pc) & 0xff;
	$init($Indify);
	this->info = $nc($Indify::INSTRUCTION_INFO)->get(this->bc);
	this->wide = 0;
	this->len = (this->info & 0x0f);
	if (this->len == 0) {
		computeLength();
	}
	return this;
...
int32_t Indify$Instruction::u1At(int32_t pos) {
	return ($nc(this->codeBase)->get(this->pc + pos) & 0xff);
}
...
void Indify$Instruction::u2AtPut(int32_t pos, int32_t x) {
	...
	this->codeBase->set(this->pc + pos + 1, (int8_t)(x >> 0));
}
...
$Class* Indify$Instruction::load$($String* name, bool initialize) {
	...
	$loadClass(Indify$Instruction, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(Indify$Instruction);
	});
	return class$;
}
