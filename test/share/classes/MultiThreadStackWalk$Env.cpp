...
$Class* MultiThreadStackWalk$Env$$Lambda$lambda$consume$0::load$($String* name, bool initialize) {
	...
	$loadClass(MultiThreadStackWalk$Env$$Lambda$lambda$consume$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MultiThreadStackWalk$Env$$Lambda$lambda$consume$0);
	});
	return class$;
}
...
void MultiThreadStackWalk$Env::init$(int64_t total, int64_t markAt, $AtomicBoolean* debug) {
	...
	$set(this, unexpected, $Collections::synchronizedSet($$cast($AbstractSet, $new($TreeSet))));
	this->max = total + 2;
...
void MultiThreadStackWalk$Env::consume($StackWalker$StackFrame* sfi) {
	...
	int64_t count = this->frameCounter->getAndIncrement();
	$var($StringBuilder, builder, $new($StringBuilder));
	builder->append("Declaring class["_s)->append(count)->append("]: "_s)->append($nc(sfi)->getDeclaringClass());
	builder->append(u'\n');
	$var($String, var$1, nullptr);
	if ($$nc(sfi->toStackTraceElement())->isNativeMethod()) {
		$assign(var$1, "(native)"_s);
	} else {
		$var($StringBuilder, var$2, $new($StringBuilder));
		var$2->append("("_s);
		var$2->append($($$nc(sfi->toStackTraceElement())->getFileName()));
		var$2->append(":"_s);
		var$2->append($$nc(sfi->toStackTraceElement())->getLineNumber());
		var$2->append(")"_s);
		$assign(var$1, $str(var$2));
	}
	builder->append("\t"_s)->append($(sfi->getClassName()))->append("."_s)->append($($$nc(sfi->toStackTraceElement())->getMethodName()))->append(var$1);
	builder->append(u'\n');
	...
	if (count == 100) {
		...
		$$nc($StackWalker::getInstance($StackWalker$Option::RETAIN_CLASS_REFERENCE))->forEach($$new(MultiThreadStackWalk$Env$$Lambda$lambda$consume$0, this));
	}
}
...
void MultiThreadStackWalk$Env::lambda$consume$0($StackWalker$StackFrame* x) {
	...
	if (st == nullptr) {
		...
		var$1->append("."_s);
		var$1->append($($nc(st)->getMethodName()));
		...
		if (st->isNativeMethod()) {
			...
			var$3->append("("_s);
			var$3->append($(st->getFileName()));
			var$3->append(":"_s);
			var$3->append(st->getLineNumber());
			var$3->append(")"_s);
			$assign(var$2, $str(var$3));
		}
		var$1->append(var$2);
...
$Class* MultiThreadStackWalk$Env::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("MultiThreadStackWalk$Env$$Lambda$lambda$consume$0")) {
	...
	$loadClass(MultiThreadStackWalk$Env, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MultiThreadStackWalk$Env);
	});
	return class$;
}
