...
void MemberFactory$Group::init$($String* $enum$name, int32_t $enum$ordinal, $MemberFactoryArray* members) {
	...
	$set(this, members, $EnumSet::copyOf($($Arrays::asList(members))));
}

$EnumSet* MemberFactory$Group::asSet($MemberFactory$GroupArray* groups) {
	$init(MemberFactory$Group);
	return $nc(groups)->length == 0 ? $EnumSet::noneOf(MemberFactory$Group::class$) : $EnumSet::copyOf($($Arrays::asList(groups)));
}

void MemberFactory$Group::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* MemberFactory$Group::load$($String* name, bool initialize) {
	...
	$loadClass(MemberFactory$Group, name, initialize, &classInfo$$, MemberFactory$Group::clinit$, []($Class* clazz) -> $Object* {
