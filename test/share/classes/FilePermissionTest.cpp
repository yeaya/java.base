...
$Class* FilePermissionTest$$Lambda$lambda$check$0::load$($String* name, bool initialize) {
	...
	$loadClass(FilePermissionTest$$Lambda$lambda$check$0, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FilePermissionTest$$Lambda$lambda$check$0);
	});
	return class$;
}
...
void FilePermissionTest::check($List* files, $String* expected) {
	...
	$nc(files)->forEach($$new(FilePermissionTest$$Lambda$lambda$check$0, actual));
...
void FilePermissionTest::lambda$check$0($StringBuilder* actual, $String* f) {
	...
	$nc(actual)->append($cast($CharSequence, result));
}
...
$Class* FilePermissionTest::load$($String* name, bool initialize) {
	if (name != nullptr) {
		if (name->equals("FilePermissionTest$$Lambda$lambda$check$0")) {
	...
	$loadClass(FilePermissionTest, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FilePermissionTest);
	});
	return class$;
}
