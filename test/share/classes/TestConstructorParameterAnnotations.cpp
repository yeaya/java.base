...
int32_t TestConstructorParameterAnnotations::checkGetParameterAnnotations($Class* clazz, $Constructor* ctor) {
	...
	$var($String, expectedString, $$sure($TestConstructorParameterAnnotations$ExpectedGetParameterAnnotations, $nc(clazz)->getAnnotation($TestConstructorParameterAnnotations$ExpectedGetParameterAnnotations::class$))->value());
...
$Class* TestConstructorParameterAnnotations::load$($String* name, bool initialize) {
	...
	$loadClass(TestConstructorParameterAnnotations, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TestConstructorParameterAnnotations);
	});
	return class$;
}
