#include <NameTooLong.h>
#include <java/io/File.h>
#include <java/io/IOException.h>
#include <java/nio/file/Path.h>
#include <jcpp.h>

using $StringArray2 = $Array<::java::lang::String, 2>;
using $File = ::java::io::File;
using $IOException = ::java::io::IOException;
using $ClassInfo = ::java::lang::ClassInfo;
using $MethodInfo = ::java::lang::MethodInfo;
using $RuntimeException = ::java::lang::RuntimeException;

void NameTooLong::init$() {
}

void NameTooLong::main($StringArray* args) {
	$useLocalObjectStack();
	$var($StringArray2, prefixSuffix, $new($StringArray2, {
		$$new($StringArray, {
			""_s,
			"txt"_s
		}),
		$$new($StringArray, {
			"prefix"_s,
			".txt"_s
		}),
		$$new($StringArray, {
			"prefix"_s,
			"."_s
		})
	}));
	int32_t failures = 0;
	int32_t index = 0;
	{
		$var($StringArray2, arr$, prefixSuffix);
		for (int32_t len$ = arr$->length, i$ = 0; i$ < len$; ++i$) {
			$var($StringArray, ps, arr$->get(i$));
			{
				$var($File, f, nullptr);
				try {
					$assign(f, $File::createTempFile($nc(ps)->get(0), $nc(ps)->get(1), $$new($File, $($System::getProperty("test.dir"_s, "."_s)))));
					$var($String, s, $$nc($$nc($nc(f)->toPath())->getFileName())->toString());
					if (!$nc(s)->startsWith($($nc(ps->get(0))->substring(0, 3)))) {
						$nc($System::err)->printf("%s did not start with %s%n"_s, $$new($ObjectArray, {
							s,
							$($nc(ps->get(0))->substring(0, 3))
						}));
						++failures;
					}
					bool var$0 = $nc(ps->get(1))->startsWith("."_s);
					if (var$0 && !s->contains($($nc(ps->get(1))->substring(0, 4)))) {
						$nc($System::err)->printf("%s did not contain %s%n"_s, $$new($ObjectArray, {
							s,
							$($nc(ps->get(1))->substring(0, 4))
						}));
						;
						++failures;
					}
				} catch ($IOException& e) {
					++failures;
					$nc($System::err)->println();
					e->printStackTrace();
					$System::err->println();
				}
				++index;
			}
		}
	}
	if (failures != 0) {
		$throwNew($RuntimeException, "Test failed!"_s);
	}
}

NameTooLong::NameTooLong() {
}

$Class* NameTooLong::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(NameTooLong, init$, void)},
		{"main", "([Ljava/lang/String;)V", nullptr, $PUBLIC | $STATIC, $staticMethod(NameTooLong, main, void, $StringArray*)},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $ACC_SUPER,
		"NameTooLong",
		"java.lang.Object",
		nullptr,
		nullptr,
		methodInfos$$
	};
	$loadClass(NameTooLong, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(NameTooLong);
	});
	return class$;
}

$Class* NameTooLong::class$ = nullptr;