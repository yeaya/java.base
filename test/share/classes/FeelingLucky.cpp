...
void FeelingLucky::main($StringArray* args) {
	...
	$nc($System::out)->printf("minedPid=%d%n"_s, $$new($ObjectArray, {$($Integer::valueOf(minedPid))}));
	...
	if ($$nc($nc(p)->getInputStream())->read() != -1) {
		$System::out->println("got a char"_s);
...
$Class* FeelingLucky::load$($String* name, bool initialize) {
	...
	$loadClass(FeelingLucky, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(FeelingLucky);
	});
	return class$;
}
