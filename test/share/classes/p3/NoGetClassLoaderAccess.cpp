...
void NoGetClassLoaderAccess::clinit$($Class* clazz) {
...
$Class* NoGetClassLoaderAccess::load$($String* name, bool initialize) {
	...
	$loadClass(NoGetClassLoaderAccess, name, initialize, &classInfo$$, NoGetClassLoaderAccess::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(NoGetClassLoaderAccess);
	});
	return class$;
}
