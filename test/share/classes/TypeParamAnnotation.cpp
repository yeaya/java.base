...
void TypeParamAnnotation::testOnClass() {
	...
	$assign(as, $nc(ts->get(0))->getAnnotations());
	check($nc(as)->length == 2);
	check($$nc($nc($cast($ParamAnno, as->get(0)))->value())->equals("t"_s));
	check($nc($cast($ParamAnno2, as->get(1)))->value() == 1);
	$assign(as, $nc(ts->get(1))->getAnnotations());
	check($nc(as)->length == 0);
	$assign(as, $nc(ts->get(2))->getAnnotations());
	check($nc(as)->length == 2);
	check($$nc($nc($cast($ParamAnno, as->get(0)))->value())->equals("v"_s));
	check($nc($cast($ParamAnno2, as->get(1)))->value() == 2);
}
...
void TypeParamAnnotation::testOnMethod() {
	...
	$assign(as, $nc(ts->get(0))->getAnnotations());
	check($nc(as)->length == 2);
	check($$nc($nc($cast($ParamAnno, as->get(0)))->value())->equals("x"_s));
	check($nc($cast($ParamAnno2, as->get(1)))->value() == 3);
	$assign(as, $nc(ts->get(1))->getAnnotations());
	check($nc(as)->length == 0);
	$assign(as, $nc(ts->get(2))->getAnnotations());
	check($nc(as)->length == 2);
	check($$nc($nc($cast($ParamAnno, as->get(0)))->value())->equals("z"_s));
	check($nc($cast($ParamAnno2, as->get(1)))->value() == 4);
}
...
void TypeParamAnnotation::testGetAnnos() {
	...
	$load($ParamAnno2);
	$assign(as, $cast($ParamAnno2Array, $nc($nc(ts)->get(0))->getAnnotationsByType($ParamAnno2::class$)));
	check($nc(as)->length == 1);
	check($nc(as->get(0))->value() == 3);
}
...
$Class* TypeParamAnnotation::load$($String* name, bool initialize) {
	...
	$loadClass(TypeParamAnnotation, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(TypeParamAnnotation);
	});
	return class$;
}
