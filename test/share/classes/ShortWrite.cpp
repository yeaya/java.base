...
void ShortWrite::test1($ExecutorService* pool, $SocketChannel* source, $SocketChannel* sink, int32_t size) {
	...
	ShortWrite::rand->nextBytes($$cast($bytes, $nc(buf)->array()));
	$var($Future, result, $nc(pool)->submit($$new($ShortWrite$Reader, sink, size)));
	...
	int64_t actual = $$sure($Long, $nc(result)->get())->longValue();
	if (actual != expected) {
...
void ShortWrite::testN($ExecutorService* pool, $SocketChannel* source, $SocketChannel* sink, $ints* sizes) {
	...
	$nc($System::out)->print("write(ByteBuffer[]), sizes="_s);
	{
		...
		for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
			...
			$System::out->print($$str({$$str(size), " "_s}));
		}
	}
	$System::out->println();
	int32_t total = 0;
	...
	$var($Future, result, $nc(pool)->submit($$new($ShortWrite$Reader, sink, total)));
								...
								try {
									try {
										...
										for (int32_t i = 0; i < 20; ++i) {
											int32_t size = ShortWrite::rand->nextInt(1024 * 1024);
											test1(pool, source, sink, size);
										}
										...
										for (int32_t i = 0; i < 20; ++i) {
											int32_t n = ShortWrite::rand->nextInt(9);
											$var($ints, sizes, $new($ints, n));
											for (int32_t j = 0; j < n; ++j) {
												sizes->set(j, ShortWrite::rand->nextInt(1024 * 1024));
											}
...
void ShortWrite::clinit$($Class* clazz) {
...
$Class* ShortWrite::load$($String* name, bool initialize) {
	...
	$loadClass(ShortWrite, name, initialize, &classInfo$$, ShortWrite::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ShortWrite);
	});
	return class$;
}
