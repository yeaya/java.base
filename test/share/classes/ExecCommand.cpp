...
void ExecCommand::main($StringArray* _args) {
	...
	try {
		...
		for (int32_t i = 0; i < ExecCommand::doCmdCopy->length; ++i) {
			{
				$var($BufferedWriter, outCmd, $new($BufferedWriter, $$new($FileWriter, ExecCommand::doCmdCopy->get(i))));
	...
	for (int32_t k = 0; k < 4; ++k) {
		...
		for (int32_t i = 0; i < ExecCommand::TEST_RTE_ARG->length; ++i) {
			...
			try {
				...
				$var($Process, exec, $$nc($Runtime::getRuntime())->exec(ExecCommand::TEST_RTE_ARG->get(i)));
				$nc(exec)->waitFor();
				switch (i) {
				case 0:
			...
			if (!$nc(outRes)->startsWith($nc(ExecCommand::TEST_RTE_GI->get(i))->get(k))) {
				$throwNew($Error, $$str({"Unexpected output! Step"_s, $$str(k), ":"_s, $$str(i), "\nArgument: "_s, ExecCommand::TEST_RTE_ARG->get(i), "\nExpected: "_s, $nc(ExecCommand::TEST_RTE_GI->get(i))->get(k), "\n  Output: "_s, outRes}));
			} else {
				$nc($System::out)->println($$str({"RTE OK:"_s, ExecCommand::TEST_RTE_ARG->get(i)}));
			}
		}
	}
}

void ExecCommand::clinit$($Class* clazz) {
	$useLocalObjectStack();
...
$Class* ExecCommand::load$($String* name, bool initialize) {
	...
	$loadClass(ExecCommand, name, initialize, &classInfo$$, ExecCommand::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(ExecCommand);
	});
	return class$;
}
