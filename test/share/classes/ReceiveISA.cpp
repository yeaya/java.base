									...
									try {
										...
										for (int32_t i = 0; i < 3;) {
											...
											if (msg->matches("Dia duit![0-2]"_s)) {
												...
												$System::out->println($$str({"random interfered packet is : "_s, msg}));
											}
											rb->clear();
										}
										if (!$nc(sa->get(0))->equals(sa->get(1))) {
											$throwNew($Exception, "Source address for packets 1 & 2 should be equal"_s);
										}
										if ($nc(sa->get(1))->equals(sa->get(2))) {
...
$Class* ReceiveISA::load$($String* name, bool initialize) {
	$MethodInfo methodInfos$$[] = {
		{"<init>", "()V", nullptr, $PUBLIC, $method(ReceiveISA, init$, void)},
		{"main", "([Ljava/lang/String;)V", nullptr, $PUBLIC | $STATIC, $staticMethod(ReceiveISA, main, void, $StringArray*), "java.lang.Exception"},
		{}
	};
	$ClassInfo classInfo$$ = {
		$PUBLIC | $ACC_SUPER,
	...
	$loadClass(ReceiveISA, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(ReceiveISA);
	});
	return class$;
}
