...
void VerifyStackTrace$TestCase3::init$() {
	...
	$set(this, walker$, $StackWalker::getInstance($($EnumSet::of($StackWalker$Option::RETAIN_CLASS_REFERENCE, $StackWalker$Option::SHOW_HIDDEN_FRAMES))));
...
$Class* VerifyStackTrace$TestCase3::load$($String* name, bool initialize) {
	...
	$loadClass(VerifyStackTrace$TestCase3, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(VerifyStackTrace$TestCase3);
	});
	return class$;
}
