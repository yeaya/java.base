...
void PathOps::init$($String* first, $StringArray* more) {
	...
	try {
		...
		PathOps::out->format("%s -> %s"_s, $$new($ObjectArray, {
			first,
			this->path$
		}));
	} catch ($Exception& x) {
...
PathOps* PathOps::root($String* expected) {
	...
	checkPath();
	check($($nc(this->path$)->getRoot()), expected);
	return this;
}
...
PathOps* PathOps::parent($String* expected) {
	...
	checkPath();
	check($($nc(this->path$)->getParent()), expected);
	return this;
}
...
PathOps* PathOps::name($String* expected) {
	...
	checkPath();
	check($($nc(this->path$)->getFileName()), expected);
	return this;
}

PathOps* PathOps::element(int32_t index, $String* expected) {
	$useLocalObjectStack();
	$nc(PathOps::out)->format("check element %d\n"_s, $$new($ObjectArray, {$($Integer::valueOf(index))}));
	checkPath();
	check($($nc(this->path$)->getName(index)), expected);
	return this;
}
...
PathOps* PathOps::notEnds($String* suffix) {
	...
	checkPath();
	$var($Path, s, $$nc($FileSystems::getDefault())->getPath(suffix, $$new($StringArray, 0)));
	check($($Boolean::valueOf($nc(this->path$)->endsWith(s))), false);
	return this;
}
...
PathOps* PathOps::absolute() {
	...
	checkPath();
	check($($Boolean::valueOf($nc(this->path$)->isAbsolute())), true);
	return this;
}
...
PathOps* PathOps::notAbsolute() {
	...
	checkPath();
	check($($Boolean::valueOf($nc(this->path$)->isAbsolute())), false);
	return this;
}

PathOps* PathOps::resolve($String* other, $String* expected) {
	$useLocalObjectStack();
	$nc(PathOps::out)->format("test resolve %s\n"_s, $$new($ObjectArray, {other}));
	checkPath();
	check($($nc(this->path$)->resolve(other)), expected);
	return this;
}
...
PathOps* PathOps::resolveSibling($String* other, $String* expected) {
	...
	checkPath();
	check($($nc(this->path$)->resolveSibling(other)), expected);
	return this;
}
...
PathOps* PathOps::relativize($String* other, $String* expected) {
	...
	check($($nc(this->path$)->relativize(that)), expected);
	return this;
}
...
PathOps* PathOps::relativizeFail($String* other) {
	...
	try {
		...
		PathOps::out->format("\tExpected: IllegalArgumentException"_s, $$new($ObjectArray, 0));
		PathOps::out->format("\tActual: %s\n"_s, $$new($ObjectArray, {result}));
		fail();
...
PathOps* PathOps::normalize($String* expected) {
	...
	checkPath();
	check($($nc(this->path$)->normalize()), expected);
	return this;
}
...
void PathOps::header($String* s) {
	...
	$nc(PathOps::out)->println();
	PathOps::out->println();
	PathOps::out->println($$str({"-- "_s, s, " --"_s}));
}
...
void PathOps::doWindowsTests() {
	...
	$$nc($$nc($$nc($$nc(test("."_s, $$new($StringArray, 0)))->makeAbsolute())->absolute())->hasRoot())->string($$str({$(cwd->toString()), "\\."_s}));
	$$nc($$nc($$nc($$nc(test("foo"_s, $$new($StringArray, 0)))->makeAbsolute())->absolute())->hasRoot())->string($$str({$(cwd->toString()), "\\foo"_s}));
	$var($String, rootAsString, $$nc(cwd->getRoot())->toString());
...
void PathOps::clinit$($Class* clazz) {
...
$Class* PathOps::load$($String* name, bool initialize) {
	...
	$loadClass(PathOps, name, initialize, &classInfo$$, PathOps::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(PathOps);
	});
	return class$;
}
