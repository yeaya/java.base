...
void NotBound::main($StringArray* args) {
	...
	try {
		...
		int32_t peerPort = $$cast($InetSocketAddress, $nc(peer)->getLocalAddress())->getPort();
	...
	try {
		...
		wakeupWhenBound(dc);
		$var($SocketAddress, sender, $nc(dc)->receive(bb));
		if (sender == nullptr) {
...
$Class* NotBound::load$($String* name, bool initialize) {
	...
	$loadClass(NotBound, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(NotBound);
	});
	return class$;
}
