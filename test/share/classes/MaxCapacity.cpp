...
void MaxCapacity::main($StringArray* args) {
	...
	if (maxHeap < (int64_t)3 * $Integer::MAX_VALUE) {
		$nc($System::out)->printf("Skipping test; max memory %sM too small%n"_s, $$new($ObjectArray, {$($Long::valueOf($div(maxHeap, (1024 * 1024))))}));
		return;
	}
	$var($ByteArrayOutputStream, baos, $new($ByteArrayOutputStream));
	for (int64_t n = 0;; ++n) {
		try {
			baos->write((int8_t)u'x');
		} catch ($Throwable& t) {
			$var($bytes, bytes, baos->toByteArray());
			if ($nc(bytes)->length != n) {
				$throwNew($AssertionError, $of("wrong length"_s));
			}
			if (bytes->get(0) != u'x' || bytes->get(bytes->length - 1) != u'x') {
			...
			$nc($System::out)->printf("gap=%dM %d%n"_s, $$new($ObjectArray, {
				$($Long::valueOf($div(gap, (1024 * 1024)))),
				$($Long::valueOf(gap))
			}));
			if (gap > 1024) {
				$throw(t);
...
$Class* MaxCapacity::load$($String* name, bool initialize) {
	...
	$loadClass(MaxCapacity, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(MaxCapacity);
	});
	return class$;
}
