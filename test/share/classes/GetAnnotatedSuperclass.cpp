	...
	for (int32_t len$ = $nc(arr$)->length, i$ = 0; i$ < len$; ++i$) {
		$Class* toTest = arr$->get(i$);
		{
			...
			if (res == nullptr) {
				...
				var$0->append(toTest);
				var$0->append(".getAnnotatedSuperclass() returns: "_s);
				var$0->append($($Arrays::asList($(res->getAnnotations()))));
				var$0->append(", should be an empty AnnotatedType"_s);
...
void GetAnnotatedSuperclass::clinit$($Class* clazz) {
	$useLocalObjectStack();
	...
	$assignStatic(GetAnnotatedSuperclass::nonNullTestData, $new($ClassArray, {
		...
		($$new($GetAnnotatedSuperclass$1))->getClass(),
		($$new($GetAnnotatedSuperclass$2))->getClass(),
		($$new($GetAnnotatedSuperclass$3))->getClass()
	}));
...
$Class* GetAnnotatedSuperclass::load$($String* name, bool initialize) {
	...
	$loadClass(GetAnnotatedSuperclass, name, initialize, &classInfo$$, GetAnnotatedSuperclass::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(GetAnnotatedSuperclass);
	});
	return class$;
}
