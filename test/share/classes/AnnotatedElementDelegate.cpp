...
int32_t AnnotatedElementDelegate::testDelegate(AnnotatedElementDelegate* delegate, $Class* annotationClass) {
	...
	failures += annotationArrayCheck(var$1, $(base->getAnnotationsByType(annotationClass)), annotationClass, "Equality failure on getAnnotationsByType(%s) on %s)%n"_s);
	$var($Object, var$2, delegate->getDeclaredAnnotation(annotationClass));
	if (!$Objects::equals(var$2, $(base->getDeclaredAnnotation(annotationClass)))) {
		++failures;
...
int32_t AnnotatedElementDelegate::annotationArrayCheck($AnnotationArray* delegate, $AnnotationArray* base, $Class* annotationClass, $String* message) {
	...
	if (!$Objects::deepEquals(delegate, base)) {
		...
		$System::err->println($$str({"Base result:\t"_s, $($Arrays::toString(base))}));
		$System::err->println($$str({"Delegate result:\t "_s, $($Arrays::toString(delegate))}));
		$System::err->println();
	}
	return failures;
}
...
$Class* AnnotatedElementDelegate::load$($String* name, bool initialize) {
	...
	$loadClass(AnnotatedElementDelegate, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(AnnotatedElementDelegate);
	});
	return class$;
}
