...
void WithSecurityManager4AsynchronousServerSocketChannel::main($StringArray* args) {
	...
	$var($SocketChannel, sc, $SocketChannel::open($$new($InetSocketAddress, lh, port)));
	$var($Future, result, listener->accept());
	if (allow) {
		$$sure($AsynchronousSocketChannel, $nc(result)->get())->close();
	} else {
		try {
			$nc(result)->get();
...
$Class* WithSecurityManager4AsynchronousServerSocketChannel::load$($String* name, bool initialize) {
	...
	$loadClass(WithSecurityManager4AsynchronousServerSocketChannel, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(WithSecurityManager4AsynchronousServerSocketChannel);
	});
	return class$;
}
