...
void OpsAfterClose4PrintWriter::main($StringArray* args) {
	...
	f->deleteOnExit();
	{
		...
		for (int32_t len$ = arr$->length, i$ = 0; i$ < len$; ++i$) {
			...
			{
				$var($PrintWriter, pw, $new($PrintWriter, $$new($FileWriter, f)));
				pw->close();
...
void OpsAfterClose4PrintWriter::clinit$($Class* clazz) {
...
$Class* OpsAfterClose4PrintWriter::load$($String* name, bool initialize) {
	...
	$loadClass(OpsAfterClose4PrintWriter, name, initialize, &classInfo$$, OpsAfterClose4PrintWriter::clinit$, []($Class* clazz) -> $Object* {
