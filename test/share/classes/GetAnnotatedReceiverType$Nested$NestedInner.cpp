...
$Class* GetAnnotatedReceiverType$Nested$NestedInner::getAnonymousClass() {
	return $$new($GetAnnotatedReceiverType$Nested$NestedInner$1, this)->getClass();
}
...
$Class* GetAnnotatedReceiverType$Nested$NestedInner::load$($String* name, bool initialize) {
	...
	$loadClass(GetAnnotatedReceiverType$Nested$NestedInner, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(GetAnnotatedReceiverType$Nested$NestedInner);
	});
	return class$;
}
