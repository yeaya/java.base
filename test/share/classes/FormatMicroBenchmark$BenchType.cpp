...
$String* FormatMicroBenchmark$BenchType::runBenchAndPrintStatistics(int32_t nbRuns, $NumberFormat* nf, bool isCurrency) {
	...
	for (int32_t i = 1; i <= nbRuns; ++i) {
		...
		if ($FormatMicroBenchmark::Verbose) {
			$System::out->println($$str({"calculated time for "_s, this->name$, " bench "_s, $$str(i), " is = "_s, $$str((estimatedTime / 1000)), " microseconds"_s}));
		} else {
			$System::out->print("."_s);
		}
		$FormatMicroBenchmark::stabilizeMemory(true);
	}
	$System::out->println($$str({this->name$, " Done."_s}));
	$FormatMicroBenchmark::printPerfResults(elapsedTimes, this->name$);
	return str;
}

void FormatMicroBenchmark$BenchType::clinit$($Class* clazz) {
...
$Class* FormatMicroBenchmark$BenchType::load$($String* name, bool initialize) {
	...
	$loadClass(FormatMicroBenchmark$BenchType, name, initialize, &classInfo$$, FormatMicroBenchmark$BenchType::clinit$, []($Class* clazz) -> $Object* {
