...
void StaticInterfaceMethodInWayOfDefault::main($StringArray* args) {
	...
	if (!$nc(res)->equals("C_v1"_s)) {
		$System::err->println($$str({"Got wrong method, expecting C_v1, got: "_s, res}));
		++errors;
	}
	$System::err->println("Running using _v2 classes:"_s);
	$assign(res, $cast($String, $nc(v2Task)->call()));
	if (!$nc(res)->equals("A_v1"_s)) {
		$System::err->println($$str({"Got wrong method, expecting A_v1, got: "_s, res}));
		++errors;
	}
	if (errors != 0) {
...
$Class* StaticInterfaceMethodInWayOfDefault::load$($String* name, bool initialize) {
	...
	$loadClass(StaticInterfaceMethodInWayOfDefault, name, initialize, &classInfo$$, []($Class* clazz) -> $Object* {
		return $alloc(StaticInterfaceMethodInWayOfDefault);
	});
	return class$;
}
