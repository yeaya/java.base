...
void StringRepeat::main($StringArray* args) {
	...
	if ($nc(args)->length > 0) {
		...
		for (int32_t i = 0; i < args->length; ++i) {
			StringRepeat::REPEATS->set(i, $Integer::parseInt(args->get(i)));
		}
	}
	test1();
	test2();
...
$String* StringRepeat::truncate($String* string) {
	...
	return $str({$(string->substring(0, 80)), "..."_s});
}
	...
	if ($nc(string)->isEmpty() || repeat == 0) {
		if (!$nc(result)->isEmpty()) {
			$nc($System::err)->format("\"%s\".repeat(%d)%n"_s, $$new($ObjectArray, {
				$(truncate(string)),
				$($Integer::valueOf(repeat))
			}));
			$System::err->format("Result \"%s\"%n"_s, $$new($ObjectArray, {$(truncate(result))}));
			$System::err->format("Result expected to be empty, found string of length %d%n"_s, $$new($ObjectArray, {$($Integer::valueOf(result->length()))}));
		...
		for (int32_t offset = $nc(result)->indexOf(string, expected); 0 <= offset; offset = result->indexOf(string, expected)) {
			...
			if (offset != expected) {
				...
				$System::err->format("Result \"%s\"%n"_s, $$new($ObjectArray, {$(truncate(result))}));
				$System::err->format("Repeat expected at %d, found at = %d%n"_s, $$new($ObjectArray, {
		...
		if (count != repeat) {
			...
			$System::err->format("Result \"%s\"%n"_s, $$new($ObjectArray, {$(truncate(result))}));
			$System::err->format("Repeat count expected to be %d, found %d%n"_s, $$new($ObjectArray, {
				$($Integer::valueOf(repeat)),
...
void StringRepeat::clinit$($Class* clazz) {
...
$Class* StringRepeat::load$($String* name, bool initialize) {
	...
	$loadClass(StringRepeat, name, initialize, &classInfo$$, StringRepeat::clinit$, []($Class* clazz) -> $Object* {
		return $alloc(StringRepeat);
	});
	return class$;
}
